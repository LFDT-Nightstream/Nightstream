...
namespace Nightstream.Implementation.Nebula.WasmResultCodec
...
def encodeDigestAux :
    (count : Nat) →
      (Fin count →
        Nightstream.Protocol.Nebula.ShiftedTernary41V1.CanonicalGoldilocks) →
        List Nat
  | 0, _ => []
  | count + 1, lanes =>
...
def encodeDigest
    (digest : Nightstream.Protocol.Nebula.Digest.Value) : List Nat :=
...
theorem encodeDigest_length
    (digest : Nightstream.Protocol.Nebula.Digest.Value) :
    (encodeDigest digest).length =
      Nightstream.Protocol.Nebula.Digest.serializedBitCount := by
...
theorem encodeDigest_exact_length
  ...
  rw [encodeDigest_length,
    Nightstream.Protocol.Nebula.Digest.serializedBitCount_eq]
...
theorem encodeDigest_injective : Function.Injective encodeDigest := by
  ...
  apply Nightstream.Protocol.Nebula.Digest.Value.ext
...
end Nightstream.Implementation.Nebula.WasmResultCodec
