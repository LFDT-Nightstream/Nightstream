...
use neo_fold_prototype::public_proof::rv32im::side_eval_claim_relation::build_rv32im_side_eval_claim_relation_from_accepted_artifact;
...
fn rv32im_side_relation_phase0_point_and_payload_match_native_claim_witness() {
    let proof =
    ...
    let (_, witness) = build_rv32im_side_eval_claim_relation_from_accepted_artifact(&accepted_artifact)
    ...
    let claim_witness = witness
        ...
        .iter()
        .find(|claim| {
            claim.claim.payload.schema == neo_fold_prototype::rv32im::kernel::FamilyEvalSchemaId::Stage2RegisterWrites
        })
...
fn rv32im_side_relation_phase0_component_counts() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let accepted_artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (_, witness) = build_rv32im_side_eval_claim_relation_from_accepted_artifact(&accepted_artifact)
