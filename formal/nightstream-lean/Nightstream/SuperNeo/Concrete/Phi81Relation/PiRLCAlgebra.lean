...
Owns: the status and dependency boundary of every concrete algebra field. Each
...
|---|---|---|---|
| `nifs.pi_rlc.verify.challenge` | `challengeValid` | `PiRLCAlgebra.Challenge`; exact Phi81 production-set membership | predicate proved; transcript refinement open |
| `nifs.pi_rlc.verify.assignment_hom` | `combineAssignment` | `EvaluationHomomorphism.PiRLCFinite.combineAssignments` | proved |
| `nifs.pi_rlc.verify.commitment_hom` | `combineCommitment`, `commit_hom` | `PiRLCAlgebra.Commitment` | proved |
| `nifs.pi_rlc.verify.public_input_hom` | `combinePublicInput`, `publicInput_hom` | `PiRLCAlgebra.PublicInput` | proved |
| `nifs.pi_rlc.verify.evaluation_hom` | `combineEvaluations`, `evaluations_hom` | `EvaluationHomomorphism.PiRLCFinite` | proved |
| `nifs.pi_rlc.verify.norm_growth` | `norm_growth` | `PiRLCAlgebra.Norm`; centered Goldilocks + exact executable Phi81 support + finite production bound | proved |
| `nifs.pi_rlc.verify.algebra` | complete `PiRLC.Algebra` | `PiRLCAlgebra.Algebra.concrete` | proved; production refinement open |
-/
