...
use super::{nc_partial_groups, MetalNcSumcheckPlan, NC_MASK_DENSE_CROSSOVER, NC_THREADS};
...
impl MetalSession {
    pub(super) fn encode_nc_mask_round(
        ...
        if source.round_encoded
            ...
            || plan.dense
            || plan.rows < 2
            || 2 * plan.width > NC_MASK_DENSE_CROSSOVER
        ...
        let (round_shape, round_shape_offset) = trace_round.map_or((&plan.shape, 0), |round| {
            (&plan.round_shapes, round * 6 * size_of::<u64>())
        });
        ...
        let groups = nc_partial_groups(plan.live_rows, false);
    ...
    fn encode_nc_mask_fold_from_buffer(
        ...
        if !direct_compact {
            ...
            self.dispatch(
                ...
                plan.active_witness_count * plan.live_rows.div_ceil(2),
            );
            encoder.endEncoding();
        } else {
            // Large inputs fold only the shared basis. Width 64 remains
            // mask-native; its fold materializes width 128 once.
            let encoder = command.computeCommandEncoder().ok_or(MetalError::Encoder)?;
            ...
            if 2 * plan.width == NC_MASK_DENSE_CROSSOVER {
                let encoder = command.computeCommandEncoder().ok_or(MetalError::Encoder)?;
                ...
                self.dispatch(
                    ...
                    plan.active_witness_count * plan.live_rows.div_ceil(2) * D,
                );
                encoder.endEncoding();
            }
        }
        ...
        source.folded = !direct_compact || 2 * plan.width == NC_MASK_DENSE_CROSSOVER;
        plan.current_slot = next_slot;
        plan.rows = next_rows;
        plan.live_rows = plan.live_rows.div_ceil(2);
        if source.folded {
