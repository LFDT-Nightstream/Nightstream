    ...
    L: neo_ccs::traits::SModuleHomomorphism<Ff, Cmt>,
{
    ...
    for (inst, wit) in mcs_list.iter().zip(witnesses.iter()) {
        let X = crate::common::project_x_from_witness_mat(&wit.Z, s.m, inst.m_in)
            .unwrap_or_else(|e| panic!("build_me_outputs_paper_exact: project_x_from_witness_mat failed: {e}"));
