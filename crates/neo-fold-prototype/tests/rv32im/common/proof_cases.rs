...
use neo_fold_prototype::rv32im::audit::audit_rv32im_accepted_proof;
use neo_fold_prototype::rv32im::layout::{RV32IM_PARITY_LOWERING_VERSION_ID, RV32IM_PARITY_PROTOCOL_VERSION_ID};
use neo_fold_prototype::rv32im::tables::Rv32FamilyTag;
use neo_fold_prototype::rv32im::{
    encode_add, encode_addi, encode_beq, encode_divu, encode_ecall, encode_lw, encode_sw, parity_source_cases,
    prove_rv32im_accepted_proof, prove_rv32im_accepted_proof_with_options, sem_inputs_digest, MemoryWord,
    Rv32imAcceptedProofArtifact, Rv32imAuditBundle, Rv32imParityCaseManifest, Rv32imParitySourceCase, Rv32imProofInput,
    Rv32imPublicProofOptions,
};

const START_PC: u32 = 0x1000;
const MAX_STEPS: usize = 32;
type AcceptedFixture = (Rv32imAcceptedProofArtifact, Rv32imAuditBundle);
...
#[allow(dead_code)]
pub fn alu_input() -> Rv32imProofInput {
    ...
    Rv32imProofInput {
        source: Rv32imParitySourceCase {
            manifest: manifest(
                "rv32im_stage_test_alu",
                "rv32im_stage_test_alu_v1",
                vec![Rv32FamilyTag::NativeAlu],
            ),
            start_pc: START_PC,
            program_words,
            initial_registers: [0; 32],
            initial_memory: Vec::<MemoryWord>::new(),
            transcript_seed: b"rv32im-stage-test-alu-v1".to_vec(),
        },
        max_steps: MAX_STEPS,
    }
}
...
#[allow(dead_code)]
pub fn branch_input() -> Rv32imProofInput {
    let program_words = vec![
        encode_addi(1, 0, 1),
        encode_addi(2, 0, 1),
        encode_beq(1, 2, 8),
    ...
    Rv32imProofInput {
        source: Rv32imParitySourceCase {
            manifest: manifest(
                "rv32im_stage_test_branch",
                "rv32im_stage_test_branch_v1",
            ...
            initial_registers: [0; 32],
            initial_memory: Vec::<MemoryWord>::new(),
            transcript_seed: b"rv32im-stage-test-branch-v1".to_vec(),
        },
        max_steps: MAX_STEPS,
    }
}
...
#[allow(dead_code)]
pub fn memory_input() -> Rv32imProofInput {
    ...
    Rv32imProofInput {
        source: Rv32imParitySourceCase {
            manifest: manifest(
                "rv32im_stage_test_memory",
                "rv32im_stage_test_memory_v1",
            ...
            initial_registers,
            initial_memory: Vec::<MemoryWord>::new(),
            transcript_seed: b"rv32im-stage-test-memory-v1".to_vec(),
        },
        max_steps: MAX_STEPS,
    }
}
...
#[allow(dead_code)]
pub fn divu_input() -> Rv32imProofInput {
    let mut initial_registers = [0; 32];
    ...
    Rv32imProofInput {
        source: Rv32imParitySourceCase {
            manifest: manifest(
                "rv32im_stage_test_divu",
                "rv32im_stage_test_divu_v1",
                vec![Rv32FamilyTag::UnsignedDivRem],
            ),
            start_pc: START_PC,
            program_words,
            initial_registers,
            initial_memory: Vec::<MemoryWord>::new(),
            transcript_seed: b"rv32im-stage-test-divu-v1".to_vec(),
        },
        max_steps: MAX_STEPS,
    }
}
...
#[allow(dead_code)]
pub fn parity_input(name: &str) -> Rv32imProofInput {
...
pub fn prove_accepted(input: &Rv32imProofInput) -> (Rv32imAcceptedProofArtifact, Rv32imAuditBundle) {
    prove_rv32im_accepted_proof(input).expect("prove accepted rv32im proof")
}

fn prove_accepted_rows_per_chunk_1(input: &Rv32imProofInput) -> (Rv32imAcceptedProofArtifact, Rv32imAuditBundle) {
    prove_rv32im_accepted_proof_with_options(
        input,
        Rv32imPublicProofOptions {
            root_fold_schedule: FoldSchedule::RowsPerChunk(1),
        },
    )
    .expect("prove accepted rv32im proof with RowsPerChunk(1)")
}
...
pub fn expect_accepted_audit_failure(artifact: &Rv32imAcceptedProofArtifact, needle: &str) {
    let err = audit_rv32im_accepted_proof(artifact).expect_err("tampered accepted proof must fail");
    assert!(
...
pub fn refresh_accepted_artifact_digest(artifact: &mut Rv32imAcceptedProofArtifact) {
    let mut artifact_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/accepted_proof_artifact");
    artifact_tr.append_message(b"rv32im/accepted_proof_artifact/claim", &artifact.claim.digest);
    artifact_tr.append_message(b"rv32im/accepted_proof_artifact/statement", &artifact.statement.digest);
    ...
    artifact_tr.append_message(b"rv32im/accepted_proof_artifact/main_lane", &artifact.main_lane.digest);
    artifact_tr.append_message(b"rv32im/accepted_proof_artifact/stage1", &artifact.stage1.digest);
    artifact_tr.append_message(b"rv32im/accepted_proof_artifact/stage2", &artifact.stage2.digest);
    artifact_tr.append_message(b"rv32im/accepted_proof_artifact/stage3", &artifact.stage3.digest);
    artifact_tr.append_message(
...
#[allow(dead_code)]
pub fn refresh_stage1_semantic_digests(artifact: &mut Rv32imAcceptedProofArtifact) {
    ...
    let mut alu_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_alu_shout_proof");
    alu_tr.append_message(b"rv32im/stage1_alu_shout_proof/sem_inputs_digest", &sem_inputs_digest);
    alu_tr.append_u64s(
    ...
    let mut branch_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_branch_shout_proof");
    ...
    let mut linkage_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_linkage_proof");
    linkage_tr.append_message(
        b"rv32im/stage1_linkage_proof/rows_digest",
        &artifact.stage1.linkage.rows_digest,
    );
    linkage_tr.append_message(b"rv32im/stage1_linkage_proof/sem_inputs_digest", &sem_inputs_digest);
    ...
    let mut semantics_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_semantics_proof");
    semantics_tr.append_message(b"rv32im/stage1_semantics_proof/sem_inputs_digest", &sem_inputs_digest);
    ...
    let mut stage1_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_proof_bundle");
    stage1_tr.append_message(b"rv32im/stage1_proof_bundle/bytecode", &artifact.stage1.bytecode.digest);
    stage1_tr.append_message(b"rv32im/stage1_proof_bundle/alu", &artifact.stage1.alu.digest);
    stage1_tr.append_message(b"rv32im/stage1_proof_bundle/branch", &artifact.stage1.branch.digest);
    stage1_tr.append_message(
        b"rv32im/stage1_proof_bundle/semantics",
    ...
    stage1_tr.append_message(b"rv32im/stage1_proof_bundle/linkage", &artifact.stage1.linkage.digest);
    stage1_tr.append_message(
...
#[allow(dead_code)]
pub fn refresh_stage3_semantic_digests(artifact: &mut Rv32imAcceptedProofArtifact) {
    let mut semantics_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage3_semantics_proof");
    semantics_tr.append_message(
    ...
    let mut stage3_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage3_proof_bundle");
    stage3_tr.append_message(b"rv32im/stage3_proof_bundle/bridge", &artifact.stage3.bridge.digest);
    stage3_tr.append_message(
        b"rv32im/stage3_proof_bundle/semantics",
        &artifact.stage3.semantics.digest,
    );
    stage3_tr.append_message(b"rv32im/stage3_proof_bundle/linkage", &artifact.stage3.linkage.digest);
    stage3_tr.append_message(
...
#[allow(dead_code)]
pub fn refresh_step_composition_surface_digest(artifact: &mut Rv32imAcceptedProofArtifact) {
...
#[allow(dead_code)]
pub fn refresh_soundness_accounting_surface_digest(artifact: &mut Rv32imAcceptedProofArtifact) {
    let mut schema_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/kernel_soundness_accounting_schema");
