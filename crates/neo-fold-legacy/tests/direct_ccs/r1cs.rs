...
use neo_fold_legacy::frontends::direct_ccs::{self, FrontendError, R1cs};
use neo_fold_legacy::{
...
fn finish_uncompressed_rejects_manual_oversized_latest() {
    ...
    let claims: Vec<_> = oversized
        ...
        .collect();
    let chunk_digest = neo_fold_legacy::paper::digest::f_prime_chunk_public_digest(0, &claims);
    let repaired_boundary = neo_fold_legacy::paper::digest::digest_fields_as_digest32(chunk_digest);
...
fn construction2_step_rejects_manual_oversized_prior_latest() {
    ...
    let z_0 = neo_fold_legacy::paper::digest::initial_boundary_digest(prep.structure_digest(), prep.public_input_len());
    let public_trace = neo_fold_legacy::paper::digest::public_trace_seed_digest(prep.structure_digest());
    let empty_acc = neo_fold_legacy::paper::digest::AccumulatorHandle::empty().digest();
    let state0 = State::base(z_0, public_trace, empty_acc, empty_acc);

    let (state1, _step0) = neo_fold_legacy::paper::construction2::step(
        prep.params(),
        prep.structure(),
    ...
    let err = neo_fold_legacy::paper::construction2::step(
        prep.params(),
        prep.structure(),
        prep.optimized_cache(),
        prep.structure_digest(),
