...
| 4 | Checked bootstrap-recursive assignment committed | in-progress: diagnostic rerun live on this box | The full-capture rerun (`RUST_BACKTRACE=1`, no truncation) is running; the old machine failed ~1.8 h in. A persisting variant (`capture_and_save_the_bootstrap_recursive_assignment`) is committed and writes `evidence/nebula-recursive-assignment.json` the moment the flow completes. |
