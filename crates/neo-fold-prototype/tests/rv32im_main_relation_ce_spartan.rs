...
use neo_fold_prototype::circuit::superneo::ce_consistency::enforce_ce_consistency;
use neo_fold_prototype::circuit::superneo::ce_spartan::{
    prove_rv32im_ce_relation, setup_rv32im_ce_relation, verify_rv32im_ce_relation,
};
use neo_fold_prototype::circuit::superneo::claim::{alloc_ce_claim, me_digest_poseidon};
use neo_fold_prototype::circuit::superneo::witness::alloc_packed_witness;
use neo_math::{D, F, K};
...
fn rv32im_main_relation_ce_spartan_round_trip() {
    ensure_toy_pp();
    ...
    let (pk, vk) =
        setup_rv32im_ce_relation(&params, &structure, &claim, &witness, F::from_u64(7)).expect("setup ce relation");
    let proof = prove_rv32im_ce_relation(&pk, &params, &structure, &claim, &witness, F::from_u64(7))
        .expect("prove ce relation");
    verify_rv32im_ce_relation(&vk, &claim, &proof).expect("verify ce relation");
}

#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_ce_spartan_rejects_tampered_claim() {
    ensure_toy_pp();
    let params = NeoParams::goldilocks_auto_r1cs_ccs(3).expect("params");
    let structure = toy_structure();
    let witness = toy_witness();
    ...
    let err = verify_rv32im_ce_relation(&vk, &tampered, &proof).expect_err("tampered claim must fail");
