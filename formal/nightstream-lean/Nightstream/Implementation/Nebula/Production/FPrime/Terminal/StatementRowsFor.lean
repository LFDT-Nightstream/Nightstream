...
namespace Nightstream.Implementation.Nebula.ProductionPaperTerminalStatementRowsFor
...
private def leafBlocks (image : PublicImage) : List (List Nat) :=
  ...
  , WasmStateCodec.encodeWord 64
      (image.result.finalMemoryRoot.lanes ⟨0, by
        norm_num [Nightstream.Protocol.Nebula.Digest.laneCount]⟩).val
  , WasmStateCodec.encodeWord 64
...
private theorem encode_eq_leafBlocks (image : PublicImage) :
  ...
  simp [leafBlocks, WasmPublicStatementCodec.encode,
    ...
    Nightstream.Protocol.Nebula.Digest.laneCount,
    Nightstream.Protocol.Nebula.Digest.laneBitWidth]
...
end Nightstream.Implementation.Nebula.ProductionPaperTerminalStatementRowsFor
