...
use neo_fold_legacy::frontends::nebula::f_prime::NebulaFPrimeStreamingLifecycleArm;
...
fn render_round(round: &neo_fold_legacy::engine::r1cs_circuit::builder::Poseidon2HashRoundAudit) -> String {
    let kind = match &round.kind {
        neo_fold_legacy::engine::r1cs_circuit::builder::Poseidon2HashRoundAuditKind::Absorb { chunk_cols } => {
            format!(".absorb {}", lean_list(chunk_cols.iter().copied()))
        }
        neo_fold_legacy::engine::r1cs_circuit::builder::Poseidon2HashRoundAuditKind::Pad => ".pad".to_owned(),
    };
    format!(
