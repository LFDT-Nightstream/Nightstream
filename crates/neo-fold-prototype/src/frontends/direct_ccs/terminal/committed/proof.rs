...
use super::types::{
    ...
    DirectCcsTerminalCommittedProofPackage, DirectCcsTerminalCommittedRelation, SimpleKernelError,
};
use crate::construction2::terminal::TerminalPrivateColumnEncoding;
...
fn setup_direct_ccs_terminal_committed_key_pair(
    ...
    Ok(DirectCcsTerminalCommittedKeyPair {
        prover: Arc::new(pk),
        verifier: Arc::new(vk),
        perf,
    })
...
pub(crate) fn setup_direct_ccs_terminal_committed_relation_cached(
    ...
    let keys = setup_direct_ccs_terminal_committed_key_pair(relation, perf)?;
    cache
        .lock()
        .map_err(|_| SimpleKernelError::Bridge("direct terminal committed-step setup cache poisoned".into()))?
...
fn direct_terminal_committed_setup_cache_key(
    ...
    let encodings = assignment
        ...
        .collect::<Vec<_>>();
    tr.append_u64s(
        b"neo.fold.next/direct_ccs/terminal_committed_setup_cache/sources",
        &encodings,
    );
    tr.digest32()
}
...
pub(crate) fn prove_direct_ccs_terminal_committed_relation(
    ...
    relation: &DirectCcsTerminalCommittedRelation,
) -> Result<DirectCcsTerminalCommittedProofPackage, SimpleKernelError> {
    let circuit = relation.committed_circuit();
    ...
    Ok(DirectCcsTerminalCommittedProofPackage {
        proof: DirectCcsTerminalCommittedProof { snark_data },
        pcs_ms: perf.pcs_prove_ms,
    })
}
