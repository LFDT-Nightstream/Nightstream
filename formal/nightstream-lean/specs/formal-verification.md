...
| `CIR-FPR-PICCS-DELAYED-RAW-CHILD-CONTRACT` | The fixed 270-coordinate delayed-projection input is materialized in canonical child-major/column-major order directly from `SplitNc.Sources.Data.runningAssignments`. Lean proves exact two-dimensional lookup, equality with the existing canonical running-source injection, and coverage by the derived 512-column/64-lane production domain. Specializing `OldPointBinding` to this table derives the complete `OldPointSumcheckRelation` or the named degree-53 projection `BadRoot` once production supplies its accepted compact identity, old-point length, and ten padding zeros. This is a model-level refinement contract, not production authority: the current Rust handoff reads `CeClaim.y_zcol` in `running_output_evaluation`, so it does not yet meet the raw-child contract; combined-NC acceptance, transcript order/domain separation, state continuity, generated padding rows, and commitment alignment remain open | `ProductionRawChildren.Fixed270.authoritativeRunningChildren_getD`, `authoritativeRunningChild_eq_orderedAssignment`, `assignmentsFitColumnDomain`, `acceptedProjectionIdentity_implies_oldPointRelation_or_badRoot` | Required Rust owner: the actual running-child witness/assignment tables; `CeClaim.y_zcol` is explicitly not an authority source | `model-proved` |
...
| `CIR-FPR-PICCSNC-PRODUCTION-DOMAIN` | The compressed fixed-point artifact has exactly 17,669,277 rows and 14,338,890 aligned columns, hence 265,535 complete Phi81 blocks. The canonical production NC domain is minimally 19 block variables plus six lane variables. Each block has 54 physical lanes and ten verifier-computed virtual zero lanes; the virtual padding is computed polynomial data, not ten physical row owners. The legacy flat carrier would require 24+6 rounds and is not the active arithmetization. A bounded release regression materializes the complete relation below the constructor guard and checks these dimensions against the compiler audit | `ProductionDomain.artifact_relationRows`, `artifact_relationColumns`, `semanticShape_blockCount`, `blockLaneDomain_covers`, `blockLaneRoundCount`, `liveLaneCount_exact`, `virtualLaneCount_exact`, `rowVariables_minimal`, `blockVariables_minimal`, `laneVariables_minimal` | Fixed-point relation artifact dimensions, `active_fixed_point_materializes_after_accumulator_ce_compression`, and independent typed production domain | `artifact-checked` |
| `CIR-FPR-PICCSNC-FULL-Z-DECODER` | For the compressed bounded profile, Rust exports one compact full-`Z` layout certificate: exact width 14,338,890, fourteen children, matrix shape `54 × 265,535`, and 64 proof-free Boolean-lane records. The generator exercises the actual `decode_superneo_coeffs_from_witness_mat` convention and all 108 one-hot cells of two complete blocks through `CcsInstance::from_low_norm_assignment`, Ajtai commitment recomputation, and `Commitment.data[row*54+lane]`; it rejects dimension, lane, and commitment-length mutations. The artifact explicitly distinguishes bounded fixture κ=4 from protocol production κ=18. Lean proves the logical-coordinate, fourteen-child, and commitment-data bijections and connects generated live cells and ten computed-zero virtual lanes to the typed production source table. This checks coordinate geometry, constructor convention, and commitment-data ordering without enumerating millions of cells or key coefficients. It does not establish native PP coefficient equality, accepted opening/extraction, transcript order, or commitment binding | `PackedWitnessDecoder.logicalColumnAt_bijective`, `childLogicalColumnAt_bijective`, `commitmentProbeColumnAt_exact`, `productionCommitmentDataIndex_bijective`, `PackedWitnessCommitment.productionFlattened_matrixCommit_at`, `unpack_at_generatedAddress`, `production_live_eq_generatedWitnessCell`, `production_padding_eq_zero`, `generated_full_decoder_and_lane_partition` | `decode_superneo_coeffs_from_witness_mat`, `CcsInstance::from_low_norm_assignment`, `Commitment::col`, compressed selective relation dimensions, and `active_selective_fixed_point_projection_artifact_matches_retained_certificate` | `artifact-checked` |
| `CIR-FPR-PICCSNC-BLOCK-DELAYED-ACTIVE` | The production block×lane combined polynomial is evaluated directly from complete packed running witnesses, has exact Boolean-cube and terminal formulas and quartic round slices, and compares the carried parent against the authoritative raw projection through a degree-one residual-weight identity. Zero `batchWeight` is an explicit residual root; producer-beta mismatch is a degree-53 projection bad root; fixed-degree SumCheck collisions remain named. Accepted successor NC truth derives the exact ordered raw running assignments—not `CeClaim.y_zcol` sidecars. Strict Π_DEC plus the narrow canonical-parent commitment/norm and raw-child commitment predicates yields exact parent recomposition or `ParentOpeningBindingCollision`; the direct active theorem assumes neither predicate, instead returning either negation as a named binding failure. The terminal path applies the same reduction to fourteen complete raw child matrices. `ActiveTrace` kernel-checks the explicit no-pending base, one-fold delay, and terminal anchor for an arbitrary nonempty digest-linked trace. Its strong headline proves `BaseNc`, every packed equation, and independent Construction-2 semantics; even an independent paper failure retains `BaseNc` and `AllPacked`, so only the dedicated parent-opening tree can prevent `y_zcol` authority. No result branch contains `OutputBindingFailure` or generic `outputUnbound`; `yRingUnbound` remains separate. The active conclusion is the exact 54-lane `PackedYZcolBoundAtBlock` relation over 19+6 rounds, not the incompatible fixed-270 flat `OldPointSumcheckRelation`. `PackedWitnessSourceProjection` connects every live source-table leaf to a full packed-`Z` cell, with exactly 54 live and ten computed-zero lanes, and the generated full-`Z` decoder artifact-checks that geometry. The current native helper is nonconformant because it reads sidecars, evaluates no raw block MLE, and is not integrated into the transcript/combined-NC verifier; exact Rust acceptance, canonical parent opening, transcript tags/order, delayed-projection/state/terminal rows, and Ajtai coordinate binding remain open | `CombinedNc.Acceptance.expectedRoundsRepresentable`, `residualWeightIdentity_exact_iff`, `DelayedRawChildren.rawChildren_recompose_eq_canonicalParent_or_bindingCollision`, `ProductionState.acceptedNext_of_stateBinding_of_parentOpening_implies_previousPackedYZcolBound_or_badEvent`, `ProductionTerminal.accepted_of_parentOpening_implies_packedYZcolBound_or_badEvent`, `PackedWitnessDecoder.production_live_eq_generatedWitnessCell`, `production_padding_eq_zero`, `ActiveBoundary.claimsAcceptedPair_of_nextPacked_implies_previousPacked_or_parentOpeningBadEvent`, `claimsAcceptedTerminal_implies_packed_or_parentOpeningBadEvent`, `ActiveTrace.Trace.terminalChecked_implies_baseAllPackedAndAllPaper_or_parentOpeningFailure_or_paperFailure` | Required production owners: complete `CcsWitness.Z`/`CeWitness.Z` handoff, block×lane terminal, adjacent-state binding, canonical parent and terminal openings, exact transcript replay, and aligned commitment key | `model-proved` |
...
| `CIR-FPR-PICCSNC-PACKED-WITNESS-LAYOUT` | Lean states Rust's intended full-witness layout `Z[lane,block] = assignment[block*54+lane]` at generic production width, proves semantic block counts and packing/unpacking inverses, proves fresh-tail zeros, and constructs `Sources.Data.runningAssignments` definitionally from complete packed witnesses. At the bounded generated domain, the compact decoder artifact proves complete coordinate ownership for all fourteen `54 × 265,535` matrices; `PackedWitnessSourceProjection` proves every live combined-NC leaf equals its full packed-witness cell, all ten virtual lanes and out-of-carrier blocks are computed zero, and the partition is exactly 54+10. The real bounded constructor/commitment probe checks 108 one-hot cells, while Lean proves the generic typed `matrixCommit`, its production κ=18 `row*54+lane` flattening, and that failure of raw-child authority is exactly existence of one verifier-indexed packed matrix whose `matrixCommit` differs from its public commitment. Successor NC truth supplies norms for those exact raw source-table assignments; the active theorem exhaustively splits commitment equality and canonical-parent commitment/norm equality into success or specifically named binding failure. Strict Π_DEC then gives recomposition or a named binding collision. The final claims-level trace theorem retains every predecessor packed equation together with independent Construction-2 semantics, explicitly handles no-pending base and raw-matrix terminal boundaries, and contains no successful premise or failure constructor based on child `CeClaim.y_zcol`, caller-supplied raw equality, `OutputBindingFailure`, or generic `outputUnbound`. Native PP coefficient equality, accepted opening/extraction, canonical opening and semantic-input rows, delayed-projection/state/terminal rows, and transcript schedule remain open, so this is not Rust-conformant production authority yet | `PackedWitness.unpack_pack`, `pack_unpack`, `coordinatesAligned_iff_unpack_eq`, `decodedData_coordinatesAligned`, `PackedWitnessDecoder.childLogicalColumnAt_bijective`, `productionCommitmentDataIndex_bijective`, `PackedWitnessCommitment.productionFlattened_matrixCommit_at`, `rawRunningCommitmentsUnbound_iff_exists_matrixCommit_ne`, `PackedWitnessProduction.parentOpeningExternalBindingFailure_iff`, `production_live_eq_generatedWitnessCell`, `production_padding_eq_zero`, `DelayedRawChildren.rawChildren_recompose_eq_canonicalParent_or_bindingCollision`, `ActiveBoundary.claimsAcceptedTerminal_implies_packed_or_parentOpeningBadEvent`, `ActiveTrace.Trace.terminalChecked_implies_baseAllPackedAndAllPaper_or_parentOpeningFailure_or_paperFailure` | `CcsInstance::from_low_norm_assignment`, `decode_superneo_coeffs_from_witness_mat`, `CcsWitness.Z`, `CeWitness.Z`, `Commitment::data`, and required production combined-NC handoff | `model-proved` |
