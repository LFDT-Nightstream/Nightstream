...
#[test]
fn riscv_trace_wiring_ccs_shared_cpu_bus_shout_sra_prove_verify() {
    // Program:
    // - LUI x1, 0x80000   (x1 = 0x8000_0000)
    // - SRAI x2, x1, 3    (x2 = 0xF000_0000)
    // - SRAI x3, x1, 0    (x3 = 0x8000_0000)
    // - HALT
    let program = vec![
    ...
    // Shout instance: SRA table, 1 lane.
    let t = exec.rows.len();
    ...
    let mut tr_prove = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sra");
    ...
    let mut tr_verify = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sra");
