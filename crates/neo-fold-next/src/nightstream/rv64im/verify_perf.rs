...
use crate::rv64im::{Rv64imIvcSnarkVerifierKey, Rv64imProofStatement, SimpleKernelError};
...
pub fn verify_rv64im_nightstream_with_perf(
    ...
    let expected_verifier_context_digest = rv64im_verifier_context_digest(
    ...
    if statement.verifier_context_digest != expected_verifier_context_digest {
        return Err(SimpleKernelError::Bridge(
            "RV64IM Nightstream verifier context digest does not match the trusted root parameters and IVC verifier key"
                .into(),
        ));
    }
