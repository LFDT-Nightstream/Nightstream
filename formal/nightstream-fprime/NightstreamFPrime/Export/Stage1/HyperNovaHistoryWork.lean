...
theorem source_calls_le_iteration
    ...
    (statement : Statement) (proof : target.Envelope) (outcomes : List (SourceResult target))
    (supported : outcomes ∈ (HyperNovaHistoryLaw.results target source statement proof).support) :
            ...
            · simp only [if_pos counter] at supported
              ...
              · simp only [if_neg base] at supported
                ...
                | some values =>
                    ...
                    have previousCount : (predecessorStatement target payload).iteration + 1 = remaining := by
                      simpa only [predecessorStatement] using counter.trans count
                    have previous := induction (predecessorStatement target payload).iteration
                      (by omega) (predecessorStatement target payload)
                      (.recursive (predecessorPayload target payload values)) tail previousSupported rfl
...
def controlAllowance (outcomes : List (SourceResult target)) : Nat := outcomes.length + 1
...
theorem expected_control_allowance_le
    ...
    Summable (fun sample => (distribution sample).toReal * (controlAllowance target sample.2.2 : ℝ)) ∧
      (∑' sample, (distribution sample).toReal * (controlAllowance target sample.2.2 : ℝ)) ≤
        ((depth + 1 : Nat) : ℝ) := by
  dsimp only
  let distribution := HyperNovaHistoryLaw.law target source initial
  have allowanceBound (sample : HyperNovaHistoryProbability.Sample target)
      (supported : sample ∈ distribution.support) : controlAllowance target sample.2.2 ≤ depth + 1 := by
    change sample ∈ (HyperNovaHistoryLaw.law target source initial).support at supported
    ...
    exact Nat.add_le_add_right
      ((source_calls_le_iteration target source input.1 input.2 outcomes outcomesSupported).trans
        (depthBound input inputSupported)) 1
  have weightedBound (sample : HyperNovaHistoryProbability.Sample target) :
      (distribution sample).toReal * (controlAllowance target sample.2.2 : ℝ) ≤
