...
use neo_fold_legacy::frontends::nebula::layout::StepPublicInput;
...
use neo_fold_legacy::paper::construction2::{NebulaConfig, StackShape};
...
fn tampered_terminal_witness_fails_slice_opening() {
    let (prep, mut audit) = honest_chain();
    let neo_fold_legacy::paper::construction2::ProofState::Active { running, .. } = &mut audit.proof.state.proof else {
