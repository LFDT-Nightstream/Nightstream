...
fn production_claim_replay_active_overlay_compiler_disposition_census_is_exact() {
    ...
    for (arm_index, arm) in audit.rows().arms().iter().enumerate() {
        ...
        assert!(rewrites.iter().all(|rewrite| {
            matches!(
                rewrite.kind(),
                neo_fold_legacy::frontends::r1cs_f_prime::SelectiveRewriteKind::ShiftedTernaryCanonical
                    | neo_fold_legacy::frontends::r1cs_f_prime::SelectiveRewriteKind::LinearDefinition
            )
        }));
        let shifted_rows = rewrites
            .iter()
            .filter(|rewrite| {
                rewrite.kind() == neo_fold_legacy::frontends::r1cs_f_prime::SelectiveRewriteKind::ShiftedTernaryCanonical
            })
