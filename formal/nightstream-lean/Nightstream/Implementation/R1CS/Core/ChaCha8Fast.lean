...
list of canonical naturals. `ChaCha8Refinement` proves generic equality to the
pure model; Rust `rand_chacha` conformance remains a separate boundary.
-/

namespace Nightstream.Implementation.R1CS.ChaCha8Fast

def getWord (state : Array UInt32) (index : Nat) : UInt32 :=
  state.getD index 0

def rotateLeft32 (value : UInt32) (amount : Nat) : UInt32 :=
...
def quarterRound (state : Array UInt32)
...
def littleEndian32 (bytes : List Nat) (offset : Nat) : UInt32 :=
  UInt32.ofNat <|
    bytes.getD offset 0 +
...
def initialState (seed : List Nat) (block : Nat) : Array UInt32 :=
...
def blockWord32s (seed : List Nat) (block : Nat) : List UInt32 :=
...
def word32s (seed : List Nat) (wordStart count : Nat) : List UInt32 :=
...
def u64s (seed : List Nat) (wordStart count : Nat) : List Nat :=
  ...
  (List.range count).map fun index =>
    stream.getD (2 * index) 0 +
      4294967296 * stream.getD (2 * index + 1) 0
