...
Independent prover replay is a separate implementation goal. Its requirements are under each phase's **Independent prover replay** group and appear on the related Protocol flow boxes. A local theorem, implemented Rust function or verifier comparison does not by itself close these witness-derived replay checks. The phase table above records their actual scope and remaining work.

The site opens on [Protocol flow](protocol-flow.md). It shows the fold in six columns above the HyperNova augmented circuit F′. The lower row checks the prior fold and one application step. After F′ returns, the outer prover creates the next fresh assignment and commitment. The feedback arrows carry that fresh claim and the 16 running claims into the next iteration. The base branch bypasses the prior-state and fold checks. Operation boxes, exact-value handoffs and phase results each own their specific Proof, Link and Rust records. Every in-scope requirement is counted once; shared support is linked without extra credit. Select an element in the [interactive flow](https://nightstream-requirements.nicarq.chatgpt.site/#protocol-flow) for its statements, evidence and assumptions. These arrows describe protocol execution, not all Lean dependencies.
...
Requirements, status, assumptions and source evidence are below each group’s proof graph. The full tree also includes all records, grouped by subject. Saved requirement and group links open the same tree at the selected entry. The group selector includes the Stage 1 exclusions.
