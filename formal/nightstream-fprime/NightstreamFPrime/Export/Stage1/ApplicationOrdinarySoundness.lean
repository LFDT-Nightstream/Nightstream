...
namespace NightstreamFPrime.Export.Stage1.ApplicationOrdinarySoundness
...
private theorem decodedEnv_location {application : Program} {logicalWidth : Nat}
  ...
  | some located =>
      ...
      simp only [ApplicationOrdinaryPlan.sourceMap, found, same]
...
theorem rowsZero_iff_rowsHold {application : Program} {logicalWidth : Nat}
    ...
    (ApplicationOrdinaryPlan.plan fits geometry).RowsZero assignment ↔
...
theorem rowsZero_implies_step {application : Program} {logicalWidth : Nat}
    ...
    (assignment : Assignment F logicalWidth)
    (one : assignment (ApplicationOrdinaryGeometry.oneColumn geometry) = 1)
    (rows : (ApplicationOrdinaryPlan.plan fits geometry).RowsZero assignment) :
...
end NightstreamFPrime.Export.Stage1.ApplicationOrdinarySoundness
