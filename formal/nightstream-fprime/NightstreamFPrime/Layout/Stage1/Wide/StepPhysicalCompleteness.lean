...
theorem complete
  ...
  have acceptedSource : Nifs.PaperNonInteractive.verify (PiRLC.Wide.Key.key relation ajtai)
    ...
    exact accepted
  obtain ⟨nifs, nifsRows, nifsScope, nifsOutput, sourceReadback⟩ := PhysicalPrefixCompleteness.complete relation ajtai prior next output.x
  ...
  exact ⟨digestFixed, completed, physical, finalSpecs.2, finalOutput, finalReadback, finalPrior, finalNext⟩
