...
fn toy_instance_with_x_value(prep: &neo_fold_legacy::Preprocessing, x: neo_math::F) -> neo_fold_legacy::CcsInstance {
    let mut z = vec![neo_math::F::ZERO; prep.structure().m];
    z[0] = x;
    neo_fold_legacy::CcsInstance::from_low_norm_assignment(
        prep.params(),
...
fn wide_kernel_preprocessing() -> neo_fold_legacy::Preprocessing {
    let cols = neo_fold_legacy::config::KAPPA as usize + 1;
    let vars = D * cols;
    let structure = CcsStructure::new(vec![Mat::zero(1, vars, F::ZERO)], SparsePoly::new(1, vec![])).expect("wide CCS");
    let params = neo_fold_legacy::config::ccs_params(structure.n, structure.m, 1, 1).expect("wide-kernel params");
    support::install_ajtai_module(&params, &structure);
    neo_fold_legacy::preprocess(params, structure, Some(D)).expect("wide-kernel preprocessing")
}
...
fn commitment_kernel_vector(prep: &neo_fold_legacy::Preprocessing) -> Vec<F> {
...
fn one_batch_proof(prep: &neo_fold_legacy::Preprocessing, value: neo_math::F) -> neo_fold_legacy::Uncompressed {
    let proof =
        neo_fold_legacy::prove(prep, vec![vec![toy_instance_with_x_value(prep, value)]]).expect("one-batch proof");
    neo_fold_legacy::finish_uncompressed(prep, proof).expect("finish one-batch proof")
}
...
fn recompute_active_running_acc_digest(proof: &neo_fold_legacy::Uncompressed) -> [u8; 32] {
    let running = final_running(proof);
    if running.claims.is_empty() {
        neo_fold_legacy::paper::digest::AccumulatorHandle::empty().digest()
    } else {
        let parent = running
            .parent_authority
            .as_ref()
            .expect("non-empty running must carry parent authority");
        neo_fold_legacy::paper::digest::AccumulatorHandle::from_running_parts(2, &running.claims, Some(parent)).digest()
    }
}

fn running_acc_digest(running: &neo_fold_legacy::RunningInstance) -> [u8; 32] {
    if running.claims.is_empty() {
        neo_fold_legacy::paper::digest::AccumulatorHandle::empty().digest()
    } else {
        let parent = running
...
fn final_running_passes_witness_authority(
    prep: &neo_fold_legacy::Preprocessing,
    proof: &neo_fold_legacy::Uncompressed,
) {
    let running = final_running(proof);
    neo_fold_legacy::lifecycle::validate_final_witness_authority(prep, &running)
...
fn verify_uncompressed_audit_rejects_commitment_kernel_terminal_witness_forge() {
    ...
    let instance = neo_fold_legacy::CcsInstance::from_low_norm_assignment(
        prep.params(),
        prep.commitment_scheme(),
        prep.structure(),
    ...
    let audit = neo_fold_legacy::prove(&prep, vec![vec![instance]]).expect("prove wide instance");
    let mut finished = neo_fold_legacy::finish_uncompressed_with_audit(&prep, audit).expect("finish wide audit");
    neo_fold_legacy::verify_uncompressed_audit(&prep, &finished).expect("honest audit verifies");
    ...
    let err = neo_fold_legacy::verify_uncompressed_audit(&prep, &finished)
        .expect_err("audit verifier accepted a same-commitment forged terminal witness");
    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::FinalAccumulatorLowNormViolation { .. }
                | neo_fold_legacy::Error::FinalAccumulatorPublicInputMismatch { .. }
                | neo_fold_legacy::Error::FinalAccumulatorCeRelationViolation { .. }
        ),
...
fn verify_uncompressed_rejects_recorded_eval_a_tamper_even_after_redigest() {
    ...
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    ...
    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::PostStateMismatch
                | neo_fold_legacy::Error::FinalAccumulatorCeRelationViolation { .. }
        ),
...
fn verify_uncompressed_rejects_recorded_r_value_tamper_even_after_redigest() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 82)]])
        .expect("one-batch uncompressed proof");
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    neo_fold_legacy::verify_uncompressed(&prep, &finished).expect("honest proof verifies");
...
fn final_witness_authority_rejects_same_shape_r_c1_limb_relabel() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![toy_instance_with_x_value(&prep, neo_math::F::ONE)]])
...
fn verify_uncompressed_rejects_locally_valid_final_accumulator_substitution() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 83)]])
        .expect("one-batch uncompressed proof");
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    neo_fold_legacy::verify_uncompressed(&prep, &finished).expect("honest proof verifies");
...
fn verify_uncompressed_rejects_cross_history_terminal_fold_replay_while_final_accumulator_opens() {
    ...
    neo_fold_legacy::verify_uncompressed(&prep, &finished_a).expect("honest proof A verifies");
    neo_fold_legacy::verify_uncompressed(&prep, &finished_b).expect("honest proof B verifies");
    ...
    final_running_passes_witness_authority(&prep, &finished_a);
    let err = neo_fold_legacy::verify_uncompressed(&prep, &finished_a)
        .expect_err("verify_uncompressed accepted a terminal fold proof replayed from a different history");
    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::PostStateMismatch | neo_fold_legacy::Error::Construction2(_)
        ),
