...
/-!
Contract: exact `stateEqual` call recipe.

The recipe compares the two canonical state coordinate strings, emits the
...
private theorem footprint_exact
    ...
    (signature parameters).callFootprint Call.stateEqual =
      equalityFootprint profile.codecs.state.width := by
  simpa [signature, callFootprint] using profile.stateEqualFootprint

private theorem temporary_layouts_exact
    (parameters : Parameters)
    (profile : DirectProfile parameters) :
    ((signature parameters).callFootprint Call.stateEqual).temporaries =
      [auxiliaryLayout profile.codecs.state.width,
        auxiliaryLayout profile.codecs.state.width,
        auxiliaryLayout profile.codecs.state.width.pred] := by
...
private def frameTemporaries
    ...
    (frame :
      CallFrame (signature := signature parameters)
        profile.family Call.stateEqual
        (Refs.cons left (Refs.cons right .nil))) :
    LayoutBundles
      [auxiliaryLayout profile.codecs.state.width,
        auxiliaryLayout profile.codecs.state.width,
        auxiliaryLayout profile.codecs.state.width.pred] :=
  ...
  have width :=
    frame.outputWidthsAgree (Ports.auxiliaryBit parameters) (by
      change Ports.auxiliaryBit parameters ∈
        callOutputs parameters Call.stateEqual
...
private theorem semantic_equality_iff
  ...
  · intro coordinatesEqual
    apply profile.codecs.state.decoded_value_unique leftExact
    rw [coordinatesEqual]
    exact rightExact
  · intro valuesEqual
    have leftEncoding :=
      (profile.codecs.state.encode_decode _ _ leftExact).2
    have rightEncoding :=
      (profile.codecs.state.encode_decode _ _ rightExact).2
    calc
      (firstBinaryOperand frame.operands).values assignment =
          profile.codecs.state.encode left :=
        leftEncoding.symm
      _ = profile.codecs.state.encode right := by rw [valuesEqual]
...
/-- Certified physical recipe for the exact direct `stateEqual` call. -/
def stateEqualRecipe
    (parameters : Parameters)
    (profile : DirectProfile parameters) :
    CallRecipe (signature parameters) profile.family Call.stateEqual := by
  refine
    { rows := ?_
      rowCount := ?_
      rowsOwned := ?_
        ...
        | cons right tail =>
            ...
            change
              (frameRecipe parameters profile frame).rows.length =
                (equalityFootprint profile.codecs.state.width).recurringRows
    ...
    | cons leftRef tail =>
        ...
        | cons rightRef tail =>
            ...
            | cons left inputs =>
                ...
                | cons right inputs =>
                    cases inputs
                    change parameters.State at left right
                    have decodedPair :=
                    ...
                    have outputCoordinate :
                        assignment
                            (frameRecipe parameters profile frame).output.id =
                          if stateEqual parameters left right then 1 else 0 := by
                      rw [raw]
                      by_cases same : left = right
                      · rw [if_pos (equalityIff.mpr same)]
                        simp [stateEqual, same]
                      · rw [if_neg (fun equal => same (equalityIff.mp equal))]
                        simp [stateEqual, same]
                    refine
                      ⟨.cons (stateEqual parameters left right) .nil,
                        rfl, ?_⟩
                    apply (unaryOutput_decodes_iff profile.family assignment
                      frame.outputs (stateEqual parameters left right)).mpr
                    exact output_decodes parameters profile frame assignment
                      (stateEqual parameters left right) outputCoordinate
            ...
            | cons left inputs =>
                ...
                | cons right inputs =>
                    ...
                    | cons output outputs =>
                        ...
                        have outputEqual :
                            output = stateEqual parameters left right := by
                        ...
                        have decodedPair := And.intro
                          ((firstBinaryOperand frame.operands
                              ...
                              (.data .state) assignment left encodedPair.1)
                          ((secondBinaryOperand frame.operands
                            ).decodes_of_encodes profile.family
                              (.data .state) assignment right encodedPair.2)
                        have equalityIff :=
                          recipe_equality_iff parameters profile frame
                            assignment left right decodedPair.1 decodedPair.2
                        have outputEncoded :=
                          (unaryOutput_encodes_iff profile.family assignment
                            frame.outputs
                            (stateEqual parameters left right)).mp
                            outputsEncoded
                        have outputCoordinate :=
                          output_coordinate_of_encodes parameters profile frame
                            assignment (stateEqual parameters left right)
                            outputEncoded
