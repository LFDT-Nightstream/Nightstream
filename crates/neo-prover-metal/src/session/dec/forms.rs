...
use crate::session::dec_seeded::DeviceSeededFormPlan;
use crate::session::{command_gpu_duration, Buffer, MetalFeOraclePlan, MetalSession, MetalWitnessMasks};
...
pub(crate) struct MetalDecFormPlan {
    ...
    pub(super) active_blocks: Buffer,
    pub(super) active_chunk_bases: Buffer,
    pub(super) active_chunk_matrices: Buffer,
    pub(super) matrix_active_offsets: Buffer,
    pub(super) matrix_chunk_offsets: Buffer,
    pub(super) matrix_identity: Buffer,
    ...
    pub(super) active_block_count: usize,
    pub(super) active_chunk_count: usize,
    pub(super) explicit_coefficient_count: usize,
    pub(super) signed_unit_coefficient_count: usize,
    pub(super) explicit_form_list_histogram: [usize; 8],
...
pub(crate) struct MetalAjtaiRingForms {
    ...
    pub(super) cache_identity: usize,
    pub(super) matrix_digest: [u64; 4],
    pub(super) chi_words: Vec<u64>,
    pub(super) row_challenge_words: Vec<u64>,
    pub(super) n_eff: usize,
}
...
impl MetalAjtaiRingForms {
    pub(super) fn matches(&self, plan: &MetalDecFormPlan, chi_words: &[u64], n_eff: usize) -> bool {
    ...
    pub(super) fn matches_row_challenges(&self, plan: &MetalDecFormPlan, row_challenges: &[K], n_eff: usize) -> bool {
        self.blocks == plan.blocks
            && self.cache_identity == plan.cache_identity
            && self.matrix_digest == plan.matrix_digest
            && self.form_rows == 2 * plan.matrix_count
...
pub(super) struct DeviceSeededFormPatch {
    pub(super) bases: Buffer,
    pub(super) coefficients: Buffer,
    pub(super) entries: usize,
}

pub(super) struct DeviceFormBuild<'a> {
    pub(super) plan: &'a MetalDecFormPlan,
...
impl MetalSession {
    ...
    pub(crate) fn prepare_dec_ring_forms(
        ...
        let local_slots = checked_product(&[blocks, D], "Pi_DEC compact form slot count overflow")?;
        ...
        let layouts = matrices
            ...
            .map(|matrix| {
                ...
                if identity {
                    let identity_blocks = rows.div_ceil(D);
    ...
    pub(super) fn prepare_seeded_form_patch(
        &self,
    ...
    fn prepare_seeded_form_patch_from_forms(
        ...
        for (matrix, form) in forms.iter().enumerate() {
            for (block, real, imaginary) in form.to_sparse_block_coeffs() {
                ...
                let base = checked_product(&[start + relative, 2, D], "Pi_DEC seeded form base overflow")?;
    ...
    pub(crate) fn eval_ajtai_y_from_signed_masks(
        ...
        let seeded_patch_bytes = seeded_patch_entries.saturating_mul(size_of::<u64>() + 2 * D * size_of::<u64>());
    ...
    pub(in crate::session) fn encode_tensor_point_k(
        &self,
    ...
    fn encode_seeded_form_patch(
        ...
        if let Some(seeded) = seeded {
            ...
            self.dispatch(&encoder, &self.dec_add_sparse_ring_forms, seeded.entries * 2 * D);
            encoder.endEncoding();
        }
        Ok(())
    }
