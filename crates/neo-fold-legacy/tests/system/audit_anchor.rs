...
use neo_fold_legacy::paper::construction2::{ProofState, State, TRIVIAL_PC};
...
fn lifecycle_surface_compiles() {
    ...
    fn _surface_check(
        _: fn(
            neo_fold_legacy::Params,
            neo_fold_legacy::Structure,
            Option<usize>,
        ) -> Result<neo_fold_legacy::Preprocessing, neo_fold_legacy::Error>,
    ) {
    }
    _surface_check(neo_fold_legacy::preprocess);
}
...
fn verify_uncompressed_surface_compiles() {
    ...
    fn _finish_terminal(
        _: fn(
            &neo_fold_legacy::Preprocessing,
            neo_fold_legacy::UncompressedAudit,
        ) -> Result<neo_fold_legacy::Uncompressed, neo_fold_legacy::Error>,
    ) {
    }
    fn _finish_audit(
        _: fn(
            &neo_fold_legacy::Preprocessing,
            neo_fold_legacy::UncompressedAudit,
        ) -> Result<neo_fold_legacy::UncompressedAudit, neo_fold_legacy::Error>,
    ) {
    }
    fn _verify_terminal(
        _: fn(&neo_fold_legacy::Preprocessing, &neo_fold_legacy::Uncompressed) -> Result<(), neo_fold_legacy::Error>,
    ) {
    }
    fn _verify_audit(
        _: fn(
            &neo_fold_legacy::Preprocessing,
            &neo_fold_legacy::UncompressedAudit,
        ) -> Result<(), neo_fold_legacy::Error>,
    ) {
    }
    _finish_terminal(neo_fold_legacy::finish_uncompressed);
    _finish_audit(neo_fold_legacy::finish_uncompressed_with_audit);
    _verify_terminal(neo_fold_legacy::verify_uncompressed);
    _verify_audit(neo_fold_legacy::verify_uncompressed_audit);
}
...
fn uncompressed_terminal_type_excludes_audit_trail_at_compile_time() {
    ...
    fn _terminal_destructure(p: &neo_fold_legacy::Uncompressed) {
        let neo_fold_legacy::Uncompressed {
            state: _,
            final_fold: _,
        } = p;
    }
    ...
    // `UncompressedAudit`.
    fn _audit_destructure(a: &neo_fold_legacy::UncompressedAudit) {
        let neo_fold_legacy::UncompressedAudit {
            proof: _,
            steps: _,
            public_batches: _,
        } = a;
...
fn fold_schedule_step_count_is_correct() {
    use neo_fold_legacy::FoldSchedule;
