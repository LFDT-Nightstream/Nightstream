...
fn build_shout_only_bus_z(
    ...
    if ell_addr != 38 {
        return Err(format!(
            "build_shout_only_bus_z: expected ell_addr=38 for packed SLL (got ell_addr={ell_addr})"
        ));
    }
    ...
    for (lane_idx, cols) in shout.lanes.iter().enumerate() {
        ...
        for j in 0..t {
            ...
            if has_lookup {
                ...
                // [lhs_u32, shamt_bits[0..5], carry_bits[0..32]].
                ...
                for bit in 0..32 {
                    packed[6 + bit] = if ((carry >> bit) & 1) == 1 { F::ONE } else { F::ZERO };
                }
...
#[test]
fn riscv_trace_wiring_ccs_shared_cpu_bus_shout_sll_prove_verify() {
    // Program:
    // - LUI x1, 1        (x1 = 4096)
    // - SLLI x2, x1, 3   (x2 = 32768)
    // - SLLI x3, x1, 0   (x3 = 4096)
    // - HALT
    let program = vec![
    ...
    // Shout instance: SLL table, 1 lane.
    let t = exec.rows.len();
    let shout_table_ids = vec![RiscvShoutTables::new(32).opcode_to_id(RiscvOpcode::Sll).0];
    ...
    let sll_z = build_shout_only_bus_z(
        ccs.m,
        layout.m_in,
        t,
        /*ell_addr=*/ sll_lut_inst.d * sll_lut_inst.ell,
        /*lanes=*/ 1,
        &shout_lanes,
        &x,
    )
    ...
    let mut tr_prove = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sll");
    ...
    let mut tr_verify = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sll");
