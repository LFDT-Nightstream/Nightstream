...
namespace Nightstream.Protocol.FPrime.ConcretePhi81.Deviations.DelayedPackedYZcol.CombinedNc.Step
...
theorem accepted_next_implies_rawProjection_or_badEvent
    ...
    (accepted : FixedPhase.Accepted ops.toOps
      (sumcheckPolynomial nextContext.covers nextData coins
  ...
  · rcases semantic with ⟨nextNcTruth, parentScalar⟩
    have leftMatches :
        DelayedPackedProjection.PairLeftScalarMatches
          (DelayedProduction.outgoingPending previousContext
            previousCertificate).parentYZcol
          (authoritativeRunningProjection nextContext.covers
...
The accepted SumCheck terminal is the raw `DelayedCombinedNc.sumcheckPolynomial`;
...
end Nightstream.Protocol.FPrime.ConcretePhi81.Deviations.DelayedPackedYZcol.CombinedNc.Step
