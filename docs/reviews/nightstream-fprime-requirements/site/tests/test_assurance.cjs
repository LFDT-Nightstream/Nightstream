...
assert.deepEqual(progress('proof', counts(leaves.filter(n => n.id.startsWith('N.') && !n.id.startsWith('N.replay.')), 'proof')), {finished: 18, total: 18});
assert.equal(leaves.find(n => n.id === 'N.replay.composed').rust, 'tested_scoped');
assert.equal(leaves.find(n => n.id === 'N.replay.composed').proof, 'partial');
