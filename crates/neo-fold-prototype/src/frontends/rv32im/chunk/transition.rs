...
use crate::chunk_folding::{
    claim_digests, prove_superneo_chunk_step,
    ...
    ChunkReplayWitness, SuperNeoChunkStep,
};
...
pub(crate) fn prove_rv32im_chunk_transition_with_perf(
    ...
    let SuperNeoChunkStep {
        replay_witness,
        relation_result: proved,
        fold_digest: verified_fold_digest,
        perf,
    } = prove_superneo_chunk_step(
        transcript,
        params,
        structure,
        &handoff.chunk_input,
    ...
    .map_err(|err| SimpleKernelError::Proof(format!("RV32IM chunk transition {chunk_index} prove failed: {err}")))?;
