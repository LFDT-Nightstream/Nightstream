...
fn final_witness_authority_rejects_commitment_shape_before_attacker_sized_allocation() {
    ...
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 0)]]).expect("one-step proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finalized proof");
    ...
    let result = neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &running);
