...
Owns: the six top-level residual families obtained by grouping the exact 74
...
| `f_prime.selective_ccs.polynomial.centered` | unary centered residual, cancellation, and one packed residual pair | 10 | checked component |
...
| `f_prime.selective_ccs.polynomial.total` | sum of the six named residuals | 74 | derived |
-/

namespace Nightstream.Implementation.R1CS.SelectiveCcs.Polynomial.Components
...
/-- One logical centered-unit residual. -/
def centeredUnitResidual (value : F) : F :=
  value ^ 3 - value

/-- Exact ten-term centered syntax before the four row images are selected.
Its association follows the sparse evaluator, which keeps recomposition
independent of any ring-normalization tactic. -/
def centeredResidualAt (general evaluation left right : F) : F :=
  general * (left * left * left) +
    (-(general * left) +
      (-(general * (left * left * left) * evaluation) +
        (general * left * evaluation +
          (general * (left * left * left * left * left * left) * evaluation +
            (-(2 * general * (left * left * left * left) * evaluation) +
              (general * (left * left) * evaluation +
                (-(7 * general *
                    (right * right * right * right * right * right) *
                    evaluation) +
                  (14 * general * (right * right * right * right) * evaluation +
                    -(7 * general * (right * right) * evaluation)))))))))

/-- Centered residual for both lifecycle modes. When `E=0`, this is the old
unary equation. When `G=E=1`, the mixed terms cancel that equation and leave
one projective pair equation. -/
def centeredResidual (point : Fin 13 -> F) : F :=
  centeredResidualAt
    (point Role.generalSelector.index)
    (point Role.evalSelector.index)
...
the existing 74-term sparse evaluator. -/
theorem evaluate_eq_combinedResidual (point : Fin 13 -> F) :
  ...
  simp [evaluate, polynomial, terms, baseTerms, canonicalTerms,
    ...
    centeredResidual, centeredResidualAt,
    evaluationResidual, canonicalResidual, evaluateTerms,
    Role.index,
    Fin.one_mul, Fin.mul_one, Fin.zero_add, Lean.Grind.Fin.neg_mul]
  simp only [fadd_assoc]

end Nightstream.Implementation.R1CS.SelectiveCcs.Polynomial.Components
