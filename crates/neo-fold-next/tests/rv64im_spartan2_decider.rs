//! Focused tests for the live RV64IM main-recursion Spartan SNARK.
...
use neo_fold_next::rv64im::audit::{
    build_rv64im_ivc_recursion_snark_setup_shape_from_components, debug_check_rv64im_ivc_recursion_snark_circuit,
};
use neo_fold_next::rv64im::final_relation::prove_rv64im_final_statement_from_accepted;
use neo_fold_next::rv64im::main_proof::Rv64imCompressedMainProof;
use neo_fold_next::rv64im::{
    build_rv64im_accepted_proof_artifact, parity_source_cases, prove_rv64im_public_proof,
    setup_rv64im_ivc_snark_from_final, setup_rv64im_ivc_snark_from_final_cached, Rv64imChunkStepIvcStatement,
    Rv64imIvcPublicImage, Rv64imIvcSnark, Rv64imIvcSnarkVerifierKey, Rv64imProofInput, SimpleKernelError,
};
use neo_math::{F, K};
...
fn compressed_main_proof_fixture(
    ...
    Rv64imCompressedMainProof::from_final_artifacts(statement, final_proof, final_pc)
        .expect("build compressed main proof")
}

fn verify_compressed_main_proof(
    proof: &Rv64imCompressedMainProof,
...
fn expect_direct_public_image_tamper_rejected(
    ...
    vk: &Rv64imIvcSnarkVerifierKey,
    mutate: impl FnOnce(&mut Rv64imIvcPublicImage),
    message: &str,
) {
...
fn rv64im_spartan2_decider_setup_shape_is_transcript_seed_invariant() {
    ...
    let shape_a = build_rv64im_ivc_recursion_snark_setup_shape_from_components(
        &statement_a,
    ...
    let shape_b = build_rv64im_ivc_recursion_snark_setup_shape_from_components(
        &statement_b,
...
fn rv64im_spartan2_decider_setup_shape_rejects_tampered_public_statement_digest() {
    ...
    let err = build_rv64im_ivc_recursion_snark_setup_shape_from_components(
        &tampered_statement,
...
fn rv64im_spartan2_decider_debug_check_only() {
    let (_proof, statement, final_proof, _) = final_fixture("control_flow_jal_skip_ecall");
    debug_check_rv64im_ivc_recursion_snark_circuit(&statement, &final_proof)
        .expect("rv64im main-recursion SNARK circuit must be satisfied");
}
...
fn rv64im_spartan2_decider_setup_only() {
    let (_proof, statement, final_proof, _) = final_fixture("control_flow_jal_skip_ecall");
    let _ = setup_rv64im_ivc_snark_from_final(&statement, &final_proof).expect("setup rv64im recursion SNARK");
}
...
fn rv64im_spartan2_decider_cached_setup_reuses_same_final_seam() {
    ...
    let first = setup_rv64im_ivc_snark_from_final_cached(&statement, &final_proof)
        .expect("setup cached rv64im recursion SNARK");
...
fn rv64im_spartan2_decider_setup_from_shape_is_reproducible() {
    ...
    let (_first_pk, first_vk) =
        setup_rv64im_ivc_snark_from_final(&statement, &final_proof).expect("setup recursion SNARK");
...
fn rv64im_spartan2_decider_cached_shape_setup_is_deterministic() {
    let (_proof, statement, final_proof, _) = final_fixture("control_flow_jal_skip_ecall");
    let first = setup_rv64im_ivc_snark_from_final_cached(&statement, &final_proof).expect("cached recursion setup");
    let second = setup_rv64im_ivc_snark_from_final_cached(&statement, &final_proof).expect("cached recursion setup");
    assert!(
        Arc::ptr_eq(&first, &second),
...
fn rv64im_spartan2_decider_round_trip_without_replay_verifier_input() {
    ...
    let (_pk, vk) = setup_rv64im_ivc_snark_from_final(&statement, &final_proof).expect("setup rv64im recursion SNARK");
    let compressed_main_proof = compressed_main_proof_fixture(&statement, &final_proof, proof.statement.final_pc);
...
#[test]
#[ignore = "expensive: main-recursion Spartan setup exceeds developer-memory budget"]
fn rv64im_spartan2_decider_rejects_public_image_and_proof_tampers() {
    let (proof, statement, final_proof, _nightstream_statement) = final_fixture("control_flow_jal_skip_ecall");
    let (_pk, vk) = setup_rv64im_ivc_snark_from_final(&statement, &final_proof).expect("setup rv64im recursion SNARK");
    let compressed_main_proof = compressed_main_proof_fixture(&statement, &final_proof, proof.statement.final_pc);
...
#[test]
#[ignore = "expensive: main-recursion Spartan setup exceeds developer-memory budget"]
fn rv64im_spartan2_decider_rejects_coherent_terminal_metadata_tamper_at_snark_boundary() {
    let (proof, statement, final_proof, _nightstream_statement) = final_fixture("control_flow_jal_skip_ecall");
    let (_pk, vk) = setup_rv64im_ivc_snark_from_final(&statement, &final_proof).expect("setup rv64im recursion SNARK");
    let compressed_main_proof = compressed_main_proof_fixture(&statement, &final_proof, proof.statement.final_pc);
...
#[test]
#[ignore = "expensive: main-recursion Spartan setup exceeds developer-memory budget"]
fn rv64im_spartan2_decider_direct_snark_verify_rejects_public_boundary_tampers() {
    let (proof, statement, final_proof, _nightstream_statement) = final_fixture("control_flow_jal_skip_ecall");
    let (_pk, vk) = setup_rv64im_ivc_snark_from_final(&statement, &final_proof).expect("setup rv64im recursion SNARK");
    let compressed_main_proof = compressed_main_proof_fixture(&statement, &final_proof, proof.statement.final_pc);
...
fn rv64im_spartan2_decider_rejects_terminal_proof_from_unrelated_chain() {
    ...
    let (_pk, vk) = setup_rv64im_ivc_snark_from_final(&statement_a, &final_proof_a).expect("setup recursion SNARK");
