    ...
    for (lane_idx, cols) in shout.lanes.iter().enumerate() {
        ...
        for j in 0..t {
            ...
            if has_lookup {
                ...
                // Signed compare via biased-unsigned compare (flip sign bit).
...
#[test]
fn riscv_trace_wiring_ccs_shared_cpu_bus_shout_slt_prove_verify() {
    // Program:
    ...
    // Shout instance: SLT table, 1 lane.
    let t = exec.rows.len();
    ...
    let slt_z = build_shout_only_bus_z(
        ccs.m,
        layout.m_in,
        t,
