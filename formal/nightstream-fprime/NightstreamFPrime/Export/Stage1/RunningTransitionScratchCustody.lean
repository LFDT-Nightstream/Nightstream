...
theorem retainedNonTransitionSource_outside (application : Stage1.Application.Program)
  ...
  | applicationLocal =>
      right
      have bound := ApplicationRetainedBlocks.source_after_localStart application slot
      change scratchEnd ≤ ((ApplicationRetainedBlocks.localBlock application).source slot).val
