//! Owns verifier-side bridge checks between the public RV32IM proof API and the private simple-kernel export.

use super::main_lane_artifact::build_simple_kernel_main_lane_artifact_from_summary;
use super::perf_diagnostics::{Rv32imPublicProofVerifyPerf, SimpleKernelBuildPerf};
use super::proof_api::Rv32imProof;
...
use super::{
    build_main_lane_surface, rv32im_simple_root_context_id_for_schedule, AjtaiFamilyKind, RootLaneColumns,
    RootLaneCommitmentSummaryArtifact, Rv32imMainLaneSurface, SelectedOpeningRef, SimpleKernelError,
...
fn validate_main_lane_opening_ref(
        ...
        || reference.expected_digest() != reference.digest
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM main-lane surface selected opening ref is inconsistent".into(),
        ));
    }
    Ok(())
}
...
fn validate_root_lane_columns_opening_ref(
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM root-lane columns selected opening ref is inconsistent".into(),
        ));
    }
    Ok(())
}
...
fn validate_public_bundle_digests(proof: &Rv32imProof) -> Result<(), SimpleKernelError> {
    ...
    if proof.kernel.stage_claims.summary != expected_stage_claims.summary {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-claim summary bundle does not match the carried stage claims".into(),
        ));
    }
    ...
    if proof.kernel.stage_packages.summary.digest != proof.kernel.stage_packages.summary.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-package digest bundle mismatch".into(),
        ));
    }
    if proof.kernel.stage_packages.digest != proof.kernel.stage_packages.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-package proof bundle digest mismatch".into(),
        ));
    }
    ...
    if proof.kernel.stage_packages.summary != expected_stage_packages {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-package summary bundle does not match the carried stage packages".into(),
        ));
    }
    if proof.kernel.kernel_opening.bindings.digest != proof.kernel.kernel_opening.bindings.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-opening binding bundle digest mismatch".into(),
        ));
    }
    if proof.kernel.kernel_opening.digest != proof.kernel.kernel_opening.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-opening proof bundle digest mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-opening summary bundle does not match the carried opening proof".into(),
        ));
    }
    if proof.kernel.kernel_claims.digest != proof.kernel.kernel_claims.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-claim proof bundle digest mismatch".into(),
        ));
    }
    ...
    if proof.kernel.kernel_claims.summary != expected_kernel_claims.summary {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-claim summary bundle does not match the carried kernel claims".into(),
        ));
    }
    ...
    if proof.kernel.main_lane.binding.digest != proof.kernel.main_lane.binding.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM main-lane proof binding digest mismatch".into(),
        ));
    }
    if proof.kernel.main_lane.digest != proof.kernel.main_lane.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM main-lane proof bundle digest mismatch".into(),
        ));
    }
    ...
    if proof.kernel.digest != proof.kernel.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel proof bundle digest mismatch".into(),
        ));
    }
    Ok(())
}

fn validate_public_bundle_bindings(proof: &Rv32imProof) -> Result<(), SimpleKernelError> {
    let derived_main_lane_surface = build_main_lane_surface(&proof.kernel.root_lane_columns);
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM joint-opening claim does not bind the expected public claims".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM accepted proof claim does not bind the expected public statement and proof digests".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM main-lane claim does not bind the expected public main-lane proof bundle".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-opening claim does not bind the expected stage and kernel terminal digests".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM root0 claim does not bind the expected carried kernel claim bundle".into(),
        ));
    }
    if proof.kernel.trace.execution_digest != proof.kernel.kernel_claims.execution_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM trace proof bundle execution digest does not match the kernel-claim proof bundle".into(),
        ));
    }
    if proof.kernel.stages.summary.stage1_row_count != proof.kernel.trace.shape.execution_row_count {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-witness proof bundle stage1 count does not match the trace row count".into(),
        ));
    }
    if proof.kernel.stages.summary.stage3_continuity_count != proof.kernel.trace.shape.real_row_count {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-witness proof bundle continuity count does not match the real trace row count".into(),
        ));
    }
    if proof.kernel.stages.summary.stage3_halted != proof.kernel.kernel_claims.halted() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-witness proof bundle halted flag does not match the kernel-claim proof bundle".into(),
        ));
    }
    Ok(())
}

fn validate_public_witness_bundle_digests(proof: &Rv32imProof) -> Result<(), SimpleKernelError> {
    if proof.witness.digest != proof.witness.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM proof witness bundle digest mismatch".into(),
        ));
    }
    if proof.witness.trace.shape.digest != proof.witness.trace.shape.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM trace witness shape bundle digest mismatch".into(),
        ));
    }
    if proof.witness.trace.digest != proof.witness.trace.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM trace witness bundle digest mismatch".into(),
        ));
    }
...
fn validate_public_witness_bindings(proof: &Rv32imProof) -> Result<(), SimpleKernelError> {
    if proof.witness.root_params_id != proof.kernel.root_params_id
        || proof.witness.root_params_id != proof.statement.root_params_id
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM proof witness bundle root-context binding mismatch".into(),
        ));
    }
    if proof.witness.trace.projection() != proof.kernel.trace {
        return Err(SimpleKernelError::Bridge(
            "RV32IM trace witness bundle does not match the carried public trace projection".into(),
        ));
    }
    if proof.witness.stages.projection_bundle() != proof.kernel.stages {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage witness bundle does not match the carried public stage projection".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-claim proof bundle does not match the carried proof witness bundle".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage-package proof bundle does not match the carried proof witness bundle".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-opening proof bundle does not match the carried proof witness bundle".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM kernel-claim proof bundle does not match the carried proof witness bundle".into(),
        ));
    }
    if proof.witness.root_lane_columns != proof.kernel.root_lane_columns {
        return Err(SimpleKernelError::Bridge(
            "RV32IM root-lane columns do not match the carried proof witness bundle".into(),
        ));
    }
    if proof.witness.root_lane_commitment != proof.kernel.root_lane_commitment {
        return Err(SimpleKernelError::Bridge(
            "RV32IM root-lane commitment does not match the carried proof witness bundle".into(),
        ));
    }
    Ok(())
}

fn public_kernel_from_proof(proof: &Rv32imProof) -> PublicSimpleKernelOutput {
...
fn public_sidecar_from_proof(proof: &Rv32imProof) -> PublicSimpleKernelWitnessSidecar {
...
> {
    ...
    if proof.statement.root_params_id != expected_root_params_id {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof root params id does not match the declared fold schedule and public-step count".into(),
        ));
    }

    if proof.kernel.trace != kernel.trace {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof trace summary does not match the canonical trace summary".into(),
        ));
    }
    if proof.kernel.stages != kernel.stages {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof stage summary does not match the canonical stage summary".into(),
        ));
    }
    ...
    if proof.kernel.stage_claims.summary_bundle() != expected_stage_claims {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof stage-claim summary does not match the canonical stage claims".into(),
        ));
    }
    if proof.kernel.stage_claims.claims != kernel.stage_claims {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof stage claims do not match the canonical stage claims".into(),
        ));
    }
    let expected_stage_packages = stage_package_summary_bundle_from_packages(&kernel.stage_packages);
    if proof.kernel.stage_packages.summary != expected_stage_packages {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof stage-package summary does not match the canonical stage packages".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof kernel-opening summary does not match the canonical kernel opening".into(),
        ));
    }
    ...
    if proof.kernel.kernel_claims.summary_bundle() != expected_kernel_claims {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof kernel-claim summary does not match the canonical kernel claims".into(),
        ));
    }
    if proof.kernel.kernel_claims.claims != kernel.kernel_claims {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof kernel claims do not match the canonical kernel claims".into(),
        ));
    }
    if proof.kernel.root_lane_columns != kernel.root_lane_columns {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof root-lane columns do not match the canonical committed columns".into(),
        ));
    }
    if proof.kernel.root_lane_commitment != kernel.root_lane_commitment {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof root-lane commitment does not match the canonical commitment artifact".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM main-lane proof binding does not match the accepted root-lane artifact".into(),
        ));
    }
...
) -> Result<(PublicSimpleKernelOutput, Rv32imPublicProofVerifyPerf), SimpleKernelError> {
...
pub(super) fn validate_public_proof_against_input_with_perf(
    ...
    let total_started = Instant::now();
    if proof.statement.initial_pc != u64::from(input.source.start_pc) {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof initial pc does not match the public input entrypoint".into(),
        ));
    }
    ...
    if sidecar.trace != rebuilt_sidecar.trace || sidecar.stages != rebuilt_sidecar.stages {
        return Err(SimpleKernelError::Bridge(
            "RV32IM public proof witness does not match the canonical build from public input".into(),
        ));
    }
