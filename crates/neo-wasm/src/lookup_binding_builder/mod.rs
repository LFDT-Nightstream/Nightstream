...
use super::layout::{
    ...
    COL_CALL_STACK_POP_RETURN_PC, COL_CALL_STACK_PUSH_PRESENT, COL_CALL_STACK_RETURN_PC_CHOICE, COL_CI_ENTRY_LIVE,
    COL_CI_LIVE, COL_CONTROL_CHOICE, COL_CURRENT_FUNCTION_NUM_LOCALS, COL_CURRENT_FUNCTION_REF, COL_EXPECTED_TYPE_ID,
    COL_FUNCTION_REF, COL_FUNCTION_TYPE_ID, COL_GLOBAL_INDEX, COL_GLOBAL_VALUE, COL_GLOBAL_VALUE_HI, COL_HALTED,
    ...
    COL_OP_TABLE_ENABLED, COL_OP_TABLE_ID, COL_OP_TABLE_VALUE, COL_OUTPUT_CAPTURED, COL_OUTPUT_ENABLED_AFTER,
    ...
    COL_TABLE_VALUE, COL_TARGET_FUNCTION_IS_GUEST, COL_TRAPPED_AFTER, COL_TRAPPED_BEFORE, COL_WIDE_VALUES_ENABLED,
    WITNESS_WIDTH,
};
...
fn build_wasm_lookup_binding_layout_uncached() -> WasmLookupBindingLayout {
    ...
    let memories = vec![
        ...
        rom_read_spec(
            ...
            // Active exactly when the call_indirect table entry is non-null;
            // a null entry (funcref 0) has no ROM row, but its type id must
            // stay bound on type-mismatch trap rows.
            WasmMemoryActivation::BooleanGate(Column(COL_CI_ENTRY_LIVE)),
        ),
        rom_read_spec(
        ...
        WasmMemorySpec {
            ...
            columns: vec![
                ...
                WasmMemoryColumnSpec {
                    ...
                    value_column: function_types.is_guest,
                    kind: WasmMemoryColumnKind::Read,
                    // De-gated on call_indirect trap rows (null entry or
                    // type mismatch): no call happens, so the callee
                    // metadata is unread and unconstrained.
                    activation: WasmMemoryActivation::BooleanGate(Column(COL_CI_LIVE)),
                },
            ],
            is_rom: true,
        },
        ...
        rom_read_spec(
            ...
            WasmMemoryActivation::BooleanGate(Column(COL_CI_LIVE)),
        ),
        rom_read_spec(
            "pc_edge_kinds",
            vec![state.pc_before],
