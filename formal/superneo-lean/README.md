...
- `SuperNeo/P13P14Golden.lean`: Lean exporter for eval-link/eval-hom conformance vectors
- `SuperNeo/Generated/P13P14Golden.csv`: Lean-generated P13/P14 conformance vectors for Rust
...
## Regenerate P13/P14 Vectors From Lean (Lean -> Rust)
...
## Paper-Protocol Coverage (`docs/superneo-paper`)

This README tracks substantial math components (`P1..P21`), but the full interactive protocol layer from the paper is only partially modeled today.

| Paper module | Paper location | Current Lean coverage | Main Lean files | Remaining gap |
|---|---|---|---|---|
| `SumCheck(T;Q)` | Def. 6 (`04_4_Preliminaries.md`), Sec. 7 (`07_7_Neo_s_folding_scheme_for_CCS.md`) | In progress (protocol shell + round-consistency surfaces) | `SuperNeo/Sumcheck.lean`, `SuperNeo/EqPoly.lean`, `SuperNeo/MLE.lean`, `SuperNeo/PolyLemmas.lean` | Missing paper-exact prover/verifier round constraints and full soundness/completeness instantiation without placeholder assumption boundaries. |
| `Π_CCS : CCS × CE -> CE^2` | Sec. 2.4 + Sec. 7.3 (`02_2_Technical_overview.md`, `07_7_Neo_s_folding_scheme_for_CCS.md`) | In progress (strong-IR theorem shell) | `SuperNeo/PiCCS.lean`, `SuperNeo/ProtocolRelations.lean`, `SuperNeo/ProtocolReduction.lean`, `SuperNeo/P20.lean`, `SuperNeo/P21.lean` | Remaining gap is paper-exact claim/transcript encoding and extractor-grade soundness statement. |
| `Π_RLC : CE^2 -> CE` | Sec. 2.4 + Sec. 7.4 (`02_2_Technical_overview.md`, `07_7_Neo_s_folding_scheme_for_CCS.md`) | In progress (weak-IR theorem shell) | `SuperNeo/PiRLC.lean`, `SuperNeo/EvalLink.lean`, `SuperNeo/EvalHom.lean`, `SuperNeo/ProtocolRelations.lean` | Remaining gap is paper-exact fold relation and full weak-IR theorem instantiation. |
| `Π_DEC` (final norm/decomposition reduction) | Sec. 2.4 + Sec. 7.5 (`02_2_Technical_overview.md`, `03_3_Overview_of_the_following_sections.md`) | In progress (upgrade/final-target shell) | `SuperNeo/PiDEC.lean`, `SuperNeo/Decomp.lean`, `SuperNeo/Norm.lean`, `SuperNeo/InvertibilityAxioms.lean` | Remaining gap is full reduction-of-knowledge theorem with paper quantifiers and extractor-facing conclusions. |
| Strong/Weak Interactive Reductions + composition theorem | Sec. 6 (`06_6_Strong_and_weak_interactive_reductions.md`) | In progress (composition theorem shell) | `SuperNeo/InteractiveReductions.lean`, `SuperNeo/ProtocolTheorem.lean` | Remaining gap is paper-exact Theorem 6 phrasing and generic IR framework formalization (beyond current concrete chain wrappers). |

Practical reading:
- `P1..P21` currently gives strong coverage of the algebraic core and protocol-facing skeletons.
- Paper-level interactive reduction security proofs are still an open workstream and are now explicitly tracked above.

## Full-Proof Completion Contract

This section is normative for repository planning:
- SuperNeo is considered **fully proven in Lean** only when every required item in `Math Breakdown (Current Status)` is marked `Done`, including protocol/security items (`M21+`), and all stated exit criteria are satisfied.
- `all_checks=true` is necessary for regression confidence, but **not sufficient** for full proof completion.
- Remaining assumption boundaries (for example, explicit hardness or trusted interfaces) must be either:
  - replaced with proved theorems in this repo, or
  - explicitly documented as accepted external assumptions in the final theorem statement.
...
| `P16` | `InvertibilityAxioms.lean` | Theorem 8 invertibility preconditions and interface boundary. | `P2`, `P5` | `P17`, `P20`, `P21` | Preconditions proven computationally; theorem-native challenge-subtraction window/invertibility bridges added; explicit `..._of_assumption` APIs and protocol-context trusted boundary (`hLowNormInvertibility`) are now in place; generic-`B` P5->P16 window/invertibility handoff lemmas are available for multiplication-bound premises, including direct operand-assumption wrappers (`withinInvertibilityWindow_mulRq_of_operand_norm_assumptions{,_inRange}`, `invertible_mulRq_of_operand_norm_assumptions{,_inRange}_of_assumption`) plus new raw-coeff assumption surfaces (`withinInvertibilityWindow_mulRq_of_operand_norm_assumptions_rawCoeff{,_fieldOp}`, `invertible_mulRq_of_operand_norm_assumptions_rawCoeff{,_fieldOp}_of_assumption`), and concrete Goldilocks-`B` instantiations (`GoldilocksRawNormBoundAssumption`, `GoldilocksRawInRangeBoundAssumption`, `GoldilocksRawCoeffBoundAssumption`, `GoldilocksRawCollapseAssumption`, `GoldilocksFieldOpCollapseAssumption`, `withinInvertibilityWindow_mulRq_of_goldilocks_operand_{assumptions,fieldOp_assumptions,rawCoeff_assumptions,rawCoeff_fieldOp_assumptions}{,_inRange}`), with explicit norm↔in-range conversion for Goldilocks raw-bound assumptions (including raw-coeff conversion bridges); core invertibility still axiomized. |
| `P17` | `SamplingSet.lean` | Definition 17 + Theorem 9 expansion-factor interface. | `P5`, `P16` | `P20`, `P21` | Rust parity + bound checks pass; theorem-native expansion constructors now include a Theorem-9-shaped path `empiricalExpansionFactor ≤ theorem9UpperBound (maxRhoNorm cset)` derived from operand norm assumptions, plus `samplingSetBoundProp` and `samplingSetBoundCheck_iff_prop` bridges; operand-bound constructors now expose raw-coeff assumption variants (`empiricalExpansionFactor_le_of_operand_norm_assumptions_rawCoeff`, `..._fieldOp_rawCoeff`) in addition to norm/in-range forms; Goldilocks wrappers now cover raw/norm/in-range/field-op and raw-coeff paths (`empiricalExpansionFactor_le_of_goldilocks_operand_rawCoeff_{assumptions,fieldOp_assumptions}` + matching `samplingSetBoundCheck_true_*` bridges); universal theorem pending. |
...
| `P5` | In progress (added theorem-native base bounds `normInf* <= q/halfQ`, reusable entry/row bound combinators, operation-sensitive inequalities for add/scale/mul/sub over field/coeff vectors, direct SuperNeo-operation wrappers for `superneoBarBlock`/`barLiftVec`/`barLiftMatrix`, challenge-compositional bounds `<= 4`, compositional constructors at both entry/coefficient and operand-norm layers for add/sub/scale, explicit add/sub/scale operand-assumption bundles (`vecAddNormBoundFromOperands`, `coeffSubNormBoundFromOperands`, `vecScaleNormBoundFromOperands`) with op-bound and monotone/`halfQ` constructors, coeff-spec-driven multiplication, explicit raw-coefficient bridge lemmas for multiplication, and a new assumption-free concrete raw-coefficient fallback path at `halfQ`; plus index-robust raw-coefficient support (`mulRqRawCoeffSpec_eq_zero_of_ge`), public raw-array interface (`mulRqRawCoeffs`), in-range/raw-array lifting APIs (`normInfF_mulRqRawCoeffSpec_le_of_inRangeBound`, `normInfCoeffs_mulRq_le_of_rawCoeffInRangeBound`, `normInfCoeffs_mulRq_le_of_norm_bounds_via_rawCoeff_inRange`, `normInfF_mulRqRawCoeffSpec_le_of_rawCoeffsNorm`, `normInfCoeffs_mulRqRawCoeffs_le_of_inRangeBound`, `normInfCoeffs_mulRq_le_of_rawCoeffsNorm`, `normInfCoeffs_mulRq_le_of_norm_bounds_via_rawCoeffsNorm`), equivalence bridges (`normInf_mulRqRawCoeffSpec_inRange_iff_rawCoeffsNorm`, `mulRqRawNormBoundFromOperands_iff_inRange`), ready-made `halfQ` wrappers for those paths (`..._halfQ`, `..._halfQ_le`), raw/collapse monotonicity APIs (`mulRqRaw{Norm,InRange}BoundFromOperands_mono`, `raw{Add,Sub,AddSub,FieldOp}CollapseBound_mono`) with halfQ-lift constructors (`mulRqRaw{Norm,InRange}BoundFromOperands_of_halfQ_le`, `raw{Add,Sub,AddSub,FieldOp}CollapseBound_of_halfQ_le`), explicit coeff-bound assumption surface and bridges (`mulRqRawCoeffBoundFromOperands`, `mulRqRawCoeffBoundFromOperands_of_inRange`, `mulRqRaw{InRange,Norm}BoundFromOperands_of_rawCoeff`, `normInfCoeffs_mulRq_le_of_operand_norm_assumptions_rawCoeff{,_fieldOp}`), and coefficient-access norm helpers (`normInfF_getCoeff_le_of_normInfCoeffs`, `normInfF_getElemBang_le_of_normInfCoeffs`)). | Remaining work is mainly tighter-than-`halfQ` multiplication/field-operation bounds from operand norms (current fully internal raw-coefficient fallback is intentionally coarse). | Norm obligations in P16/P17 are now consumable theorem-first through raw-bound and generic-`B` APIs (not check-only side conditions). |
| `P6` | In progress (both `splitRoundTrip_sound`/`complete`, full bool↔prop bridges for decomposition/per-digit bounds, monotonicity lemmas, structural split/recompose shape invariants (`splitBalancedVec_size`, `splitBalancedVec_row_size`, `splitRoundTrip_digits_size`, `splitRoundTrip_recompose_size_eq`, `splitRoundTrip_digit_row_size`), and direct quantified extraction lemmas from successful checks are now available; P20/P21 now expose direct digit-row-width projections via `p20DecompProp_digit_row_size`, `p20ArithmeticBundle_decomp_digit_row_size`, `p21ProtocolTarget_decomp_digit_row_size`, `p21FullMathTarget_decomp_digit_row_size`). | Still missing a fully constructive theorem layer that derives reconstruction and per-digit constraints from decomposition definitions directly (not only via check predicates). | Universal decomposition theorem with bound guarantees and direct reuse in P20/P21 without check wrappers. |
| `P7` | In progress (bool↔prop interfaces for Boolean-point predicates and hypercube-indicator checks are now available, with direct proposition extraction lemmas for size/Boolean guards/evaluation cases). | Still missing the fully quantified selector theorem over all Boolean vectors without relying on check wrappers. | Full theorem for Boolean selector behavior of `eq` over `{0,1}^ell`. |
| `P8` | In progress (bool↔prop bridge layer for `mleIdentity` is now expanded with iff, explicit size-form proposition bridge, and direct proposition extraction lemmas in addition to check-extraction lemmas for size/equality). | Still missing quantified theorem over all valid `v,r` (beyond check-bridge interface). | Quantified theorem equating inner-product and folding MLE formulations. |
...
| `P16` | In progress (critical boundary identified; precondition bridge in P20 now uses theorem constants directly, invertibility-window bool has sound/complete theorem bridges, direct invertibility extraction lemmas are available, challenge-coefficient norm bounds imply `withinInvertibilityWindow` for both direct vectors and `coeffSub` differences, `invertible_of_*_of_assumption` APIs are available, protocol context carries `hLowNormInvertibility` as an explicit trusted interface, protocol-side `invDelta` now threads through P20/P21 with assumption-driven inverse witness extraction, unparameterized global invertibility axiom wrappers were removed, and new P5->P16 generic-`B` bridges for multiplication-bound handoff are in place (`withinInvertibilityWindow_of_norm_le_of_lt`, `withinInvertibilityWindow_mulRq_of_{norm_le_of_lt,rawCoeffInRangeBound,rawCoeffsNorm,norm_bounds_via_rawCoeffInRange,norm_bounds_via_rawCoeffsNorm}`, plus invertibility extraction helpers for raw-bound premises). | Theorem 8 itself is still not formally proven in Lean; invertibility remains an explicit input assumption. | Replace assumption boundary with proved theorem, or keep as explicit trusted contract with documented provenance. |
...
| M16 | Theorem 8 low-norm invertibility | `SuperNeo/InvertibilityAxioms.lean` | Add explicit assumption boundary and concrete precondition checks | Appendix B.2 constants + D.7 bound interface | Captures the invertibility condition required by SuperNeo’s soundness-critical reduction step. | In progress (explicit assumption boundary + preconditions implemented; theorem bridges from all-challenge direct and subtraction norms to window/invertibility added; generic-`B` multiplication-bound -> window/invertibility handoff lemmas are now available for P5 integration, including concrete Goldilocks-`B` assumption bundles/wrappers; no global theorem axiom wrapper) |
| M17 | Definition 17 + Theorem 9 (`C`, expansion factor) | `SuperNeo/SamplingSet.lean` | Formalize set conditions and expansion-factor theorem interface | generated Rust `samplingCases` | Formalizes sampling guarantees that control error amplification in SuperNeo analysis. | In progress (implementation + parity passing; theorem-native Theorem-9-shaped constructors and check↔prop bridges are now in place, including field-op collapse surfaces; full universal theorem pending) |
...
| M21 | SumCheck protocol formalization (Def. 6) | `SuperNeo/Sumcheck.lean` | Formalize transcript, completeness, and soundness interface over field/extension-field setting used by paper | Optional transcript-level vectors (if useful) | Required protocol primitive for `Π_CCS` theorem, not just helper polynomial checks. | In progress (module now includes transcript/claim/result shells, soundness/completeness boundaries, protocol bundle, root-table check↔prop bridge, explicit per-round consistency surfaces (`sumcheckLineEval01`, `sumcheckLineSum01`, `SumcheckRoundShape`, `SumcheckRoundConsistency`, `SumcheckAcceptedStrongProp`, `SumcheckStrongSoundnessAssumption`), a theorem-native projection layer for round-consistency/strong-acceptance obligations (`sumcheckRoundConsistency_*`, `sumcheckAcceptedStrong_*`), and a standard→strong soundness conversion (`sumcheckStrongSoundnessAssumption_of_soundnessAssumption`); remaining gap is paper-faithful round-polynomial constraint completeness). |
| M22 | CCS / CE relation formalization (Defs. 12/13 style) | `SuperNeo/ProtocolRelations.lean` (+ planned relation refinements) | Complete relation definitions and theorem-native equivalence to current skeleton predicates | Existing check harness + relation sanity examples | Needed so reductions are proved against paper relations, not only internal shell predicates. | In progress (explicit paper-facing predicates are now present: `CCSRelation`, `CERelation`, `CERelationRelaxed`, with bridge lemmas (`ceValid_iff_relations`, `ccsRelation_of_ceValid`, `ceRelation_of_ceValid`, `ceValid_of_relations`, `ceRelationRelaxed_of_ceRelation`, `ceRelation_of_relaxed_and_norm`) and protocol-target lift lemmas (`p21ProtocolTarget_to_CERelation`, `p21FullMathTarget_to_CCSRelation`, `p21FullMathTarget_to_CERelation`); remaining work is full paper-definition alignment/details for Defs. 12/13 and extractor-facing relation variants used by `Π_CCS/Π_RLC/Π_DEC`). |
| M23 | `Π_CCS` strong interactive reduction theorem | `SuperNeo/PiCCS.lean` + `ProtocolReduction.lean` | Prove completeness + strong-IR properties in paper form, using sumcheck and helper polynomials | Derived checks from existing arithmetic stack | First major protocol reduction in SuperNeo security chain. | In progress (theorem-native `PiCCS` skeleton now includes both minimal and round-consistent SumCheck paths, concrete bridge lemmas from `p20ForClaim` (`PiCCSP20{,Strong}LinkAssumption` -> `PiCCSArithmetic{,Strong}LinkAssumption`), a named protocol-check bundle layer (`PiCCSProtocolChecks`) with assumption conversions (`PiCCS{,Strong}CheckBundleLinkAssumption` <-> legacy conjunction assumptions), check-bundle -> `p20ForClaim` lifting (`p20ForClaim_of_piCCSProtocolChecks`), direct bundle-protocol assumptions (`PiCCS{,Strong}CheckBundleProtocolAssumption`) with conversion into standard/strong protocol assumptions, and protocol wrappers that accept strong transcripts under standard assumptions; remaining work is paper-exact claim encoding and full strong-IR theorem instantiation). |
| M24 | `Π_RLC` weak interactive reduction theorem | `SuperNeo/PiRLC.lean` + `P21/ProtocolReduction` integration | Prove weak-IR properties and relaxed-witness behavior in paper form | Existing eval-hom + matrix/eval checks | Second protocol reduction composing with `Π_CCS`. | In progress (new theorem-native `PiRLC` skeleton added with weak relation boundary (`PiRLCWeakRelationAssumption`), norm-rebind boundary (`PiRLCNormRebindAssumption`), protocol bundle, CE/relaxed bridge theorems (`piRLCWeakIR_*`), and composition hook consuming `PiCCS` outputs; remaining work is paper-exact fold relation/constraints and full weak-IR instantiation). |
| M25 | `Π_DEC` final reduction-of-knowledge theorem | `SuperNeo/PiDEC.lean` + decomposition/norm integration | Prove final norm/decomposition reduction and extraction claims | Decomp + norm checks as regression support | Final reduction in `Π_SuperNeo := Π_DEC ∘ Π_RLC ∘ Π_CCS`. | In progress (new theorem-native `PiDEC` skeleton added with relaxed→CE upgrade boundary (`PiDECUpgradeAssumption`), final target packaging (`PiDECFinalTarget`), CE/decomposition extraction lemmas (`piDECFinalTarget_of_assumption`, `piDEC_ceRelation_of_assumption`, `piDECDecomp_of_assumption`), invertibility-witness extraction from CE arithmetic validity, and direct composition hook from `Π_RLC`; remaining work is paper-exact reduction-of-knowledge theorem instantiation and extractor-level statement). |
| M26 | Strong/Weak composition theorem instantiation (Theorem 6) | `SuperNeo/InteractiveReductions.lean` | Formalize strong/weak IR framework and compose instantiated reductions | N/A (theorem-level composition) | Bridges individual reduction proofs into end-to-end reduction-of-knowledge. | In progress (composition layer now includes protocol-level assumption bundles for standard/strong CCS paths plus check-bundle variants (`SuperNeo{,StrongCCS,CheckBundle,StrongCheckBundle}ReductionAssumption`), chain theorems (`superneoReduction_chain_of_*`, `superneoReduction_ceRelation_of_assumptions`, `superneoReduction_ceValid_of_assumptions`), conversions among assumption surfaces (`superneoReductionAssumption_of_checkBundleReductionAssumption`, `superneoStrongCCSReductionAssumption_of_{reductionAssumption,checkBundleReductionAssumption,strongCheckBundleReductionAssumption}`), and strong-transcript wrappers under standard assumptions (`..._with_strongAccepted`); remaining work is paper-exact theorem-6 phrasing and stronger generic IR framework statements). |
| M27 | Final SuperNeo protocol theorem | `SuperNeo/ProtocolReduction.lean` + `SuperNeo/ProtocolTheorem.lean` | End-to-end theorem matching paper claim for `Π_SuperNeo` under explicit assumptions/bounds | Aggregate checks as regression only | Final target theorem certifying protocol soundness/completeness path. | In progress (wrapper module `ProtocolTheorem.lean` now provides end-to-end theorem families for standard, strong-CCS, check-bundle, and strong-check-bundle assumption surfaces, with and without invertibility witnesses, including bundle-driven entrypoints (`superneoProtocolTheorem_of_{assumptions,checkBundle_assumptions,strongCCS_assumptionBundle,strongCheckBundle_assumptions}`, `..._with_invertibility_of_{assumptions,checkBundle_assumptions,strongCCS_assumptionBundle,strongCheckBundle_assumptions}`) plus standard-assumption wrappers that consume strong accepted transcripts directly (`..._of_assumptions_with_strongAccepted`); remaining work is paper-exact final theorem statement/quantifiers and full extraction-level instantiation). |
