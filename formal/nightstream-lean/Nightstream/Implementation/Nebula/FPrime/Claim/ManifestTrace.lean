...
namespace Nightstream.Implementation.Nebula.FullClaimManifestTrace
...
structure ExactChain
    ...
    (configuration : Configuration Params StructureDigest Header Digest.Value
      Running (Value widths) NifsProof Nebula NebulaDigest NebulaOpen)
    (selected : SelectedVerifier widths)
    (initial final : Nightstream.Protocol.Nebula.FPrime.ClosedCarry
      Digest.Value)
  ...
  memory : Nightstream.Protocol.Nebula.GlobalFPrime.Chain
...
end Nightstream.Implementation.Nebula.FullClaimManifestTrace
