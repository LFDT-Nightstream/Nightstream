...
use super::super::state::{
    DirectCcsFPrimeChunkPerf, DirectCcsFPrimeCircuit, DirectCcsFPrimeCommittedPerf, DirectCcsFPrimeCommittedSourcePerf,
...
use super::committed::{
    ...
    DirectCcsTerminalCommittedKeyPair, DirectCcsTerminalCommittedPerf, DirectCcsTerminalCommittedProofPackage,
    DirectCcsTerminalCommittedRelation,
};
...
pub(crate) struct DirectCcsTerminalSnarkPackage {
    pub(crate) snark: DirectCcsIvcSnark,
    pub(crate) verifier_key: DirectCcsIvcSnarkVerifierKey,
    pub(crate) perf: DirectCcsFPrimeSnarkPerf,
}

pub(crate) fn prove_direct_ccs_terminal_snark(
    circuit: DirectCcsFPrimeCircuit,
    emit: &mut dyn FnMut(&str),
) -> Result<DirectCcsTerminalSnarkPackage, DirectCcsFPrimeSnarkError> {
...
fn prove_terminal_committed_step(
    ...
    let committed: DirectCcsTerminalCommittedProofPackage =
        prove_direct_ccs_terminal_committed_relation(&setup.keys.prover, &terminal.terminal_relation)
...
fn package_terminal_snark(
    ...
    Ok(DirectCcsTerminalSnarkPackage {
        snark: DirectCcsIvcSnark::from_parts(proof, public_image),
        verifier_key,
        perf,
    })
}
...
fn terminal_perf_accounting(
    ...
    Ok(DirectCcsFPrimeSnarkPerf {
        timing: DirectCcsFPrimeTimingPerf {
        ...
        proof: DirectCcsFPrimeProofSizePerf {
            final_proof_bytes,
            snark_bytes: proof.snark_bytes_len(),
        },
        r1cs: DirectCcsFPrimeR1csPerf {
            sizes: setup.r1cs_sizes,
            nnz: setup.r1cs_nnz,
            public_inputs: setup.terminal_committed_perf.public_inputs,
        },
        chunks: DirectCcsFPrimeChunkPerf {
            constraints_first4: terminal.terminal_shape.chunk_constraints_first4,
            constraints_by_chunk: terminal.terminal_shape.chunk_constraints_by_chunk.clone(),
            count: terminal.terminal_shape.chunk_count,
            done_constraints: terminal.terminal_shape.chunk_done_constraints,
        },
        constraints: DirectCcsFPrimeConstraintPerf {
            public_link: terminal.terminal_shape.public_link_constraints,
            construction2_fold: terminal.terminal_shape.construction2_fold_constraints,
            construction2_fold_final_ce_consistency: 0,
        },
        committed: DirectCcsFPrimeCommittedPerf {
            constraints: setup.terminal_committed_perf.constraints,
            width: setup.terminal_committed_perf.committed_width,
            commitment_words: setup.terminal_committed_perf.commitment_words,
            source: DirectCcsFPrimeCommittedSourcePerf {
                values: setup.terminal_committed_perf.source_values,
                bit_values: setup.terminal_committed_perf.source_bit_values,
                u32_values: setup.terminal_committed_perf.source_u32_values,
                u64_values: setup.terminal_committed_perf.source_u64_values,
                unclassified_private_values: setup.terminal_committed_perf.unclassified_private_values,
            },
            breakdown: setup.terminal_committed_perf.breakdown,
        },
        final_ce: DirectCcsFPrimeFinalCePerf {
            relation_constraints: terminal.final_ce_shape.total_relation_constraints,
            relation_breakdown: terminal.final_ce_shape.relation_breakdown,
            bundle_constraints: 0,
            bundle_digest_constraints: 0,
            bundle_digest_match_constraints: 0,
            r1cs_sizes: [0; 10],
        },
    })
}
