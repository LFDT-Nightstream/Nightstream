...
fn chunk_step_relations_fixture() -> Vec<neo_fold_next::rv32im::audit::Rv32imChunkStepIvcRelation> {
    static FIXTURE: OnceLock<Vec<neo_fold_next::rv32im::audit::Rv32imChunkStepIvcRelation>> = OnceLock::new();
    FIXTURE
        .get_or_init(|| {
...
fn rv32im_main_recursion_step_spartan_canonical_payload_localizes_first_failing_step() {
    ...
    let spartan_shape = neo_fold_next::rv32im::audit::build_rv32im_main_recursion_step_spartan_shape(&raw_relations)
...
fn rv32im_main_recursion_step_spartan_published_target_is_authoritative() {
    ...
    tampered.spartan_statement.x_out = neo_fold_next::rv32im::Rv32imEncodedPublicInput::from_digest_bytes([7u8; 32]);
...
fn rv32im_main_recursion_step_spartan_rejects_stale_construction2_u_next_shell() {
    let (spartan_shape, backend_relations) = single_relation_backend_fixture();
    let first = backend_relations.first().expect("first backend relation");
    let full_width = neo_fold_next::rv32im::build_rv32im_main_recursion_construction2_canonical_full_width(
        first.f_prime_advice.verifier_key_fs(),
        first.f_prime_advice.phi_side(),
    )
    .expect("build canonical Construction-2 full width");
    let stale_u_i = neo_fold_next::rv32im::build_rv32im_main_recursion_construction2_default_fresh_instance(
        first.f_prime_advice.verifier_key_fs(),
        full_width,
    )
...
fn rv32im_main_recursion_step_spartan_print_first_last_metadata() {
    ...
    let first_live_state_in_shapes = first
        ...
        .claims
        .iter()
        .map(neo_fold_next::rv32im::audit::Rv32imCeClaimDigestShape::from_claim)
        .collect::<Vec<_>>();
    ...
    let first_fresh_claim_shapes = first
        ...
        .fresh_claims
        .iter()
        .map(neo_fold_next::rv32im::audit::Rv32imCcsClaimShape::from_claim)
        .collect::<Vec<_>>();
    ...
    let first_fresh_witness_shapes = first
        ...
        .fresh_witnesses
        .iter()
        .map(neo_fold_next::rv32im::audit::Rv32imCcsWitnessShape::from_witness)
        .collect::<Vec<_>>();
    ...
    let first_parent_shape =
        neo_fold_next::rv32im::audit::Rv32imCeClaimDigestShape::from_claim(&first.payload.pi_rlc.parent);
    let last_parent_shape =
        neo_fold_next::rv32im::audit::Rv32imCeClaimDigestShape::from_claim(&last.payload.pi_rlc.parent);
