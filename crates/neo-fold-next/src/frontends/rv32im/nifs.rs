...
#[derive(Clone, Debug)]
pub(crate) struct Rv32imNifsRunningWitness {
    pub state: Rv32imChunkFoldState,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct Rv32imNifsFreshInstance {
    pub step_public: Rv32imChunkStepPublic,
    pub chunk_summary: FixedShapeChunkSummary,
}

#[derive(Clone, Debug)]
pub(crate) struct Rv32imNifsFreshWitness {
    pub handoff: Rv32imVerifiedKernelChunkHandoff,
    pub state_out: Rv32imChunkFoldState,
}

#[derive(Clone, Debug)]
pub(crate) struct Rv32imNifsProof {
    ccs_outputs: Vec<CeClaim<Commitment, F, K>>,
    // Lower Pi_CCS replay transport. RV32IM NIFS derives its carried fold state from the
...
fn build_rv32im_nifs_proof_from_trace(trace: Rv32imChunkRelationTrace) -> Rv32imNifsProof {
    Rv32imNifsProof::from_replay_parts(trace.ccs_outputs, trace.ccs_replay_proof, trace.parent, trace.children)
}

fn validate_rv32im_nifs_ce_claim_surface(
...
fn verify_rv32im_nifs_pi_ccs_trace(
    running: &Rv32imNifsRunningWitness,
    fresh_instance: &Rv32imNifsFreshInstance,
    fresh_witness: &Rv32imNifsFreshWitness,
    proof: &Rv32imNifsProof,
) -> Result<(Rv32imChunkRelationTrace, Poseidon2Transcript), SimpleKernelError> {
    validate_rv32im_nifs_surface(running, fresh_instance, fresh_witness)?;
    let (params, log, structure) =
        rv32im_root_main_lane_context_for_claim_count(running.state.carry.main.claims.len())?;
    ...
    let me_input_accumulator_handle = crate::finalize::digest32_as_fields(
        crate::rv32im::final_relation::rv32im_chunk_fold_carry_recursive_accumulator_digest(&running.state.carry),
    );
    let mut transcript =
    ...
    .map_err(|err| SimpleKernelError::Proof(format!("RV32IM NIFS proof Pi_CCS replay failed: {err}")))?;
    Ok((trace, transcript))
}

fn verify_rv32im_nifs_pi_rlc_parent(
    trace: &Rv32imChunkRelationTrace,
    proof: &Rv32imNifsProof,
) -> Result<(), SimpleKernelError> {
    validate_rv32im_nifs_ce_claim_surface(&trace.parent, "RV32IM NIFS proof Pi_RLC parent claim")?;
    if trace.parent != proof.parent {
        return Err(SimpleKernelError::Proof(
            "RV32IM NIFS proof RLC parent claim does not match the verified chunk relation trace".into(),
        ));
    }
    Ok(())
}

fn verify_rv32im_nifs_pi_dec_children(
    trace: &Rv32imChunkRelationTrace,
    proof: &Rv32imNifsProof,
) -> Result<(), SimpleKernelError> {
    for (child_index, claim) in trace.children.iter().enumerate() {
        validate_rv32im_nifs_ce_claim_surface(claim, &format!("RV32IM NIFS proof Pi_DEC child {child_index}"))?;
    }
    if trace.children != proof.children {
        return Err(SimpleKernelError::Proof(
            "RV32IM NIFS proof DEC child claims do not match the verified chunk relation trace".into(),
        ));
    }
    Ok(())
}

fn derive_rv32im_nifs_state_from_trace(
    running: &Rv32imNifsRunningWitness,
    fresh_instance: &Rv32imNifsFreshInstance,
    fresh_witness: &Rv32imNifsFreshWitness,
    trace: &Rv32imChunkRelationTrace,
    ...
    let fresh = adapt_rv32im_chunk_to_fresh_ccs(&fresh_witness.handoff);
    let expected_step_public = build_rv32im_chunk_step_public(
    ...
    if expected_step_public != fresh_instance.step_public {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS fresh instance public step does not match the verified fold-step output".into(),
        ));
    }
    ...
    if expected_summary != fresh_instance.chunk_summary {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS fresh instance chunk summary does not match the verified fold-step output".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS carried next-main state does not match the verified fold-step output".into(),
        ));
    }
    if next_carry.terminal_handle != fresh_witness.state_out.carry.terminal_handle {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS carried terminal handle does not match the verified fold-step output".into(),
        ));
    }
    ...
    if transcript_out != fresh_witness.state_out.transcript {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS carried transcript_out does not match the verified fold-step output".into(),
        ));
    }
...
pub(crate) fn prove_rv32im_nifs_step(
    running: &Rv32imNifsRunningWitness,
    fresh_instance: &Rv32imNifsFreshInstance,
    fresh_witness: &Rv32imNifsFreshWitness,
) -> Result<Rv32imNifsProof, SimpleKernelError> {
    validate_rv32im_nifs_surface(running, fresh_instance, fresh_witness)?;
    let (params, log, structure) =
        rv32im_root_main_lane_context_for_claim_count(running.state.carry.main.claims.len())?;
    let optimized_cache = rv32im_cached_root_main_lane_optimized_cache()?;
    ...
    let mut prove_transcript =
        Poseidon2Transcript::from_state_and_absorbed(running.state.transcript.state, running.state.transcript.absorbed);
    let ((replay_witness, _, _, _), _) = prove_rv32im_chunk_transition_with_perf(
        fresh_instance.step_public.chunk_index as usize,
        &fresh_witness.handoff,
        &running.state.carry.main,
        me_input_accumulator_handle,
    ...
    let _ = derive_rv32im_nifs_state_from_trace(running, fresh_instance, fresh_witness, &trace, &trace_transcript)?;
    Ok(build_rv32im_nifs_proof_from_trace(trace))
}

fn verify_rv32im_nifs_structured_surfaces(
    running: &Rv32imNifsRunningWitness,
    fresh_instance: &Rv32imNifsFreshInstance,
    fresh_witness: &Rv32imNifsFreshWitness,
    proof: &Rv32imNifsProof,
) -> Result<Rv32imChunkFoldState, SimpleKernelError> {
    let (trace, transcript) = verify_rv32im_nifs_pi_ccs_trace(running, fresh_instance, fresh_witness, proof)?;
    verify_rv32im_nifs_pi_rlc_parent(&trace, proof)?;
    verify_rv32im_nifs_pi_dec_children(&trace, proof)?;
    derive_rv32im_nifs_state_from_trace(running, fresh_instance, fresh_witness, &trace, &transcript)
}

pub(crate) fn verify_rv32im_nifs_step(
    running: &Rv32imNifsRunningWitness,
    fresh_instance: &Rv32imNifsFreshInstance,
    fresh_witness: &Rv32imNifsFreshWitness,
    proof: &Rv32imNifsProof,
) -> Result<Rv32imNifsRunningWitness, SimpleKernelError> {
    Ok(Rv32imNifsRunningWitness {
        state: verify_rv32im_nifs_structured_surfaces(running, fresh_instance, fresh_witness, proof)?,
    })
}

fn validate_rv32im_nifs_surface(
    running: &Rv32imNifsRunningWitness,
    fresh_instance: &Rv32imNifsFreshInstance,
    fresh_witness: &Rv32imNifsFreshWitness,
) -> Result<(), SimpleKernelError> {
    let expected_step_count = fresh_instance
        ...
        .ok_or_else(|| SimpleKernelError::Bridge("RV32IM NIFS step bounds are not monotone".into()))?;
    if fresh_instance.chunk_summary.start_index != fresh_instance.step_public.step_lo {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS summary start index does not match step_public.step_lo".into(),
        ));
    }
    if fresh_instance.chunk_summary.public_step_count != expected_step_count {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS summary public_step_count does not match step_public span".into(),
        ));
    }
    if fresh_witness.handoff.public_chunk.start_index as u64 != fresh_instance.step_public.step_lo {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS handoff start index does not match step_public.step_lo".into(),
        ));
    }
    if fresh_witness.handoff.public_chunk.steps.len() as u64 != expected_step_count {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS handoff step count does not match step_public span".into(),
        ));
    }
    if running.state.carry.terminal_handle.0 != fresh_instance.step_public.state_in {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS running state terminal handle does not match step_public.state_in".into(),
        ));
    }
    if fresh_witness.state_out.carry.terminal_handle.0 != fresh_instance.step_public.state_out {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS carried state_out terminal handle does not match step_public.state_out".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM NIFS transcript snapshot absorbed count exceeds the Poseidon2 rate".into(),
        ));
    }
    Ok(())
}
