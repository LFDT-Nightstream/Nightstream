//! Owns witness-side CE checks over SuperNeo packed `Z` for the RV32IM main relation circuit.
...
pub fn enforce_x_projection<CS: ConstraintSystem<SpartanF>>(
    ...
    if claim.public_input.x.len() == claim.public_input.m_in {
        for col in 0..claim.public_input.m_in {
    ...
    let active_cols = claim.public_input.m_in.min(witness.cols);
    for col in 0..claim.public_input.m_in {
        for row in 0..D {
            let claim_idx = row
...
pub fn enforce_claim_y_zcol<CS: ConstraintSystem<SpartanF>>(
    ...
    if computed.len() != claim.norm_check.y_zcol.len() {
        return Err(SynthesisError::Unsatisfiable);
    }
    for (idx, (computed_var, claim_var)) in computed
        .iter()
        .zip(claim.norm_check.y_zcol.iter())
        .enumerate()
    {
