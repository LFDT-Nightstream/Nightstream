...
AXES = [('proof', 'Proof', {'proved'}), ('connection', 'Link', {'connected'}),
        ('rust', 'Rust', {'implemented', 'tested_scoped'})]
...
def export_markdown(data, guide):
    nodes = data['nodes']
    ...
    def counts(node_id):
        included = [node for node in leaves(node_id) if node['origin'] != 'out_of_scope']
        result = []
        for key, label, complete in AXES:
            applicable = [node for node in included if node[key] not in {'not_required', 'assumption'}]
            finished = sum(node[key] in complete for node in applicable)
            result.append(f'{finished}/{len(applicable)}' if applicable else 'n/a')
        return result
    ...
    def record(node, source):
        ...
        if node['kind'] == 'leaf':
            status = '; '.join(f'{label}: `{node[key]}`' for key, label, _ in AXES)
        else:
        ...
        lines.extend([
            ...
            '- Remaining: ' + prose(node.get('remaining', 'Not recorded.')),
    ...
    index = ['# Nightstream requirements — Markdown index', '',
             ...
             f'- Base repository commit: `{data["commit"]}`.',
             ...
             'HTML, Markdown and JSON are generated from the same snapshot. This export does not rerun proofs or tests, and the base commit alone does not include the recorded working-tree updates.', '',
    ...
    index.extend(['', '## Snapshot metadata', '',
                  'These fields are copied from the source snapshot, including its recorded validation claims. They are not new validation results from the export.', '',
                  '```json', json.dumps({key: value for key, value in data.items() if key != 'nodes'},
    ...
    for group in groups:
        ...
        lines = [f'# {group["id"]} — {prose(group["label"])}', '',
                 ...
                 f'Base commit: `{data["commit"]}`. Recorded update: {prose(data.get("source_note", "None recorded."))}', '',
