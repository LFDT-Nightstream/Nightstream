...
/-!
Owns the compact replay fixture for the canonical `Poseidon2HashChainV1`
verification-key binding. It exports the complete binding preimage and digest;
the digest is evidence only when a consumer replays this preimage.
-/
...
def parityValue (structural relation application nifsKey commitmentKey :
  ...
  let currentBinding := bindingFromComponentDigests structural relation
    application nifsKey commitmentKey
  .array [
    .atom schema,
