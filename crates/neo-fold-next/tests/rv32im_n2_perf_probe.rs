...
use neo_fold_next::rv32im::{
    ...
    build_rv32im_kernel_export_source_from_accepted_artifact, prove_rv32im_public_proof_with_perf, Rv32imProofInput,
};
...
fn rv32im_n2_latency_probe() {
    ...
    let (proof, proof_perf) = prove_rv32im_public_proof_with_perf(&input).expect("prove n=2 public proof");
    ...
    let started = Instant::now();
    let accepted_artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build n=2 accepted artifact");
    print_probe("build_accepted_artifact.wall", elapsed_ms(started));

    println!("n2-latency|start|build_kernel_export_source");
    let started = Instant::now();
    let _kernel_export_source = build_rv32im_kernel_export_source_from_accepted_artifact(&accepted_artifact)
    ...
    let (final_statement, final_proof) =
        prove_rv32im_final_statement_from_accepted(&accepted_artifact).expect("prove n=2 final statement");
    ...
    let started = Instant::now();
    let advices = build_rv32im_main_recursion_f_prime_advices(&relations).expect("build n=2 f-prime advices");
    ...
    let started = Instant::now();
    debug_profile_rv32im_main_recursion_step_chunk_replay_stages(first_relation)
    ...
    let first_step_shape =
        debug_measure_rv32im_main_recursion_step_spartan_circuit_shape(&spartan_shape, first_relation)
    ...
    let commitment_key_ms =
        debug_measure_rv32im_main_recursion_step_spartan_commitment_key(&spartan_shape, first_relation)
...
fn rv32im_n2_shape_probe() {
    let source = build_mixed_opcode_perf_source_case(2);
    let input = Rv32imProofInput {
        max_steps: source.program_words.len(),
        source,
    };
    let (proof, _) = prove_rv32im_public_proof_with_perf(&input).expect("prove n=2 public proof");
    let accepted_artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build n=2 accepted artifact");
    let (final_statement, final_proof) =
        prove_rv32im_final_statement_from_accepted(&accepted_artifact).expect("prove n=2 final statement");
    let relations = build_rv32im_chunk_step_ivc_relations(&final_statement, &final_proof)
