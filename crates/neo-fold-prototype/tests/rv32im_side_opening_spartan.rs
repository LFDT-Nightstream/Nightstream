...
use neo_fold_prototype::public_proof::rv32im::side_opening_relation::build_rv32im_side_opening_relation_from_accepted_artifact;
use neo_fold_prototype::public_proof::rv32im::side_opening_spartan::{
...
use neo_fold_prototype::public_proof::rv32im::side_relation_circuit::circuit_stage1_opening_packaged_statement_digest;
use neo_fold_prototype::rv32im::kernel::Stage1SelectedOpeningClaim;
...
fn rv32im_side_opening_spartan_round_trip() {
    let proof =
    ...
    debug_check_rv32im_side_opening_spartan_circuit(&statement, &witness)
        .expect("debug-check side-opening spartan circuit");
    let keys = setup_rv32im_side_opening_spartan_cached(&statement, &witness).expect("setup side-opening spartan");
    let proof =
        prove_rv32im_side_opening_spartan(&keys.as_ref().0, &statement, &witness).expect("prove side-opening spartan");

    verify_rv32im_side_opening_spartan(&keys.as_ref().1, &statement, &proof).expect("verify side-opening spartan");
}

#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_rejects_tampered_stage1_row_witness() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, mut witness) =
        build_rv32im_side_opening_relation_from_accepted_artifact(&artifact).expect("build side-opening relation");
    let (pk, _) = setup_rv32im_side_opening_spartan(&statement, &witness).expect("setup side-opening spartan");

    witness.stage1_selected_rows.first.fetched_word ^= 1;

    let err = prove_rv32im_side_opening_spartan(&pk, &statement, &witness)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_circuit_rejects_tampered_kernel_packaged_witness() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, mut witness) =
    ...
    let err = debug_check_rv32im_side_opening_spartan_circuit(&statement, &witness)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_cached_setup_reuses_same_shape_for_same_case() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let keys_a =
        setup_rv32im_side_opening_spartan_cached(&statement, &witness).expect("cached setup side-opening spartan A");
    let keys_b =
        setup_rv32im_side_opening_spartan_cached(&statement, &witness).expect("cached setup side-opening spartan B");
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_cached_setup_ignores_selected_witness_replay() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let keys_b = setup_rv32im_side_opening_spartan_cached(&statement, &tampered_selected)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_cached_setup_ignores_packaged_step_replay() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    tampered_packaged_step
        ...
        .label
        .push_str("/tampered");
    let keys_b = setup_rv32im_side_opening_spartan_cached(&statement, &tampered_packaged_step)
        .expect("cached setup side-opening spartan B");
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_cached_setup_ignores_packaged_proof_digest_replay() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let keys_b = setup_rv32im_side_opening_spartan_cached(&statement, &tampered_packaged_proof)
        .expect("cached setup side-opening spartan B");
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_cached_setup_ignores_packaged_final_main_claim_digests() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let keys_b = setup_rv32im_side_opening_spartan_cached(&statement, &tampered_packaged_final_claims)
        .expect("cached setup side-opening spartan B");
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_reduced_setup_without_packaged_final_main_claims_round_trips() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let attempt = (|| {
        let (pk, vk) =
            debug_setup_rv32im_side_opening_spartan_without_packaged_final_main_claims(&statement, &witness)?;
        let proof = prove_rv32im_side_opening_spartan(&pk, &statement, &witness)?;
        verify_rv32im_side_opening_spartan(&vk, &statement, &proof)
    })();
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_reduced_setup_without_packaged_final_main_claims_verifies_after_prove() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let (pk, vk) = debug_setup_rv32im_side_opening_spartan_without_packaged_final_main_claims(&statement, &witness)
        .expect("reduced setup should still synthesize");

    let proof = prove_rv32im_side_opening_spartan(&pk, &statement, &witness).expect("reduced setup should still prove");
    verify_rv32im_side_opening_spartan(&vk, &statement, &proof)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_reduced_setup_without_stage1_packaged_final_main_claims_verifies_after_prove() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let (pk, vk) =
        debug_setup_rv32im_side_opening_spartan_without_stage1_packaged_final_main_claims(&statement, &witness)
            .expect("reduced stage1 setup should still synthesize");

    let proof =
        prove_rv32im_side_opening_spartan(&pk, &statement, &witness).expect("reduced stage1 setup should still prove");
    verify_rv32im_side_opening_spartan(&vk, &statement, &proof).expect(
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_current_setup_shape_matches_real_circuit() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let delta = debug_compare_rv32im_side_opening_spartan_setup_shape(&statement, &witness)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_zeroing_packaged_final_main_claims_preserves_circuit_shape() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let delta =
        debug_compare_rv32im_side_opening_spartan_without_packaged_final_main_claims_shape(&statement, &witness)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_stage1_packaged_statement_digest_r1cs_is_stable_under_final_main_claim_changes() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_stage1_packaged_opening_digest_ignores_final_main_claims_in_setup_shape() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let delta = debug_compare_rv32im_stage1_packaged_opening_digest_without_packaged_final_main_claims_shape(
        &statement, &witness,
    )
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_stage1_statement_digest_reduced_setup_still_round_trips() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_stage1_packaged_opening_digest_reduced_setup_round_trips() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    debug_round_trip_rv32im_stage1_packaged_opening_digest_with_reduced_setup(&statement, &witness)
        .expect("minimal stage1 packaged-opening wrapper should now round-trip under reduced setup");
}

#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_stage1_packaged_opening_digest_zeroing_only_final_main_claims_preserves_shape() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
    ...
    let delta =
        debug_compare_rv32im_stage1_packaged_opening_digest_zeroing_only_final_main_claims_shape(&statement, &witness)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_stage1_packaged_opening_digest_fixed_statement_stays_shape_stable() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, witness) =
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_side_opening_spartan_rejects_wrong_packaged_final_main_claim_width_at_prove() {
    let proof =
        prove_rv32im_public_proof(&proof_input("control_flow_jal_skip_ecall")).expect("prove rv32im public proof");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    let (statement, mut witness) =
    ...
    let err = match setup_rv32im_side_opening_spartan(&statement, &witness) {
        Ok((pk, _)) => {
            prove_rv32im_side_opening_spartan(&pk, &statement, &witness).expect_err("wrong packaged width must fail")
        }
        Err(err) => err,
    };
