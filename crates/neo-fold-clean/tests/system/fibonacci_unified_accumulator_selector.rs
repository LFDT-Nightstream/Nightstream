...
fn make_plan(unified: bool) -> RecursiveStepImagePlan {
    ...
    let probe_layout = FibonacciFPrimeImageLayout::new(build_recursive_step_image_config(&plan_probe));
...
fn unified_plan_drops_direct_new_acc_digest_binding() {
    ...
    use neo_fold_clean::frontends::fibonacci_f_prime::image::StateOutDigestTarget;
...
fn unified_structure_builds_without_panicking() {
    ...
    // panics. Reaching the end of `build_fibonacci_f_prime_structure`
    // is the M3a structural smoke test.
    let plan = make_plan(/* unified = */ true);
    let layout = FibonacciFPrimeImageLayout::new(build_recursive_step_image_config(&plan));
    let _structure = build_fibonacci_f_prime_structure(layout);
...
fn unified_accumulator_selector_accepts_base_digest() {
    let fixture = build_unified_image(SelectedDigest::Base);
    let structure = build_fibonacci_f_prime_structure(fixture.layout.clone());
...
fn unified_accumulator_selector_flip_failure_is_a_selector_row() {
    use neo_fold_clean::frontends::fibonacci_f_prime::image::UnifiedAccumulatorSelector;
