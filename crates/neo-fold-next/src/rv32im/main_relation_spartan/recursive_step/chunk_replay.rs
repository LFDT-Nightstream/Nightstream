...
//! carried transcript matches native state_out before `chunk_done`. Despite the
//! historic module name, this does not re-execute RV32IM instructions; it
//! verifies the chunk NIFS transcript inside the recursive step.

use bellpepper_core::{num::AllocatedNum, ConstraintSystem, SynthesisError};
use neo_reductions::engines::utils::{build_dims_and_policy, digest_ccs_matrices_with_sparse_cache};
use p3_goldilocks::Goldilocks;

use super::super::nifs_v_stages::Rv32imPiCcsStageOutput;
...
use super::super::{synthesize_rv32im_chunk_nifs_verifier_body_with_outer_relation_mode, Rv32imClaimBundle};
...
use crate::rv32im::main_recursion::Rv32imMainRecursionFPrimeAdvice;
...
pub(super) struct Rv32imMainRecursionStepChunkReplayOutput {
    ...
    pub(super) state_in_claims: Vec<crate::superneo_circuit::claim::CeClaimVar>,
    pub(super) pi_ccs: Rv32imPiCcsStageOutput,
}

pub(super) fn synthesize_rv32im_main_recursion_step_chunk_replay<CS: ConstraintSystem<SpartanF>>(
    cs: &mut CS,
    witness: &Rv32imMainRecursionFPrimeAdvice,
    payload: &Rv32imMainRecursionFPrimePayload,
    state_in_var: &Rv32imRecursiveCoverStateVar,
    state_out_var: &Rv32imRecursiveCoverStateVar,
    bridge_handoff_digest: &[AllocatedNum<SpartanF>; 4],
    trace_prefix: Option<&str>,
) -> Result<Rv32imMainRecursionStepChunkReplayOutput, SynthesisError> {
    let (params, _, structure) = crate::rv32im::kernel::rv32im_root_main_lane_context_for_claim_count(
        witness.running_state().carry.main.claims.len(),
    )
    .map_err(|_| SynthesisError::Unsatisfiable)?;
    let optimized_cache = rv32im_cached_root_main_lane_optimized_cache().map_err(|_| SynthesisError::Unsatisfiable)?;
    ...
    let replayed_body = synthesize_rv32im_chunk_nifs_verifier_body_with_outer_relation_mode(
        &params,
        structure,
        dims,
        &mat_digest,
    ...
    Ok(Rv32imMainRecursionStepChunkReplayOutput {
