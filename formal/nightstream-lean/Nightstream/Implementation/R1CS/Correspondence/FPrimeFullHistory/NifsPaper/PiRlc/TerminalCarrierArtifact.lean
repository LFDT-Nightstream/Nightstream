...
/-!
Terminal fixed-carrier diagnostic for `Pi_RLC`.
...
Protocol: SuperNeo `Pi_RLC` inside terminal full-history F'.
Phase: generated projection carrier and strict-`Pi_DEC` parent binding.
Constraint family: 29 diagnostic projection leaves across 15 inputs; challenge
rows are owned by `TerminalSamplerArtifact`.

Owns: the exact first-29-trace census; all input/output 54-column widths; the
terminal local-to-global strict-`Pi_DEC` parent layout; exact output-parent
...
`terminalColumnMap`; no assignment values or digests are premises. The input
...
| `nifs.pi_rlc.shape.parent` | output names the terminally relabeled strict-PiDEC parent | computed | `parentClaim`, `columns` |
...
namespace Nightstream.Implementation.R1CS.FPrimeFullHistoryNifsPaper.PiRlc.TerminalCarrierArtifact
...
/-- Strict-PiDEC parent in terminal full-history global column space. -/
def parentClaim : PiDecStrictCompiler.ClaimLayout :=
  RelabeledCarrier.relabelClaim terminalColumnMap layout.parent

/-- The verifier-selected parent point, shared by all 15 reduction inputs. -/
def point : PointColumns :=
  { r := parentClaim.rCols }

/-- Exact diagnostic terminal batch extracted from generated traces. -/
def columns : BatchColumns Concrete.productionGlobalParams terminalArity
    DiagnosticProfile.matrixCount :=
  TraceCarrier.batchColumns TerminalSamplerArtifact.tree parentClaim
    TerminalSamplerArtifact.challengeColumns point
...
def publicTraces : List ProjectionProgram.ProjectionTrace :=
  terminalTraces.take DiagnosticProfile.publicLeafCount
...
def delayedTraces : List ProjectionProgram.ProjectionTrace :=
  terminalTraces.drop DiagnosticProfile.publicLeafCount

set_option maxRecDepth 1000000
set_option maxHeartbeats 8000000

theorem publicTrace_census :
    TerminalSamplerArtifact.tree.flatten = publicTraces := by
  native_decide
...
theorem trace_partition : publicTraces ++ delayedTraces = terminalTraces := by
  exact List.take_append_drop DiagnosticProfile.publicLeafCount terminalTraces

theorem publicTrace_positions :
    (publicOrder DiagnosticProfile.matrixCount).map
      TerminalSamplerArtifact.publicRoleIndex =
        List.range DiagnosticProfile.publicLeafCount :=
  TerminalSamplerArtifact.publicRoleIndex_census

theorem inputWidth : forall role index,
    (TerminalSamplerArtifact.tree.publicPairAt role index).inputColumns.length =
...
theorem outputWidth : forall role,
    (TerminalSamplerArtifact.tree.publicTrace role).outputColumns.length =
      Concrete.ringDegree := by
  intro role
  cases role with
  | commitment lane =>
...
theorem equationWiringArtifact :
    EquationWiringArtifact columns TerminalSamplerArtifact.tree :=
  TraceCarrier.equationWiringArtifact TerminalSamplerArtifact.tree parentClaim
    TerminalSamplerArtifact.challengeColumns point
...
theorem carrierArtifact :
    CarrierArtifact (CarrierCodec.canonical DiagnosticProfile.matrixCount) columns
      TerminalSamplerArtifact.tree publicTraces :=
  TraceCarrier.carrierArtifact
    (CarrierCodec.canonical DiagnosticProfile.matrixCount)
    TerminalSamplerArtifact.tree parentClaim
    TerminalSamplerArtifact.challengeColumns point publicTraces
...
theorem bindChallenges_columns :
    TerminalSamplerArtifact.bindChallenges columns = columns := by
  unfold TerminalSamplerArtifact.bindChallenges columns
    TraceCarrier.batchColumns
  rfl

/-- Static challenge wiring for this exact diagnostic batch. -/
theorem challengeWiringArtifact :
    ChallengeWiringArtifact columns TerminalSamplerArtifact.tree := by
  rw [← bindChallenges_columns]
  exact TerminalSamplerArtifact.challengeWiringArtifact columns

end Nightstream.Implementation.R1CS.FPrimeFullHistoryNifsPaper.PiRlc.TerminalCarrierArtifact
