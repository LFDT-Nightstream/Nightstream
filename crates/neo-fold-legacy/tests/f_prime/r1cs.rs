...
use neo_fold_legacy::paper::construction2::{LaneCommitmentMode, RunningInstance};
...
use neo_fold_legacy::paper::reductions::pi_ccs_circuit::PiCcsVerifierConfig;
...
fn build_fixture_with_divergent_fresh(divergent_index: usize) -> Fixture {
    ...
    let (running, _first_proof) = neo_fold_legacy::paper::nifs::prove(
        &mut first_tr,
        prep.params(),
        prep.structure(),
...
fn pi_ccs_config<'a>(prep: &'a neo_fold_legacy::Preprocessing) -> PiCcsVerifierConfig<'a> {
    PiCcsVerifierConfig {
        params: prep.params(),
...
fn make_step_config<'a>(prep: &'a neo_fold_legacy::Preprocessing) -> FPrimeStepConfig<'a> {
    FPrimeStepConfig {
        ...
        state_x_out_digest_mode: match prep.semantic_state_mode() {
            neo_fold_legacy::paper::construction2::SemanticStateMode::Stateless => {
                neo_fold_legacy::paper::digest::StateXOutDigestMode::Stateless
            }
            neo_fold_legacy::paper::construction2::SemanticStateMode::Stateful => {
                neo_fold_legacy::paper::digest::StateXOutDigestMode::Stateful
            }
        },
    }
}
...
fn canonical_base_acc_digest(prep: &neo_fold_legacy::Preprocessing) -> [F; 4] {
    let m_in = prep
        .public_input_len()
