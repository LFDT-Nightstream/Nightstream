...
use neo_fold_next::rv64im::audit::Rv64imCeClaimDigestShape;
...
use neo_fold_next::rv64im::{
    ...
    build_rv64im_main_recursion_f_prime_advices, debug_measure_rv64im_main_recursion_step_chunk_replay_fingerprint,
    prove_rv64im_accepted_proof_with_options_and_perf, Rv64imProofInput, Rv64imProofProvePerf,
...
fn unwrap_accepted_artifact_with_schedule_context<T>(
    ...
    result.unwrap_or_else(|err| match root_fold_schedule {
        FoldSchedule::WholeTrace => {
            panic!(
                "{context}: WholeTrace overflowed the live RV64IM DEC/k_rho budget; retry with --rows-per-chunk-1.\nunderlying error: {err}"
            )
        }
...
fn print_named_constraint_breakdown(
    ...
    stages: &[Rv64imNamedConstraintDelta],
) {
    print_section(title);
...
fn print_pi_rlc_public_child_families(
    ...
    pi_rlc_parent_shape: &Rv64imCeClaimDigestShape,
) {
...
fn print_backend_relation_commitment_sparsity(relations: &[Rv64imMainRecursionFPrimeBackendRelation]) {
...
fn perturb_state_in_projection_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
...
fn perturb_pi_ccs_alpha_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
    if let Some(first) = relation.payload.pi_ccs.public_challenges.alpha.first_mut() {
        *first += K::ONE;
    }
}

fn perturb_pi_ccs_gamma_value(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
    relation.payload.pi_ccs.public_challenges.gamma += K::ONE;
}

fn perturb_state_out_projection_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
...
fn perturb_pi_ccs_output_y_ring_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
...
fn perturb_pi_ccs_output_y_zcol_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
    if let Some(claim) = relation.payload.pi_ccs.ccs_outputs.first_mut() {
        if let Some(first) = claim.y_zcol.first_mut() {
            *first += K::ONE;
        }
...
fn perturb_pi_dec_child_y_ring_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
...
fn perturb_pi_rlc_parent_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
    perturb_ce_claim_values(&mut relation.payload.pi_rlc.parent);
}

fn perturb_fresh_claim_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
    if let Some(claim) = relation.payload.fresh_claims.first_mut() {
        perturb_ccs_claim_values(claim);
    }
}

fn perturb_fresh_witness_values(relation: &mut Rv64imMainRecursionFPrimeBackendRelation) {
...
fn fixed_shape_family_status(
    ...
    match debug_measure_rv64im_main_recursion_step_spartan_circuit_shape(spartan_shape, relation) {
        Ok(shape) => {
...
fn first_pi_ccs_stage_diff<'a>(
    baseline: &'a neo_fold_next::rv64im::audit::Rv64imPiCcsStageFingerprint,
    perturbed: &'a neo_fold_next::rv64im::audit::Rv64imPiCcsStageFingerprint,
...
struct FastSummaryPerf {
    ...
    accepted_perf: Rv64imProofProvePerf,
...
fn accepted_root_rlc_dec_ms(accepted_perf: &Rv64imProofProvePerf) -> f64 {
...
fn measure_fast_summary_perf(input: &Rv64imProofInput) -> FastSummaryPerf {
    ...
    let ((accepted, _), accepted_perf) = unwrap_accepted_artifact_with_schedule_context(
        prove_rv64im_accepted_proof_with_options_and_perf(input, Rv64imPublicProofOptions { root_fold_schedule }),
        root_fold_schedule,
    ...
    let (final_statement, final_proof) =
        prove_rv64im_final_statement_from_accepted(&accepted).expect("prove final statement for fast summary rerun");
    ...
    let advices =
        build_rv64im_main_recursion_f_prime_advices(&relations).expect("build f-prime advices for fast summary rerun");
    ...
    let _ = debug_measure_rv64im_main_recursion_step_shape_only_circuit_shape(&spartan_shape)
...
fn main() {
    ...
    let state_in_claim_shape = first_relation
        ...
        .state_in_claims
        .first()
        .map(Rv64imCeClaimDigestShape::from_claim)
    ...
    let pi_rlc_parent_shape = Rv64imCeClaimDigestShape::from_claim(&first_relation.payload.pi_rlc.parent);
    ...
    print_section("RV64IM Main Recursion Shape Probe");
    print_kv(
        "mode",
        match probe_mode {
            ProbeMode::Full => "full",
    ...
    let chunk_replay_tail_aux = debug_measure_rv64im_main_recursion_step_chunk_replay_tail_aux_counts(first_relation)
    ...
    let pi_ccs_aux = debug_measure_rv64im_main_recursion_step_pi_ccs_aux_counts(first_relation)
    ...
    let pi_ccs_sumcheck = debug_measure_rv64im_main_recursion_step_pi_ccs_sumcheck_constraint_breakdown(first_relation)
    ...
    let pi_rlc_public_stage = debug_measure_rv64im_main_recursion_step_pi_rlc_public_stage_breakdown(first_relation)
    ...
    let live_shape = debug_measure_rv64im_main_recursion_step_spartan_circuit_shape(&spartan_shape, first_relation)
    ...
    let pi_ccs_fingerprint = debug_measure_rv64im_main_recursion_step_pi_ccs_fingerprint(first_relation)
    ...
    let chunk_replay_fingerprint = debug_measure_rv64im_main_recursion_step_chunk_replay_fingerprint(first_relation)
    ...
    let perturbed_shape =
        debug_measure_rv64im_main_recursion_step_spartan_circuit_shape(&spartan_shape, &perturbed_relation);
    let perturbed_pi_ccs_fingerprint = debug_measure_rv64im_main_recursion_step_pi_ccs_fingerprint(&perturbed_relation);
    let perturbed_chunk_replay_fingerprint =
        debug_measure_rv64im_main_recursion_step_chunk_replay_fingerprint(&perturbed_relation);
    ...
    let state_in_projection_pi_ccs_fingerprint =
        debug_measure_rv64im_main_recursion_step_pi_ccs_fingerprint(&state_in_projection_relation);
    let fresh_claim_pi_ccs_fingerprint =
        debug_measure_rv64im_main_recursion_step_pi_ccs_fingerprint(&fresh_claim_relation);
    ...
    match &perturbed_chunk_replay_fingerprint {
        Ok(perturbed_chunk_replay_fingerprint) => {
            ...
            {
                print_kv("chunk_replay.first_diff_stage", stage);
                print_kv("chunk_replay.baseline", baseline);
                print_kv("chunk_replay.perturbed", perturbed);
            } else {
                print_kv("chunk_replay.first_diff_stage", "none");
            }
        }
        Err(_) => {
            print_kv("chunk_replay.first_diff_stage", "unsat");
        }
    }
