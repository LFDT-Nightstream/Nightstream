...
use neo_fold_next::rv32im::build_all_parity_cases;
        ...
        match derived.manifest.name.as_str() {
            ...
            "multiply_low_mul_ecall" => {
                ...
                assert_eq!(
                    ...
                    Some(neo_fold_next::rv32im::Rv32Opcode::Mul)
                );
                ...
                assert_eq!(
                    ...
                    Some(neo_fold_next::rv32im::Rv32TraceVirtualOpcode::SignExtendWord)
                );
            ...
            "multiply_high_mulh_mulhu_mulhsu_ecall" => {
                ...
                assert_eq!(
                    derived
                        ...
                        .filter(|row| row.trace_virtual_opcode
                            == Some(neo_fold_next::rv32im::Rv32TraceVirtualOpcode::Movsign))
                        .count(),
                    3
                );
                assert_eq!(
                    derived
                        .execution_rows
                        .iter()
                        .filter(|row| row.trace_opcode == Some(neo_fold_next::rv32im::Rv32Opcode::Mulh) && row.is_real)
                        .count(),
                    0
                );
                assert_eq!(
                    derived
                        ...
                        .iter()
                        .filter(
                            |row| row.trace_opcode == Some(neo_fold_next::rv32im::Rv32Opcode::Mulhsu) && row.is_real
                        )
                        .count(),
                    0
                );
                assert_eq!(
                    derived
                        ...
                        .iter()
                        .filter(|row| {
                            row.opcode == neo_fold_next::rv32im::Rv32Opcode::Mulhu
                ...
                assert!(!derived.execution_rows.iter().any(|row| {
                    row.trace_opcode == Some(neo_fold_next::rv32im::Rv32Opcode::Addi)
                ...
                assert!(
                    derived
                        ...
                        .any(|row| row.trace_virtual_opcode
                            == Some(neo_fold_next::rv32im::Rv32TraceVirtualOpcode::Advice))
                );
                assert!(!derived.execution_rows.iter().any(|row| {
                    row.trace_virtual_opcode
                        == Some(neo_fold_next::rv32im::Rv32TraceVirtualOpcode::AssertValidUnsignedRemainder)
                }));
                assert!(derived
                    .stage2
                    .register_writes
            ...
            "signed_divrem_chain_ecall" => {
                ...
                assert!(derived
                    ...
                    .iter()
                    .any(|row| row.trace_virtual_opcode
                        == Some(neo_fold_next::rv32im::Rv32TraceVirtualOpcode::ChangeDivisor)));
                assert!(!derived.execution_rows.iter().any(|row| {
                    row.trace_virtual_opcode
                        == Some(neo_fold_next::rv32im::Rv32TraceVirtualOpcode::AssertSignedDivIdentity)
                }));
                assert!(derived
                    .stage2
                    .register_writes
