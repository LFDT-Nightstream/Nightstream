use crate::common::proof_cases::{
    accepted_alu, accepted_branch, accepted_divu, accepted_memory, accepted_multiply_high, accepted_test_guard,
...
use neo_fold_prototype::rv32im::Rv32Opcode;
...
fn redteam_rebuild_dependence_check_fails_without_audit_mode() {
    ...
    assert!(
        audit_rv32im_accepted_proof(&artifact).is_err(),
...
#[test]
fn redteam_mulh_side_bundle_cannot_bind_unrelated_main_lane() {
    let _serial = accepted_test_guard();
    let (mulh_artifact, _) = accepted_multiply_high();
    let (main_artifact, _) = accepted_alu();
    ...
    assert!(
        mulh_artifact
            ...
            .execution_rows
            .iter()
            .any(|row| row.trace_opcode == Some(Rv32Opcode::Mulh)),
        "donor side artifact must exercise MULH"
    );
    assert!(
        main_artifact
            .root_execution
            .execution_rows
            .iter()
            .all(|row| row.trace_opcode != Some(Rv32Opcode::Mulh)),
        "target main-lane artifact must not exercise MULH"
    );

    let side_bundle =
        build_rv32im_side_proof_bundle_from_accepted_artifact(&mulh_artifact).expect("build MULH side bundle");
    let statement = build_rv32im_side_opening_relation_statement(&main_artifact.statement, &side_bundle)
        .expect("build mixed side/main relation statement");
    let witness = build_rv32im_side_opening_relation_witness_from_accepted_artifact(&mulh_artifact);

    let err = verify_rv32im_side_opening_relation(&statement, &witness)
        .expect_err("MULH side bundle must not verify against an unrelated main lane");
    let err = err.to_string();
    assert!(
        err.contains("main-lane")
            || err.contains("kernel-claim surface")
            || err.contains("public statement")
            || err.contains("kernel-opening proof surface"),
