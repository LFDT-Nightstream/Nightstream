use super::Rv32imMainRecursionFPrimePayload;
use crate::rv32im::final_relation::Rv32imChunkFoldTranscriptSnapshot;
use crate::rv32im::main_recursion::{Rv32imMainRecursionFPrimeAdvice, RV32IM_MAIN_RECURSION_TRIVIAL_PC};
use crate::rv32im::main_relation_trace::{
    build_rv32im_main_circuit_chunk_replay_surface, build_rv32im_main_circuit_pi_ccs_replay_surface,
    Rv32imMainCircuitChunkReplaySurface,
};
use crate::rv32im::SimpleKernelError;

impl Rv32imMainRecursionFPrimePayload {
    ...
    pub fn fixed_transcript_out(&self) -> &Rv32imChunkFoldTranscriptSnapshot {
    ...
    pub(crate) fn padded_chunk_replay_surface(&self) -> Result<Rv32imMainCircuitChunkReplaySurface, SimpleKernelError> {
    ...
    pub(crate) fn effective_chunk_replay_surface(
            ...
            .zip(self.step_shape.fe_round_lengths.iter())
        {
            if round.len() < *live_len as usize {
                return Err(SimpleKernelError::Bridge(
                    "RV32IM recursive-step payload cannot truncate a padded FE round to the live coefficient count"
                        .into(),
                ));
            }
            ...
            .zip(self.step_shape.nc_round_lengths.iter())
        {
            if round.len() < *live_len as usize {
                return Err(SimpleKernelError::Bridge(
                    "RV32IM recursive-step payload cannot truncate a padded NC round to the live coefficient count"
                        .into(),
                ));
            }
            round.truncate(*live_len as usize);
        ...
        let effective_me_output_count = effective_output_count
            ...
            .ok_or_else(|| {
                SimpleKernelError::Bridge(
                    "RV32IM recursive-step payload effective CCS output count cannot be smaller than the active fresh-output count"
                        .into(),
                )
            })?;
        ...
        if self.pi_ccs.ccs_outputs.len() < effective_me_end {
            return Err(SimpleKernelError::Bridge(
                "RV32IM recursive-step payload cannot recover the effective ME-output suffix from the canonical padded CCS-output layout"
                    .into(),
            ));
        }
    ...
    pub fn matches_explicit_semantics(&self, advice: &Rv32imMainRecursionFPrimeAdvice) -> bool {
        self.z_0 == *advice.z_0()
            && self.z_i == *advice.z_i()
            && self.z_next == advice.fresh_state_out().carry.terminal_handle.0
            && self.pc_i == advice.pc_i()
            && self.pc_next == RV32IM_MAIN_RECURSION_TRIVIAL_PC
