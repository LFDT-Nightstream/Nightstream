...
fn assert_side_proof_rejected(
    fixture: &SideFixture,
    side_proof: &neo_fold_prototype::public_proof::rv32im::Rv32imSideProof,
...
fn rv32im_side_soundness_positive_exact_cover_rejects_missing_or_duplicate_targets() {
    ...
    let stage1_positions = witness
        ...
        .enumerate()
        .filter(|(_, claim)| {
            claim.claim.payload.schema == neo_fold_prototype::rv32im::kernel::FamilyEvalSchemaId::Stage1Rows
        })
        .map(|(index, _)| index)
        .collect::<Vec<_>>();
