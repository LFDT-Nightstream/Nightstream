...
pub fn rv64im_public_step_digest<CS: ConstraintSystem<SpartanF>>(
    cs: &mut CS,
    step: &PublicStepVar,
    label: &str,
) -> Result<[AllocatedNum<SpartanF>; 4], SynthesisError> {
    Ok(rv64im_public_step_digest_with_values(cs, step, label)?.0)
}

fn rv64im_public_step_digest_with_values<CS: ConstraintSystem<SpartanF>>(
    cs: &mut CS,
    step: &PublicStepVar,
    label: &str,
    ...
    let mut transcript = Poseidon2TranscriptCircuit::new(
        ...
        b"neo.fold.next/rv64im/public_step",
    )?;
...
pub fn rv64im_public_chunk_digest<CS: ConstraintSystem<SpartanF>>(
    cs: &mut CS,
    ...
    let mut transcript = Poseidon2TranscriptCircuit::new(
        cs.namespace(|| format!("{label}_init")),
        b"neo.fold.next/rv64im/public_chunk",
    )?;
    let meta_halves = [
    ...
    for (step_idx, step) in steps.iter().enumerate() {
        let (digest, digest_values) = rv64im_public_step_digest_with_values(
