//! Owns the below-export RV32IM Phase 0 eval-claim theorem seam.
//!
...
use crate::rv32im::kernel::{
    build_commitment_vector, build_rv32im_eval_claim_bundle_from_claim_witnesses,
    build_rv32im_eval_claim_bundle_from_claim_witnesses_trusted_local,
    build_rv32im_eval_claim_witnesses_from_accepted_artifact, derive_phase0_point, phase0_binding_digest,
    ...
    OpenedAjtaiObjectWitness, Rv32imAcceptedProofArtifact, Rv32imEvalClaimBundle, Rv32imPhase0BindingSurface,
    Rv32imPhase0BindingTarget, SimpleKernelError,
};
use crate::rv32im::Rv32imProofStatement;

use super::side_bridges::validate_rv32im_side_proof_bundle_structure;
use super::{
    bind_rv32im_side_proof_bundle_to_statement_core, build_rv32im_side_proof_bundle_from_accepted_artifact,
    Rv32imSideProofBundle,
};

pub(super) fn active_phase0_schemas_from_side_bundle(side_bundle: &Rv32imSideProofBundle) -> Vec<FamilyEvalSchemaId> {
...
fn rv32im_side_eval_claim_relation_statement_digest_from_surfaces(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    phase0_opened_objects: &Rv32imPhase0OpenedObjectBundle,
    eval_claim_bundle: &Rv32imEvalClaimBundle,
) -> [u8; 32] {
...
pub fn rv32im_side_eval_claim_relation_statement_digest(statement: &Rv32imSideEvalClaimRelationStatement) -> [u8; 32] {
    rv32im_side_eval_claim_relation_statement_digest_from_surfaces(
...
pub fn build_rv32im_phase0_opened_object_bundle_from_claim_witnesses(
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<Rv32imPhase0OpenedObjectBundle, SimpleKernelError> {
    ...
    for claim_witness in claim_witnesses {
        ...
        if let Some((expected_object, expected_context)) = by_schema.get(&schema) {
            if *expected_object != opened_object || *expected_context != commitment_context {
                return Err(SimpleKernelError::Bridge(format!(
                    "RV32IM side-eval-claim relation found inconsistent opened-object identities for {:?}",
                    schema
                )));
            }
            continue;
    ...
    for (schema, (opened_object, commitment_context)) in by_schema {
        let mut summary = Rv32imPhase0OpenedObjectSummary {
            schema,
    ...
    let mut bundle = Rv32imPhase0OpenedObjectBundle {
        objects,
        digest: [0; 32],
    };
...
fn build_rv32im_phase0_opening_target_bundle_from_claim_witnesses(
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<Rv32imPhase0OpeningTargetBundle, SimpleKernelError> {
    ...
    for claim_witness in claim_witnesses {
        ...
        if let Some(expected) = by_schema.get(&schema) {
            ...
            {
                return Err(SimpleKernelError::Bridge(format!(
                    "RV32IM side-eval-claim artifact found inconsistent opened-object identities for {:?}",
                    schema
                )));
            }
            continue;
    ...
    for (index, (schema, representative)) in by_schema.into_iter().enumerate() {
        ...
        .map_err(|err| {
            SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim artifact could not build the Phase 0 opened commitment for {:?}: {err}",
                schema
            ))
        })?;
        ...
        .map_err(|err| {
            SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim artifact could not rebuild the Phase 0 witness for {:?}: {err}",
                schema
            ))
        })?;
        let target = Rv32imPhase0OpeningTarget {
            schema,
            opened_commitment,
            opening_proof,
            digest: [0; 32],
        };
        targets.push(Rv32imPhase0OpeningTarget {
            digest: target.expected_digest(),
            ..target
        });
    }

    let bundle = Rv32imPhase0OpeningTargetBundle {
        targets,
        digest: [0; 32],
    };
    Ok(Rv32imPhase0OpeningTargetBundle {
        digest: bundle.expected_digest(),
        ..bundle
    })
}

pub(crate) fn build_rv32im_phase0_opened_object_bundle_from_opening_targets(
    eval_claim_bundle: &Rv32imEvalClaimBundle,
    phase0_opening_targets: &Rv32imPhase0OpeningTargetBundle,
) -> Result<Rv32imPhase0OpenedObjectBundle, SimpleKernelError> {
    ...
    for target in &phase0_opening_targets.targets {
        ...
        let representative = claims.next().ok_or_else(|| {
            SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim artifact is missing claims for {:?}",
                schema
            ))
        })?;
        if representative.opened_object != target.opened_commitment.opened_object {
            return Err(SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim artifact {:?} opened-object mismatch between carried claims and opening target",
                schema
            )));
        }
        for claim in claims {
            ...
            {
                return Err(SimpleKernelError::Bridge(format!(
                    "RV32IM side-eval-claim artifact found inconsistent claim contexts for {:?}",
                    schema
                )));
            }
        }
        let summary = Rv32imPhase0OpenedObjectSummary {
            schema,
            opened_object: target.opened_commitment.opened_object.clone(),
            commitment_context: representative.commitment_context,
            digest: [0; 32],
        };
        objects.push(Rv32imPhase0OpenedObjectSummary {
            digest: summary.expected_digest(),
            ..summary
        });
    }

    let bundle = Rv32imPhase0OpenedObjectBundle {
        objects,
        digest: [0; 32],
    };
    Ok(Rv32imPhase0OpenedObjectBundle {
        digest: bundle.expected_digest(),
        ..bundle
    })
}

fn validate_rv32im_side_eval_claim_relation_inputs(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    phase0_opened_objects: &Rv32imPhase0OpenedObjectBundle,
    eval_claim_bundle: &Rv32imEvalClaimBundle,
    ...
    if public_statement.digest != public_statement.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim relation public statement digest mismatch".into(),
        ));
    }
    ...
    if phase0_opened_objects.digest != phase0_opened_objects.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim relation Phase 0 opened-object bundle digest mismatch".into(),
        ));
    }
    if eval_claim_bundle.digest != eval_claim_bundle.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim relation eval-claim bundle digest mismatch".into(),
        ));
    }
    Ok(())
}

pub fn build_rv32im_side_eval_claim_relation_statement(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    phase0_opened_objects: &Rv32imPhase0OpenedObjectBundle,
    eval_claim_bundle: &Rv32imEvalClaimBundle,
) -> Result<Rv32imSideEvalClaimRelationStatement, SimpleKernelError> {
    validate_rv32im_side_eval_claim_relation_inputs(
        public_statement,
        side_bundle,
        phase0_opened_objects,
        eval_claim_bundle,
    )?;
    Ok(Rv32imSideEvalClaimRelationStatement {
...
pub fn build_rv32im_side_eval_claim_relation_witness_from_accepted_artifact(
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<Rv32imSideEvalClaimRelationWitness, SimpleKernelError> {
...
pub(crate) fn build_rv32im_side_eval_claim_relation_witness_from_accepted_artifact_and_side_bundle(
    side_bundle: &Rv32imSideProofBundle,
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<Rv32imSideEvalClaimRelationWitness, SimpleKernelError> {
    Ok(Rv32imSideEvalClaimRelationWitness {
...
pub fn build_rv32im_side_eval_claim_relation_from_accepted_artifact(
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<(Rv32imSideEvalClaimRelationStatement, Rv32imSideEvalClaimRelationWitness), SimpleKernelError> {
    let side_bundle = build_rv32im_side_proof_bundle_from_accepted_artifact(artifact)?;
    let witness =
        build_rv32im_side_eval_claim_relation_witness_from_accepted_artifact_and_side_bundle(&side_bundle, artifact)?;
    let phase0_opened_objects =
        build_rv32im_phase0_opened_object_bundle_from_claim_witnesses(&witness.claim_witnesses)?;
    let eval_claim_bundle = build_rv32im_eval_claim_bundle_from_claim_witnesses(&witness.claim_witnesses)?;
    let statement = build_rv32im_side_eval_claim_relation_statement(
        &artifact.statement,
        &side_bundle,
        &phase0_opened_objects,
        &eval_claim_bundle,
...
pub fn verify_rv32im_side_eval_claim_relation(
    statement: &Rv32imSideEvalClaimRelationStatement,
    witness: &Rv32imSideEvalClaimRelationWitness,
) -> Result<(), SimpleKernelError> {
    verify_rv32im_side_eval_claim_relation_surfaces(statement, &witness.claim_witnesses)
}

fn verify_rv32im_side_eval_claim_relation_surfaces(
    statement: &Rv32imSideEvalClaimRelationStatement,
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<(), SimpleKernelError> {
    ...
    validate_rv32im_side_proof_bundle_structure(&statement.side_bundle)?;
    ...
    let expected_opened_objects = build_rv32im_phase0_opened_object_bundle_from_claim_witnesses(claim_witnesses)?;
    if expected_opened_objects != statement.phase0_opened_objects {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim relation opened-object summaries do not match the carried Phase 0 bundle".into(),
        ));
    }

    let expected_claim_bundle = build_rv32im_eval_claim_bundle_from_claim_witnesses(claim_witnesses)?;
    if expected_claim_bundle != statement.eval_claim_bundle {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim relation claim witnesses do not match the carried eval-claim bundle".into(),
        ));
    }
...
fn verify_phase0_claim_witness_consistency(
    ...
    for claim_witness in claim_witnesses {
        FamilyEvalClaimWitness::new(claim_witness.claim.clone(), claim_witness.witness.clone()).map_err(|err| {
            SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim relation {:?} claim witness is internally inconsistent: {err}",
        ...
        .map_err(|err| {
            SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim relation {:?} could not rebuild the commitment vector: {err}",
                claim_witness.claim.payload.schema
            ))
        })?;
        if rebuilt_commitment_vector != claim_witness.witness.commitment_vector {
            return Err(SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim relation {:?} packed columns do not match the carried commitment vector",
                claim_witness.claim.payload.schema
            )));
        }
    }
...
pub fn build_rv32im_side_eval_claim_artifact(
    statement: &Rv32imSideEvalClaimRelationStatement,
    witness: &Rv32imSideEvalClaimRelationWitness,
) -> Result<Rv32imSideEvalClaimArtifact, SimpleKernelError> {
    verify_rv32im_side_eval_claim_relation(statement, witness)?;
    let phase0_opening_targets =
        build_rv32im_phase0_opening_target_bundle_from_claim_witnesses(&witness.claim_witnesses)?;
    let mut artifact = Rv32imSideEvalClaimArtifact {
        statement_digest: rv32im_side_eval_claim_relation_statement_digest(statement),
...
pub fn build_rv32im_side_eval_claim_artifact_from_accepted_artifact(
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<Rv32imSideEvalClaimArtifact, SimpleKernelError> {
    let (statement, witness) = build_rv32im_side_eval_claim_relation_from_accepted_artifact(artifact)?;
    build_rv32im_side_eval_claim_artifact(&statement, &witness)
}

pub(crate) fn build_rv32im_side_eval_claim_artifact_from_accepted_artifact_and_side_bundle(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<Rv32imSideEvalClaimArtifact, SimpleKernelError> {
    let expected_side_bundle = bind_rv32im_side_proof_bundle_to_statement_core(
    ...
    if &expected_side_bundle != side_bundle {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim artifact side-proof bundle does not match the accepted artifact".into(),
        ));
    }
    let witness =
        build_rv32im_side_eval_claim_relation_witness_from_accepted_artifact_and_side_bundle(side_bundle, artifact)?;
    build_rv32im_side_eval_claim_artifact_from_claim_witnesses_and_side_bundle(
        public_statement,
        side_bundle,
...
pub(super) fn build_rv32im_side_eval_claim_artifact_from_claim_witnesses_and_side_bundle(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<Rv32imSideEvalClaimArtifact, SimpleKernelError> {
    let phase0_opened_objects = build_rv32im_phase0_opened_object_bundle_from_claim_witnesses(claim_witnesses)?;
    let phase0_opening_targets = build_rv32im_phase0_opening_target_bundle_from_claim_witnesses(claim_witnesses)?;
    let eval_claim_bundle = build_rv32im_eval_claim_bundle_from_claim_witnesses(claim_witnesses)?;
    validate_rv32im_side_eval_claim_relation_inputs(
        public_statement,
        side_bundle,
        &phase0_opened_objects,
    ...
    build_rv32im_side_eval_claim_artifact_from_trusted_surfaces(
        public_statement,
        side_bundle,
        phase0_opening_targets,
        eval_claim_bundle,
...
pub(super) fn build_rv32im_side_eval_claim_artifact_from_claim_witnesses_and_trusted_side_bundle(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<Rv32imSideEvalClaimArtifact, SimpleKernelError> {
    let phase0_opening_targets = build_rv32im_phase0_opening_target_bundle_from_claim_witnesses(claim_witnesses)?;
    let eval_claim_bundle = build_rv32im_eval_claim_bundle_from_claim_witnesses_trusted_local(claim_witnesses)?;
    build_rv32im_side_eval_claim_artifact_from_trusted_surfaces(
        public_statement,
        side_bundle,
        phase0_opening_targets,
...
fn build_rv32im_side_eval_claim_artifact_from_trusted_surfaces(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    phase0_opening_targets: Rv32imPhase0OpeningTargetBundle,
    eval_claim_bundle: Rv32imEvalClaimBundle,
    ...
    let phase0_opened_objects =
        build_rv32im_phase0_opened_object_bundle_from_opening_targets(&eval_claim_bundle, &phase0_opening_targets)?;
    let mut artifact = Rv32imSideEvalClaimArtifact {
        statement_digest: rv32im_side_eval_claim_relation_statement_digest_from_surfaces(
            public_statement,
            side_bundle,
            &phase0_opened_objects,
            &eval_claim_bundle,
...
pub fn build_rv32im_side_eval_claim_relation_statement_from_artifact(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    artifact: &Rv32imSideEvalClaimArtifact,
) -> Result<Rv32imSideEvalClaimRelationStatement, SimpleKernelError> {
    if artifact.digest != artifact.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim artifact digest mismatch".into(),
        ));
    }
    ...
    build_rv32im_side_eval_claim_relation_statement(
        public_statement,
        side_bundle,
        &phase0_opened_objects,
        &artifact.eval_claim_bundle,
...
pub fn verify_rv32im_side_eval_claim_artifact(
    public_statement: &Rv32imProofStatement,
    side_bundle: &Rv32imSideProofBundle,
    artifact: &Rv32imSideEvalClaimArtifact,
) -> Result<(), SimpleKernelError> {
    validate_rv32im_side_eval_claim_artifact_structure(artifact)?;
    let statement =
        build_rv32im_side_eval_claim_relation_statement_from_artifact(public_statement, side_bundle, artifact)?;
    if artifact.statement_digest != rv32im_side_eval_claim_relation_statement_digest(&statement) {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim artifact statement digest does not match the carried relation statement".into(),
        ));
    }
    let claim_witnesses = rebuild_phase0_claim_witnesses_from_artifact(artifact)?;
    verify_rv32im_side_eval_claim_relation_surfaces(&statement, &claim_witnesses)
}

pub(crate) fn validate_rv32im_side_eval_claim_artifact_structure(
    artifact: &Rv32imSideEvalClaimArtifact,
) -> Result<(), SimpleKernelError> {
    ...
    if artifact.phase0_opening_targets.digest != artifact.phase0_opening_targets.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim artifact Phase 0 opening-target bundle digest mismatch".into(),
        ));
    }
    for target in &artifact.phase0_opening_targets.targets {
        if target.opened_commitment.digest != target.opened_commitment.expected_digest() {
            return Err(SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim artifact {:?} opened-commitment digest mismatch",
                target.schema
            )));
        }
        if target.opening_proof.digest != target.opening_proof.expected_digest() {
            return Err(SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim artifact {:?} opening-proof digest mismatch",
                target.schema
            )));
        }
        if target.digest != target.expected_digest() {
            return Err(SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim artifact {:?} opening target digest mismatch",
                target.schema
            )));
        }
    }
    if artifact.eval_claim_bundle.digest != artifact.eval_claim_bundle.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side-eval-claim artifact eval-claim bundle digest mismatch".into(),
        ));
    }
    if artifact.digest != artifact.expected_digest() {
        return Err(SimpleKernelError::Bridge(
    ...
    artifact
        ...
        .map(|claim| {
            let witness = witness_by_schema
                ...
                .ok_or_else(|| {
                    SimpleKernelError::Bridge(format!(
                        "RV32IM side-eval-claim artifact is missing a reconstructed witness for {:?}",
            ...
            FamilyEvalClaimWitness::new(claim.clone(), witness).map_err(|err| {
                SimpleKernelError::Bridge(format!(
                    "RV32IM side-eval-claim artifact {:?} claim payload does not match the carried opening target: {err}",
                    claim.payload.schema
                ))
            })
        })
...
fn verify_phase0_claim_surface(
    ...
    if claim.opened_object != summary.opened_object || claim.commitment_context != summary.commitment_context {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM side-eval-claim relation {:?} claim does not match the carried opened-object summary",
            claim.payload.schema
        )));
    }
    ...
    if expected_binding_digest != claim.binding_digest {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM side-eval-claim relation {:?} binding digest does not match the carried theorem surfaces",
            claim.payload.schema
        )));
    }
    ...
    if expected_point != claim.point {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM side-eval-claim relation {:?} point does not match the carried Phase 0 derivation",
            claim.payload.schema
        )));
    }
...
fn verify_phase0_claim_coverage(
    ...
    if actual_schemas != expected_schemas {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM side-eval-claim relation active schema mismatch: expected {:?}, got {:?}",
    ...
    for schema in expected_schemas {
        ...
        if actual_slots != expected_slots {
            return Err(SimpleKernelError::Bridge(format!(
                "RV32IM side-eval-claim relation {:?} slot coverage mismatch: expected {:?}, got {:?}",
...
fn family_binding_anchor_digest(side_bundle: &Rv32imSideProofBundle, schema: FamilyEvalSchemaId) -> [u8; 32] {
    match schema {
...
pub(crate) fn build_rv32im_phase0_binding_surface_from_side_bundle(
    side_bundle: &Rv32imSideProofBundle,
) -> Rv32imPhase0BindingSurface {
    let targets = active_phase0_schemas_from_side_bundle(side_bundle)
        .into_iter()
        .map(|schema| {
            let mut target = Rv32imPhase0BindingTarget {
                schema,
        ...
        .collect();
    let mut surface = Rv32imPhase0BindingSurface {
        targets,
        digest: [0; 32],
    };
...
pub(crate) fn nightstream_phase0_binding_digest(
    ...
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/nightstream/rv32im/phase0_binding");
    tr.append_message(
        b"neo.fold.next/nightstream/rv32im/phase0_binding/statement_core_digest",
        &statement_core_digest,
    );
    tr.append_message(
        b"neo.fold.next/nightstream/rv32im/phase0_binding/base_binding_digest",
        &base_binding_digest,
    );
    tr.digest32()
}
        ...
        .map(|claim_witness| {
            ...
            let payload = FamilyEvalPayload::new(
                ...
                claim_witness
                    ...
                    .map_err(|err| {
                        SimpleKernelError::Bridge(format!(
                            "RV32IM Nightstream Phase 0 {:?} payload evaluation failed while rebinding to the carried side bundle: {err}",
                            schema
                        ))
                    })?,
            )
            .map_err(|err| {
                SimpleKernelError::Bridge(format!(
                    "RV32IM Nightstream Phase 0 {:?} payload rebuild failed while rebinding to the carried side bundle: {err}",
                    schema
                ))
            })?;
            ...
            .map_err(|err| {
                SimpleKernelError::Bridge(format!(
                    "RV32IM Nightstream Phase 0 {:?} claim rebuild failed while rebinding to the carried side bundle: {err}",
                    schema
                ))
            })?;
            FamilyEvalClaimWitness::new(claim, claim_witness.witness.clone()).map_err(|err| {
                SimpleKernelError::Bridge(format!(
                    "RV32IM Nightstream Phase 0 {:?} rebound claim does not match the carried opened-object witness: {err}",
                    schema
                ))
            })
        })
