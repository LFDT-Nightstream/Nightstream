...
/-! Candidate PiRLC parent with the one-window wide sampler. The four
combination families and the input/output bindings keep their existing
interfaces. The parent adds no witness or row. Production selection is
separate from this candidate circuit. -/
...
structure Interface (logicalWidth : Nat)
  ...
  baseOffset : Nat := 0
  initialState : Nat → ProjectedBatch.EState
...
def samplerInterface
    ...
    {publicFits : ringDegree * publicRingColumns ≤
      Phi81CarrierLayout.carrierWidth logicalWidth}
    (interface : Interface logicalWidth publicFits) : ProjectedBatch.Interface where
...
def commitmentInterface
  ...
  challenge := fun _ source =>
    ProjectedBatch.outputChallenge interface.baseOffset source
...
def commitmentOffset (offset : Nat) : Nat :=
  samplerOffset offset + ProjectedBatch.privateCount

def publicInputOffset (offset : Nat) : Nat :=
  commitmentOffset offset + 20196
...
def samplerCircuit
    ...
    {publicFits : ringDegree * publicRingColumns ≤
      Phi81CarrierLayout.carrierWidth logicalWidth}
    (interface : Interface logicalWidth publicFits) : FormalCircuit :=
  ProjectedBatch.circuit (samplerInterface interface)
...
structure Assumptions
    ...
    (interface : Interface logicalWidth publicFits) (offset : Nat)
    (env : Env) : Prop where
  sampler : ProjectedBatch.Assumptions (samplerInterface (atOffset interface offset))
...
structure SpecHolds
  ...
  sampler : ProjectedBatch.SpecHolds
...
theorem soundness
  ...
  refine {
    ...
    sampler := childSpec_of_rows "pirlc.wide.sampler"
...
@[simp] private theorem samplerOp_localLength
    ...
    {publicFits : ringDegree * publicRingColumns ≤
      Phi81CarrierLayout.carrierWidth logicalWidth}
    (interface : Interface logicalWidth publicFits) (offset : Nat) :
    (childOp "pirlc.wide.sampler" (samplerCircuit interface) offset).localLength =
      ProjectedBatch.privateCount := by
  rw [childOp, Sequence.childOp_localLength]
  exact ProjectedBatch.localLength_eq (samplerInterface interface) offset
...
@[simp] private theorem samplerOp_rowCount
    ...
    {publicFits : ringDegree * publicRingColumns ≤
      Phi81CarrierLayout.carrierWidth logicalWidth}
    (interface : Interface logicalWidth publicFits) (offset : Nat) :
    (childOp "pirlc.wide.sampler" (samplerCircuit interface) offset).rowCount =
      ProjectedBatch.logicalRowCount := by rfl
...
theorem flatConstraints_length
  ...
  simp [opsAt, rowCount, logicalRowCount, ProjectedBatch.counts.2]
