...
theorem piCcs_endpoint_value (program : Program)
    ...
    (slot : Fin 8) : Form program ((PiCCSOrdinaryRetainedBlocks.outputEndpointBlock program).form
...
theorem piCcs_endpoint (program : Program)
    ...
    Form program (PiCCSOrdinaryDirectPlan.endpointForm geometry column) := by
...
theorem piCcs_source (program : Program)
    (geometry : PiCCSOrdinaryRetainedGeometry.Geometry program (PerApplicationFixedPoint.logicalWidth program))
    (column : Fin Layout.Stage1.Spartan.spartanColumnCount) :
    Form program ((PiCCSOrdinaryDirectPlan.sourceMap geometry).form column) := by
  dsimp only [PiCCSOrdinaryDirectPlan.sourceMap]
  split
  · exact piCcs_endpoint program geometry column
  · exact common_form program _ (InputSupport.location program geometry _)
...
theorem piCcs_ordinary (program : Program)
    ...
    (geometry : PiCCSOrdinaryRetainedGeometry.Geometry program (PerApplicationFixedPoint.logicalWidth program)) :
    Plans program (PiCCSOrdinaryDirectPlan.plan relation geometry) := by
  apply ordinary_plan
  intro row
  exact source_row _ _ (piCcs_source program geometry) (one program _ rfl) _ _

theorem nextPreimage (program : Program)
    (geometry : PiCCSOrdinaryRetainedGeometry.Geometry program (PerApplicationFixedPoint.logicalWidth program)) :
    Plans program (NextPreimageDirectPlan.plan geometry) := by
  apply source_plan
