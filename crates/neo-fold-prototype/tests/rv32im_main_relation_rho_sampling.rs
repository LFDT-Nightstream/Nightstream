...
use neo_fold_prototype::circuit::superneo::rho_sampling::{
    materialize_goldilocks_rot_matrices, sample_goldilocks_rot_rhos,
};
use neo_fold_prototype::circuit::superneo::transcript::Poseidon2TranscriptCircuit;
use neo_math::D;
...
fn rv32im_main_relation_rho_sampler_matches_native_goldilocks_sampler() {
    let params = NeoParams::goldilocks_paper_b2();
    let ring = RotRing::goldilocks();
    let mut native_tr = Poseidon2Transcript::new(b"test/rv32im/rho_sampling");
    ...
    let mut circuit_tr =
        Poseidon2TranscriptCircuit::new(cs.namespace(|| "tr"), b"test/rv32im/rho_sampling").expect("transcript");
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_rho_sampler_handles_k_rho_plus_one_claims() {
    let params = NeoParams::goldilocks_paper_b2();
    let mut cs = TestConstraintSystem::<SpartanF>::new();
    let mut circuit_tr =
        Poseidon2TranscriptCircuit::new(cs.namespace(|| "tr"), b"test/rv32im/rho_sampling/full").expect("transcript");
