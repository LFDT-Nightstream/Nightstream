use neo_ccs::{
    matrix::Mat, poly::{SparsePoly, Term}, relations::{check_ccs_rowwise_zero, check_ccs_rowwise_relaxed},
    CcsStructure
};
use neo_math::F;
...
fn ccs_relaxed_rejects_wrong_rhs() {
    ...
    let x = vec![F::from_u64(3)]; let w = vec![F::from_u64(4)]; // M z = 11
    // Claim e*u = 12, but actual is 11 → reject
    let u = vec![F::from_u64(6)]; let e = F::from_u64(2);
...
fn ccs_relaxed_accepts_correct_rhs() {
    ...
    let s = CcsStructure::new(vec![m], f).unwrap();
    let x = vec![F::from_u64(3)]; let w = vec![F::from_u64(4)]; // M z = 11
    let u = vec![F::ONE]; let e = F::from_u64(11);
...
fn ccs_structure_validation_edge_cases() {
    ...
    let f2 = SparsePoly::new(3, vec![Term{ coeff: F::ONE, exps: vec![1,0,0] }]); // arity 3
...
fn polynomial_evaluation_tampering_detection() {
    ...
    let f = SparsePoly::new(2, vec![
        Term{ coeff: F::ONE, exps: vec![2,0] },      // x^2
        Term{ coeff: F::ONE, exps: vec![0,2] },      // y^2  
        Term{ coeff: -F::from_u64(5), exps: vec![0,0] }, // -5
    ]);
...
fn matrix_polynomial_consistency_attack() {
    ...
    let f_single = SparsePoly::new(1, vec![Term{ coeff: F::ONE, exps: vec![1] }]);
    let f_double = SparsePoly::new(2, vec![Term{ coeff: F::ONE, exps: vec![1,1] }]);
...
fn large_polynomial_degree_handling() {
    ...
    let f = SparsePoly::new(2, vec![
        Term{ coeff: F::ONE, exps: vec![10,0] },     // x^10
        Term{ coeff: F::ONE, exps: vec![0,10] },     // y^10
        Term{ coeff: -F::from_u64(2), exps: vec![5,5] }, // -2*x^5*y^5
    ]);
