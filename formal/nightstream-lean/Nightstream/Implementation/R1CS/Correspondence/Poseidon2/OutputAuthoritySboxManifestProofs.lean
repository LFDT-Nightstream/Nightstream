...
|---|---|---|---|---|
| `allCalls_transport` | 422 renamed isolated layouts | `poseidon2Call_transport` holds for every listed call | source rows still need `SourceCallRowsMatch` | no |
| `candidate_count` | compact Cartesian census | exactly 36,292 derived candidate entries | manifest certificate | no |
...
theorem use_census
    (manifest : Manifest) (certificate : manifest.Certificate) :
    manifest.census.definitionCUses = 36292 ∧
      manifest.census.linearAUses = 290336 ∧
      manifest.census.totalMatrixUses = 326628 := by
  rcases certificate.censusValid with
