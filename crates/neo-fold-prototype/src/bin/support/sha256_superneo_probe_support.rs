...
pub(super) fn print_spartan(
    ...
    let public_input_len = steps
        .first()
        .map(|step| step.mcs.m_in)
        .ok_or_else(|| invalid_input("direct SHA256 Spartan compression requires at least one fold claim"))?;
    let program = DirectCcsProgram::new_with_public_input_len(params, ccs, public_input_len)?;
    let mut recursive_state = start_direct_ccs_proof_state(program)?;
    ...
    if ccs.n > 1024 {
        println!("direct CCS recursive compression: skipped");
        println!(
            "reason: SHA256 CCS shape is large (rows={}, cols={}, matrices={}); full Spartan terminal proving is intentionally not run by default",
            ccs.n,
            ccs.m,
            ccs.t()
        );
        println!("spartan proof: not produced for this SHA256 diagnostic run");
        println!();
        return Ok(None);
    }
    ...
    println!(
        ...
        perf.r1cs.public_inputs,
        perf.chunks.count,
        perf.chunks.constraints_first4,
        perf.constraints.construction2_fold,
        perf.constraints.construction2_fold_final_ce_consistency,
        perf.constraints.public_link,
        perf.chunks.done_constraints,
        perf.committed.constraints,
        perf.final_ce.relation_constraints,
        perf.final_ce.relation_constraints
    );
    println!(
        "terminal Construction-2 boundary: committed_width={}, source_values={}, commitment_words={}",
        perf.committed.width, perf.committed.source.values, perf.committed.commitment_words
    );
    println!(
        "terminal source encodings: bit={}, u32={}, u64={}, unclassified={}",
        perf.committed.source.bit_values,
        perf.committed.source.u32_values,
        perf.committed.source.u64_values,
        perf.committed.source.unclassified_private_values
    );
    let tb = perf.committed.breakdown;
    ...
    println!(
        ...
        perf.chunks.constraints_by_chunk,
        perf.constraints.construction2_fold,
        perf.constraints.construction2_fold_final_ce_consistency,
        perf.constraints.public_link,
        perf.chunks.done_constraints,
    ...
    println!();
    println!("terminal_nifs_chunk_constraints={:?}", perf.chunks.constraints_by_chunk);
    println!(
        "final_ce_private_checks=[projection_digest=0, digest_match=0, relation={}]",
        perf.final_ce.relation_constraints
    );
    let ce = perf.final_ce.relation_breakdown;
    println!(
    ...
    println!(
        ...
        perf.r1cs.nnz,
        perf.r1cs.nnz as f64 / perf.r1cs.sizes[0].max(1) as f64
    );
    println!(
        "proof bytes: final_serialized={}, snark_data={}, wrapper_overhead={}",
        recursive_perf.total_proof_bytes,
        perf.proof.snark_bytes + recursive_perf.f_prime_chain_proof_bytes + recursive_perf.f_prime_final_ce_proof_bytes,
        recursive_perf.total_proof_bytes.saturating_sub(
            perf.proof.snark_bytes
                + recursive_perf.f_prime_chain_proof_bytes
                + recursive_perf.f_prime_final_ce_proof_bytes
        )
    );
    ...
    match spartan_perf {
        Some(spartan_perf) => {
            ...
            println!(
                ...
                terminal.timing.setup_ms + spartan_perf.f_prime_chain_setup_ms + spartan_perf.f_prime_final_ce_setup_ms
            );
            println!(
                "proving (total): {:.3} ms",
            ...
            let total_constraints = terminal.r1cs.sizes[0]
                + spartan_perf.f_prime_chain_constraints
                + spartan_perf.f_prime_final_ce_constraints;
            let total_padded = terminal.r1cs.sizes[4]
