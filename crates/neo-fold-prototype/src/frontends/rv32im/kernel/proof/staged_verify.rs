//! Owns accepted-proof staged verification and transcript replay for RV32IM.

use neo_transcript::{Poseidon2Transcript, Transcript};
use serde::{Deserialize, Serialize};
use std::borrow::Cow;

use crate::rv32im::kernel::root_lane_witness::{
...
use super::perf_diagnostics::{
    ...
    AcceptedStagePackageVerifyPerf, Rv32imPublicProofVerifyPerf,
};
use super::proof_accepted::Rv32imAcceptedProofArtifact;
use super::proof_api::{Rv32imMainLaneProofBundle, Rv32imProofStatement};
...
use super::proof_witness::{
    ...
    verify_stage_claim_packaged_proof, Rv32imKernelExportClaimProof, Rv32imKernelOpeningProofBundle,
...
use super::{
    rv32im_simple_root_context_id_for_schedule, RootLaneColumns, RootLaneCommitmentSummaryArtifact,
...
pub struct Rv32imStage1ExportProof {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Rv32imStage2ExportProof {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Rv32imStage3ExportProof {
...
pub(crate) struct Rv32imAcceptedProofCoreInputs<'a> {
    pub statement: &'a Rv32imProofStatement,
    pub stage_claims: &'a Rv32imStageClaimProofBundle,
    pub stage_packages: &'a Rv32imStagePackageProofBundle,
    pub kernel_opening: &'a Rv32imKernelOpeningProofBundle,
    ...
    pub main_lane: &'a Rv32imMainLaneProofBundle,
    pub stage1: Rv32imStage1ExportProof,
    pub stage2: Rv32imStage2ExportProof,
    pub stage3: Rv32imStage3ExportProof,
    pub root_execution: &'a RootExecutionBundle,
}

impl Rv32imStage1ExportProof {
    fn expected_digest_from_parts(
        rows_digest: [u8; 32],
        semantics_digest: [u8; 32],
        selected_opening_digest: [u8; 32],
    ) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_export_proof");
        tr.append_message(b"rv32im/stage1_export_proof/rows_digest", &rows_digest);
        tr.append_message(b"rv32im/stage1_export_proof/semantics", &semantics_digest);
        tr.append_message(b"rv32im/stage1_export_proof/selected_opening", &selected_opening_digest);
        tr.digest32()
    }
...
impl Rv32imStage2ExportProof {
    fn expected_digest_from_parts(
        ...
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage2_export_proof");
        tr.append_message(
            b"rv32im/stage2_export_proof/register_reads_family_digest",
            &register_reads_family_digest,
        );
        tr.append_message(
            b"rv32im/stage2_export_proof/register_writes_family_digest",
            &register_writes_family_digest,
        );
        tr.append_message(
            b"rv32im/stage2_export_proof/ram_events_family_digest",
            &ram_events_family_digest,
        );
        tr.append_message(
            b"rv32im/stage2_export_proof/twist_links_family_digest",
            &twist_links_family_digest,
        );
        tr.append_message(
            b"rv32im/stage2_export_proof/register_timeline_digest",
            &register_timeline_digest,
        );
        tr.append_message(b"rv32im/stage2_export_proof/ram_timeline_digest", &ram_timeline_digest);
        tr.append_message(b"rv32im/stage2_export_proof/twist_links_digest", &twist_links_digest);
        tr.append_message(b"rv32im/stage2_export_proof/temporal_digest", &temporal_digest);
        tr.append_message(b"rv32im/stage2_export_proof/semantics", &semantics_digest);
        tr.append_message(b"rv32im/stage2_export_proof/selected_opening", &selected_opening_digest);
        tr.digest32()
    }
...
impl Rv32imStage3ExportProof {
    pub(crate) fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage3_export_proof");
        tr.append_message(b"rv32im/stage3_export_proof/bridge", &self.bridge.digest);
        tr.append_message(b"rv32im/stage3_export_proof/semantics", &self.semantics.digest);
        tr.append_message(
            b"rv32im/stage3_export_proof/selected_opening",
            &self.selected_opening.digest,
        );
        tr.digest32()
    }
...
pub(crate) fn stage1_export_proof_from_bundle(bundle: &Stage1ProofBundle) -> Rv32imStage1ExportProof {
    let proof = Rv32imStage1ExportProof {
        rows: bundle.row_bindings.clone(),
        semantics: bundle.semantics.clone(),
        selected_opening: bundle.selected_opening.clone(),
        digest: [0; 32],
    };
    Rv32imStage1ExportProof {
        digest: proof.expected_digest(),
        ..proof
    }
}

pub(crate) fn stage2_export_proof_from_bundle(bundle: &Stage2ProofBundle) -> Rv32imStage2ExportProof {
    let proof = Rv32imStage2ExportProof {
    ...
    Rv32imStage2ExportProof {
        digest: proof.expected_digest(),
        ..proof
    }
}

pub(crate) fn stage3_export_proof_from_bundle(bundle: &Stage3ProofBundle) -> Rv32imStage3ExportProof {
    let proof = Rv32imStage3ExportProof {
        bridge: bundle.bridge.clone(),
        semantics: bundle.semantics.clone(),
        selected_opening: bundle.selected_opening.clone(),
        digest: [0; 32],
    };
    Rv32imStage3ExportProof {
        digest: proof.expected_digest(),
        ..proof
    }
}
...
pub(crate) fn derive_stage1_export_proof(
    execution_rows: &[Rv32ExpandedRow],
...
impl<'a> From<&'a Rv32imAcceptedProofArtifact> for Rv32imAcceptedProofCoreInputs<'a> {
    fn from(value: &'a Rv32imAcceptedProofArtifact) -> Self {
        Self {
            ...
            kernel_claims: kernel_export_claim_proof_from_bundle(&value.kernel_claims)
                .expect("RV32IM accepted proof should derive a compact export kernel-claim proof"),
...
fn export_terminal_row(inputs: &Rv32imAcceptedProofCoreInputs<'_>) -> Result<(u64, bool), SimpleKernelError> {
...
fn verify_kernel_claim_surface_bindings(inputs: &Rv32imAcceptedProofCoreInputs<'_>) -> Result<(), SimpleKernelError> {
...
impl Stage1VerifiedClaims {
    pub(crate) fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_verified_claims");
...
impl Stage2VerifiedClaims {
    pub(crate) fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage2_verified_claims");
...
impl Stage3VerifiedClaims {
    pub(crate) fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage3_verified_claims");
...
fn validate_stage1_carried_sem_inputs(
    ...
    if sem_inputs.len() != rows.len() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 semantic inputs length mismatch".into(),
        ));
    }

    let mut digest_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_sem_inputs");
    digest_tr.append_u64s(b"rv32im/stage1_sem_inputs/len", &[sem_inputs.len() as u64]);
    ...
    for (row, sem_input) in rows.iter().zip(sem_inputs.iter()) {
        ...
        if sem_input != &expected {
            return Err(SimpleKernelError::Bridge(
                "RV32IM stage1 semantic inputs mismatch".into(),
            ));
        }
        digest_tr.append_message(b"rv32im/stage1_sem_inputs/entry", &sem_in_digest(sem_input));
...
fn build_stage1_sem_inputs_surface(rows: &[Rv32ExpandedRow]) -> (Vec<SemIn>, ValidatedStage1SemInputSurface) {
    let mut sem_inputs = Vec::with_capacity(rows.len());
    let mut digest_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_sem_inputs");
    digest_tr.append_u64s(b"rv32im/stage1_sem_inputs/len", &[rows.len() as u64]);
    let mut sequence_count = 0u64;
    let mut helper_row_count = 0u64;
...
fn validate_stage1_carried_row_bindings(
    ...
    if row_bindings.len() != rows.len() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 row-binding length mismatch".into(),
        ));
    }

    let mut digest_tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/stage1_rows_family");
    digest_tr.append_u64s(b"rv32im/stage1_rows_family/len", &[row_bindings.len() as u64]);
    ...
    for (index, (row, row_binding)) in rows.iter().zip(row_bindings.iter()).enumerate() {
        ...
        if row_binding != &expected {
            return Err(SimpleKernelError::Bridge("RV32IM stage1 row bindings mismatch".into()));
        }
        digest_tr.append_message(b"rv32im/stage1_rows_family/row_digest", &stage1_row_digest(row_binding));
...
fn verify_stage1_native_bundle_surface_with_reuse<'a>(
    artifact: &'a Rv32imAcceptedProofArtifact,
    ...
    if artifact.stage1.digest != artifact.stage1.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 proof bundle digest mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 native bytecode surface mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 native ALU surface mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 native branch surface mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 native address-correctness surface mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 native linkage surface mismatch".into(),
        ));
    }
...
fn verify_stage1_with_perf(
    inputs: &Rv32imAcceptedProofCoreInputs<'_>,
    ...
    if inputs.stage1.semantics.digest != inputs.stage1.semantics.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 semantics proof digest mismatch".into(),
        ));
    }
    ...
    if inputs.stage1.digest
        != Rv32imStage1ExportProof::expected_digest_from_parts(
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 export proof digest mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 semantic surface digest mismatch".into(),
        ));
    }
    ...
    if inputs.stage_packages.packages.stage1.digest != inputs.stage1.selected_opening.digest {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage1 selected opening does not match the carried stage package".into(),
        ));
    }
...
fn verify_stage2_with_perf(
    ...
    if inputs.stage2.semantics.digest != inputs.stage2.semantics.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage2 export sub-proof digest mismatch".into(),
        ));
    }
    let semantics_started = Instant::now();
    ...
    if inputs.stage2.digest
        != Rv32imStage2ExportProof::expected_digest_from_parts(
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage2 export proof digest mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage2 semantic surface digest mismatch".into(),
        ));
    }
    if inputs
        .stage_claims
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage2 linkage digest mismatch".into(),
        ));
    }
    ...
    if inputs.stage_packages.packages.stage2.digest != inputs.stage2.selected_opening.digest {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage2 selected opening does not match the carried stage package".into(),
        ));
    }
    let selected_opening_ms = millis_since(selected_opening_started);
...
pub(crate) fn verify_stage3(
    ...
    if inputs.stage3.digest != inputs.stage3.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage3 export proof digest mismatch".into(),
        ));
    }
    ...
    if summary != expected_summary {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage3 continuity surface mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage3 export sub-proof digest mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage3 semantic bridge mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage3 continuity claim digest mismatch".into(),
        ));
    }
    ...
    if inputs.stage_packages.packages.stage3.digest != inputs.stage3.selected_opening.digest {
        return Err(SimpleKernelError::Bridge(
            "RV32IM stage3 selected opening does not match the carried stage package".into(),
        ));
    }
...
pub(crate) fn build_verified_stage3_claim_from_accepted_artifact(
    ...
    let inputs = Rv32imAcceptedProofCoreInputs::from(artifact);
...
fn verify_transcript_surface_bindings(
    ...
    if transcript.digest != transcript.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM verified transcript surface digest mismatch".into(),
        ));
    }
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM transcript surface does not match the carried stage-claim or terminal bindings".into(),
        ));
    }
    Ok(())
}
...
fn verify_root_execution(
    ...
    if inputs.root_execution.digest != inputs.root_execution.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM root execution bundle digest mismatch".into(),
        ));
    }
    if inputs.root_execution.family_digest != inputs.root_lane_columns.family_digest {
        return Err(SimpleKernelError::Bridge(
            "RV32IM root execution family digest mismatch".into(),
        ));
    }
...
fn verify_step_composition_surface(artifact: &Rv32imAcceptedProofArtifact) -> Result<(), SimpleKernelError> {
    ...
    if artifact.step_composition != expected {
        return Err(SimpleKernelError::Bridge(
            "RV32IM step composition surface mismatch".into(),
        ));
    }
    Ok(())
}

fn verify_soundness_accounting_surface(artifact: &Rv32imAcceptedProofArtifact) -> Result<(), SimpleKernelError> {
    let expected = canonical_kernel_soundness_accounting_surface();
    if artifact.soundness_accounting != expected {
        return Err(SimpleKernelError::Bridge(
            "RV32IM soundness accounting surface mismatch".into(),
        ));
    }
    Ok(())
}
...
pub(crate) fn verify_accepted_proof_core_with_transcript_surface_with_perf(
    ...
    {
        return Err(SimpleKernelError::Bridge(
            "RV32IM accepted proof core bundle digest mismatch".into(),
        ));
    }
    ...
    Ok(Rv32imPublicProofVerifyPerf {
...
pub(crate) fn verify_accepted_proof_artifact_export_core_with_perf(
    ...
    let mut perf = verify_accepted_proof_core_with_transcript_surface_with_perf(
        &Rv32imAcceptedProofCoreInputs::from(artifact),
        &transcript,
        None,
    )?;
...
pub(crate) fn verify_accepted_proof_artifact_with_perf(
    ...
    let total_started = Instant::now();
    let expected_root_params_id = rv32im_simple_root_context_id_for_schedule(
        artifact.statement.fold_schedule,
        usize::try_from(artifact.statement.public_step_count).map_err(|_| {
            SimpleKernelError::Bridge(
                "RV32IM accepted proof public_step_count does not fit into the local root-family selector".into(),
            )
        })?,
    )?;
    if artifact.statement.root_params_id != expected_root_params_id {
        return Err(SimpleKernelError::Bridge(
            "RV32IM accepted proof root params id does not match the declared fold schedule and public-step count"
                .into(),
        ));
    }
    if artifact.digest != artifact.expected_digest() {
