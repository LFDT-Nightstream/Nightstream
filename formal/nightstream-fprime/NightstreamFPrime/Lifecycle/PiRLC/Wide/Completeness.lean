...
/-! Constructive completion of the candidate seven-child PiRLC phase.
The wide sampler is total, so this parent has no availability premise.
The existing four ring-combination witnesses are composed without new rows. -/
...
private theorem appendSampler
  ...
  have builtAssumptions : ProjectedBatch.Assumptions
  ...
  have childScope : ∀ expression ∈ flatConstraints
    ...
    change ∀ expression ∈ flatConstraints (ProjectedBatch.operations (samplerInterface shared) (samplerOffset offset)),
      expression.VarsBelow (samplerOffset offset + localLength (ProjectedBatch.operations (samplerInterface shared) (samplerOffset offset)))
    rw [ProjectedBatch.localLength_eq]
    exact ProjectedBatch.scope (samplerInterface shared) (samplerOffset offset) builtAssumptions
  rcases Sequence.appendBuiltAt before "pirlc.wide.sampler"
  ...
  have endEq : offset + localLength after.operations = commitmentOffset offset := by
    calc
      ...
      _ = commitmentOffset offset := by
        change samplerOffset offset + localLength (ProjectedBatch.operations (samplerInterface shared) (samplerOffset offset)) = _
        rw [ProjectedBatch.localLength_eq]
        rfl
...
/-- Every input transcript has a complete local PiRLC witness. No sampler
availability or shortfall premise is required. -/
theorem completePrefix_constructive
    {logicalWidth : Nat}
  ...
  obtain ⟨completed, operations⟩ := completePrefixFromSampler relation interface env offset assumptions
    (ProjectedBatch.complete (samplerInterface (atOffset interface offset)) env (samplerOffset offset) assumptions.sampler)
  ...
  have specification := soundness relation interface offset completed.current
    (assumptionsAt assumptions completed.current) (holdsFlat_implies_holds completed.current _ rows)
...
theorem completeness
  ...
  rcases completePrefix relation ajtai interface env offset assumptions _phase with
