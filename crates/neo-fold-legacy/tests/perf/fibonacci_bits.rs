...
use neo_fold_legacy::config::R1CS_PROFILE;
use neo_fold_legacy::engine::decider::synthesize_statement_r1cs;
use neo_fold_legacy::frontends::direct_ccs::{self, R1cs};
use neo_fold_legacy::paper::construction2::{self, FoldProof, ProofState, State};
...
use neo_fold_legacy::paper::f_prime::r1cs::{encode_f_prime_superneo_public_input, F_PRIME_SUPERNEO_PUBLIC_INPUT_LEN};
...
fn f_prime_base_state(prep: &neo_fold_legacy::lifecycle::Preprocessing) -> State {
...
fn f_prime_state_x_out(prep: &neo_fold_legacy::lifecycle::Preprocessing, state: &State) -> [F; 4] {
...
fn f_prime_peek_next_state(
    prep: &neo_fold_legacy::lifecycle::Preprocessing,
    state: &State,
    batch: CcsInstance,
) -> State {
...
fn proof_shape(
    audit: &neo_fold_legacy::lifecycle::UncompressedAudit,
    step_prove_ms: &[f64],
    finish_ms: f64,
...
fn fibonacci_decider_r1cs_shape_snapshot() {
    ...
    let mut public_batches: Vec<Vec<neo_fold_legacy::paper::relations::CcsClaim>> = Vec::with_capacity(transitions);
...
fn print_report(
    r1cs: &R1cs,
    prep: &neo_fold_legacy::lifecycle::Preprocessing,
    finished: &neo_fold_legacy::lifecycle::UncompressedAudit,
    shape: &ProofShape,
    fold_rows: &[FoldRow],
    values: &[u64],
    transitions: usize,
