...
use neo_fold_legacy::engine::ccs_native::poseidon2::{
...
use neo_fold_legacy::paper::reductions::pi_ccs;
use neo_fold_legacy::{config, CcsInstance, Params, Structure};
use neo_math::{D, F, K};
...
fn prove_verify_single_fresh(structure: &Structure, z: Vec<F>) -> Result<Vec<neo_fold_legacy::CeClaim>, pi_ccs::Error> {
    let params = config::ccs_params(structure.n, structure.m, structure.t(), structure.max_degree()).expect("params");
    install_ajtai_module(&params, structure);
    let cols = structure.m.div_ceil(D);
    let log = AjtaiSModule::from_global_for_dims(D, cols).expect("Ajtai module");
...
fn r1cs_poseidon2_hash_shape(input: &[F]) -> (usize, usize) {
    use neo_fold_legacy::engine::r1cs_circuit::poseidon2::enforce_poseidon2_hash;
    let mut builder = R1csBuilder::new();
...
fn bit_backed_hash_values_walk_matches_builder() {
    use neo_fold_legacy::engine::ccs_native::poseidon2::build_bit_backed_poseidon2_hash_values;
