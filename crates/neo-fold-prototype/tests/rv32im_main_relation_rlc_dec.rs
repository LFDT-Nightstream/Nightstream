...
use neo_fold_prototype::circuit::superneo::claim::alloc_ce_claim;
use neo_fold_prototype::circuit::superneo::pi_dec::{
    enforce_dec_public_non_commitment, enforce_dec_public_with_constant_children,
};
use neo_fold_prototype::circuit::superneo::pi_rlc::{
    enforce_rlc_public_non_commitment, enforce_rlc_public_non_commitment_with_rho_vars,
};
use neo_fold_prototype::circuit::superneo::rho_sampling::alloc_rot_rho_matrices_from_native;
use neo_math::{D, F, K};
...
fn rv32im_main_relation_rlc_dec_gadgets_accept_public_equalities() {
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_rlc_dec_gadgets_reject_tampered_parent() {
    let child0 = claim_from_scalars(2, 3, 5, 7, 11, 13, 17);
    let child1 = claim_from_scalars(19, 23, 29, 31, 37, 41, 43);
    ...
    assert!(!cs.is_satisfied(), "tampered RLC parent CE value must fail");
}

#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_dec_constant_children_accept_public_equalities() {
    let child0 = claim_from_scalars(2, 3, 5, 7, 11, 13, 17);
    let child1 = claim_from_scalars(19, 23, 29, 31, 37, 41, 43);
    let dec_parent = claim_from_scalars(
        2 + 4 * 19,
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_dec_constant_children_reject_tampered_parent() {
    let child0 = claim_from_scalars(2, 3, 5, 7, 11, 13, 17);
    let child1 = claim_from_scalars(19, 23, 29, 31, 37, 41, 43);
    let mut bad_parent = claim_from_scalars(
        2 + 4 * 19,
    ...
    assert!(
        !cs.is_satisfied(),
        "tampered constant-child DEC parent CE value must fail"
    );
}
...
fn rv32im_main_relation_dec_public_rejects_embedded_x_lane_tamper() {
    ...
    enforce_dec_public_with_constant_children(&mut cs, &parent_var, &[child0, child1], 4, "dec_const")
        .expect("enforce dec const");
