...
fn build_shout_only_bus_z(
    ...
    if ell_addr != 35 {
        return Err(format!(
            "build_shout_only_bus_z: expected ell_addr=35 for packed SLTU (got ell_addr={ell_addr})"
        ));
    }
    ...
    for (lane_idx, cols) in shout.lanes.iter().enumerate() {
        ...
        for j in 0..t {
            ...
            if has_lookup {
                // addr_bits: [lhs_u32, rhs_u32, diff_u32, diff_bits[0..32]]
                ...
                for bit in 0..32 {
                    let b = (diff >> bit) & 1;
                    packed[3 + bit] = if b == 1 { F::ONE } else { F::ZERO };
                }
...
#[test]
fn riscv_trace_shared_cpu_bus_shout_sltu_semantics_redteam() {
    // Program:
    // - LUI x1, 0        (x1 = 0)
    // - LUI x2, 1        (x2 = 4096)
    // - SLTU x3, x1, x2  (1)
    ...
    // Shout instance: SLTU table, 1 lane (tamper one diff bit).
    let t = exec.rows.len();
    let shout_table_ids = vec![RiscvShoutTables::new(32).opcode_to_id(RiscvOpcode::Sltu).0];
    let shout_lanes = extract_shout_lanes_over_time(&exec, &shout_table_ids, /*xlen=*/ 32).expect("extract shout lanes");

    let sltu_lut_inst = LutInstance::<Cmt, F> {
        table_id: 0,
        comms: Vec::new(),
        k: 0,
        d: 35,
    ...
    let mut sltu_z = build_shout_only_bus_z(
        ccs.m,
        layout.m_in,
        t,
        /*ell_addr=*/ sltu_lut_inst.d * sltu_lut_inst.ell,
        /*lanes=*/ 1,
        &shout_lanes,
        &x,
    )
    ...
    let j = lane0
        ...
        .iter()
        .position(|&b| b)
        .expect("expected at least one SLTU lookup");

    let bus = build_bus_layout_for_instances_with_shout_and_twist_lanes(
        ccs.m,
        layout.m_in,
        t,
        core::iter::once((/*ell_addr=*/ 35usize, /*lanes=*/ 1usize)),
        core::iter::empty::<(usize, usize)>(),
    )
    .expect("bus layout");
    ...
    let diff_bit0_col_id = cols
        ...
        .clone()
        .nth(3)
        .expect("expected addr_bits[3] for diff bit 0");
    let cell = bus.bus_cell(diff_bit0_col_id, j);
    sltu_z[cell] = if sltu_z[cell] == F::ONE { F::ZERO } else { F::ONE };

    let sltu_Z = neo_memory::ajtai::encode_vector_balanced_to_mat(&params, &sltu_z);
    let sltu_c = l.commit(&sltu_Z);
    let sltu_lut_inst = LutInstance::<Cmt, F> {
        table_id: 0,
        comms: vec![sltu_c],
        ..sltu_lut_inst
    };
    let sltu_lut_wit = LutWitness { mats: vec![sltu_Z] };

    let steps_witness = vec![crate::common_setup::canonicalize_step_time_columns(StepWitnessBundle {
        mcs,
        lut_instances: vec![(sltu_lut_inst, sltu_lut_wit)],
    ...
    let mut tr_prove = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sltu-semantics-redteam");
    if let Ok(proof) = fold_shard_prove(
        ...
        let mut tr_verify = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sltu-semantics-redteam");
        fold_shard_verify(
            FoldingMode::Optimized,
            &mut tr_verify,
            &params,
        ...
        .expect_err("tampered packed SLTU diff bit must be caught by Route-A time constraints");
    }
}
