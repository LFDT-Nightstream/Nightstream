use nightstream_constraint_exporter::{
    render_redundancy_candidate_lean, render_removal_counterexample_candidate_lean,
...
#[test]
fn renders_a_rechecked_family_candidate_as_lean_data() {
    ...
    let lean = render_redundancy_candidate_lean(
        &complete,
        &problem,
        &certificate,
        "Generated.Artifact",
    ...
    assert!(lean.contains("familyCertificate is exact data only"));
...
#[test]
fn renders_a_rechecked_terminal_family_candidate_as_lean_data() {
    let problem = duplicate_problem();
    let complete = complete_duplicate_problem();
    let certificate = derive_scalar_certificate(&problem, &Selection::Family("duplicate".to_owned()))
        .expect("derive scalar certificate")
        .expect("duplicate row has a scalar certificate");
    let lean = render_terminal_redundancy_candidate_lean(
        &complete,
        &problem,
        &certificate,
...
fn redundancy_renderer_rejects_a_query_row_that_differs_from_the_complete_relation() {
    ...
    assert!(render_redundancy_candidate_lean(
        &complete,
        &query,
        &certificate,
        "Generated.Artifact",
...
#[test]
fn renders_a_complete_replayed_removal_candidate_as_lean_data() {
    let problem = necessary_problem();
    let model = one_model();
    let plan = ["bitness".to_owned(), "zero".to_owned()];
    let lean = render_removal_counterexample_candidate_lean(
        &problem,
        &model,
        "zero",
        "Generated.Artifact",
    ...
    assert!(lean.contains("removalCounterexample is exact data only"));
    assert!(!lean.contains("theorem removalCounterexample_valid"));
    assert!(!lean.contains("theorem necessary"));
    assert!(!lean.contains("native_decide"));

    let terminal = render_terminal_removal_counterexample_candidate_lean(
        &problem,
        &model,
        "zero",
        "Generated.TerminalArtifact",
    ...
    assert!(terminal.contains("removalCounterexample is exact data only"));
    assert!(!terminal.contains("theorem removalCounterexample_valid"));
    assert!(!terminal.contains("theorem necessary"));
    assert!(!terminal.contains("native_decide"));
}
...
fn removal_counterexample_rejects_an_incomplete_relation() {
    ...
    assert!(render_removal_counterexample_candidate_lean(
        &problem,
        &one_model(),
        "zero",
        "Generated.Artifact",
