...
use super::{
    ...
    debug_profile_rv32im_main_relation_chunk_stage_progress, digest32_as_spartan_fields,
    synthesize_rv32im_chunk_nifs_verifier_body_with_synthetic_chunk_relation_io, Rv32imChunkBoundaryPlan,
    Rv32imClaimBundle, CHUNK_META_RAW_TAG,
};
use crate::rv32im::final_relation::{Rv32imChunkFoldTranscriptSnapshot, RV32IM_CHUNK_DONE_RAW_TAG};
use crate::rv32im::kernel::{rv32im_cached_root_main_lane_optimized_cache, SimpleKernelError};
use crate::rv32im::main_relation_trace::{Rv32imMainCircuitChunkCover, Rv32imMainCircuitChunkReplaySurface};
use crate::spartan_backend::SpartanF;
use crate::superneo_circuit::pi_ccs::{bind_header_and_instance_digest, bind_me_inputs};
...
pub(super) fn derive_rv32im_fixed_transcript_out_from_chunk_body(
    payload: &Rv32imMainRecursionFPrimePayload,
    transcript_in: &Rv32imChunkFoldTranscriptSnapshot,
    replay_chunk: &Rv32imMainCircuitChunkReplaySurface,
    ...
    let (params, _, structure) =
        crate::rv32im::kernel::rv32im_root_main_lane_context_for_claim_count(payload.state_in_claims.len())?;
    let optimized_cache = rv32im_cached_root_main_lane_optimized_cache()?;
    let dims = build_dims_and_policy(&params, structure)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript dims failed: {err}")))?;
    let mat_digest: [Goldilocks; 4] = digest_ccs_matrices_with_sparse_cache(structure, Some(optimized_cache.sparse()))
        .try_into()
        .map_err(|_| SimpleKernelError::Bridge("RV32IM fixed transcript matrix digest length mismatch".into()))?;
...
fn derive_fixed_transcript_out_from_parts(
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript state allocation failed: {err}")))?;
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript state import failed: {err}")))?;
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript live-state allocation failed: {err}")))?;
    ...
    if let Err(err) = synthesize_rv32im_chunk_nifs_verifier_body_with_synthetic_chunk_relation_io(
        params,
        structure,
        dims,
        mat_digest,
        ...
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript chunk replay failed: {err}"
        )));
    }
    ...
    replayed_transcript
        ...
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript chunk_done failed: {err}")))?;
...
fn debug_profile_fixed_transcript_chunk_body(
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript profile state allocation failed: {err}"
        ))
    })?;
    let transcript_values = transcript_in
        .state
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript profile state import failed: {err}")))?;
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript profile relation digest allocation failed: {err}"
        ))
    })?;
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript profile live-state allocation failed: {err}"
        ))
    })?;
    let carried_claims = Rv32imClaimBundle::from_effective_claims(
        live_state_in_vars
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript stage profile failed: {err}")))?;
    Ok(())
}
...
fn compare_transcript_state(
    ...
    if circuit.absorbed() != native.absorbed() {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript mismatch after {label}: absorbed {} != {}",
            circuit.absorbed(),
            native.absorbed()
        )));
    }
        ...
        .enumerate()
    {
        ...
        if *circuit_value != expected {
            return Err(SimpleKernelError::Bridge(format!(
                "RV32IM fixed transcript mismatch after {label} at limb {idx}: {} != {}",
...
fn enforce_transcript_state_against_native(
    ...
    circuit
        ...
        .map_err(|err| {
            SimpleKernelError::Bridge(format!(
                "RV32IM fixed transcript {label} state enforcement failed: {err}"
            ))
        })?;
    if !cs.is_satisfied() {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript latent mismatch after {label}: {}",
...
fn append_chunk_meta_native(transcript: &mut Poseidon2Transcript, replay_chunk: &Rv32imMainCircuitChunkReplaySurface) {
...
fn debug_check_fixed_transcript_prefix(
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!("RV32IM fixed transcript prefix state allocation failed: {err}"))
    })?;
    let transcript_values = transcript_in
        .state
        .map(|value| SpartanF::from_canonical_u64(value.as_canonical_u64()));
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript prefix state import failed: {err}")))?;
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript prefix meta failed: {err}")))?;
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript prefix header binding failed: {err}")))?;
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript prefix native header binding failed: {err}"
        ))
    })?;
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript prefix live-state allocation failed: {err}"
        ))
    })?;
    let carried_claims = Rv32imClaimBundle::from_effective_claims(
        live_state_in_vars
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript prefix circuit ME binding failed: {err}"
        ))
    })?;
    bind_me_inputs_native(&mut native, state_in_claims).map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript prefix native ME binding failed: {err}"
        ))
    })?;
...
fn debug_locate_fixed_transcript_chunk_stage(
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!("RV32IM fixed transcript staged state allocation failed: {err}"))
    })?;
    let transcript_values = transcript_in
        .state
        .map(|value| SpartanF::from_canonical_u64(value.as_canonical_u64()));
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript staged state import failed: {err}")))?;
    let synthetic_chunk_relation_digest = alloc_const_field_values(
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript staged relation digest allocation failed: {err}"
        ))
    })?;
    let mut synthetic_chunk_relation_cursor = 0usize;
    let live_state_in_vars = alloc_recursive_carried_projection_claims(
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM fixed transcript staged live-state allocation failed: {err}"
        ))
    })?;
    let carried_claims = Rv32imClaimBundle::from_effective_claims(
        live_state_in_vars
    ...
    debug_locate_rv32im_main_relation_chunk_stage(
        params,
        structure,
        dims,
        mat_digest,
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM fixed transcript staged chunk replay failed: {err}")))?;
    Ok(())
}
