...
use neo_fold_legacy::paper::construction2::{LatestInstance, ProofState, RunningInstance, SemanticStateMode, State};
use neo_fold_legacy::paper::digest::{digest32_as_fields, state_x_out_digest_with_mode, StateXOutDigestMode};
use neo_fold_legacy::paper::f_prime::r1cs::encode_f_prime_superneo_public_input;
use neo_fold_legacy::paper::relations::{CcsClaim, CcsWitness};
use neo_fold_legacy::{Preprocessing, Uncompressed};
use neo_math::F;
...
fn build_fixture() -> Fixture {
    ...
    neo_fold_legacy::verify_uncompressed(&step.prep, &base).expect("bounded base proof verifies");
...
fn add_base_case(
    ...
    let rust_result = neo_fold_legacy::verify_uncompressed(prep, &proof);
...
fn add_recursive_case(
    ...
    let link_accepted =
        neo_fold_legacy::lifecycle::validate_required_f_prime_latest_link(prep, &proof.state, latest).is_ok();
    assert_eq!(
        link_accepted,
    ...
    let running_relation_accepted =
        neo_fold_legacy::lifecycle::validate_final_witness_authority(prep, &running_value).is_ok();
    let fresh_relation_accepted = neo_fold_legacy::lifecycle::validate_latest_witness_authority(prep, latest).is_ok();
