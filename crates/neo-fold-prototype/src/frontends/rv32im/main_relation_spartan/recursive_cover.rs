...
use crate::rv32im::final_relation::rv32im_recursive_accumulator_instance_digest_from_parts;
use crate::rv32im::final_relation::Rv32imChunkFoldTranscriptSnapshot;
use crate::rv32im::kernel::rv32im_root_main_lane_context_for_claim_count;
...
use crate::superneo_circuit::claim::{
    ...
    packed_bytes_field_values, CircuitCeClaim,
};
use crate::superneo_circuit::transcript::hash_field_linear_combinations_raw;

#[derive(Clone)]
pub(super) struct Rv32imRecursiveCoverClaimVar {
    pub(super) claim: CircuitCeClaim,
}

#[derive(Clone)]
pub(super) struct Rv32imRecursiveCoverStateVar {
...
pub(crate) struct Rv32imRecursiveAccumulatorProjectionDigestAuxBreakdown {
...
fn alloc_recursive_cover_public_state_fields<CS: ConstraintSystem<SpartanF>>(
    ...
    transcript: &Rv32imChunkFoldTranscriptSnapshot,
...
> {
    ...
    let folded_accumulator_digest =
        rv32im_recursive_accumulator_instance_digest_from_parts(claims, terminal_handle_digest);
...
pub(super) fn alloc_recursive_cover_state<CS: ConstraintSystem<SpartanF>>(
    ...
    terminal_handle_digest: [u8; 32],
    label: &str,
) -> Result<Rv32imRecursiveCoverStateVar, SynthesisError> {
    ...
    Ok(Rv32imRecursiveCoverStateVar {
        transcript_state,
        transcript_absorbed,
        terminal_handle,
        folded_accumulator_digest,
...
pub(super) fn alloc_recursive_cover_claims<CS: ConstraintSystem<SpartanF>>(
    ...
    let shared_r = first_var.openings.r.clone();
    let shared_r_values = first_var.openings.r_values.clone();
    let shared_s_col = first_var.norm_check.s_col.clone();
    let shared_s_col_values = first_var.norm_check.s_col_values.clone();
    ...
    base_claims
        .into_iter()
        .map(|claim| Ok::<_, SynthesisError>(Rv32imRecursiveCoverClaimVar { claim }))
        .collect()
}
...
pub(crate) fn debug_measure_recursive_accumulator_instance_digest_circuit_from_claims_aux(
    ...
    terminal_handle: &[AllocatedNum<SpartanF>; 4],
    label: &str,
) -> Result<Rv32imRecursiveAccumulatorProjectionDigestAuxBreakdown, SynthesisError> {
    ...
    Ok(Rv32imRecursiveAccumulatorProjectionDigestAuxBreakdown {
        after_header,
        after_claim_digests,
        after_outer_hash,
    })
...
fn recursive_accumulator_instance_digest_circuit_from_phi_dec_parent_vars<CS: ConstraintSystem<SpartanF>>(
    ...
    let domain =
        packed_bytes_field_values(b"neo.fold.next/rv32im/main_recursion_recursive_accumulator_phi_dec_parent/v1");
    let parent_field_count = claims
        .first()
        .map(|claim| 1 + claim.commitment.data.len())
        .unwrap_or(0);
    ...
    if let Some(first_claim) = claims.first() {
        let (params, _, _) =
            rv32im_root_main_lane_context_for_claim_count(claims.len()).map_err(|_| SynthesisError::Unsatisfiable)?;
        let parent_len = first_claim.commitment.data.len();
        ...
        for claim in claims {
            if !claim.commitment.data.is_empty() || !claim.commitment.data_values.is_empty() {
                return Err(SynthesisError::Unsatisfiable);
            }
        }
    }
