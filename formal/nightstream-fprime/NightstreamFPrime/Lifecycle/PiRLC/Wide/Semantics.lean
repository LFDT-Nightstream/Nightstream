...
/-! The candidate PiRLC phase uses the existing paper combination relation
with challenges from the exact one-window sampler. It also proves the
outgoing transcript state, rather than assuming a supplied challenge vector. -/

namespace NightstreamFPrime.Lifecycle.PiRLC.Wide.Semantics
...
theorem arityTotal_eq_sourceCount :
    Nifs.PaperProfile.arity.total = ProjectedBatch.sourceCount := by
  rw [Nifs.PaperProfile.arity_total, ProjectedBatch.sourceCount_eq]

def sourceIndex (source : Fin Nifs.PaperProfile.arity.total) :
    Fin ProjectedBatch.sourceCount :=
...
def evalChallenges
  ...
  fun source lane => (ProjectedBatch.outputChallenge (Formal.samplerOffset offset) (sourceIndex source) lane).eval env
...
theorem combinationChallenge_eval
    ...
    (lane : Fin ringDegree) :
    (ProjectedBatch.outputChallenge offset (Fin.cast CombinationFamily.sourceCount_eq.symm source) lane).eval env =
...
theorem PhaseHolds.response
    ...
    (fun source : Fin Nifs.PaperProfile.arity.total =>
      Nifs.NonInteractive.PiRlcWideSampler.Transcript.challengeAt
        (Scalar.evalState env (interface.initialState offset)) source.val) =
      evalChallenges interface offset env := by
  funext source
  exact (ProjectedBatch.outputChallenge_eval
    (Formal.samplerInterface (Formal.atOffset interface offset)) env offset phase.sampler (sourceIndex source)).symm
...
theorem PhaseHolds.outgoingState
    ...
    {offset : Nat} {env : Env}
    (phase : PhaseHolds relation ajtai interface offset env) :
    Scalar.evalState env (ProjectedBatch.outputState
