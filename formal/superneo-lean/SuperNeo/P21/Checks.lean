import SuperNeo.P21.Constructors

/-! P21 check-subset equivalences and check-driven constructors. -/
...
theorem p21ProtocolTarget_of_checks_with_native_p6_base2
  {bar : Array (Array F)}
  {m : Array (Array F)}
  {z z1 z2 zDecomp r : Array F}
  {ρ1 ρ2 : F}
...
theorem p21FullMathTarget_of_checks_with_native_p6_base2
  {bar : Array (Array F)}
  {a b : Array F}
  {m : Array (Array F)}
  {z z1 z2 zDecomp r : Array F}
  ...
  (hP10 : p10CoreCheck bar a b = true)
  (hKPos : 0 < kSplit)
  (hZero : splitScalarTerminalZeroProp zDecomp 2 kSplit)
  (hCanon : zDecomp.all (fun x => decide (F.Canonical x)) = true)
  (hP12 : matrixTransformIdentity bar m z = true)
  (hP14 : evalHom2 bar m z1 z2 r ρ1 ρ2 = true)
  (hVecAdd : preservesAddVec hVec z1 z2 = true)
  ...
  p21FullMathTarget bar a b m z z1 z2 zDecomp r ρ1 ρ2 2 kSplit cset samples invDelta qVals
    xs ys expectedCoeffs evalPoint expectedEval ell totalDegree setSize := by
  have hP6 : splitRoundTrip zDecomp 2 kSplit = true := by
    exact splitRoundTrip_true_of_base_two_of_state_zero_of_allCanonical
      (z := zDecomp) (k := kSplit) hKPos hZero hCanon
