//! The real SHA-256 serial packed-state workload for the `e2e_bench` gate.
//!
...
//! Mirrors the CPU-reference workload in
//! `neo-fold-clean/tests/system/sha256_bellpepper_batching.rs` (packed-state
//! serial pairs) — keep the two in sync if the reference shape changes.
