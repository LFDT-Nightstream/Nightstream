...
| `P5` | Done (native non-coarse blockers are proved and exposed: `schoolbookAddTriangleBound_theorem`, `schoolbookSubTriangleBound_theorem`, `schoolbookMulUniversalBound_theorem`; assumption-free endpoints now exist for operation/collapse bundles (`vecAddNormBoundFromOperands_native`, `coeffSubNormBoundFromOperands_native`, `vecScaleNormBoundFromOperands_native`, `raw{Add,Sub,AddSub,FieldOp}CollapseBound_native`); and both coarse/tight native multiplication endpoints are available (`normInfCoeffs_mulRq_le_native`, `normInfCoeffs_mulRq_le_native_tight`) and wired into P16/P17/P20/P21/ProtocolReduction). | None for P5 core math. | P16/P17/P20/P21 can consume native-tight P5 paths without introducing new blocker assumptions. |
...
| M5 | Definition 3 (centered `l_inf` norm) | `SuperNeo/Norm.lean` | Define centered representatives and prove basic norm bounds | `neo_math::inf_norm` + generated norm vectors | Norm bounds are prerequisites for low-norm assumptions and soundness-side constraints. | Done (implementation + parity passing; theorem-native blocker proofs and assumption-free native endpoints are complete, including native-tight `mulRq` norm bounds consumed by downstream P16/P17/P20/P21 wrappers) |
