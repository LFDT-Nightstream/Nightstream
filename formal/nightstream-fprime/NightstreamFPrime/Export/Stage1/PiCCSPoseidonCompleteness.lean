...
private theorem source_sbox (index : InvocationIndex) (row : Fin PoseidonRetainedSlots.rows.length) :
  ...
  change (physicalInvocation index).witnessStart +
    (PoseidonRetainedSlots.localOutput row).val < PerApplicationPackage.basePackage.layout.totalColumnCount
...
private theorem sbox_form (index : InvocationIndex) (row : Fin PoseidonRetainedSlots.rows.length) :
  ...
  refine (PoseidonRetainedFamily.form_eval (PiCCSPoseidonPlan.schedule application)
    (PiCCSPoseidonPlan.retainedStart application) (PiCCSPoseidonPlan.retainedFits geometry)
...
private theorem slice_values
  ...
  have kindEq (current : Fin (Invocations.invocationCount actions)) :
      PiCCSActionPayloadBlock.kindAt
        (PoseidonActionSemantics.sliceIndex offset (Invocations.invocationCount actions) fits current) =
        PoseidonActionSchedule.kindAt actions current := by
    simpa only [Fin.cast_eq_self] using kind current
