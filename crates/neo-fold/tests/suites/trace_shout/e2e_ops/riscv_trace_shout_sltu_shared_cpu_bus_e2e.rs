...
#[test]
fn riscv_trace_wiring_ccs_shared_cpu_bus_shout_sltu_prove_verify() {
    // Program:
    // - LUI x1, 0        (x1 = 0)
    // - LUI x2, 1        (x2 = 4096)
    // - SLTU x3, x1, x2  (1)
    ...
    // Shout instance: SLTU table, 1 lane.
    let t = exec.rows.len();
    ...
    let sltu_z = build_shout_only_bus_z(
        ccs.m,
        layout.m_in,
        t,
    ...
    let mut tr_prove = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sltu");
    ...
    let mut tr_verify = Poseidon2Transcript::new(b"riscv-trace-no-shared-bus-shout-sltu");
