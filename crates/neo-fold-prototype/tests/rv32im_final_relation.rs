...
use neo_fold_prototype::rv32im::final_relation::{
    build_rv32im_terminal_chunk_fold_witness, prove_rv32im_final_statement_from_accepted,
    prove_rv32im_folded_statement_from_accepted,
};
use neo_fold_prototype::rv32im::{
    build_rv32im_accepted_proof_artifact, build_rv32im_kernel_export_source_from_accepted_artifact,
    build_rv32im_kernel_export_witness, parity_source_cases, prove_rv32im_accepted_proof,
    prove_rv32im_accepted_proof_with_options, prove_rv32im_public_proof,
    prove_rv32im_public_proof_and_published_seam_with_perf, Rv32imProofInput, Rv32imPublicProofOptions,
};
...
#[test]
fn rv32im_folded_statement_round_trip() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (folded, proof) =
        prove_rv32im_folded_statement_from_accepted(&artifact).expect("prove rv32im folded statement");
    ...
    audit_check_rv32im_folded_statement_replay(&folded, &proof).expect("audit-check rv32im folded statement replay");
}

#[test]
fn rv32im_final_statement_round_trip() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, proof) =
    ...
    audit_check_rv32im_final_statement_replay(&statement, &proof).expect("audit-check rv32im final statement replay");
}

#[test]
fn rv32im_terminal_chunk_fold_witness_matches_final_accumulator() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
    ...
    let witness =
        build_rv32im_terminal_chunk_fold_witness(&statement, &proof).expect("build terminal chunk-fold witness");
...
fn rv32im_direct_prover_seam_matches_reconstructive_final_path() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let ((proof, published_seam), _) =
        prove_rv32im_public_proof_and_published_seam_with_perf(&input).expect("prove rv32im public proof and seam");
    let artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted rv32im artifact");
    ...
    audit_check_rv32im_final_statement_replay(&rebuilt_statement, &final_proof)
        .expect("audit-check rv32im direct final statement replay");
}

#[test]
fn rv32im_final_statement_kernel_export_matches_public_export_components() {
    let input = proof_input("control_flow_jal_skip_ecall");
    ...
    let expected_source =
        build_rv32im_kernel_export_source_from_accepted_artifact(&artifact).expect("build export source");
    let expected_witness = build_rv32im_kernel_export_witness(&proof).expect("build export witness");
...
#[test]
fn rv32im_final_statement_rejects_tampered_accepted_artifact() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    ...
    let err = audit_check_rv32im_final_statement_replay(&statement, &proof)
...
#[test]
fn rv32im_final_statement_rejects_tampered_chunk_replay_witness() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_tampered_bridge_binding() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_tampered_transcript_surface() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_tampered_transcript_mix_surface() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_tampered_root_execution_rows() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_tampered_export_kernel_claim_surface() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_tampered_export_terminal_row_endpoint() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_tampered_export_main_lane_surface() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof(&input).expect("prove accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
...
#[test]
fn rv32im_final_statement_rejects_reordered_chunk_witnesses() {
    let input = proof_input("control_flow_jal_skip_ecall");
    let (artifact, _audit) = prove_rv32im_accepted_proof_with_options(
        &input,
        Rv32imPublicProofOptions {
            root_fold_schedule: FoldSchedule::RowsPerChunk(1),
        },
    )
    .expect("prove chunked accepted rv32im proof");
    let (statement, mut proof) =
        prove_rv32im_final_statement_from_accepted(&artifact).expect("prove rv32im final statement");
