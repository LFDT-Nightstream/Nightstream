...
Owns: all 74 sparse monomials, their order, Goldilocks coefficients, the exact
...
| `f_prime.selective_ccs.polynomial.centered` | unary centered residual plus `g*e*(left^2-7*right^2)` packing | 10 | 8 | `selective_polynomial` |
...
| **total** | exact sparse polynomial | **74** | **8** | `selective_polynomial` |
-/

namespace Nightstream.Implementation.R1CS.SelectiveCcs.Polynomial.Semantics
...
/-- The twenty-one terms shared by ordinary, packed-domain, and evaluation
rows. The eight mixed-selector terms preserve the unary centered equation when
`E=0` and replace it with one projective residual pair when `G=E=1`. -/
def baseTerms : List (Monomial F 13) := [
  ...
  monomial (-1) [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.centeredUnit, 3)],
  monomial 1 [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.centeredUnit, 1)],
  monomial 1 [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.centeredUnit, 6)],
  monomial (-2) [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.centeredUnit, 4)],
  monomial 1 [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.centeredUnit, 2)],
  monomial (-7) [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.a, 6)],
  monomial 14 [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.a, 4)],
  monomial (-7) [(Role.generalSelector, 1), (Role.evalSelector, 1),
    (Role.a, 2)],
...
/-- Exact ordered 74-term sparse syntax produced by Rust's
`selective_polynomial`. -/
def terms : List (Monomial F 13) :=
  baseTerms ++ canonicalTerms

theorem base_term_count_exact : baseTerms.length = 21 := by decide

theorem canonical_term_count_exact : canonicalTerms.length = 53 := by decide

theorem term_count_exact : terms.length = 74 := by decide
...
theorem total_degree_sum_exact :
    (terms.map Monomial.totalDegree).sum = 324 := by
  decide
...
end Nightstream.Implementation.R1CS.SelectiveCcs.Polynomial.Semantics
