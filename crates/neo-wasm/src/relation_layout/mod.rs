...
use super::layout::{
    ...
    COL_PARAM_INIT_ACTIVE_BEFORE, COL_PC_AFTER, COL_PC_BEFORE, COL_PC_EDGE_KIND, COL_PC_FREF_ACTIVE, COL_PC_ROM_ACTIVE,
...
fn build_wasm_relation_layout_uncached() -> WasmRelationLayout {
    ...
    let memories = vec![
        ...
        WasmMemorySpec {
            name: "grammar_event_counts_post",
            columns: vec![
                WasmMemoryColumnSpec {
                    address_columns: vec![Column(COL_HOST_CALLEE_FREF_BEFORE)],
                    value_column: Column(COL_GRAMMAR_POST_COUNT),
                    kind: WasmMemoryColumnKind::Read,
                    activation: WasmMemoryActivation::BooleanGate(Column(COL_GRAMMAR_RESULT_ACTIVE)),
                },
                // Exit latch: the export's exit-event count.
                WasmMemoryColumnSpec {
                    address_columns: vec![Column(COL_HOST_CALLEE_FREF_AFTER)],
                    value_column: Column(COL_GRAMMAR_POST_COUNT),
                    kind: WasmMemoryColumnKind::Read,
