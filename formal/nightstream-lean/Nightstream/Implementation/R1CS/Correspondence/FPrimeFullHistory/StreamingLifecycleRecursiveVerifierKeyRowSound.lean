import Nightstream.Implementation.R1CS.Correspondence.FPrimeFullHistory.StreamingLifecycleRecursiveVerifierKeyProgramBridge
import Nightstream.Implementation.R1CS.Correspondence.FPrimeFullHistory.StreamingVariableHashRecipeConstantSound

/-!
Contract: the three exact recursive verifier-key recipe traces compute
Poseidon2 on their complete typed preimages.

Owns the semantic consequence of satisfying the formulaic trace rows. It does
not prove source-input authority or that final selective rows equal these
trace rows.

Assurance tier: artifact-checked for
`FPRIME-STREAMING-LIFECYCLE-RECURSIVE-VERIFIER-KEY-PROVENANCE-V1` and
model-level for `FPRIME-STREAMING-RECURSIVE-VERIFIER-KEY-PROGRAM-V1`,
Nightstream b2/k16.
...
namespace Nightstream.Implementation.R1CS.Correspondence.FPrimeFullHistory.StreamingLifecycleRecursiveVerifierKeyRowSound
...
open Nightstream.Implementation.R1CS.Artifacts.FPrimeFullHistory.StreamingLifecycleRecursiveVerifierKey
...
/-- Structural satisfaction of the exact recursive verifier-key source
program. The fields preserve the source-program partition and do not evaluate
the complete generated stage. -/
structure StageSatisfied (assignment : Nat -> Nat) : Prop where
  ...
  publicTraceBinding : Satisfies
    rawArtifact.publicTraceBinding.rows assignment
...
structure StageOutputs
  ...
  initialBoundary : forall lane : Fin 4,
    assignment
      ...
      computedInitialBoundary inputs lane
  publicTrace : forall lane : Fin 4,
    assignment
        (rawArtifact.publicTraceBinding.leftColumns.getD lane.val 0) =
      assignment
        (rawArtifact.publicTraceBinding.rightColumns.getD lane.val 0)
...
private theorem initialBoundary_fixedSegments
  ...
  · have tailValues := congrArg (List.drop 6) constants
    simpa [List.map_append, readColumns, initialBoundaryTagColumns,
      initialBoundaryConstantFields, initialBoundaryTag] using tailValues
...
/-- Exact source-program rows derive every recursive verifier-key stage
output. The four dynamic preimages remain tied to verifier-owned columns;
neither computed digest is accepted as independent authority. -/
theorem stage_rows_imply_outputs
  ...
  have publicTraceBinding := digestBinding_rows_sound assignment canonical one
    rawArtifact.publicTraceBinding satisfied.publicTraceBinding
  refine {
    ...
    initialBoundary := ?_
    publicTrace := publicTraceBinding
  }
  · intro lane
...
end Nightstream.Implementation.R1CS.Correspondence.FPrimeFullHistory.StreamingLifecycleRecursiveVerifierKeyRowSound
