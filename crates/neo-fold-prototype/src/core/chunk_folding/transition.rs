...
use neo_reductions::engines::utils::{self, Dims};
    ...
    MB: Fn(&[Commitment], u32) -> Commitment + Clone + Copy,
{
    ...
    let (dims, dims_ms) = build_transition_dims(params, s)?;
    ...
    let dec_children = dec.children.len();
    ...
    let perf = ChunkProvePerf {
        ...
        ccs_ms,
        dims_ms,
        rlc_prepare_ms: rlc.prepare_ms,
        rlc_ms: rlc.prove_ms,
        dec_split_ms: dec.split_ms,
        dec_commit_ms: dec.commit_ms,
        dec_ms: dec.prove_ms,
...
fn build_transition_dims(params: &NeoParams, s: &CcsStructure<F>) -> Result<(Dims, f64), PiCcsError> {
    let started = Instant::now();
    let dims = utils::build_dims_and_policy(params, s)?;
    Ok((dims, started.elapsed().as_secs_f64() * 1_000.0))
}
...
fn run_pi_rlc<MR>(
    ...
    fresh_witnesses: &[CcsWitness<F>],
    ccs_outputs: &[CeClaim<Commitment, F, K>],
    rlc_rhos: &[neo_reductions::api::RotRho],
    ell_d: usize,
    is_optimized: bool,
    mix_rhos_commits: MR,
) -> Result<PiRlcTransition, PiCcsError>
where
    MR: Fn(&[Mat<F>], &[Commitment]) -> Commitment + Clone + Copy,
{
    let prepare_started = Instant::now();
    let mut rlc_input_witnesses = Vec::with_capacity(fresh_step_count + incoming_main.witnesses.len());
    rlc_input_witnesses.extend(chunk_fresh_witness_mats(fresh_witnesses));
    rlc_input_witnesses.extend(incoming_main.witnesses.iter());
    ...
    let (parent, z_mix) = if is_optimized {
        ...
        let owned_rlc_input_witnesses: Vec<Mat<F>> = rlc_input_witnesses.iter().map(|z| (*z).clone()).collect();
        rlc_with_commit(
            mode,
            s,
            params,
...
fn run_pi_dec<L, MB>(
    ...
    optimized_cache: Option<&OptimizedStructureCache>,
    combine_b_pows: MB,
) -> Result<PiDecTransition, PiCcsError>
where
    L: SModuleHomomorphism<F, Commitment> + Sync,
    MB: Fn(&[Commitment], u32) -> Commitment + Clone + Copy,
{
    let split_started = Instant::now();
    let (z_split, digit_nonzero) = split_b_matrix_k_with_nonzero_flags(z_mix, params.k_rho as usize, params.b)?;
    let split_ms = split_started.elapsed().as_secs_f64() * 1_000.0;
