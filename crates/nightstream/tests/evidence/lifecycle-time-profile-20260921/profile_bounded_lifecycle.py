...
def run(engine, label):
    ...
    command = ['xcrun', 'xctrace', 'record', '--template', 'Time Profiler',
               ...
               '--launch', '--', str(BINARY), '--engine', engine, '--steps', '3']
    ...
    state = {'label': label, 'engine': engine, 'controller_pid': controller,
             ...
             'target_command': str(BINARY) + ' --engine ' + engine + ' --steps 3',
    ...
    if path.exists():
        for line in path.read_text().splitlines():
            try:
                ...
                pass
    verified = any(e.get('event') == 'benchmark_finished' and e.get('verified') is True and e.get('steps') == 3 for e in events)
    if outcome == 'running':
    ...
    record = {'command': command, 'state': state, 'outcome': outcome, 'error': error, 'exit': code,
              ...
              'timing_scope': 'Full lifecycle under Time Profiler; do not compare its time with an uninstrumented run as a controlled speed ratio.',
