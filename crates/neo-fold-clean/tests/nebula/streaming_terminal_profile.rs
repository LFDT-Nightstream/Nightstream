...
fn terminal_profile_binds_the_trailing_fresh_after_state_to_final_rows() {
    ...
    assert_eq!(profile.accepted_work_items(), 400);
    assert_eq!(profile.terminal_arm(), 399);
    ...
    assert_eq!(profile.lifecycle_scope(), "recursive-terminal-arm-399");
    assert_eq!(
    ...
    let mut expect_reject = |column: usize, change: fn(F) -> F, label: &str| {
        let original = terminal.witness()[column];
        terminal.tamper_witness(column, change(original));
        assert!(!terminal.is_satisfied(), "changed {label} must reject");
        terminal.tamper_witness(column, original);
        assert!(terminal.is_satisfied(), "restored {label} must satisfy");
    };
    expect_reject(
        final_witness[profile.schedule_selector_column()].col(),
        |_| F::ZERO,
        "terminal schedule selector",
    );
    expect_reject(
        public.vk_fs_digest[0].col(),
        |value| value + F::ONE,
        "verifier-key digest",
    );
    expect_reject(
        terminal_output.post_phase_lane.program_binding_digest[0].col(),
        |value| value + F::ONE,
        "Nebula program binding",
    );
    expect_reject(
        terminal_output.delayed_payload[0].col(),
        |value| F::ONE - value,
        "delayed payload",
    );
    expect_reject(adv_wires.ops.data[0].col(), |value| value + F::ONE, "fresh adv opening");
    expect_reject(
        terminal_output.final_lane.h[0].c0.col(),
        |_| F::from_u64(2),
        "final closed lane",
    );
}
