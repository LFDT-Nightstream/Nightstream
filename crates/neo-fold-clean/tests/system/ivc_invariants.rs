...
//! | `running_accumulator_witness_must_carry_f_prime_encoded_size` | ignored | A recursive production-shape fold exceeds five minutes; the source census and fixed-point artifacts are the default gates. |
...
//! | `canonical_encoder_verifies_multistep_memory_chain` | ignored (in `tests/nebula/f_prime.rs`) | The full three-segment fixed-point chain exceeds the 24 GiB audit limit. |
//!
...
#[test]
#[ignore = "the recursive production-shape fold exceeds the five-minute test cap; run this audit alone with `cargo test --release -p neo-fold-clean --test system_ivc_invariants running_accumulator_witness_must_carry_f_prime_encoded_size -- --ignored --exact`"]
