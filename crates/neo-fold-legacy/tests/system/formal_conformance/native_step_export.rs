...
use neo_fold_legacy::paper::construction2::{self, FoldProof, ProofState, SemanticStateMode, State, StepProof};
use neo_fold_legacy::paper::digest::{self, AccumulatorHandle};
use neo_fold_legacy::paper::nifs::{self, NifsProof};
use neo_fold_legacy::paper::relations::{CcsClaim, CeClaim};
use neo_fold_legacy::{CcsInstance, Preprocessing, RunningInstance};
use neo_math::{F, K};
...
fn build_native_step_corpus() -> NativeStepCorpus {
    ...
    let base_audit =
        neo_fold_legacy::prove(&prep, Vec::<Vec<CcsInstance>>::new()).expect("construct base native-step fixture");
    let base_input = base_audit.proof.state.clone();
    let after_base = neo_fold_legacy::extend(&prep, base_audit, vec![super::support::toy_instance(&prep, 10_001)])
    ...
    let after_recursive = neo_fold_legacy::extend(&prep, after_base, vec![super::support::toy_instance(&prep, 10_002)])
    ...
    NativeStepCorpus {
        ...
        profile: Profile {
            ...
            nebula: "absent",
            transcript_label: ascii(neo_fold_legacy::paper::f_prime::native::F_PRIME_STEP_TRANSCRIPT_LABEL),
...
fn stable_nifs_error(case: &str, error: &nifs::Error) -> StableError {
    match error {
        nifs::Error::PiDec(neo_fold_legacy::paper::pi_dec::Error::VerifyRejected) => {
            StableError::NifsPiDecVerifyRejected
        }
        nifs::Error::PiCcs(neo_fold_legacy::paper::pi_ccs::Error::Engine(
            neo_fold_legacy::engine::optimized::Error::Reductions(neo_reductions::error::PiCcsError::InvalidInput(
                message,
            )),
