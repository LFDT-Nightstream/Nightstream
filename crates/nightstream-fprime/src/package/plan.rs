...
pub(super) const COMBINATION_TEMPLATE_COUNT: usize = 2 * RING_DEGREE;
const PI_CCS_SOURCE_START: usize = 14_751_804;
const PI_CCS_TARGET_START: usize = 14_751_526;
const PILOT_PRIOR_PUBLIC_SOURCE_START: usize = 49_393;
...
fn expand_first54(source_count: usize, round_count: usize) -> Result<Vec<RawCompactRowInvocation>, PackageError> {
    ...
    for source in 0..source_count {
        let selector_logical = linear(20_073_351, &[(source, 15_504)])?;
        let selector_row = linear(19_955_287, &[(source, 59_344)])?;
        let selector_fresh = linear(20_390_413, &[(source, 43_743)])?;
...
fn combination_challenge(source: usize) -> Result<usize, PackageError> {
    final_column(linear(20_080_273, &[(source, 15_504)])?)
}

fn combination_value(family: usize, source: usize, block: usize, cell: usize) -> Result<usize, PackageError> {
    match family {
        0 if source == 0 => linear(98_786, &[(block, RING_DEGREE)]),
        0 => linear(97, &[(source - 1, 3_081), (block, RING_DEGREE)]),
        1 if source == 0 => source_to_spartan(linear(PILOT_PRIOR_PUBLIC_SOURCE_START, &[(block, RING_DEGREE)])?),
        1 => linear(1_286, &[(source - 1, 3_081), (block, RING_DEGREE)]),
        2 => linear(100_534, &[(source, 1_620), (cell, 1)]),
        3 => linear(100_642, &[(source, 1_620), (block, 108), (cell, 1)]),
