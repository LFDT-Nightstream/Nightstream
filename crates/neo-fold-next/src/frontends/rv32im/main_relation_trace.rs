//! Owns the native RV32IM main-circuit trace builder one chunk at a time.
//!
...
use crate::rv32im::kernel::{
    ...
    rv32im_root_main_lane_context_for_claim_count, Rv32imVerifiedKernelChunkHandoff, SimpleKernelError,
};
use crate::superneo_circuit::structure::pad_ccs_structure_to_block_width;

pub(crate) const CHUNK_META_RAW_TAG: u64 = 14;

#[derive(Clone, Debug)]
pub(crate) struct Rv32imMainCircuitHandoff {
    ...
    pub(crate) public_input_layout: Rv32imMainCircuitPublicInputLayout,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum Rv32imMainCircuitPublicInputLayout {
    PackedPrefix,
}

#[derive(Clone, Debug)]
pub(crate) struct Rv32imMainCircuitChunkTrace {
    ...
    pub(crate) ccs_trace: Rv32imChunkRelationTrace,
}

#[derive(Clone, Debug)]
pub(crate) struct Rv32imMainCircuitPiCcsReplaySurface {
...
#[derive(Clone, Debug)]
pub(crate) struct Rv32imMainCircuitPiRlcReplaySurface {
    pub(crate) parent: neo_ccs::CeClaim<Commitment, F, K>,
}

#[derive(Clone, Debug)]
pub(crate) struct Rv32imMainCircuitPiDecReplaySurface {
    pub(crate) children: Vec<neo_ccs::CeClaim<Commitment, F, K>>,
}

#[derive(Clone, Debug)]
pub(crate) struct Rv32imMainCircuitChunkReplaySurface {
    pub(crate) handoff: Rv32imMainCircuitHandoff,
    pub(crate) fresh_claims: Vec<CcsClaim<Commitment, F>>,
    pub(crate) fresh_witnesses: Vec<CcsWitness<F>>,
    pub(crate) pi_ccs: Rv32imMainCircuitPiCcsReplaySurface,
    pub(crate) pi_rlc: Rv32imMainCircuitPiRlcReplaySurface,
    pub(crate) pi_dec: Rv32imMainCircuitPiDecReplaySurface,
}

impl Rv32imMainCircuitChunkTrace {
    ...
    pub(crate) fn replay_surface(&self) -> Result<Rv32imMainCircuitChunkReplaySurface, SimpleKernelError> {
        build_rv32im_main_circuit_chunk_replay_surface(
            &self.handoff,
            &self.fresh_claims,
            &self.fresh_witnesses,
            build_rv32im_main_circuit_pi_ccs_replay_surface(
...
pub(crate) fn build_rv32im_main_circuit_chunk_replay_surface(
    handoff: &Rv32imMainCircuitHandoff,
    fresh_claims: &[CcsClaim<Commitment, F>],
    fresh_witnesses: &[CcsWitness<F>],
    pi_ccs: Rv32imMainCircuitPiCcsReplaySurface,
    parent: neo_ccs::CeClaim<Commitment, F, K>,
    children: Vec<neo_ccs::CeClaim<Commitment, F, K>>,
) -> Result<Rv32imMainCircuitChunkReplaySurface, SimpleKernelError> {
    Ok(Rv32imMainCircuitChunkReplaySurface {
        handoff: handoff.clone(),
        fresh_claims: fresh_claims.to_vec(),
        fresh_witnesses: fresh_witnesses.to_vec(),
        pi_ccs,
        pi_rlc: Rv32imMainCircuitPiRlcReplaySurface { parent },
        pi_dec: Rv32imMainCircuitPiDecReplaySurface { children },
    })
}

pub(crate) fn build_rv32im_main_circuit_pi_ccs_replay_surface(
...
) -> Rv32imMainCircuitPiCcsReplaySurface {
    Rv32imMainCircuitPiCcsReplaySurface {
        ccs_outputs,
        replay_proof,
        public_challenges,
...
pub(crate) struct Rv32imMainCircuitCeClaimShape {
...
impl Rv32imMainCircuitCeClaimShape {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct Rv32imMainCircuitCcsClaimShape {
    pub(crate) commitment_d: u64,
    pub(crate) commitment_kappa: u64,
    pub(crate) c_data_len: u64,
    pub(crate) x_len: u64,
...
impl Rv32imMainCircuitCcsClaimShape {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct Rv32imMainCircuitCcsWitnessShape {
    pub(crate) w_len: u64,
    pub(crate) z_rows: u64,
    pub(crate) z_cols: u64,
}

impl Rv32imMainCircuitCcsWitnessShape {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct Rv32imMainCircuitChunkCover {
    pub(crate) fresh_claim_count: u64,
    pub(crate) fresh_witness_count: u64,
    pub(crate) fresh_claim_shapes: Vec<Rv32imMainCircuitCcsClaimShape>,
    pub(crate) fresh_witness_shapes: Vec<Rv32imMainCircuitCcsWitnessShape>,
    pub(crate) ccs_output_count: u64,
    pub(crate) child_count: u64,
    pub(crate) parent_claim_shape: Rv32imMainCircuitCeClaimShape,
    pub(crate) ccs_output_shapes: Vec<Rv32imMainCircuitCeClaimShape>,
    pub(crate) child_claim_shapes: Vec<Rv32imMainCircuitCeClaimShape>,
    pub(crate) fe_round_lengths: Vec<u64>,
    pub(crate) nc_round_lengths: Vec<u64>,
}

impl Rv32imMainCircuitChunkCover {
    pub(crate) fn from_trace(trace: &Rv32imMainCircuitChunkTrace) -> Self {
        Self {
            ...
            parent_claim_shape: Rv32imMainCircuitCeClaimShape::from_claim(&trace.ccs_trace.parent),
    ...
    pub(crate) fn from_replay_surface(surface: &Rv32imMainCircuitChunkReplaySurface) -> Self {
        Self {
            ...
            parent_claim_shape: Rv32imMainCircuitCeClaimShape::from_claim(&surface.pi_rlc.parent),
    ...
    pub(crate) fn covers_replay_surface(&self, surface: &Rv32imMainCircuitChunkReplaySurface) -> bool {
...
struct Rv32imMainCircuitTraceBuildContext<'a> {
    params: &'a NeoParams,
    log: &'a AjtaiSModule,
...
pub(crate) fn build_rv32im_main_circuit_chunk_trace_from_authoritative_parts(
    chunk_index: usize,
    ...
    let (params, log, structure) = rv32im_root_main_lane_context_for_claim_count(carry_in.main.claims.len())?;
    let ce_structure = pad_ccs_structure_to_block_width(structure)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM padded CE structure failed: {err}")))?;
    let optimized_cache = rv32im_cached_root_main_lane_optimized_cache()?;
    let dims = build_dims_and_policy(&params, structure)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM main relation dims failed: {err}")))?;
    let ctx = Rv32imMainCircuitTraceBuildContext {
        params: &params,
        log,
        structure,
    ...
    let chunk_trace = build_rv32im_main_circuit_chunk_trace_from_parts(
        &ctx,
        chunk_index,
        handoff,
        chunk_summary,
    ...
    let replayed_transcript_out = rv32im_chunk_fold_carried_transcript_snapshot(&Rv32imChunkFoldTranscriptSnapshot {
        state: transcript.state(),
        absorbed: transcript.absorbed(),
    });
    if &replayed_transcript_out != transcript_out {
        return Err(SimpleKernelError::Bridge(
            "RV32IM main relation circuit trace transcript_out does not match the carried private transcript state"
                .into(),
        ));
    }
    Ok(chunk_trace)
...
fn build_rv32im_main_circuit_chunk_trace_from_parts(
    ctx: &Rv32imMainCircuitTraceBuildContext<'_>,
    chunk_index: usize,
    handoff: &Rv32imVerifiedKernelChunkHandoff,
    chunk_summary: &FixedShapeChunkSummary,
    carry_in: &Rv32imChunkFoldCarry,
    ...
    let trace = trace_rv32im_chunk_relation_with_replay(
        chunk_index,
        handoff,
        &carry_in.main,
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} fold-digest binding failed: {err}"
        ))
    })?;
    ...
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} Pi_RLC public recompute failed: {err}"
        ))
    })?;
    if let Some(mismatch) = describe_ce_claim_mismatch(&expected_parent, &trace.parent) {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} Pi_RLC parent claim does not match the independently recomputed fold: {mismatch}"
        )));
    }
    ...
    if trace.ccs_outputs.len() != ccs_output_zs.len() {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} output/witness arity mismatch"
        )));
    }
        ...
        .enumerate()
    {
        check_output_claim_consistency(ctx.params, ctx.structure, ctx.ce_structure, claim, z_matrix).map_err(|err| {
            SimpleKernelError::Bridge(format!(
                "RV32IM main relation chunk {chunk_index} backend consistency failed for ccs_output {output_index}: {err}"
            ))
        })?;
    }
    if chunk_summary.public_chunk_digest != handoff.public_chunk_digest {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} verified public chunk digest mismatch"
        )));
    }
    if carry_out.main.claims != trace.children || carry_out.main.witnesses != trace.z_split {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} trace/verify next-main mismatch"
        )));
    }
    if chunk_summary.chunk_relation_digest != trace.chunk_relation_digest {
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} trace chunk_relation_digest does not match the carried summary"
        )));
    }
    for (child_index, (claim, z_matrix)) in trace.children.iter().zip(trace.z_split.iter()).enumerate() {
        check_dec_child_claim_consistency(ctx.params, ctx.structure, ctx.ce_structure, ctx.log, claim, z_matrix)
            .map_err(|err| {
                SimpleKernelError::Bridge(format!(
                    "RV32IM main relation chunk {chunk_index} child {child_index} backend consistency failed: {err}"
                ))
            })?;
    }
    ...
    Ok(Rv32imMainCircuitChunkTrace {
        handoff: Rv32imMainCircuitHandoff {
            ...
            public_input_layout: Rv32imMainCircuitPublicInputLayout::PackedPrefix,
        },
...
fn replay_main_relation_pi_ccs_terminal_state(
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM main relation Pi_CCS transcript replay failed: {err}")))?;
    Ok((terminal_state, transcript))
}

pub(crate) fn debug_replay_rv32im_main_relation_pi_ccs_transcript_state(
    transcript_in: &Rv32imChunkFoldTranscriptSnapshot,
    handoff: &Rv32imMainCircuitHandoff,
    fresh_claims: &[CcsClaim<Commitment, F>],
    fresh_witnesses: &[CcsWitness<F>],
    me_inputs: &[neo_ccs::CeClaim<Commitment, F, K>],
    me_witnesses: &[Mat<F>],
) -> Result<Rv32imChunkFoldTranscriptSnapshot, SimpleKernelError> {
    let (params, log, structure) = rv32im_cached_root_main_lane_context()?;
    let ce_structure = pad_ccs_structure_to_block_width(structure)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM padded CE structure failed: {err}")))?;
    let optimized_cache = rv32im_cached_root_main_lane_optimized_cache()?;
    let dims = build_dims_and_policy(params, structure)
    ...
    Ok(Rv32imChunkFoldTranscriptSnapshot {
        state: transcript.state(),
        absorbed: transcript.absorbed(),
    })
}

pub(crate) fn debug_describe_rv32im_main_relation_pi_ccs_terminal_state_mismatch(
    transcript_in: &Rv32imChunkFoldTranscriptSnapshot,
    handoff: &Rv32imMainCircuitHandoff,
    fresh_claims: &[CcsClaim<Commitment, F>],
    fresh_witnesses: &[CcsWitness<F>],
...
pub(crate) fn debug_describe_rv32im_main_relation_pi_rlc_parent_mismatch(
    transcript_in: &Rv32imChunkFoldTranscriptSnapshot,
    handoff: &Rv32imMainCircuitHandoff,
    fresh_claims: &[CcsClaim<Commitment, F>],
    fresh_witnesses: &[CcsWitness<F>],
    ...
    .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM main relation Pi_RLC public recompute failed: {err}")))?;
    Ok(describe_ce_claim_mismatch(&expected_parent, live_parent).unwrap_or_else(|| "pi_rlc_parent_match".into()))
}

pub(crate) fn debug_describe_rv32im_main_relation_pi_rlc_x_flat_mismatch(
    transcript_in: &Rv32imChunkFoldTranscriptSnapshot,
    handoff: &Rv32imMainCircuitHandoff,
    fresh_claims: &[CcsClaim<Commitment, F>],
    fresh_witnesses: &[CcsWitness<F>],
...
fn check_pi_ccs_terminal_state_native(
        ...
        || live.fold_digest != replayed.fold_digest
    {
        ...
        return Err(SimpleKernelError::Bridge(format!(
            "RV32IM main relation chunk {chunk_index} Pi_CCS terminal replay drifted from the verified chunk trace: {detail}"
        )));
    }
    Ok(())
}
...
fn sample_main_relation_pi_rlc_rhos(
    ...
    sample_rot_rhos_n_typed(transcript, params, &ring, claim_count)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM main relation Pi_RLC rho sampling failed: {err}")))
}
