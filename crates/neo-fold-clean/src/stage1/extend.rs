...
use super::{
    step_inputs::digest_bytes, CompleteStepError, PiCcsV1_1PackageBridgeError, Poseidon2HashChainV1Package, ProveError,
    Stage1Envelope, StepInputError,
};
use crate::paper::{
    construction2::{ProofState, RunningInstance},
    nifs,
    params::Params,
    relations::{ajtai_dec_mixer, CeClaim},
};
...
pub enum ExtendError {
    ...
    Sampling(#[from] crate::engine::optimized::Error),
...
impl Poseidon2HashChainV1Package {
    ...
    pub fn extend(&self, envelope: Stage1Envelope, message: [F; 4]) -> Result<Stage1Envelope, ExtendError> {
        ...
        match proof {
            ProofState::Initial => {
                ...
                let (inputs, witnesses) = self.base_inputs(&params, state.z0(), message)?;
            ...
            ProofState::Active {
                ...
                let inputs = self.step_inputs(&state, &prior, &fresh_claim, &proof, message)?;
