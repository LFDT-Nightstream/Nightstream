...
use neo_fold_legacy::engine::r1cs_circuit::{Lc, R1csBuilder, Var};
...
use neo_fold_legacy::paper::f_prime::digest_circuit::StateXOutDigestInputs;
use neo_fold_legacy::paper::f_prime::nebula_lane_circuit::delayed_nebula_public_suffix_len;
use neo_fold_legacy::paper::f_prime::r1cs::encode_x_out_public_bits;
use neo_fold_legacy::paper::f_prime::stage as fprime_stage;
use neo_fold_legacy::paper::params::Params;
use neo_fold_legacy::paper::relations::CcsInstance;
use neo_math::{D, F, K};
...
fn streaming_lifecycle_source_arms_own_complete_context_and_fold_stages() {
    ...
    let preprocessing = neo_fold_legacy::lifecycle::preprocess_with_test_log(
        params,
...
fn streaming_recursive_source_assignment_uses_a_real_nifs_proof() {
    ...
    let log = neo_fold_legacy::frontends::direct_ccs::ajtai::setup_seeded(&params, &seed_structure, 0x5354_5245_414d);
    let preprocessing = neo_fold_legacy::lifecycle::preprocess_with_test_log(
        params,
        seed_structure,
        log,
    ...
    if let Err(error) = recursive.is_satisfied_by(assignment) {
        let row = match error {
            neo_fold_legacy::frontends::direct_ccs::FrontendError::Unsatisfied { row } => row,
