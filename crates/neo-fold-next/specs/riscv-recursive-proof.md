...
not required merely because some legacy export path carries them.
...
### 7.1 Normalized Public API Boundary
...
Any repo-local auxiliary digest bundle, side-lane tuple, or derived claim
bundle carried by a concrete instantiation shall be classified into exactly one
of:
...
That concrete classification is owned by
`riscv-recursive-instantiation.md`.

Side-lane public-tuple, packaged-verifier, and linkage details are owned by
`riscv-authoritative-side-proof-bundle.md`.

No auxiliary bundle may remain theorem-facing solely because a concrete repo
export path happens to carry it.
...
## 10. Direct-Verifier Replay Is Not The Export Format
...
- an audit path,
- or a proof-complete debug artifact.
...
- `riscv-recursive-instantiation.md` owns the concrete recursive backend
  ...
  adapter from concrete repo-local chunk artifacts, and the final public Rust
  API,
- `riscv-authoritative-side-proof-bundle.md` owns the side-lane public tuple,
  optional packaged side verifier, and Spartan linkage relation that any
  recursive or compression layer must preserve,
