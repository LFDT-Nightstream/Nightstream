...
fn shifting_byte_offsets_preserves_program_tables_and_trace() {
    ...
    let run = collect_wasmtime_steps(&wasm, &HostEventBindings::default(), "main", &[]).unwrap();
    let shifted_run = collect_wasmtime_steps(&shifted, &HostEventBindings::default(), "main", &[]).unwrap();
