...
fn binding_surface() -> &'static Rv32imPhase0BindingSurface {
    static SURFACE: OnceLock<Rv32imPhase0BindingSurface> = OnceLock::new();
    SURFACE.get_or_init(|| build_rv32im_phase0_binding_surface_from_accepted_artifact(artifact()))
}

fn convergence_proof() -> &'static Rv32imOpeningConvergenceProof {
    static PROOF: OnceLock<Rv32imOpeningConvergenceProof> = OnceLock::new();
    PROOF.get_or_init(|| {
        build_rv32im_opening_convergence_proof_from_witnesses(binding_surface(), claim_witnesses())
            .expect("final opening convergence proof should build from real phase0 witnesses")
    })
}

fn convergence_artifact() -> &'static Rv32imOpeningConvergenceArtifact {
    static ARTIFACT: OnceLock<Rv32imOpeningConvergenceArtifact> = OnceLock::new();
    ARTIFACT.get_or_init(|| {
        build_rv32im_opening_convergence_artifact_from_proof(convergence_proof())
            .expect("compact convergence artifact should project from the full convergence proof")
    })
}

fn direct_convergence_artifact() -> &'static Rv32imOpeningConvergenceArtifact {
    static ARTIFACT: OnceLock<Rv32imOpeningConvergenceArtifact> = OnceLock::new();
    ARTIFACT.get_or_init(|| {
        build_rv32im_opening_convergence_artifact_from_witnesses(binding_surface(), claim_witnesses())
            .expect("compact convergence artifact should build directly from real phase0 witnesses")
    })
}

fn rebuild_compact_artifact_after_phase1_proof_mutation(artifact: &mut Rv32imOpeningConvergenceArtifact) {
...
fn final_bundle_roundtrips_from_real_phase0_witnesses() {
    ...
    let rebuilt = build_rv32im_opening_convergence_proof_from_witnesses(binding_surface(), claim_witnesses())
...
fn final_bundle_rejects_missing_bucket_or_duplicate_bucket_claim() {
    ...
    let err = verify_rv32im_opening_convergence_proof(&proof)
...
fn final_openings_follow_reduced_claim_order() {
    ...
    let err =
        verify_rv32im_opening_convergence_proof(&swapped).expect_err("final openings must stay in reduced-claim order");
    match err {
...
fn compact_convergence_artifact_roundtrips_from_full_proof() {
    ...
    verify_rv32im_opening_convergence_artifact_from_proof(artifact, convergence_proof())
        .expect("compact convergence artifact should match the full convergence proof");

    let rebuilt = build_rv32im_opening_convergence_artifact_from_proof(convergence_proof())
...
fn direct_compact_convergence_artifact_matches_full_proof_projection() {
    ...
    verify_rv32im_opening_convergence_artifact(direct)
        .expect("direct compact convergence artifact should be self-consistent");
    verify_rv32im_opening_convergence_artifact_from_proof(direct, convergence_proof())
...
fn compact_convergence_artifact_is_smaller_than_full_proof() {
    ...
    println!(
        "rv32im opening convergence sizes: full_proof={} compact_artifact={}",
...
fn compact_convergence_artifact_rejects_rebound_final_target_tamper() {
    ...
    let err = verify_rv32im_opening_convergence_artifact_from_proof(&artifact, convergence_proof())
...
fn compact_convergence_artifact_rejects_tampered_phase0_digest() {
    ...
    let err = verify_rv32im_opening_convergence_artifact(&artifact)
...
fn standalone_convergence_artifact_rejects_rebound_binding_digest_forgery() {
    let forged = build_rv32im_opening_convergence_artifact_from_witnesses(
        binding_surface(),
        &rebound_binding_digest_claim_witnesses(),
    )
    .expect("forged compact convergence artifact should still build from rebound witnesses");

    verify_rv32im_opening_convergence_artifact(&forged)
