...
/-- V6: with sampler-drawn challenges, the existing extractor still returns
with probability at least the sampled acceptance minus the extraction loss
`n / |C|` and the distance term `n * distance`. -/
