...
Its three block addresses include the selected candidate's last carrier block.
...
def shape : Phi81Relation.Shape := Lifecycle.PaperAlgebra.fullShape
  (Wide.RetainedLayout.logicalWidth Wide.SetupBinding.application)
  (Wide.FixedPoint.publicFits Wide.SetupBinding.application)

def firstBlock : Fin Wide.SetupBinding.messageColumns :=
  ⟨0, by rw [Wide.SetupBinding.messageColumns_eq]; decide⟩

def middleBlock : Fin Wide.SetupBinding.messageColumns :=
...
/-- The complete carrier contains exactly the three selected coordinates. -/
...
def sparseCommitment : Commitment.Value Wide.SetupBinding.verifierRows :=
  fun row =>
    let first := cacheRing (Wide.SetupBinding.productionAjtaiKey row firstBlock)
    let middle := cacheRing (Wide.SetupBinding.productionAjtaiKey row middleBlock)
    let last := cacheRing (Wide.SetupBinding.productionAjtaiKey row lastBlock)
...
theorem sparseCommitment_eq_commit :
    sparseCommitment =
      Commitment.commit (shape := shape) Wide.SetupBinding.productionAjtaiKey assignment := by
  unfold assignment
...
private def rowValue (row : Fin Wide.SetupBinding.verifierRows) : Value :=
  let firstCoordinates := Array.ofFn
    (Wide.SetupBinding.productionAjtaiKey row firstBlock)
  let middleCoordinates := Array.ofFn
    (Wide.SetupBinding.productionAjtaiKey row middleBlock)
  let lastCoordinates := Array.ofFn
    (Wide.SetupBinding.productionAjtaiKey row lastBlock)
...
theorem rowValue_eq_commit (row : Fin Wide.SetupBinding.verifierRows) :
    rowValue row = .array (List.ofFn fun lane : Fin ringDegree =>
      .atom ((Commitment.commit (shape := shape)
        Wide.SetupBinding.productionAjtaiKey assignment row) lane).val) := by
  rw [← sparseCommitment_eq_commit]
  rfl

/-- Schema 1: authority words, original block/lane/scalar support, and all
22 by 54 commitment coefficients. The authority words use the selected wide setup. -/
def parityValue (_delay : Unit := ()) : Value :=
  .array [.atom 1,
    .array (Wide.SetupBinding.commitmentKeyWords.map fun word => Value.atom word.val),
    .array [
