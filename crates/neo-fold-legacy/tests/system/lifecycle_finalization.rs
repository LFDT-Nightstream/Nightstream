...
use neo_fold_legacy::paper::construction2::{ProofState, RunningInstance};
use neo_fold_legacy::paper::nifs::{self, NifsProof, NifsProverAdapter, NifsProverRequest};
use neo_fold_legacy::CcsInstance;
...
fn toy_instance_with_x_value(prep: &neo_fold_legacy::Preprocessing, x: F) -> CcsInstance {
...
fn valid_bitness_instance(prep: &neo_fold_legacy::Preprocessing, r1cs: &R1cs, z: F) -> CcsInstance {
    let mut assignment = vec![F::ZERO; D];
    assignment[0] = z;
    direct_ccs::build_instance(prep, r1cs, &assignment).expect("valid bitness instance")
}

fn invalid_bitness_instance_with_valid_shape(prep: &neo_fold_legacy::Preprocessing) -> CcsInstance {
...
fn final_running(proof: &neo_fold_legacy::Uncompressed) -> neo_fold_legacy::RunningInstance {
...
fn final_running_mut(proof: &mut neo_fold_legacy::Uncompressed) -> &mut neo_fold_legacy::RunningInstance {
...
fn lifecycle_default_cpu_prover_finishes() {
    ...
    let audit = neo_fold_legacy::prove(
        &prep,
        [
    ...
    neo_fold_legacy::finish_uncompressed_with_audit(&prep, audit).expect("CPU backend finalize");
}
...
fn lifecycle_nifs_adapter_covers_recursive_and_terminal_folds() {
    ...
    let audit = neo_fold_legacy::lifecycle::prove_with_nifs_adapter(
        &prep,
        &mut adapter,
        [
    ...
    let finalized =
        neo_fold_legacy::lifecycle::finish_uncompressed_with_audit_and_nifs_adapter(&prep, &mut adapter, audit)
            .expect("adapter-backed lifecycle finalization");
    assert_eq!(adapter.calls, 2, "finalization should run the terminal NIFS.P");

    neo_fold_legacy::verify_uncompressed_audit(&prep, &finalized).expect("adapter-backed lifecycle proof verifies");
}

#[test]
fn verify_uncompressed_rejects_unfolded_trailing_latest() {
    let prep = support::toy_preprocessing();
    let audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 23)]])
    ...
    assert!(
        neo_fold_legacy::verify_uncompressed(&prep, &audit.proof).is_err(),
...
fn prove_rejects_empty_batch() {
    ...
    let err = neo_fold_legacy::prove(&prep, vec![Vec::<CcsInstance>::new()])
        .expect_err("prover must reject an empty batch before it creates an empty active state");

    assert!(
        matches!(err, neo_fold_legacy::Error::EmptyBatch),
...
fn extend_rejects_noncanonical_chunk_counter() {
    let prep = support::toy_preprocessing();
    let mut audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 31)]])
        .expect("one-batch uncompressed proof");
    audit.proof.state.chunk_count = u64::MAX;

    let err = neo_fold_legacy::extend(&prep, audit, vec![support::toy_instance(&prep, 32)])
        .expect_err("extending a noncanonical chunk counter must fail");

    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::Construction2(neo_fold_legacy::paper::construction2::Error::NoncanonicalCounter {
                counter: "chunk_count"
            })
        ),
...
fn extend_rejects_noncanonical_step_counter() {
    let prep = support::toy_preprocessing();
    let mut audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 33)]])
        .expect("one-batch uncompressed proof");
    audit.proof.state.step_count = u64::MAX;

    let err = neo_fold_legacy::extend(&prep, audit, vec![support::toy_instance(&prep, 34)])
...
fn extend_rejects_chunk_counter_past_the_canonical_field_range() {
    let prep = support::toy_preprocessing();
    let mut audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 35)]])
        .expect("one-batch uncompressed proof");
    audit.proof.state.chunk_count = F::ORDER_U64 - 1;

    let err = neo_fold_legacy::extend(&prep, audit, vec![support::toy_instance(&prep, 36)])
        .expect_err("the next chunk counter must remain a canonical field value");

    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::Construction2(neo_fold_legacy::paper::construction2::Error::CounterOverflow {
                counter: "chunk_count"
            })
        ),
...
fn extend_rejects_step_counter_past_the_canonical_field_range() {
    let prep = support::toy_preprocessing();
    let mut audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 37)]])
        .expect("one-batch uncompressed proof");
    audit.proof.state.step_count = F::ORDER_U64 - 1;

    let err = neo_fold_legacy::extend(&prep, audit, vec![support::toy_instance(&prep, 38)])
...
fn verify_uncompressed_rejects_forged_active_empty_without_terminal_fold() {
    let prep = support::toy_preprocessing();
    let mut forged = neo_fold_legacy::prove(&prep, std::iter::empty::<Vec<CcsInstance>>())
    ...
    forged.state.proof = ProofState::active(
        neo_fold_legacy::RunningInstance::default(),
        neo_fold_legacy::LatestInstance::from_instances(Vec::new()),
    );

    let err = neo_fold_legacy::verify_uncompressed(&prep, &forged)
        .expect_err("verify_uncompressed must reject Active+empty+final_fold=None");
    assert!(
        matches!(err, neo_fold_legacy::Error::MissingTerminalFoldProof),
...
fn verify_uncompressed_rejects_compact_anchor_tampers() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 24)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish uncompressed proof");

    let cases: [(&str, fn(&mut neo_fold_legacy::Uncompressed)); 3] = [
    ...
    for (label, tamper) in cases {
        ...
        tamper(&mut tampered);
        let err = neo_fold_legacy::verify_uncompressed(&prep, &tampered)
            .expect_err(&format!("verify_uncompressed must reject tampered {label}"));
        assert!(
            matches!(err, neo_fold_legacy::Error::PostStateMismatch),
...
fn verify_uncompressed_rejects_wrong_preprocessing_structure() {
    ...
    let proof = neo_fold_legacy::prove(&honest_prep, vec![vec![support::toy_instance(&honest_prep, 25)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&honest_prep, proof).expect("finish uncompressed proof");

    let wrong_prep = direct_ccs::preprocess_seeded(&bitness_r1cs(), 0x51_7A_C7).expect("wrong-shape preprocessing");
    assert!(
        neo_fold_legacy::verify_uncompressed(&wrong_prep, &finished).is_err(),
...
fn verify_uncompressed_rejects_terminal_input_relabel_tampers() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 26)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish uncompressed proof");
    neo_fold_legacy::verify_uncompressed(&prep, &finished).expect("honest proof verifies");
    ...
    for (label, tamper) in cases {
        ...
        tamper(&mut tampered);
        assert!(
            neo_fold_legacy::verify_uncompressed(&prep, &tampered).is_err(),
...
fn finish_uncompressed_folds_trailing_latest_and_verifies() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 29)]])
        .expect("one-batch uncompressed proof");
    ...
    neo_fold_legacy::verify_uncompressed(&prep, &finished).expect("finished proof verifies");
}
...
fn verify_uncompressed_rejects_tampered_recorded_acc_digest() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 33)]])
        .expect("one-batch uncompressed proof");
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish uncompressed proof");
    ...
    let err = neo_fold_legacy::verify_uncompressed(&prep, &finished)
        .expect_err("verify_uncompressed must reject a tampered acc_digest");
    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::PostStateMismatch | neo_fold_legacy::Error::AccDigestMismatch
        ),
...
fn verify_uncompressed_rejects_invalid_terminal_fold_even_if_final_accumulator_opens() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(
        &prep,
        vec![
    ...
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish proof");
    ...
    assert!(
        neo_fold_legacy::verify_uncompressed(&prep, &finished).is_err(),
...
fn verify_uncompressed_rejects_final_accumulator_from_different_fold_history() {
    ...
    let proof_a = neo_fold_legacy::prove(
        &prep,
        vec![
            vec![toy_instance_with_x_value(&prep, F::ZERO)],
            vec![toy_instance_with_x_value(&prep, F::ONE)],
    ...
    .expect("prove a");
    let mut finished_a = neo_fold_legacy::finish_uncompressed(&prep, proof_a).expect("finish a");

    let proof_b = neo_fold_legacy::prove(
        &prep,
        vec![
            vec![toy_instance_with_x_value(&prep, F::ONE)],
            vec![toy_instance_with_x_value(&prep, F::ZERO)],
    ...
    .expect("prove b");
    let finished_b = neo_fold_legacy::finish_uncompressed(&prep, proof_b).expect("finish b");
    ...
    assert!(
        neo_fold_legacy::verify_uncompressed(&prep, &finished_a).is_err(),
...
fn prove_rejects_shape_valid_invalid_previous_fold_before_it_enters_accumulator() {
    ...
    // build on top of it.
    assert!(
        neo_fold_legacy::prove(&prep, vec![vec![invalid_previous], vec![valid_current]]).is_err(),
...
fn finish_uncompressed_rejects_inconsistent_already_finalized_proof() {
    let prep = support::toy_preprocessing();
    let audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 37)]])
        .expect("one-batch uncompressed proof");
    let mut finished =
        neo_fold_legacy::finish_uncompressed_with_audit(&prep, audit).expect("finish uncompressed proof");
    ...
    assert!(
        matches!(
            neo_fold_legacy::finish_uncompressed_with_audit(&prep, finished),
            Err(neo_fold_legacy::Error::FinalizedProofInconsistent)
        ),
...
fn prove_rejects_public_input_len_mismatch() {
    ...
    let mismatched = neo_fold_legacy::CcsInstance::from_low_norm_assignment(
        prep.params(),
        prep.commitment_scheme(),
        prep.structure(),
        &z,
    ...
    assert!(
        matches!(
            neo_fold_legacy::prove(&prep, vec![vec![mismatched]]),
            Err(neo_fold_legacy::Error::PublicInputLenMismatch { expected: D, got: 0 })
        ),
...
fn validate(
    prep: &neo_fold_legacy::Preprocessing,
    statement: &neo_fold_legacy::paper::decider::Statement,
) -> Result<(), neo_fold_legacy::paper::decider::Error> {
    neo_fold_legacy::paper::decider::validate_witness(
        prep.params(),
        prep.structure(),
...
fn decider_validate_witness_accepts_finished_proof() {
    ...
    let finished = neo_fold_legacy::finish_uncompressed_with_audit(&prep, proof).expect("finish");
    let statement = neo_fold_legacy::build_decider_statement(&prep, &finished);
...
fn decider_validate_witness_rejects_tampered_public_image_acc_digest() {
    ...
    .expect("two-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed_with_audit(&prep, proof).expect("finish");
    let mut statement = neo_fold_legacy::build_decider_statement(&prep, &finished);
    ...
    assert!(
        matches!(
            ...
            Err(neo_fold_legacy::paper::decider::Error::PublicImageMismatch)
        ),
...
fn decider_validate_witness_rejects_missing_final_fold() {
    ...
    match err {
        neo_fold_legacy::paper::decider::Error::WalkFailed(reason) => {
            assert!(
...
fn decider_validate_witness_rejects_final_state_claims_not_matching_walk() {
    ...
    let final_running = match &mut statement.witness.final_state.proof {
        neo_fold_legacy::ProofState::Active { running, .. } => running,
        neo_fold_legacy::ProofState::Initial => panic!("test setup: state must be Active"),
    };
    ...
    assert!(
        matches!(
            validate(&prep, &statement),
            Err(neo_fold_legacy::paper::decider::Error::WitnessShape)
        ),
...
fn decider_validate_witness_rejects_final_state_latest_not_empty() {
    ...
    match &mut statement.witness.final_state.proof {
        neo_fold_legacy::ProofState::Active { latest, .. } => {
...
fn same_shape_different_batches_have_same_f_prime_trace_but_different_acc_digest() {
    ...
    let proof_a =
        neo_fold_legacy::prove(&prep, vec![vec![toy_instance_with_x_value(&prep, F::ZERO)]]).expect("prove a");
    let finished_a = neo_fold_legacy::finish_uncompressed(&prep, proof_a).expect("finish a");

    let proof_b = neo_fold_legacy::prove(&prep, vec![vec![toy_instance_with_x_value(&prep, F::ONE)]]).expect("prove b");
    let finished_b = neo_fold_legacy::finish_uncompressed(&prep, proof_b).expect("finish b");
    ...
    neo_fold_legacy::verify_uncompressed(&prep, &finished_a).expect("proof a verifies");
    neo_fold_legacy::verify_uncompressed(&prep, &finished_b).expect("proof b verifies");
}
...
fn verify_uncompressed_rejects_tampered_stateless_semantic_state_digest() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 51)]]).expect("one-batch proof");
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finalize");

    // Honest stateless proof must verify cleanly under verify_uncompressed.
    neo_fold_legacy::verify_uncompressed(&prep, &finished).expect("honest stateless proof verifies");
    ...
    assert!(
        matches!(err, neo_fold_legacy::Error::StatelessSemanticInvariantViolated),
...
fn verify_uncompressed_audit_rejects_tampered_stateless_step_proof_semantic_state_digest() {
    ...
    .expect("two-batch proof");
    let mut audit = neo_fold_legacy::finish_uncompressed_with_audit(&prep, proof).expect("finalize with audit");

    // Honest audit verifies cleanly.
    neo_fold_legacy::verify_uncompressed_audit(&prep, &audit).expect("honest stateless audit verifies");
    ...
    let err = neo_fold_legacy::verify_uncompressed_audit(&prep, &audit)
...
// all four terminal obligations against each `(claim, witness Z)`:
//
//   commit(Z) == claim.c           | Ajtai opening
//   project_x(Z) == claim.X        | public-input projection
//   ||Z||_∞ < b                    | low-norm digit-range
//   Eval_K == Pad · Z(r) and
//   Eval_A[j] == M_j · Z(r)        | CE-relation closure (this test)
// The `acc_digest` chain hash only carries commitment data, so a
// malformed evaluation family slips past the binding pipeline. The CE-relation
...
#[test]
fn final_witness_authority_rejects_eval_k_inconsistent_with_pad_z_at_r() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 71)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish uncompressed proof");
    ...
    neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &honest_running)
        .expect("honest running passes the full witness-authority gate");

    // Tamper the separate Pad family on the first claim. Witness Z stays
    // honest, so commit/X/low-norm still pass. Only the CE-relation
    // row can fail.
    let mut tampered_running = honest_running;
    assert!(
        !tampered_running.claims.is_empty() && !tampered_running.claims[0].eval_k.is_empty(),
        "test setup requires a non-empty Eval_K"
    );
    let original = tampered_running.claims[0].eval_k[0];
    tampered_running.claims[0].eval_k[0] = original + neo_math::K::ONE;
    assert_ne!(
        tampered_running.claims[0].eval_k[0], original,
        "mutation must actually change Eval_K[0]"
    );

    let err = neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &tampered_running)
        .expect_err("verify_uncompressed's witness-authority gate must reject Eval_K inconsistent with Pad·Z at r");
    assert!(
        matches!(err, neo_fold_legacy::Error::FinalAccumulatorCeRelationViolation { .. }),
        "expected FinalAccumulatorCeRelationViolation on Eval_K tamper, got {err:?}"
    );
}
...
fn final_witness_authority_rejects_extra_claim_without_witness() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 76)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);

    running.claims.push(running.claims[0].clone());

    let err = neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &running)
        .expect_err("extra claim without matching witness must reject");
    assert!(
        matches!(err, neo_fold_legacy::Error::FinalAccumulatorWitnessShapeMismatch),
...
fn final_witness_authority_rejects_extra_witness_without_claim() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 77)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
...
fn final_witness_authority_rejects_zero_commitment_with_wrong_ajtai_shape() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 78)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
    ...
    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::FinalAccumulatorWitnessCommitmentMismatch { .. }
        ),
...
fn final_witness_authority_rejects_zero_witness_with_wrong_packed_shape() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 79)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
...
fn final_witness_authority_rejects_zero_witness_m_in_exceeds_structure_m() {
    ...
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 80)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
    ...
    assert!(
        matches!(err, neo_fold_legacy::Error::FinalAccumulatorPublicInputMismatch { .. }),
...
fn final_witness_authority_rejects_zero_witness_partial_ring_public_input() {
    let prep = support::toy_preprocessing_unfixed_public_input_len();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 801)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
...
fn final_witness_authority_rejects_m_in_relabel_below_program_public_input_len() {
    ...
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 81)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
    ...
    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::PublicInputLenMismatch { expected: D, got: 0 }
        ),
...
fn final_witness_authority_rejects_nonzero_witness_m_in_relabel_below_program_public_input_len() {
    ...
    let proof = neo_fold_legacy::prove(&prep, vec![vec![toy_instance_with_x_value(&prep, F::ONE)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
    ...
    // Hacker model: keep c, Eval_K, Eval_A, and Z untouched, but relabel the
    ...
    // holds, and the evaluation families are independent of L_in's projection width.
...
/// End-to-end version of the Eval_K tamper attack: build a real
/// finished proof, mutate `proof.state.proof.running.claims[0].eval_k[0]`
...
/// [`final_witness_authority_rejects_eval_k_inconsistent_with_pad_z_at_r`]
...
/// Eval_K is bound end-to-end:
///
///   - Helper-direct test: CE-relation rows are load-bearing.
///   - This end-to-end test: any Eval_K tamper in the recorded state
///     is rejected by *some* gate in the verifier pipeline.
#[test]
fn verify_uncompressed_rejects_recorded_eval_k_tamper_via_binding_step() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 72)]])
        .expect("one-batch uncompressed proof");
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish uncompressed proof");

    neo_fold_legacy::verify_uncompressed(&prep, &finished).expect("honest finished proof verifies");

    let running = final_running_mut(&mut finished);
    assert!(
        !running.claims.is_empty() && !running.claims[0].eval_k.is_empty(),
        "test setup requires non-empty Eval_K on the recorded running"
    );
    running.claims[0].eval_k[0] = running.claims[0].eval_k[0] + neo_math::K::ONE;

    let err = neo_fold_legacy::verify_uncompressed(&prep, &finished)
        .expect_err("verify_uncompressed must reject a recorded Eval_K tamper");
    assert!(
        matches!(err, neo_fold_legacy::Error::PostStateMismatch),
...
/// `claim.eval_a.len() != structure.t()` — outer-dimension shape
/// mismatch. The CE-relation gadget cannot compute expected values for
/// the missing matrix and rejects up-front.
#[test]
fn final_witness_authority_rejects_eval_a_outer_length_mismatch() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 73)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);

    running.claims[0].eval_a.pop();

    let err = neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &running)
        .expect_err("Eval_A outer-length mismatch must reject");
    assert!(
        matches!(err, neo_fold_legacy::Error::FinalAccumulatorCeRelationViolation { .. }),
...
fn final_witness_authority_rejects_r_length_mismatch() {
    let prep = support::toy_preprocessing();
    let proof = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 75)]])
        .expect("one-batch uncompressed proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, proof).expect("finish");
    let mut running = final_running(&finished);
    ...
    assert!(
        matches!(
            err,
            neo_fold_legacy::Error::FinalAccumulatorEvaluationPointShapeMismatch { .. }
        ),
