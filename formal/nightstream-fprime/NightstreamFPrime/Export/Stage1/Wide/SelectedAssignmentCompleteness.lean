...
theorem complete_with_values (compiled : PiRlcWideSampler.RangePlan.Compiled)
    (ajtai : AjtaiKey
    ...
    ∃ (env : Env) (message : Fin 4 → F),
      ...
      (SourceAssignment.raw application env suffix).outputDigest = output.x ∧
      PiRLC.Wide.Formal.RangesCompleted
        (Layout.Stage1.Wide.PiRLCInputs.interface
          (logicalWidth := RetainedLayout.logicalWidth application)
          (publicFits := FixedPoint.publicFits application)) Layout.Stage1.Wide.PiRLCInputs.phaseOffset env ∧
      Layout.Stage1.Wide.PilotPiCCSPiRLCPiDECRunningTransition.PhysicalHolds
        (FixedPoint.relation application compiled fits) env := by
  ...
  exact ⟨env, message, rows, CarrierAssignment.norm application env suffix relation running,
    publicOutput, advice, outputDigest, rangeValues, physical⟩
...
theorem complete (compiled : PiRlcWideSampler.RangePlan.Compiled)
  ...
  obtain ⟨env, message, rows, norm, publicOutput, advice, digest, _, _⟩ := complete_with_values
    compiled ajtai context input output result step priorWellFormed nextWellFormed freshPublic accepted recursiveResult witnessWidth
  exact ⟨env, message, rows, norm, publicOutput, advice, digest⟩
