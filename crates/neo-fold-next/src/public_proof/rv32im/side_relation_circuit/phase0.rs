...
use crate::rv32im::kernel::phase0_full_width_for_schema;
use crate::rv32im::kernel::rv32im_exact_stage_pp_seed;
use crate::rv32im::kernel::{CommitmentContextId, FamilyEvalSchemaId, PackedColumnOracleRef};
use crate::superneo_circuit::k_field::{
...
pub fn derive_phase0_point<CS: ConstraintSystem<SpartanF>>(
    ...
    let opened_object = crate::rv32im::kernel::OpenedAjtaiObjectId {
    ...
    let point =
        crate::rv32im::kernel::derive_phase0_point(&opened_object, commitment_context, schema, slot, binding_digest);

    let mut point_tr = Poseidon2TranscriptCircuit::new(
        cs.namespace(|| format!("{label}_point_tr")),
        b"neo.fold.next/rv32im/opening_convergence/phase0/point",
    )?;
...
pub fn enforce_commitment_root_and_opened_object_digest<CS: ConstraintSystem<SpartanF>>(
    ...
    opened_object: &crate::rv32im::kernel::OpenedAjtaiObjectId,
    label: &str,
    ...
    let mut commitment_tr = Poseidon2TranscriptCircuit::new(
        ...
        b"neo.fold.next/rv32im/opening_convergence/phase0/commitment_root",
    )?;
    ...
    let mut opened_object_tr = Poseidon2TranscriptCircuit::new(
        ...
        b"neo.fold.next/rv32im/opening_convergence/phase0/opened_object",
    )?;
...
fn ensure_phase0_exact_pp(schema: FamilyEvalSchemaId, cols: usize) -> Result<(), SynthesisError> {
    ...
    if has_global_pp_for_dims(D, cols) {
        if let Ok((kappa, seed)) = get_global_pp_seeded_params_for_dims(D, cols) {
            if kappa != want_kappa || seed != rv32im_exact_stage_pp_seed() {
                return Err(SynthesisError::Unsatisfiable);
            }
        } else {
        ...
        set_global_pp_seeded(D, want_kappa, cols, rv32im_exact_stage_pp_seed())
            .map_err(|_| SynthesisError::Unsatisfiable)?;
    }
    Ok(())
}
