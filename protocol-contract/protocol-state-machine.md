...
ordered PiCCS inputs. It then samples 28 `alpha` coordinates and one `gamma`.
...
challenge `j`. There are exactly 28 rounds.

After the 238 ordered PiCCS ring outputs and the joint terminal check are
...
PiRLC output, the 16 PiDEC children, and fold finalization are absorbed in
...
|---|---:|---|---|
| `REP-PICCS-ROUNDS` | exactly 28 | round 0 through 27 | reject |
| `REP-RHO-SOURCES` | exactly 17 | source 0 through 16 | reject |
...
Thus one fold derives exactly 918 accepted strong-set digits and uses at most
2,754 sampler candidates.
...
For each adjacent pair, the 16 ordered PiDEC children of fold `j` are the 16
