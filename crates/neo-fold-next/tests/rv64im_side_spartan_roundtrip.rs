...
use neo_fold_next::nightstream::rv64im::audit::{
    build_rv64im_bound_phase0_claim_witnesses_from_accepted_artifact, debug_check_rv64im_side_binding_circuit,
...
fn rv64im_side_binding_roundtrip_with_same_and_rebuilt_vk() {
    ...
    let claim_witnesses =
        build_rv64im_bound_phase0_claim_witnesses_from_accepted_artifact(&nightstream_statement, &accepted_artifact)
    ...
    let proof = prove_rv64im_side_binding(&pk, &statement, &witness, &claim_witnesses).expect("prove side binding");
...
fn rv64im_side_binding_setup_reuses_same_shape_for_rebuilt_same_case() {
    let public_proof_a = prove_rv64im_public_proof_with_options(
        &proof_input("control_flow_jal_skip_ecall"),
        Rv64imPublicProofOptions {
            root_fold_schedule: FoldSchedule::RowsPerChunk(1),
        },
    )
    .expect("prove rv64im public proof A");
    ...
    let public_proof_b = prove_rv64im_public_proof_with_options(
        &proof_input("control_flow_jal_skip_ecall"),
        Rv64imPublicProofOptions {
            root_fold_schedule: FoldSchedule::RowsPerChunk(1),
        },
    )
    .expect("prove rv64im public proof B");
    ...
    let claim_witnesses_b = build_rv64im_bound_phase0_claim_witnesses_from_accepted_artifact(
    ...
    debug_check_rv64im_side_binding_circuit(&statement_b, &witness_b, &claim_witnesses_b)
    ...
    let proof_b = prove_rv64im_side_binding(&keys_a.as_ref().0, &statement_b, &witness_b, &claim_witnesses_b)
