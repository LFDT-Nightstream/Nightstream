...
Research at checkpoint `961ba3d2e`, 2026-09-22. This check concerns circuit
size only. It did not run Rust benchmarks or regenerate packages or fixtures.
The saved package remains at 184,359,564 committed coordinates,
4,703,127 logical rows, and 3,001,571,645 normalized matrix entries. Lean
production source now selects the 86-row retained template below. Rust and
saved artifacts still use the checkpoint template; integration is pending.
...
use the same 86-row indexing. The emitted packages, Rust row decoder, pins,
and fixtures remain at the preserved checkpoint until the Lean batch is
stable. Their conformance checks are still required before installation.
