...
use neo_fold_clean::{extend, finish_uncompressed, prove, verify_uncompressed, CcsInstance, FoldSchedule};
...
fn proof_shape(
    proof: &neo_fold_clean::lifecycle::Uncompressed,
    step_prove_ms: &[f64],
    finish_ms: f64,
...
fn fibonacci_bits_perf_snapshot() {
    ...
    let final_running_claims = finished
        ...
        .unwrap_or(0);
    let final_latest_instances = match &finished.state.proof {
...
fn print_report(
    ...
    finished: &neo_fold_clean::lifecycle::Uncompressed,
    shape: &ProofShape,
    fold_rows: &[FoldRow],
    values: &[u64],
    transitions: usize,
    ...
    kv("final folds", usize::from(finished.final_fold.is_some()));
