...
impl NifsProfileSample {
    pub(crate) fn from_profiles(profiles: Vec<MetalNifsProfile>) -> Self {
        ...
        for profile in profiles {
            ...
            sample.pi_ccs += profile.pi_ccs.elapsed;
            sample.ajtai_y_eval += profile.pi_ccs.ajtai.y_eval;
            sample.pi_rlc += profile.pi_rlc.elapsed;
            sample.pi_dec += profile.pi_dec.elapsed;
            sample.dec_form_build += profile.pi_dec.form_build;
            sample.dec_projection += profile.pi_dec.projection;
            sample.dec_host_materialization += profile.pi_dec.host_materialization;
            sample.folds += 1;
            sample.fe_on_metal &= profile.pi_ccs.fe.on_metal;
            sample.ajtai_y_eval_on_metal &= profile.pi_ccs.ajtai.y_eval_on_metal;
            sample.nc_on_metal &= profile.pi_ccs.nc.on_metal;
            sample.nc_mask_native_on_metal &= profile.pi_ccs.nc.mask_native_on_metal;
            sample.rlc_witness_on_metal &= profile.pi_rlc.witness_on_metal;
            sample.rlc_witness_resident_only &= profile.pi_rlc.witness_resident_only;
            sample.rlc_rho_small_coefficients &= profile.pi_rlc.rho_small_coefficients;
            sample.dec_split_on_metal &= profile.pi_dec.split_on_metal;
            sample.dec_recomposition_on_metal &= profile.pi_dec.recomposition_on_metal;
            sample.dec_forms_on_metal &= profile.pi_dec.forms_on_metal;
            sample.dec_y_on_metal &= profile.pi_dec.y_on_metal;
            sample.dec_commit_on_metal &= profile.pi_dec.commit_on_metal;
            sample.resident_input_folds += usize::from(profile.residency.running_input);
            sample.resident_output_folds += usize::from(profile.residency.running_output);
            sample.deferred_proof_folds += usize::from(profile.residency.proof_deferred);
            sample.deferred_running_folds += usize::from(profile.residency.running_deferred);
            sample.recursive_compile_reverify_required |= profile.residency.recursive_compile_reverify_required;
