...
//! (44 bits), `cnt` (running non-pad count), and the running `h_rs`/`h_ws`
...
use crate::frontends::nebula::layout::{
    x_offsets, CellRecord, LayoutError, MemOpRecord, NebulaParams, StepPublicInput, H_FS, H_IS, H_RS, H_WS, K_BITS,
    STEP_IDX_BITS, TS_BITS, VAL_BITS, X_BITS,
};
...
impl SMemCircuit {
    ...
    pub fn witness(&self, gammas: &Gammas, data: &StepData<'_>) -> Result<(Vec<F>, StepPublicInput), LayoutError> {
        ...
        z[0] = F::ONE;
        for (i, bit) in x.encode()?.into_iter().enumerate() {
            z[1 + i] = bit;
        }
        for (j, slot) in slots.iter().enumerate() {
            ...
            write_bits(&mut z, off + 2, slot.seg as u64, 1);
            write_bits(&mut z, off + 3, slot.addr, p.addr_bits());
            write_bits(&mut z, off + 3 + p.addr_bits(), slot.v_r as u64, VAL_BITS);
            write_bits(&mut z, off + 3 + p.addr_bits() + VAL_BITS, slot.v_w as u64, VAL_BITS);
            write_bits(&mut z, off + 3 + p.addr_bits() + 2 * VAL_BITS, slot.rt, TS_BITS);
    ...
    for j in 0..p.b_ops {
        let slot = match data.ops.get(j) {
            Some(op) => {
                ...
                let g = addr + if op.seg { p.rom_cells() } else { 0 };
                ...
                OpSlotValues {
                    ...
                    diff: wt.wrapping_sub(rt).wrapping_sub(1) & ts_mask,
                    h_rs,
                    h_ws,
                }
            }
...
fn emit_rows(p: &NebulaParams, zm: &ZMap, rb: &mut RowBuilder) {
    ...
    // Op slots: E1–E9 (spec §4.1).
    for j in 0..p.b_ops {
        let off = zm.op_slot(j);
        let aux = &zm.op_aux[j];
        let pad = Lin::bit(off);
        let is_write = Lin::bit(off + 1);
        let seg = Lin::bit(off + 2);
        let addr = Lin::word(off + 3, p.addr_bits());
        let v_r = Lin::word(off + 3 + p.addr_bits(), VAL_BITS);
        let v_w = Lin::word(off + 3 + p.addr_bits() + VAL_BITS, VAL_BITS);
        let rt = Lin::word(off + 3 + p.addr_bits() + 2 * VAL_BITS, TS_BITS);
        let diff = Lin::word(aux.diff, TS_BITS);
        let cnt = Lin::word(aux.cnt, zm.cnt_bits);
        let not_pad = one.clone().minus(pad.clone());

        // E1: bitness of the lane slot and its aux bits.
        ...
        // E3: reads write back what they read.
        rb.product_row(&one.clone().minus(is_write.clone()), &v_w.clone().minus(v_r.clone()));

        // E4: rt < wt, gated on real ops: (1−pad)·(wt − rt − 1 − diff) = 0,
        // wt = ts_in + cnt_j.
        let wt = ts_in.clone().plus(cnt.clone());
        rb.product_row(&not_pad, &wt.clone().minus(rt.clone()).minus(one.clone()).minus(diff));

        // E5: no writes to ROM.
        rb.product_row(&is_write, &one.clone().minus(seg.clone()));

        // E6: ROM addresses stay below R.
        for k in p.r as usize..p.addr_bits() {
            rb.product_row(&one.clone().minus(seg.clone()), &Lin::bit(off + 3 + k));
        }

        // E7: pad slots are all-zero.
        for field in [&is_write, &seg, &addr, &v_r, &v_w, &rt] {
            rb.product_row(&pad, field);
        }

        // E8/E9: RS and WS product updates. The fingerprint's packed prefix
        // is affine in lane bits: packed = t + 2^TS_BITS · (addr + seg·R).
        let g_lin = addr
            .clone()
...
/// `c_ops`/`c_is`/`c_fs` before γ), or the E2-constrained `cnt` aux (a
/// deterministic function of lane pads). Run at construction, so a layout
/// change that leaks a free interpretation bit into the products fails the
/// build — not a review.
fn audit_lane_residency(p: &NebulaParams, zm: &ZMap, rb: &RowBuilder) {
    ...
    let in_cnt_aux = |col: usize| {
        zm.op_aux
            .iter()
            .any(|a| (a.cnt..a.cnt + zm.cnt_bits).contains(&col))
    };
    for m in [M_P, M_Q, M_FA, M_FB, M_GA, M_GB, M_V] {
        for &(row, col, _) in &rb.trips[m] {
            assert!(
                col < 1 + X_BITS || in_lane(col) || in_cnt_aux(col),
