...
- all output CCS-matrix coefficients, and no Pad coefficient;
...
structure Interface where
  ...
  gamma : Nat → KExpr
  outputEval_A : Nat → MatrixCoordinate productionShape → KExpr

def coefficientExprs (interface : Interface) (offset : Nat) : List KExpr :=
  (canonicalMatrixCoordinates productionShape).map
    (interface.outputEval_A offset)
...
theorem flatConstraints_varsBelow (interface : Interface) (offset : Nat)
  ...
  PointWeightedHorner.Owned.flatConstraints_varsBelow (coreInterface interface)
    cubeVariables_positive offset env assumptions

/-- The owned `Eval_A` result lies inside this leaf's declared interval. -/
...
theorem coefficientExprs_length (interface : Interface) (offset : Nat) :
    (coefficientExprs interface offset).length = 12096 := by
  simp [coefficientExprs, canonicalMatrixCoordinates_length, productionShape,
...
exactly production `ProtocolPolynomial.matrixAtMessage`. -/
theorem spec_implies_keyMatrixAtMessage
    {logicalWidth : Nat}
    ...
    (outputEvalEq : ∀ coordinate,
      (interface.outputEval_A offset coordinate).eval env =
