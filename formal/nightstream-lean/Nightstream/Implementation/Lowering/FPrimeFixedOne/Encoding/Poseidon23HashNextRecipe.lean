import Nightstream.Implementation.Lowering.FPrimeFixedOne.Encoding.Poseidon23HashNextFrame

/-!
Contract: total canonical recipe for the next Construction-2 binding hash.
...
namespace Poseidon23HashNextRecipe

private theorem footprint_exact
    (parameters : Parameters)
    (profile : Poseidon23ApplicationProfile parameters) :
    (signature parameters).callFootprint Call.hashNext =
...
private theorem honest_source_values
    ...
    Poseidon23HashOccurrence.Honest.sourceValues
        (Poseidon23HashNextFrame.occurrence parameters profile frame)
        assignment =
      Poseidon23Hash.sourceCoordinates profile.codecs true
  ...
  rw [Poseidon23HashNextFrame.iteration_values_eq_singleton
    parameters profile frame assignment] at iterationValues
  have iterationHead :
      assignment
          ((Poseidon23HashNextFrame.occurrence parameters profile frame).iteration.id) =
        (boundedNatCodec.encode iteration).getD 0 0 := by
    rw [← iterationValues]
    rfl
  rw [iterationHead]
  rw [Poseidon23HashNextFrame.source_tail_values
    parameters profile frame assignment]
  rw [coordinates.2.1, coordinates.2.2.1, coordinates.2.2.2]
  have nextTrue :
      (Poseidon23HashNextFrame.occurrence parameters profile frame).next =
        true := rfl
  simp [nextTrue]
...
private theorem active_source_values
    ...
    (assignment : ColumnId -> Field)
    (normalized :
      assignment ((Poseidon23HashNextFrame.occurrence
          parameters profile frame).normalizedColumn.id) =
        assignment ((Poseidon23HashNextFrame.occurrence
          parameters profile frame).iteration.id) + 1) :
...
private theorem temporary_disjoint_visible
    ...
    IdsDisjoint
      (Poseidon23HashNextFrame.occurrence
  ...
  · rw [← Poseidon23HashNextFrame.temporary_ids_exact
      parameters profile frame]
    exact temporary
  · exact Poseidon23HashNextFrame.visible_subset
...
def recipe
    (parameters : Parameters)
    (profile : Poseidon23ApplicationProfile parameters) :
    CallRecipe (signature parameters) profile.family Call.hashNext := by
  refine
    { rows := ?_
      rowCount := ?_
      rowsOwned := ?_
            ...
            | cons current tail =>
                ...
                | cons running tail =>
                    ...
                    exact Poseidon23HashOccurrence.rows
                      (Poseidon23HashNextFrame.occurrence
                        parameters profile frame)
                      (Poseidon23HashNextFrame.coreFacts
                        parameters profile frame)
  · intro context references frame
    cases references with
    | cons iteration tail =>
            ...
            | cons current tail =>
                ...
                | cons running tail =>
                    ...
                    · exact List.mem_append_left _
                        (Poseidon23HashNextFrame.visible_subset
                                    ...
                                    have semanticSource :
                                        Poseidon23HashOccurrence.sourceValues
                                          ...
                                          Poseidon23Hash.sourceCoordinates
                                            profile.codecs true iteration z0
                                            current running := by
