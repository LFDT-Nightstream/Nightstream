...
use neo_fold_prototype::rv32im::audit::{build_rv32im_chunk_step_relations, verify_rv32im_chunk_step_relation};
use p3_field::PrimeCharacteristicRing;

#[test]
fn rv32im_chunk_step_relation_round_trip() {
    let fixture = rv32im_n2_support::build_rv32im_n2_fixture().expect("build rv32im n=2 fixture");
    let relations = build_rv32im_chunk_step_relations(&fixture.final_statement, &fixture.final_proof)
    ...
    assert!(
        !relations.is_empty(),
        "expected at least one RV32IM chunk-step relation"
    );

    for relation in &relations {
        let state_out = verify_rv32im_chunk_step_relation(&relation.statement, &relation.witness)
...
#[test]
fn rv32im_chunk_step_relation_rejects_tampered_transcript_out() {
    let fixture = rv32im_n2_support::build_rv32im_n2_fixture().expect("build rv32im n=2 fixture");
    let mut relation = build_rv32im_chunk_step_relations(&fixture.final_statement, &fixture.final_proof)
        .expect("build chunk-step relations")
        .into_iter()
        .next()
    ...
    let err = verify_rv32im_chunk_step_relation(&relation.statement, &relation.witness)
        .expect_err("tampered transcript_out must fail");
    assert!(format!("{err}").contains("transcript_out"), "unexpected error: {err}");
}

#[test]
fn rv32im_chunk_step_relation_rejects_tampered_next_carry() {
    let fixture = rv32im_n2_support::build_rv32im_n2_fixture().expect("build rv32im n=2 fixture");
    let mut relation = build_rv32im_chunk_step_relations(&fixture.final_statement, &fixture.final_proof)
        .expect("build chunk-step relations")
        .into_iter()
