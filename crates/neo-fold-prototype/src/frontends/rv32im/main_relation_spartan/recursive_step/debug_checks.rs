...
use super::*;
use crate::rv32im::kernel::rv32im_root_main_lane_context_for_step_cap;
...
fn ensure_debug_cs_satisfied(
    ...
    if cs.is_satisfied() {
        ...
        Err(Rv32imMainRecursionStepSpartanError::Prepare(format!(
            "{stage}: {}",
...
pub fn debug_check_rv32im_main_recursion_step_spartan_circuit(
...
pub fn debug_check_rv32im_main_recursion_step_spartan_embedded_body(
    spartan_shape: &Rv32imMainRecursionStepSpartanShape,
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<(), Rv32imMainRecursionStepSpartanError> {
    let circuit = build_rv32im_main_recursion_step_circuit(spartan_shape, backend_relation)?;
    ...
    if relation_public_cursor != relation_public_inputs.len() {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion embedded step body did not consume all expected public values".into(),
        ));
    }
    if !cs.is_satisfied() {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
...
pub fn debug_measure_rv32im_main_recursion_step_spartan_circuit_shape(
    spartan_shape: &Rv32imMainRecursionStepSpartanShape,
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
...
pub fn debug_check_rv32im_main_recursion_step_spartan_inactive_side_lane_constraints(
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<(), Rv32imMainRecursionStepSpartanError> {
    let mut cs = TestConstraintSystem::<SpartanF>::new();
...
pub fn debug_check_rv32im_main_recursion_x_out_gadget_parity(
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<(), Rv32imMainRecursionStepSpartanError> {
...
pub fn debug_check_rv32im_main_recursion_step_spartan_chunk_replay_surface(
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<(), Rv32imMainRecursionStepSpartanError> {
    ...
    {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step payload replay surface is not dominated by the carried chunk cover".into(),
        ));
    }
    if replay_chunk.pi_ccs.ccs_outputs.len() < replay_chunk.fresh_claims.len() {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step replay surface has fewer CCS outputs than fresh claims".into(),
        ));
    }
    Ok(())
}

pub fn debug_check_rv32im_main_recursion_step_spartan_pi_ccs_replay_lengths(
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<(), Rv32imMainRecursionStepSpartanError> {
    let replay_chunk = backend_relation
        .payload
    ...
    let step_cap = backend_relation
        ...
        .step_cap()
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    let (params, _, structure) = rv32im_root_main_lane_context_for_step_cap(step_cap)
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    ...
    {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step Pi_CCS FE replay round count does not match row_chals + alpha_prime".into(),
        ));
    }
    ...
    {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step Pi_CCS NC replay round count does not match s_col + alpha_prime_nc".into(),
        ));
    }
    if replay_chunk.pi_ccs.row_chals.len() != dims.ell_n {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step Pi_CCS row challenge count does not match ell_n".into(),
        ));
    }
    if replay_chunk.pi_ccs.s_col.len() != dims.ell_m {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step Pi_CCS column challenge count does not match ell_m".into(),
        ));
    }
    Ok(())
}
