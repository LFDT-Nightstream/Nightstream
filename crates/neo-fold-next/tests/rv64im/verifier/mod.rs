...
use neo_fold_next::rv64im::audit::{
    audit_rv64im_accepted_proof, audit_rv64im_accepted_proof_against_input, audit_rv64im_accepted_proof_with_perf,
};
...
fn accepted_artifact_audit_replays_transcript_without_input_audit() {
    ...
    let perf = audit_rv64im_accepted_proof_with_perf(&artifact).expect("accepted audit");
...
fn audit_path_checks_public_input_only_when_requested() {
    ...
    audit_rv64im_accepted_proof(&artifact).expect("accepted audit");
    audit_rv64im_accepted_proof_against_input(&input, &artifact, &audit).expect("input audit");
}
