...
use neo_fold_prototype::circuit::superneo::k_field::alloc_constant_k;
use neo_fold_prototype::circuit::superneo::output_binding::{alloc_fresh_ccs_claim, enforce_me_outputs_against_inputs};
use neo_math::{D, F, K};
...
fn rv32im_main_relation_output_binding_accepts_fresh_and_me_inputs() {
    ...
    let r_vars = r
        ...
        .map(|(idx, value)| {
            alloc_constant_k(
                &mut cs,
                neo_fold_prototype::circuit::superneo::k_field::KNum::from_neo_k(*value),
                &format!("r_{idx}"),
            )
        })
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_output_binding_rejects_tampered_fresh_commitment() {
    let params = NeoParams::goldilocks_auto_r1cs_ccs(3).expect("params");
    let structure = toy_structure();
    let fresh = fresh_claim(3, 5);
    let r = vec![K::from(F::from_u64(11))];
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_output_binding_ignores_s_col_shell() {
    let params = NeoParams::goldilocks_auto_r1cs_ccs(3).expect("params");
    let structure = toy_structure();
    let fresh = fresh_claim(3, 5);
    let r = vec![K::from(F::from_u64(11))];
