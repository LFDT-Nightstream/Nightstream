//! Owns the RV32IM side binding proof.
//!
...
use super::authoritative_side::Rv32imSideBindingStatement;
...
pub type Rv32imSideBindingProverKey = spartan2::spartan::SpartanProverKey<Rv32imSideBindingEngine>;
pub type Rv32imSideBindingVerifierKey = spartan2::spartan::SpartanVerifierKey<Rv32imSideBindingEngine>;

type Rv32imSideBindingKeyPair = Arc<(Rv32imSideBindingProverKey, Rv32imSideBindingVerifierKey)>;

static RV32IM_SIDE_BINDING_SETUP_CACHE: OnceLock<Mutex<HashMap<[u8; 32], Rv32imSideBindingKeyPair>>> = OnceLock::new();

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct Rv32imSideBindingProof {
    pub snark_data: Vec<u8>,
}

impl Rv32imSideBindingProof {
...
#[derive(Clone)]
struct Rv32imSideBindingCircuit {
...
impl SpartanCircuit<Rv32imSideBindingEngine> for Rv32imSideBindingCircuit {
    ...
    fn synthesize<CS: ConstraintSystem<SpartanF>>(
        ...
        let mut tr = Poseidon2TranscriptCircuit::new(
            ...
            b"neo.fold.next/nightstream/rv32im/authoritative_side/public_instance",
        )?;
        tr.append_u64s(
        ...
        let mut statement_tr = Poseidon2TranscriptCircuit::new(
            ...
            b"neo.fold.next/nightstream/rv32im/authoritative_side/statement",
        )?;
...
pub fn setup_rv32im_side_binding(
    statement: &Rv32imSideBindingStatement,
    public: &Rv32imSideOpeningPublic,
) -> Result<(Rv32imSideBindingProverKey, Rv32imSideBindingVerifierKey), SimpleKernelError> {
    Rv32imSideBindingSnark::setup(Rv32imSideBindingCircuit::dummy(statement, public)?)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM side binding setup failed: {err}")))
}

pub fn setup_rv32im_side_binding_cached(
    statement: &Rv32imSideBindingStatement,
    public: &Rv32imSideOpeningPublic,
) -> Result<Rv32imSideBindingKeyPair, SimpleKernelError> {
    let shape_digest = rv32im_side_binding_shape_digest(public);
    let cache = RV32IM_SIDE_BINDING_SETUP_CACHE.get_or_init(|| Mutex::new(HashMap::new()));
    if let Some(keys) = cache
        .lock()
        .map_err(|_| SimpleKernelError::Bridge("RV32IM side binding setup cache poisoned".into()))?
        .get(&shape_digest)
        .cloned()
    {
        return Ok(keys);
    ...
    let keys = Arc::new(setup_rv32im_side_binding(statement, public)?);
    cache
        .lock()
        .map_err(|_| SimpleKernelError::Bridge("RV32IM side binding setup cache poisoned".into()))?
...
pub fn prove_rv32im_side_binding(
    pk: &Rv32imSideBindingProverKey,
    statement: &Rv32imSideBindingStatement,
    public: &Rv32imSideOpeningPublic,
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<Rv32imSideBindingProof, SimpleKernelError> {
    let circuit = Rv32imSideBindingCircuit::from_claim_witnesses(statement, public, claim_witnesses)?;
    let prep = Rv32imSideBindingSnark::prep_prove(pk, circuit.clone(), true)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM side binding prepare failed: {err}")))?;
    let proof = Rv32imSideBindingSnark::prove(pk, circuit, &prep, true)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM side binding prove failed: {err}")))?;
    let snark_data = bincode::serialize(&proof)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM side binding encode failed: {err}")))?;
    Ok(Rv32imSideBindingProof { snark_data })
}

pub fn verify_rv32im_side_binding(
    vk: &Rv32imSideBindingVerifierKey,
    statement: &Rv32imSideBindingStatement,
    proof: &Rv32imSideBindingProof,
) -> Result<(), SimpleKernelError> {
    let snark: Rv32imSideBindingSnark = bincode::deserialize(&proof.snark_data)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM side binding decode failed: {err}")))?;
    let public_values = snark
        .verify(vk)
        .map_err(|err| SimpleKernelError::Bridge(format!("RV32IM side binding verify failed: {err}")))?;
...
fn verify_side_public_io(
    ...
    if expected != public_values {
        return Err(SimpleKernelError::Bridge(
            "RV32IM side binding public IO mismatch".into(),
        ));
    }
    Ok(())
}
