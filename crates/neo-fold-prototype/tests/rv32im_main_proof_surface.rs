...
use neo_fold_prototype::rv32im::final_relation::{Rv32imFinalBuildProof, Rv32imFinalStatement};
use neo_fold_prototype::rv32im::main_proof::Rv32imMainFinalProofSurface;
...
fn n2_final_pc() -> u64 {
    rv32im_n2_support::build_rv32im_n2_fixture()
        .expect("build rv32im n=2 fixture")
        .accepted_artifact
        .statement
        .final_pc
}
...
#[test]
fn rv32im_main_proof_surface_chunk_summary_chain_digest_is_stable() {
...
#[test]
fn rv32im_main_proof_surface_digest_tracks_chunk_summary_chain_digest() {
    let (final_statement, final_proof) = n2_final_case();
...
#[test]
fn rv32im_main_proof_surface_matches_last_native_f_prime_step_image() {
    let (final_statement, final_proof) = n2_final_case();
    let relations =
...
fn rv32im_main_nightstream_proof_digest_tracks_compact_compressed_main_proof() {
    let compact = rv32im_n2_support::build_rv32im_n2_published_seam()
        .expect("build one-step published seam")
        .main_proof
        .clone();
    let baseline = rv32im_main_nightstream_proof_digest(&compact);
    ...
    assert_ne!(
        baseline,
        rv32im_main_nightstream_proof_digest(&recursive_proof_tamper),
    ...
    assert_ne!(
        baseline,
        rv32im_main_nightstream_proof_digest(&final_ce_proof_tamper),
    ...
    assert_ne!(
        baseline,
        rv32im_main_nightstream_proof_digest(&statement_tamper),
...
#[test]
#[ignore = "expensive: published seam construction includes compressed IVC proof material"]
fn rv32im_accumulator_public_statement_rejects_x_last_drift() {
    let mut compact = rv32im_n2_support::build_rv32im_n2_published_seam()
        .expect("build one-step published seam")
        .main_proof
        .clone();
    compact
