...
Phi81 quotient coefficients. Each invocation contributes 1 general-field
quotient value in invocation-major order.
...
def sourceWidth (baseSourceWidth invocationCount : Nat) : Nat :=
  baseSourceWidth + invocationCount * 1
...
def groupColumn (baseSourceWidth invocationCount : Nat)
    (invocation : Fin invocationCount) (group : Fin 1) :
    Fin (sourceWidth baseSourceWidth invocationCount) :=
...
def block (baseSourceWidth invocationCount : Nat) :
  ...
  kind := .field
  slotCount := invocationCount * 1
  source := fun slot =>
    let decoded : Fin invocationCount × Fin 1 := Fin.decodeProd slot
...
@[simp] theorem block_slotCount (baseSourceWidth invocationCount : Nat) :
    (block baseSourceWidth invocationCount).slotCount = invocationCount * 1 := by
  rfl
