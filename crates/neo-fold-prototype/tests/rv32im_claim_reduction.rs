...
use neo_fold_prototype::rv32im::kernel::{
    ...
    OpenedAjtaiObjectId, QuadraticRoundPoly, Rv32imAcceptedProofArtifact, Rv32imEvalClaimBundle,
...
fn eval_claim_bundle() -> &'static Rv32imEvalClaimBundle {
    static BUNDLE: OnceLock<Rv32imEvalClaimBundle> = OnceLock::new();
    BUNDLE.get_or_init(|| {
        build_rv32im_eval_claim_bundle_from_accepted_artifact(artifact())
...
fn phase0_binding_surface() -> &'static Rv32imPhase0BindingSurface {
...
fn claim_reduction_bucket_builder_rejects_mixed_point_arities_within_schema() {
    ...
    let bundle = Rv32imEvalClaimBundle::new(claims).expect("bundle should still canonicalize");
