//! Owns the verifier-facing Phase 0 RV32IM evaluation-claim surface and its canonical ordering.
//!
//! It owns:
...
impl OpenedAjtaiObjectId {
    ...
    pub fn expected_digest(&self, commitment_context: &CommitmentContextId) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase0/opened_object");
...
impl FamilyEvalClaim {
    ...
    pub fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase0/claim");
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase0/claim/opened_object_digest",
            &self.opened_object.digest,
        );
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase0/claim/pp_seed_digest",
            &self.commitment_context.pp_seed_digest,
        );
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase0/claim/module_shape_digest",
            &self.commitment_context.module_shape_digest,
        );
        tr.append_u64s(
            b"neo.fold.next/rv32im/opening_convergence/phase0/claim/meta",
            &[self.payload.schema.tag(), self.id.slot as u64],
        );
        append_k_vec(
            &mut tr,
            b"neo.fold.next/rv32im/opening_convergence/phase0/claim/point",
            &self.point,
        );
        append_packed_column_evals(
            &mut tr,
            b"neo.fold.next/rv32im/opening_convergence/phase0/claim/payload",
            &self.payload.column_evals,
        );
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase0/claim/binding_digest",
            &self.binding_digest,
        );
        tr.digest32()
    }
...
impl OpeningClaimAccumulator {
    ...
    pub fn into_bundle(mut self) -> Rv32imEvalClaimBundle {
        self.claims.sort_by(canonical_claim_cmp);
        Rv32imEvalClaimBundle::from_canonical_claims(self.claims)
    }
}

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct Rv32imEvalClaimBundle {
    pub claims: Vec<FamilyEvalClaim>,
    pub digest: [u8; 32],
}

impl Rv32imEvalClaimBundle {
    ...
    pub fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase0/bundle");
        tr.append_u64s(
            b"neo.fold.next/rv32im/opening_convergence/phase0/bundle/claim_count",
            &[self.claims.len() as u64],
        );
        for claim in &self.claims {
            tr.append_message(
                b"neo.fold.next/rv32im/opening_convergence/phase0/bundle/claim_digest",
...
fn append_k_vec(tr: &mut Poseidon2Transcript, label: &'static [u8], values: &[K]) {
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase0/k_vec_len",
        &[values.len() as u64],
    );
...
fn append_packed_column_evals(tr: &mut Poseidon2Transcript, label: &'static [u8], values: &[PackedColumnEval]) {
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase0/column_eval_count",
        &[values.len() as u64],
    );
    tr.append_fields_iter(
        label,
