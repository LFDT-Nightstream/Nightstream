...
use neo_fold_legacy::frontends::f_prime::FPrimeImageLayout;
...
fn audit_verifier_rejects_malformed_final_witness_without_panicking() {
    ...
    let audit =
        neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 0)]]).expect("one-step audit proof");
    let mut finalized = neo_fold_legacy::finish_uncompressed_with_audit(&prep, audit).expect("finalized audit proof");
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        neo_fold_legacy::verify_uncompressed_audit(&prep, &finalized)
    }));
...
fn terminal_verifier_rejects_m_in_x_column_mismatch_without_panicking() {
    let prep = support::toy_preprocessing();
    let audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 30)]]).expect("one-step proof");
    let mut finished = neo_fold_legacy::finish_uncompressed(&prep, audit).expect("finalized proof");
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        neo_fold_legacy::verify_uncompressed(&prep, &finished)
    }));
...
fn lifecycle_rejects_mixed_public_input_relations_without_panicking() {
    ...
    let narrow = neo_fold_legacy::CcsInstance::from_low_norm_assignment(
        prep.params(),
    ...
    let audit = neo_fold_legacy::prove(&prep, [vec![wide, narrow]])
        .expect("the first lifecycle step currently accepts the mixed batch");
    let result = catch_unwind(AssertUnwindSafe(|| neo_fold_legacy::finish_uncompressed(&prep, audit)));
