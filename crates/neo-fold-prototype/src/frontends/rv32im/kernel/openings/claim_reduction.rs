...
use crate::multilinear::{build_eq_table, eq_eval_le};
...
use super::opening_eval_claims::{
    ...
    PackedColumnEval, Rv32imEvalClaimBundle,
};
...
impl ClaimReductionBucket {
    ...
    pub fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase1/bucket");
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase1/bucket/pp_seed_digest",
            &self.commitment_context.pp_seed_digest,
        );
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase1/bucket/module_shape_digest",
            &self.commitment_context.module_shape_digest,
        );
        tr.append_u64s(
            b"neo.fold.next/rv32im/opening_convergence/phase1/bucket/meta",
            &[
                self.schema.tag(),
        ...
        for claim in &self.claims {
            tr.append_message(
                b"neo.fold.next/rv32im/opening_convergence/phase1/bucket/claim_digest",
...
impl ClaimReductionProof {
    pub fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase1/proof");
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase1/proof/bucket_digest",
            &self.bucket_digest,
        );
        append_k(
            &mut tr,
            b"neo.fold.next/rv32im/opening_convergence/phase1/proof/eta",
            &self.eta,
        );
        append_k(
            &mut tr,
            b"neo.fold.next/rv32im/opening_convergence/phase1/proof/gamma",
            &self.gamma.unwrap_or(K::ZERO),
        );
        append_k(
            &mut tr,
            b"neo.fold.next/rv32im/opening_convergence/phase1/proof/rho",
            &self.rho,
        );
        tr.append_u64s(
            b"neo.fold.next/rv32im/opening_convergence/phase1/proof/round_poly_count",
            &[self.round_polys.len() as u64],
        );
        for poly in &self.round_polys {
            append_quadratic_round_poly(
                &mut tr,
                b"neo.fold.next/rv32im/opening_convergence/phase1/proof/round_poly",
                poly,
            );
        }
        append_k_vec(
            &mut tr,
            b"neo.fold.next/rv32im/opening_convergence/phase1/proof/scalar_evals_at_r_star",
...
fn claim_reduction_result_digest_from_parts(
    ...
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase1/result");
    tr.append_message(
        b"neo.fold.next/rv32im/opening_convergence/phase1/result/bucket_digest",
        &bucket_digest,
    );
    append_k(
        &mut tr,
        b"neo.fold.next/rv32im/opening_convergence/phase1/result/gamma",
        &gamma.unwrap_or(K::ZERO),
    );
    append_k(
        &mut tr,
        b"neo.fold.next/rv32im/opening_convergence/phase1/result/rho",
        &rho,
    );
    append_k_vec(
        &mut tr,
        b"neo.fold.next/rv32im/opening_convergence/phase1/result/unified_point",
        unified_point,
    );
    tr.append_message(
        b"neo.fold.next/rv32im/opening_convergence/phase1/result/proof_digest",
        &proof_digest,
    );
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase1/result/source_claim_count",
        &[source_claims.len() as u64],
    );
    for claim in source_claims {
        append_claim_id(
            &mut tr,
            b"neo.fold.next/rv32im/opening_convergence/phase1/result/source_claim_id",
            &claim.id,
        );
    }
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase1/result/unified_claim_count",
        &[unified_claims.len() as u64],
    );
    for claim in unified_claims {
        tr.append_message(
            b"neo.fold.next/rv32im/opening_convergence/phase1/result/unified_claim_digest",
...
pub fn build_claim_reduction_results_from_witnesses(
    ...
    let bundle = Rv32imEvalClaimBundle::new(claim_witnesses.iter().map(|w| w.claim.clone()).collect())
...
pub(crate) fn validate_claim_reduction_result_with_witnesses(
    ...
    let eta = sample_k(&mut transcript, b"neo.fold.next/rv32im/opening_convergence/phase1/eta");
    ...
    let gamma = if result.bucket.payload_width() > 1 {
        let gamma = sample_k(
            &mut transcript,
            b"neo.fold.next/rv32im/opening_convergence/phase1/gamma",
        );
    ...
    let rho = sample_k(&mut transcript, b"neo.fold.next/rv32im/opening_convergence/phase1/rho");
    ...
    append_k_vec(
        &mut transcript,
        b"neo.fold.next/rv32im/opening_convergence/phase1/scalar_evals_at_r_star",
...
pub(crate) fn validate_claim_reduction_results_with_witnesses(
    ...
    let expected_buckets = build_claim_reduction_buckets(
        &Rv32imEvalClaimBundle::new(claim_witnesses.iter().map(|w| w.claim.clone()).collect())
            .map_err(ClaimReductionError::WitnessClaimBundleBuildFailed)?,
    )?;
...
pub fn phase1_claim_digest(claim: &FamilyEvalClaim) -> [u8; 32] {
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase1/claim");
    append_phase1_claim_body(&mut tr, claim);
    tr.digest32()
}

pub fn phase1_unified_claim_digest(claim: &FamilyEvalClaim) -> [u8; 32] {
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase1/unified_claim");
    append_phase1_claim_body(&mut tr, claim);
    tr.digest32()
}
...
fn phase1_transcript(bucket_digest: [u8; 32]) -> Poseidon2Transcript {
    let mut transcript = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase1");
    transcript.append_message(
        b"neo.fold.next/rv32im/opening_convergence/phase1/bucket_digest",
        &bucket_digest,
    );
    transcript
}

fn validate_phase0_binding_surface(surface: &Rv32imPhase0BindingSurface) -> Result<(), ClaimReductionError> {
...
fn verify_phase0_claim_bindings_against_surface(
    claim_witnesses: &[FamilyEvalClaimWitness],
    surface: &Rv32imPhase0BindingSurface,
) -> Result<(), ClaimReductionError> {
    for claim_witness in claim_witnesses {
...
fn run_phase1_sumcheck_prover(
    ...
    for round in 0..oracle.rounds {
        ...
        append_quadratic_round_poly(
            transcript,
            b"neo.fold.next/rv32im/opening_convergence/phase1/round_poly",
            &round_poly,
        );
        let challenge = sample_k(
            transcript,
            b"neo.fold.next/rv32im/opening_convergence/phase1/r_star_round",
        );
...
fn append_phase1_claim_body(tr: &mut Poseidon2Transcript, claim: &FamilyEvalClaim) {
    tr.append_message(
        b"neo.fold.next/rv32im/opening_convergence/phase1/claim/opened_object_digest",
        &claim.opened_object.digest,
    );
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase1/claim/meta",
        &[claim.id.slot as u64, claim.payload.schema.tag()],
    );
    append_k_vec(
        tr,
        b"neo.fold.next/rv32im/opening_convergence/phase1/claim/point",
        &claim.point,
    );
    append_packed_column_evals(
        tr,
        b"neo.fold.next/rv32im/opening_convergence/phase1/claim/payload",
        &claim.payload.column_evals,
    );
    tr.append_message(
        b"neo.fold.next/rv32im/opening_convergence/phase1/claim/binding_digest",
        &claim.binding_digest,
    );
}
...
fn append_k_vec(tr: &mut Poseidon2Transcript, label: &'static [u8], values: &[K]) {
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase1/k_vec_len",
        &[values.len() as u64],
    );
...
fn append_packed_column_evals(tr: &mut Poseidon2Transcript, label: &'static [u8], values: &[PackedColumnEval]) {
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase1/column_eval_count",
        &[values.len() as u64],
    );
    tr.append_fields_iter(
        label,
...
fn append_claim_id(tr: &mut Poseidon2Transcript, label: &'static [u8], id: &FamilyEvalClaimId) {
    ...
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase1/claim_id/slot",
        &[id.slot as u64],
    );
}
