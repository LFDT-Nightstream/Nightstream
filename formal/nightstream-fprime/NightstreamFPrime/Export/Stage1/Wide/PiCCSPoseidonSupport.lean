...
theorem piCcs_payload (program : Program)
    ...
    (index : Fin PiCCSActionPayloadBlock.payloadCount) : CommonForm program (PiCCSPayloadWiring.form geometry index) := by
...
theorem piCcs_sbox (program : Program)
    ...
    CommonForm program ((PiCCSPoseidonPlan.retainedBlock program).form
...
theorem piCcs_previous (program : Program)
    ...
    CommonForm program (PiCCSPoseidonPlan.previousOutput geometry invocation lane) := by
  unfold PiCCSPoseidonPlan.previousOutput
  split
  · exact empty _
  · exact InputSupport.piCcsOutput program geometry _ lane
...
theorem piCcs_poseidon (program : Program)
    ...
    (geometry : PiCCSPoseidonPlan.Geometry program (PerApplicationFixedPoint.logicalWidth program)) :
    CommonPlans program (PiCCSPoseidonPlan.plan (PiCCSPayloadWiring.form ordinary) geometry) := by
  have payload (invocation : Fin PiCCSPoseidonPlan.invocationCount) (lane : Fin 8) :
      CommonForm program (PiCCSPoseidonPlan.payloadForm (PiCCSPayloadWiring.form ordinary) invocation lane) := by
  ...
  · apply pin_plan
    ...
    · intro row
      ...
      · apply add (payload _ _)
        ...
        · exact InputSupport.piCcsOutput program geometry _ _
      · exact empty _
      · exact empty _
...
theorem piCcs_pins (program : Program)
    (ordinary : PiCCSOrdinaryRetainedGeometry.Geometry program (PerApplicationFixedPoint.logicalWidth program))
    (geometry : PiCCSPoseidonPlan.Geometry program (PerApplicationFixedPoint.logicalWidth program)) :
    CommonPlans program (PiCCSTranscriptEndpointPlan.plan geometry ordinary) := by
  apply pin_plan
  · exact one_common program _ rfl
  · intro row
    apply add (InputSupport.piCcsOutput program geometry _ _)
    apply scale
    unfold PiCCSTranscriptEndpointPlan.sourceForm
    split
    · exact piCcs_endpoint_value program ordinary _
    · exact InputSupport.location program ordinary _
