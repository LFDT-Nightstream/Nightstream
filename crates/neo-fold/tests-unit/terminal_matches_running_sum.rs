...
// use neo_fold::pi_ccs::pi_ccs_derive_transcript_tail_with_me_inputs_and_label; // unused after switching to bound-transcript helper
...
fn terminal_rhs_matches_running_sum() {
    ...
    let _ = sample_challenges(&mut tr_v, dims.ell_d, dims.ell).unwrap();

    // Use prover-carried sum-check challenges directly (authoritative r, α').
    let (r_prime, alpha_prime) = proof.sumcheck_challenges.split_at(dims.ell_n);
    let rhs = rhs_Q_apr(&s, &proof.challenges_public, r_prime, alpha_prime, &[mcs1], &[me_input], &out_me, &params).unwrap();
    assert_eq!(proof.sumcheck_final, rhs, "Final running sum must equal Q(α', r') computed from outputs.");
}
