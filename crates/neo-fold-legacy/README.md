...
Deprecated for new application work. Use [`nightstream`](../nightstream/README.md).
This crate retains the previous implementation for existing consumers, manual
comparisons, and reference. Its tests are not part of the Rust CI test job.

`neo-fold-legacy` owns the public folding lifecycle, SuperNeo NIFS composition,
...
```sh
RUSTC_WRAPPER="" timeout 300s cargo test -p neo-fold-legacy --release --test nifs_round_trip
RUSTC_WRAPPER="" timeout 300s cargo test -p neo-fold-legacy --release --test f_prime_package_production
RUSTC_WRAPPER="" timeout 300s cargo test -p neo-fold-legacy --release --test system_preprocessing_cache_hygiene
```
