...
The goal is to prove the protocol rules and their implementation links, check
Rust against those rules, and support later reductions in constraint count.
...
Standard cryptographic security results can remain explicit assumptions.
...
parameters. Per-call bounds conditional on the full preceding history give
an aggregate bound `min(1, sum of the per-call bounds)`; independence is not
required. Sampler abort probability, decoder distribution error, verifier
test error, and extraction loss are different terms. A union bound for one
...
cvc5 can propose removals, column reuse, and counterexamples. Lean must prove
that the changed constraints still imply the required relation and that
valid inputs retain a satisfying witness. Removing rows alone preserves
completeness for the same witness layout; changing columns or witness
construction needs its own preservation proof. An unchecked `UNSAT` result
does not authorize a change.

Rust conformance uses the same selected relation, key, parameters,
authenticated input, transcript, proof bytes, and complete phase outputs.
