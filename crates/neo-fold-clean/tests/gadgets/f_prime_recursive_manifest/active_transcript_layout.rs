...
open Nightstream.Implementation.R1CS.FPrimeRecursivePiRlcChallengeTranscriptLayoutSchema\n\n",
    ...
    let pin_split = artifact.constant_pins.len().div_ceil(2);
    for (index, pin) in artifact.constant_pins.iter().enumerate() {
        let local_index = if index < pin_split { index } else { index - pin_split };
        if index == pin_split {
            out.push_str("\n  ] ++\n    [ ");
    ...
    let call_split = artifact.calls.len().div_ceil(2);
    ...
    let emission_split = artifact.emission_order.len().div_ceil(2);
    for (index, emission) in artifact.emission_order.iter().enumerate() {
        let local_index = if index < emission_split {
            index
        } else {
            index - emission_split
        };
        if index == emission_split {
            out.push_str(" ] ++\n    [ ");
        } else if local_index != 0 {
            if local_index % 8 == 0 {
                out.push_str(",\n    ");
    ...
    let continuity_split = artifact.state_continuity.len().div_ceil(2);
    for (index, continuity) in artifact.state_continuity.iter().enumerate() {
        let local_index = if index < continuity_split {
            index
        } else {
            index - continuity_split
        };
        if index == continuity_split {
            out.push_str("\n  ] ++\n    [ ");
        } else if local_index != 0 {
            if local_index % 2 == 0 {
                out.push_str("\n  , ");
    ...
    let alias_split = artifact.field_output_aliases.len().div_ceil(2);
    for (index, alias) in artifact.field_output_aliases.iter().enumerate() {
        if index == alias_split {
            out.push_str("  ] ++\n    [ ");
        } else if index != 0 {
            out.push_str("  , ");
        }
