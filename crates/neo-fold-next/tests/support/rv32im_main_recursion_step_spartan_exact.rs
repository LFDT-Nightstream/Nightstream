...
pub fn assert_backend_relation_exact_surface_contract(
    ...
    debug_check_rv32im_chunk_step_recursive_effective_chunk_trace_matches_native(relation).unwrap_or_else(|err| {
        panic!("{label}: exact-step payload should reconstruct the native chunk replay trace: {err}")
    });
    debug_check_rv32im_main_recursion_step_spartan_live_claim_me_digest_parity(relation).unwrap_or_else(|err| {
        panic!("{label}: live carried claims should hash to the authoritative native ME digests: {err}")
    });
    debug_check_rv32im_main_recursion_x_out_gadget_parity(relation)
...
pub fn single_relation_backend_fixture() -> (
    ...
    FIXTURE
        .get_or_init(|| {
            ...
            build_rv32im_main_recursion_f_prime_backend_relations_with_spartan_shape(&relations[..1])
