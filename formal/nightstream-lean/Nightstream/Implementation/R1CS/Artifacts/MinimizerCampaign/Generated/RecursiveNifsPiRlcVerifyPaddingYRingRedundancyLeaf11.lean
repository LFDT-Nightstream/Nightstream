...
namespace Nightstream.Implementation.R1CS.Artifacts.MinimizerCampaign.Generated.RecursiveNifsPiRlcVerifyPaddingYRingRedundancyLeaf11
...
theorem candLeaf57 :
    (rowsChunk wire 57).filter
        (fun row => decide (row.family = certFamily)) =
      (certParts 57).map (fun scalar => scalar.candidate) := by
  native_decide
...
theorem scalarsGroup :
    ∀ k, 57 ≤ k → k < 58 → ∀ scalar ∈ certParts k,
      scalar.Valid ∧
  ...
  · subst is57
    intro scalar member
    rw [show certParts 57 = [] from rfl] at member
    cases member
  exact absurd upper (by omega)


end Nightstream.Implementation.R1CS.Artifacts.MinimizerCampaign.Generated.RecursiveNifsPiRlcVerifyPaddingYRingRedundancyLeaf11
