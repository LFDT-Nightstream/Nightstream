...
use neo_fold_legacy::lifecycle::preprocess_with_test_log;
use neo_fold_legacy::{config, finish_uncompressed, preprocess, prove, verify_uncompressed, CcsInstance};
use neo_math::{Rq, D, F};
...
fn verifier_key_distinguishes_unbounded_from_fixed_zero_public_input_arity() {
    ...
    let same_boundary = neo_fold_legacy::paper::digest::initial_boundary_digest(unbounded.structure_digest(), None)
        == neo_fold_legacy::paper::digest::initial_boundary_digest(fixed_zero.structure_digest(), Some(0));
    assert!(
...
fn preprocessing_rejects_public_input_arity_above_structure_width() {
    ...
    assert!(matches!(
        ...
        Err(neo_fold_legacy::Error::PreprocessingPublicInputTooLarge { m_in: usize::MAX, m: D })
    ));
}
...
fn ccs_parameter_selection_rejects_unrepresentable_safety_margin() {
    let result = neo_fold_legacy::Params::for_ccs_shape_with(1, 1, 1, 0, 100, u32::MAX);
    assert!(
        result.is_err(),
