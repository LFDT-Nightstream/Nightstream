...
use neo_fold_legacy::paper::construction2::{FoldProof, ProofState};
use neo_fold_legacy::{Error, RunningInstance, Uncompressed};
use neo_math::{F, K};
...
fn one_batch_finished() -> (neo_fold_legacy::Preprocessing, Uncompressed) {
    let prep = support::toy_preprocessing();
    let audit = neo_fold_legacy::prove(&prep, vec![vec![support::toy_instance(&prep, 700)]]).expect("one-batch proof");
    let finished = neo_fold_legacy::finish_uncompressed(&prep, audit).expect("terminal fold");
    (prep, finished)
}
...
fn terminal_ce_native_success_and_each_authority_rejection_are_live() {
    ...
    neo_fold_legacy::verify_uncompressed(&prep, &finished).expect("honest terminal proof verifies");
    ...
    assert!(matches!(
        neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &bad_commitment),
    ...
    assert!(matches!(
        neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &bad_projection),
    ...
    assert!(matches!(
        neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &bad_norm),
    ...
    assert!(matches!(
        neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &bad_point),
    ...
    assert!(matches!(
        neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &bad_eval_k),
    ...
    assert!(matches!(
        neo_fold_legacy::lifecycle::validate_final_witness_authority(&prep, &bad_eval_a),
        Err(Error::FinalAccumulatorCeRelationViolation { index: 0, .. })
    ));
    ...
    assert!(
        neo_fold_legacy::verify_uncompressed(&prep, &disconnected_child).is_err(),
...
fn lifecycle_replay_exercises_fprime_success_and_rejection_paths() {
    ...
    let audit = neo_fold_legacy::prove(&prep, batches).expect("two-step F' audit");
    let finished = neo_fold_legacy::finish_uncompressed_with_audit(&prep, audit).expect("terminal fold");
    neo_fold_legacy::verify_uncompressed_audit(&prep, &finished).expect("honest lifecycle replay");

    let mut wrong_variant = finished.clone();
    wrong_variant.steps[1].fold = FoldProof::NoFold;
    assert!(
        neo_fold_legacy::verify_uncompressed_audit(&prep, &wrong_variant).is_err(),
    ...
    assert!(
        neo_fold_legacy::verify_uncompressed_audit(&prep, &empty_step).is_err(),
    ...
    assert!(
        neo_fold_legacy::verify_uncompressed_audit(&prep, &semantic_forgery).is_err(),
