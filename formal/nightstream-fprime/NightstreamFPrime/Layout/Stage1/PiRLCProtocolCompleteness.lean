...
private theorem protocol_readback
    (env : Env)
    (agrees : ∀ index, index < PiCCSInputs.phaseOffset → env index =
  ...
  have preserved := input_readback relation env _ (relationProof relation values template) agrees
  exact ⟨preserved.1.trans (loaded.1.trans original.1),
    preserved.2.1.trans (loaded.2.1.trans original.2.1),
    preserved.2.2.trans (loaded.2.2.trans original.2.2)⟩
...
theorem completePrefix
  ...
  obtain ⟨c, cOperations, cPhase⟩ := PiCCSProtocolCompleteness.completePrefix
    prior priorPublic output digest priorFixed outputFixed digestFixed values context relation
    ajtai template priorPc outputPc priorContext outputContext accepted
  have cRead := protocol_readback relation prior priorPublic output digest priorFixed outputFixed
    digestFixed values context template c.current (fun index below => c.agrees index (Or.inl below))
  ...
  have finalRead := protocol_readback relation prior priorPublic output digest priorFixed outputFixed
    digestFixed values context template r.current (fun index below =>
