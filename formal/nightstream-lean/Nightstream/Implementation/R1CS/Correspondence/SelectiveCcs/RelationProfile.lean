...
namespace Nightstream.Implementation.R1CS.SelectiveCcs.RelationProfile
...
structure FiniteRelation (rows columns : Nat) where
  matrices :
    Nightstream.Implementation.R1CS.SelectiveCcs.Polynomial.Ports.Role ->
...
def shape {rows columns : Nat} (profile : Profile rows columns) :
  ...
  publicRingColumns := profile.publicRingColumns
...
end Nightstream.Implementation.R1CS.SelectiveCcs.RelationProfile
