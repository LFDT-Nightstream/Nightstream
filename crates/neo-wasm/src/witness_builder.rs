use super::ccs::{
    ...
    LOCAL_ZERO_REMAINING_AFTER_ZERO_TEST, PARAM_INIT_REMAINING_AFTER_ZERO_TEST, PC_EDGE_KIND_ZERO_TEST,
    PERM_ROUND_ZERO_TEST, SELECT_COND_ZERO_TEST, SELECT_HI_MUX, SELECT_LO_MUX,
};
...
use super::layout::{
    ...
    COL_CI_ENTRY_IS_NULL, COL_CI_HOST_CALL, COL_CI_OOB, COL_CI_TYPE_EQ, COL_CMP_GE, COL_CMP_LOW, COL_COMM_CHAIN_AFTER,
    COL_COMM_CHAIN_BEFORE, COL_CONTROL_CHOICE, COL_CURRENT_FUNCTION_NUM_LOCALS, COL_CURRENT_FUNCTION_REF,
    COL_DIV_DIVIDEND_IS_MIN, COL_DIV_DIVISOR_IS_NEG1, COL_DIV_DIVISOR_IS_ZERO, COL_DIV_OVERFLOW, COL_DIV_OVERFLOW_COND,
    COL_DIV_TRAP, COL_ENTERED_FUNCTION_NUM_LOCALS, COL_EXPECTED_TYPE_ID, COL_FUNCTION_CALL_TYPE_LOOKUP_GATE,
    COL_FUNCTION_REF, COL_FUNCTION_TYPE_ID, COL_GLOBAL_INDEX, COL_GLOBAL_VALUE, COL_GLOBAL_VALUE_HI, COL_GROW_SUCCESS,
    ...
    COL_LOCAL_VALUE_HI, COL_LOCAL_WRITE_ENABLED, COL_LOCAL_ZERO_ACTIVE_AFTER, COL_LOCAL_ZERO_ACTIVE_BEFORE,
    COL_LOCAL_ZERO_REMAINING_AFTER, COL_LOCAL_ZERO_REMAINING_BEFORE, COL_MAX_MEMORY_PAGES_AFTER,
...
pub fn build_witness_vector(trace: &WasmVmStep) -> Vec<F> {
    ...
    write_local_zero_state(&mut wit, true, trace.state_before.local_zero);
    write_local_zero_state(&mut wit, false, trace.state_after.local_zero);
    LOCAL_ZERO_REMAINING_AFTER_ZERO_TEST.assign(&mut wit);
    ...
    if trace.target_function_is_guest || trace.row_kind.is_turn_boundary() {
        // This row still belongs to the outgoing frame. Reconstruct the
        // entered frame's count; its ROM read binds the claim to the callee.
        wit[COL_ENTERED_FUNCTION_NUM_LOCALS] = F::from_u64(
            u64::from(trace.state_after.local_zero.remaining) + u64::from(trace.call_param_count.unwrap_or(0)),
        );
    }
    ...
    DIVISOR_ZERO_TEST.assign(&mut wit);
    let divisor_is_zero = wit[COL_DIV_DIVISOR_IS_ZERO];
    ...
    dividend_min_zero_test().assign(&mut wit);
    let dividend_is_min = wit[COL_DIV_DIVIDEND_IS_MIN];
    divisor_neg1_zero_test().assign(&mut wit);
    let divisor_is_neg1 = wit[COL_DIV_DIVISOR_IS_NEG1];
    ...
    ) || trace.row_kind.is_call_param_init() || trace.row_kind.is_local_zero()
    ...
    CALL_INDIRECT_TYPE_ZERO_TEST.assign(&mut wit);
    let type_eq = wit[COL_CI_TYPE_EQ];
...
fn write_local_zero_state(wit: &mut [F], before: bool, state: super::ir::WasmCountdownState) {
    let (active_col, remaining_col) = if before {
        (COL_LOCAL_ZERO_ACTIVE_BEFORE, COL_LOCAL_ZERO_REMAINING_BEFORE)
    } else {
        (COL_LOCAL_ZERO_ACTIVE_AFTER, COL_LOCAL_ZERO_REMAINING_AFTER)
    };
