...
/-!
Contract: exact typed-call placement of the next binding-hash occurrence.
...
namespace Poseidon23HashNextFrame
...
private theorem temporary_layouts_exact
    (parameters : Parameters)
    (profile : Poseidon23ApplicationProfile parameters) :
    ((signature parameters).callFootprint Call.hashNext).temporaries =
...
private theorem output_length
  ...
  have width :=
    frame.outputWidthsAgree (Ports.publicDigest parameters) (by
      change Ports.publicDigest parameters ∈
        callOutputs parameters Call.hashNext
      exact List.mem_cons_self)
  have exact :
      (Ports.publicDigest parameters).layout.owners.length =
...
/-- Exact next-hash physical occurrence derived from its typed call frame. -/
def occurrence
    (parameters : Parameters)
    (profile : Poseidon23ApplicationProfile parameters)
    {context : Schema (typeSystem parameters)}
  ...
  Poseidon23HashCallCommon.occurrence frame.owner frame.one frame.active true
...
def coreFacts
  ...
  apply coreAllocationFacts frame.owner frame.one frame.active true
    (iterationColumn parameters profile frame)
    (sourceTail parameters profile frame)
    (sourceTail_length parameters profile frame)
    (unaryOutput frame.outputs).columns
