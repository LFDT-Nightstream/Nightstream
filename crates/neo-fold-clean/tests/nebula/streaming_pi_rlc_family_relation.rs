...
use neo_fold_clean::frontends::r1cs_f_prime::{
    SelectiveEmittedRowFamily, SelectiveProjectedDecoderRunProvenance, SelectiveProjectedSourceDecoderRun,
    SelectiveProjectedSourceDecoderStridedRun, SelectiveProjectedSourceDecoderTemplateInstances,
...
fn lean_resolution(resolution: SelectiveProjectedSourceResolutionRun) -> String {
    match resolution {
...
fn write_runs(rendered: &mut String, name: &str, runs: &[SelectiveProjectedSourceDecoderRun]) {
    writeln!(rendered, "def {name} : List RawRun :=\n  [").expect("render decoder run header");
...
open Nightstream.Implementation.R1CS.Artifacts.FPrimeFullHistory.StreamingPiRLCFamilyBodyDecoderSchema\n"
    ...
    for (label, decoder) in [("even", even), ("odd", odd)] {
        ...
        for (index, template) in decoder.repeated_templates().iter().enumerate() {
            ...
            writeln!(
                rendered,
                "{separator}{{ sourceWidth := {}, relativeRuns := templateRules{index:02}, instances := {label}TemplateInstances{index:02} }}",
