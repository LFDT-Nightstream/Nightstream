...
pub fn enforce_dec_public<CS: ConstraintSystem<SpartanF>>(
    ...
    enforce_scalar_power_sum_on_canonical_embedded_x(
        cs,
        &parent.x,
        &children
            .iter()
...
pub fn enforce_dec_public_with_constant_children<CS: ConstraintSystem<SpartanF>>(
    ...
    enforce_scalar_power_sum_on_canonical_embedded_x_constant_children(
        cs,
        &parent.x,
        &parent.x_values,
        &children
...
fn enforce_scalar_power_sum_on_canonical_embedded_x<CS: ConstraintSystem<SpartanF>>(
    cs: &mut CS,
    parent: &[AllocatedNum<SpartanF>],
    children: &[Vec<AllocatedNum<SpartanF>>],
    base_b: u32,
    ...
    if children.is_empty() || parent.is_empty() || !parent.len().is_multiple_of(D) {
        return Err(SynthesisError::Unsatisfiable);
    }
    let cols = parent.len() / D;
    for child in children {
        if child.len() != parent.len() {
            return Err(SynthesisError::Unsatisfiable);
        }
...
fn enforce_scalar_power_sum_on_canonical_embedded_x_constant_children<CS: ConstraintSystem<SpartanF>>(
    cs: &mut CS,
    parent: &[AllocatedNum<SpartanF>],
    parent_values: &[F],
    children: &[Vec<F>],
    ...
    if children.is_empty()
        ...
        || !parent_values.len().is_multiple_of(D)
        || (!parent.is_empty() && parent.len() != parent_values.len())
    {
        return Err(SynthesisError::Unsatisfiable);
    }
    let cols = parent_values.len() / D;
    for child in children {
        if child.len() != parent_values.len() {
            return Err(SynthesisError::Unsatisfiable);
        }
