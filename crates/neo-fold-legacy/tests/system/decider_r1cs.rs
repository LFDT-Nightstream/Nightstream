...
use neo_fold_legacy::engine::decider::{
    __test_isolation::{
...
use neo_fold_legacy::paper::construction2::{self, EncInst, ProofState, State, TRIVIAL_PC};
use neo_fold_legacy::paper::decider::{self, PublicImage};
...
fn compute_x_out_native(prep: &neo_fold_legacy::Preprocessing, state: &State) -> [F; 4] {
    let mode = match prep.semantic_state_mode() {
        neo_fold_legacy::paper::construction2::SemanticStateMode::Stateless => StateXOutDigestMode::Stateless,
        neo_fold_legacy::paper::construction2::SemanticStateMode::Stateful => StateXOutDigestMode::Stateful,
    };
...
fn public_image_pin_fixture(prep: &neo_fold_legacy::Preprocessing) -> PublicImage {
...
fn refresh_public_image_x_out(prep: &neo_fold_legacy::Preprocessing, public: &mut PublicImage) {
    let mode = match prep.semantic_state_mode() {
        neo_fold_legacy::paper::construction2::SemanticStateMode::Stateless => StateXOutDigestMode::Stateless,
        neo_fold_legacy::paper::construction2::SemanticStateMode::Stateful => StateXOutDigestMode::Stateful,
    };
...
fn base_state(prep: &neo_fold_legacy::Preprocessing) -> State {
...
fn build_link_instance(prep: &neo_fold_legacy::Preprocessing, r1cs: &R1cs, x_out_target: [F; 4]) -> CcsInstance {
...
fn peek_next_state(prep: &neo_fold_legacy::Preprocessing, state: &State, batch: &[CcsInstance]) -> State {
...
/// step.
fn build_honest_finished_proof(len: usize) -> (neo_fold_legacy::Preprocessing, neo_fold_legacy::UncompressedAudit) {
    assert!(len >= 1);
...
fn build_honest_finished_proof_with_prep(
    ...
    r1cs: &R1cs,
    len: usize,
) -> (neo_fold_legacy::Preprocessing, neo_fold_legacy::UncompressedAudit) {
    assert!(len >= 1);
    ...
    let mut public_batches: Vec<Vec<neo_fold_legacy::paper::relations::CcsClaim>> = Vec::with_capacity(len);
    ...
    let in_flight = neo_fold_legacy::UncompressedAudit {
    ...
    let finished = neo_fold_legacy::finish_uncompressed_with_audit(&prep, in_flight).expect("finish");
    (prep, finished)
}
...
fn build_honest_finished_proof_with_sizes(
    ...
    let mut public_batches: Vec<Vec<neo_fold_legacy::paper::relations::CcsClaim>> = Vec::with_capacity(sizes.len());
...
fn decider_r1cs_honors_explicit_verifier_owned_ajtai_setup() {
    ...
    let prep = neo_fold_legacy::lifecycle::preprocess_with_test_log(params, structure, owned_log, public_input_len)
    ...
    neo_fold_legacy::verify_uncompressed_audit(&prep, &finished).expect("owned-log audit must pass native verification");
    let statement = neo_fold_legacy::build_decider_statement(&prep, &finished);
...
fn decider_r1cs_synthesis_rejects_tampered_step_proof() {
    ...
    let recursive_idx = statement
        ...
        .steps
        .iter()
        .position(|s| matches!(s.fold, neo_fold_legacy::paper::construction2::FoldProof::Recursive(_)))
        .expect("at least one recursive step");
    if let neo_fold_legacy::paper::construction2::FoldProof::Recursive(ref mut nifs) =
    ...
    assert!(
        matches!(err, neo_fold_legacy::paper::decider::Error::WalkFailed(_)),
...
fn decider_base_state_pins_reject_tampered_seed_wires() {
    ...
    let cases: [(
        &str,
        fn(&neo_fold_legacy::engine::decider::__test_isolation::BaseStateProbeWires) -> usize,
    ); 10] = [
...
fn decider_state_link_rejects_tampered_state_field_wires() {
    let cases: [(
        &str,
        fn(&neo_fold_legacy::engine::decider::__test_isolation::StateLinkProbeWires) -> usize,
    ); 10] = [
        ("vk_fs_digest", |p| p.vk_fs0.col()),
        ("structure_digest", |p| p.structure0.col()),
        ("chunk_count", |p| p.chunk_count.col()),
