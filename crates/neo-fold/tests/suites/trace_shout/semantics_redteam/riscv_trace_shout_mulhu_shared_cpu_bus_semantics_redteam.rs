...
#[test]
fn riscv_trace_wiring_ccs_shared_cpu_bus_shout_mulhu_semantics_redteam() {
    // Program:
    // - LUI x1, 16          (x1 = 65536)
    // - LUI x2, 16          (x2 = 65536)
    ...
    for row in exec.rows.iter_mut() {
        if row.active {
            row.shout_events.retain(|ev| ev.shout_id == mulhu_id);
        }
    }
    let mulhu_rows = exec
        .rows
    ...
    let mulhu_shout_rows = exec
        ...
        .filter(|row| {
            row.active
                ...
                && row.shout_events.iter().any(|ev| ev.shout_id == mulhu_id)
        })
        .count();
    assert!(mulhu_rows > 0, "expected at least one MULHU row");
    assert_eq!(
        mulhu_shout_rows, mulhu_rows,
        "native MULHU shout coverage mismatch (mulhu_rows={mulhu_rows}, shout_rows={mulhu_shout_rows})"
    );
    ...
    // Shout instance: MULHU table, 1 lane (tamper one low-bit).
    let t = exec.rows.len();
    ...
    let mut mulhu_z = build_shout_only_bus_z(
        ccs.m,
        layout.m_in,
        t,
    ...
    // Flip lo bit 0 on any active lookup row.
    let lane0 = &shout_lanes[0];
    let j = lane0
        .has_lookup
        .iter()
        .position(|&b| b)
        .expect("expected at least one MULHU lookup");
    ...
    let lo_bit0_col_id = cols
        ...
        .clone()
        .nth(2)
        .expect("expected addr_bits[2] for lo bit 0");
    let cell = bus.bus_cell(lo_bit0_col_id, j);
    mulhu_z[cell] = if mulhu_z[cell] == F::ONE { F::ZERO } else { F::ONE };
    ...
    if let Ok(proof) = fold_shard_prove(
        ...
        .expect_err("tampered packed MULHU lo bit must be caught by Route-A time constraints");
    }
}
