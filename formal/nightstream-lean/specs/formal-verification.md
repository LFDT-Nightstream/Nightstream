...
| `FPR-NIFS-BRIDGE` | The concrete row-decoded recursive verifier refines the fixed-active `Nifs.PaperProfile.Transition` over the exact same statements, or exposes the named SumCheck, sampler, extraction, or binding event, and then reaches the composition knowledge theorem over `Concrete.relationSemantics` (or a proved relation isomorphism). It must enforce verifier-computed PiDEC public/copy fields, exact evaluation-vector layout, and the two message recomposition equations, but must not require child commitments/evaluations to equal one deterministic private split. Fixed-layout arity may be discharged structurally rather than by arithmetic rows, but that refinement must be proved. A public carrier with trivial private predicates may establish field/equation alignment but cannot close this property. No old R1CS relation or caller-supplied verifier predicate is semantic authority | Missing complete transcript/security and concrete decoder/refinement theorem | `ConcreteNifs` plus `SuperNeo.Folding.{PiCCS,PiRLC,PiDEC,Composition,Nifs.PaperProfile}` | `planned` |
...
| `FPR-OBLIGATION-NECESSITY` | Removing any one of the six retained macro obligations while retaining the other five admits a concrete invalid fixed candidate. This is exhaustive inclusion-minimality for that macro plan, not necessity of every internal PiDEC equation, source-binding coordinate, physical row, or a globally minimal algebraic circuit | `SelectedNifs.PaperSemanticMinimality.{necessityWitnesses,provedNecessaryLeaves_eq_checks,inclusionMinimalSound}` | Fixed-active reduced verifier macro-obligation tree | `model-proved` |
