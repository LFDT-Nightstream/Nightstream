...
use neo_fold_legacy::paper::reductions::pi_dec_circuit::{alloc_dec_inputs, enforce_dec_v_strict};
...
use neo_fold_legacy::{config, preprocess, CcsInstance, CeClaim, Preprocessing};
use neo_math::{D, F, K};
use neo_transcript::{Poseidon2Transcript, Transcript};
use p3_field::PrimeCharacteristicRing;

fn lifecycle_accepts(prep: &Preprocessing, fresh: CcsInstance) -> bool {
    let Ok(audit) = neo_fold_legacy::prove(prep, [vec![fresh]]) else {
        return false;
    };
    let Ok(proof) = neo_fold_legacy::finish_uncompressed(prep, audit) else {
        return false;
    };
    neo_fold_legacy::verify_uncompressed(prep, &proof).is_ok()
}
...
fn rot_rho_rejects_rotation_outside_configured_strong_set() {
    ...
    let params = neo_fold_legacy::Params::production();
    assert!(
