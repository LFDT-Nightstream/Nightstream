...
private theorem substitution_agrees_on_row
    ...
    Ordinary.AgreesOnTerms (substitution application)
        (ApplicationOrdinaryPlan.sourceMap geometry) row.a.terms ∧
      Ordinary.AgreesOnTerms (substitution application)
        (ApplicationOrdinaryPlan.sourceMap geometry) row.b.terms ∧
      Ordinary.AgreesOnTerms (substitution application)
        (ApplicationOrdinaryPlan.sourceMap geometry) row.c.terms := by
  refine ⟨?_, ?_, ?_⟩
...
def directForms
  ...
  SourceCompiler.compileRow (ApplicationOrdinaryPlan.sourceMap geometry)
    (oneColumn geometry)
...
theorem plan_forms
    ...
    (geometry : Geometry application logicalWidth)
    (index : Fin (ApplicationDirectSource.program application fits).rowCount) :
    (ApplicationOrdinaryPlan.plan fits geometry).forms index =
      (directForms fits geometry index).meaningfulForm := by
  simpa only [ApplicationOrdinaryPlan.plan, directForms,
    ApplicationOrdinaryPlan.inputs] using!
      (OrdinarySourcePlan.Program.compile_toPlan_forms
        (ApplicationDirectSource.program application fits)
        (ApplicationOrdinaryPlan.inputs fits geometry) index)
...
theorem ordinaryBlock_row?
  ...
  exact Ordinary.Block.row?_eq_compileRow (ordinaryBlock geometry) sourceRow
    ...
    (ApplicationOrdinaryPlan.sourceMap geometry) (oneColumn geometry) rfl
    ((ApplicationDirectSource.program application fits).bounded index)
    agreesA agreesB agreesC
...
theorem matrixProgram_row?
    ...
    (loaded : ∀ index :
      ...
      sourceRow (PerApplicationPackage.basePackage.layout.rowCount +
          index.val) =
        some ((ApplicationDirectSource.program application fits).row index))
    (global : Fin (ApplicationOrdinaryPlan.plan fits geometry).rowCount) :
    (matrixProgram geometry).row? logicalWidth sourceRow global.val =
      some ((ApplicationOrdinaryPlan.plan fits geometry).forms global) := by
  have blockBound : global.val <
    ...
    have count := ApplicationOrdinaryPlan.plan_rowCount fits geometry
    omega
