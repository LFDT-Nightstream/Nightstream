...
/-!
Exact checked rows for the recursive accumulator core owner.
...
def accumulatorClaimSourceColumns : List Nat :=
  FPrimeFullHistoryRecursiveAccumulatorCorePoseidonHashes.accumulatorDigestTrace.inputColumns

def accumulatorDigestColumns : List Nat := [1127468, 1127469, 1127470, 1127471]
def rowStart : Nat := 863861
def rowEnd : Nat := 1118772
...
def rowPieces : List (List Row) :=
  [segment0Rows, FPrimeFullHistoryRecursiveAccumulatorCorePoseidonHashes.rows]
...
theorem rows_length : rows.length = rowCount := by
  simp only [rows, rowPieces, List.flatten_cons, List.flatten_nil,
    ...
    FPrimeFullHistoryRecursiveAccumulatorCorePoseidonHashes.rows_length]
  native_decide
