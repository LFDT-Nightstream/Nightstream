...
def initialState : Transcript.State := PiCCSNonzero.compute.outgoingState

/-- The exact fixed-size sampler result. `none` is an explicit phase
rejection; no challenge fallback exists. -/
def sampled : Option (Transcript.PiRlcSampler.Batch SourceCount) :=
...
def inputPublicInput (source : Fin SourceCount) :
    PublicInput (logicalWidth := Data.logicalWidth)
...
def point : Point := PiCCSNonzero.compute.verifierRoundPoint
...
def inputInstance
    ...
    NightstreamFPrime.Lifecycle.PiRLC.v1_1.InputBinding.InputInstance
      Data.logicalWidth Data.publicFits where
  constraintSystem :=
...
def attempt
    ...
    PiRLC.Attempt (PaperAlgebra.Structure Data.logicalWidth)
      (PaperAlgebra.PublicInput (logicalWidth := Data.logicalWidth)
        (publicFits := Data.publicFits))
