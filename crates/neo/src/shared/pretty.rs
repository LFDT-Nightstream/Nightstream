...
mod tests {
    ...
    #[test]
    fn test_pretty_field_basic() {
        let f = Goldilocks::from_u64(0);
        assert_eq!(pretty_field(f), "0");
        
        let f = Goldilocks::from_u64(1);
        assert_eq!(pretty_field(f), "1");
        
        let f = Goldilocks::from_u64(42);
        assert_eq!(pretty_field(f), "42");
    }
    
    #[test]
    fn test_pretty_field_negative() {
        // p-1 should show as -1
        let p = Goldilocks::ORDER_U64;
        let f = Goldilocks::from_u64(p - 1);
        assert_eq!(pretty_field(f), "-1");
        
        let f = Goldilocks::from_u64(p - 5);
        assert_eq!(pretty_field(f), "-5");
    }
    
    #[test]
    fn test_pretty_field_vec_short() {
        let v = vec![
            Goldilocks::from_u64(0),
            Goldilocks::from_u64(1),
            Goldilocks::from_u64(2),
        ];
        assert_eq!(pretty_field_vec(&v), "[0, 1, 2]");
    }
    
    #[test]
    fn test_pretty_field_vec_long() {
        let v = vec![Goldilocks::from_u64(0); 20];
        let result = pretty_field_vec(&v);
        assert!(result.contains("0, 0, 0"));
        assert!(result.contains("17 more"));
    }
    
    #[test]
    fn test_pretty_field_vec_empty() {
        let v: Vec<Goldilocks> = vec![];
        assert_eq!(pretty_field_vec(&v), "[]");
    }
}
