...
namespace Nightstream.Implementation.Lowering.FPrimeFixedOne.Encoding.TerminalR1cs.Ajtai
...
theorem row_supported
  ...
  change
    column ∈
      ((terms frame (verifierRowAt coordinate) (outputAt coordinate) ++
        Goldilocks.singleton frame.one 1 ++
        Goldilocks.singleton
          (frame.commitment
  ...
  · exact Or.inr (Or.inr
      (by simpa [Goldilocks.singleton] using commitment))
...
end Nightstream.Implementation.Lowering.FPrimeFixedOne.Encoding.TerminalR1cs.Ajtai
