...
use spartan2::{
    bellpepper::{r1cs::SpartanWitness as _, solver::SatisfyingAssignment},
...
impl DirectCcsTerminalR2Assignment {
    pub(super) fn from_terminal_circuit(circuit: DirectCcsTerminalFPrimeCircuit) -> Result<Self, SimpleKernelError> {
        ...
        let witness_values = layout.source_witness_values(&witness, &private_witness_labels)?;
...
impl DirectCcsTerminalR2Layout {
    ...
    fn source_witness_values(
        &self,
        witness: &R1CSWitness<NeoFoldDeciderEngine>,
        private_witness_labels: &[Option<String>],
    ) -> Result<Vec<F>, SimpleKernelError> {
        let mut values = Vec::with_capacity(self.source_limb_width + 1);
        for (witness_idx, value) in witness.values().iter().enumerate() {
            let Some(Some(label)) = private_witness_labels.get(witness_idx) else {
                continue;
            };
            let Some((offset, encoding)) = self.source_binding(label) else {
                continue;
            };
        ...
        self.pad_source_witness_values(&mut values)?;
        values.push(F::ONE);
        Ok(values)
    }

    fn pad_source_witness_values(&self, values: &mut Vec<F>) -> Result<(), SimpleKernelError> {
        while values.len() < self.source_limb_width {
            values.push(F::ZERO);
        }
        if values.len() != self.source_limb_width {
