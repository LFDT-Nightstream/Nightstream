...
//! | calls | 422 call geometries | order, range, 600-row/column ABI | `CallGeometry.Valid` |
...
const EXPECTED_CANDIDATES: usize = 36_292;
const EXPECTED_LINEAR_USES: usize = 290_336;
const EXPECTED_TOTAL_USES: usize = 326_628;
...
impl LeanBridgeManifest {
    ...
    fn validate_boundaries(&self, source_rows: usize, source_columns: usize) -> Result<(), String> {
        let expected_claimed = std::array::from_fn(|lane| self.stage_columns.start + lane);
        ...
        if self.stage_rows.len() != EXPECTED_STAGE_ROWS
            ...
            || self.prehash_columns.start != self.stage_columns.start + 4
