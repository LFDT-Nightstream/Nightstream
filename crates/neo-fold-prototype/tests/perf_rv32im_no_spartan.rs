...
use neo_fold_prototype::rv32im::audit::build_rv32im_chunk_step_ivc_relations;
use neo_fold_prototype::rv32im::final_relation::prove_rv32im_final_statement_from_accepted;
use neo_fold_prototype::rv32im::ivc::{derive_rv32im_ivc_step_cap, Rv32imIvcAppendPerf, Rv32imIvcState};
use neo_fold_prototype::rv32im::{
    build_mixed_opcode_perf_source_case, build_rv32im_recursion_shape_for_step_cap,
    prove_rv32im_accepted_proof_with_options, rv32im_simple_root_params_for_step_cap, Rv32imProofInput,
    Rv32imPublicProofOptions, RV32IM_MIXED_OPCODE_PERF_DEFAULT_N,
};
use serde::Serialize;
...
fn append_stage_rows(perfs: &[Rv32imIvcAppendPerf]) -> Vec<(&'static str, f64)> {
...
fn print_append_step_rows(perfs: &[Rv32imIvcAppendPerf], opcode_count: usize) {
...
fn print_append_step_summary(perfs: &[Rv32imIvcAppendPerf], opcode_count: usize) {
...
fn run_rv32im_mixed_opcode_no_spartan_ivc_perf_snapshot(schedule: FoldSchedule, title: &str) {
    ...
    let (accepted_artifact, _audit) =
        prove_rv32im_accepted_proof_with_options(&input, public_proof_options).expect("prove rv32im accepted artifact");
    ...
    let kernel_params = rv32im_simple_root_params_for_step_cap(step_cap);
    let recursion_shape = build_rv32im_recursion_shape_for_step_cap(step_cap).expect("build rv32im recursion shape");
...
fn rv32im_mixed_opcode_no_spartan_ivc_perf_snapshot() {
    run_rv32im_mixed_opcode_no_spartan_ivc_perf_snapshot(
        FoldSchedule::WholeTrace,
        "RV32IM No-Spartan IVC Perf Snapshot (whole trace)",
    );
}

#[test]
#[ignore = "performance/debugging snapshot; run with --release -- --ignored --nocapture"]
fn rv32im_mixed_opcode_no_spartan_ivc_perf_snapshot_rows_per_chunk_1() {
    run_rv32im_mixed_opcode_no_spartan_ivc_perf_snapshot(
        FoldSchedule::RowsPerChunk(1),
        "RV32IM No-Spartan IVC Perf Snapshot (per-op folds)",
    );
}

#[test]
#[ignore = "performance/debugging snapshot; run with --release -- --ignored --nocapture"]
fn rv32im_mixed_opcode_no_spartan_ivc_perf_snapshot_whole_trace() {
    run_rv32im_mixed_opcode_no_spartan_ivc_perf_snapshot(
        FoldSchedule::WholeTrace,
        "RV32IM No-Spartan IVC Perf Snapshot (whole trace)",
    );
