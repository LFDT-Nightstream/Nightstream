...
#[test]
fn host_advice_stays_with_the_caller_after_cross_instance_reentry() {
    let caller_bytes = wat::parse_str(
        r#"(module
        (import "host" "invoke" (func $invoke (result i32)))
        (func (export "run") (result i32) call $invoke))"#,
    )
    .unwrap();
    let callee_bytes = wat::parse_str(
        r#"(module
        (func (export "run") (result i32) i32.const 17))"#,
    )
    .unwrap();
    let mut registry = WasmtimeTraceRegistry::default();
    registry
        .register_module(&caller_bytes, HostEventBindings::default())
        .unwrap();
    registry
        .register_module(&callee_bytes, HostEventBindings::default())
        .unwrap();
    let mut store = store(registry);
    let module = Module::new(store.engine(), &callee_bytes).unwrap();
    let callee_instance = instantiate(&mut store, &module);
    let callee = callee_instance.get_func(&mut store, "run").unwrap();
    let mut linker = Linker::<WasmtimeTraceRegistry>::new(store.engine());
    linker
        .func_wrap_async("host", "invoke", move |mut caller, (): ()| {
            Box::new(async move {
                let frame = caller.debug_exit_frames().next().unwrap();
                let caller_index = frame.instance(&mut caller)?.debug_index_in_store();
                let mut results = [Val::I32(0)];
                callee.call_async(&mut caller, &[], &mut results).await?;
                caller.data_mut().record_call_inputs(caller_index, &[99])?;
    ...
    assert_eq!(call(&mut store, caller_instance, "run"), 17);
    let caller_trace = store
        .data()
        .instance(caller_instance.debug_index_in_store())
        .unwrap();
    assert_eq!(caller_trace.steps()[0].host_call_inputs, [99]);
    let callee_trace = store
        .data()
        .instance(callee_instance.debug_index_in_store())
        .unwrap();
    assert!(callee_trace
        .steps()
        .iter()
        .all(|row| row.host_call_inputs.is_empty()));
}
