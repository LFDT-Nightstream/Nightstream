//! Owns the fixed-step Spartan backend for RV32IM main recursion.
//!
...
//! body by the chunk NIFS verifier stage, which reuses the inner verifier body
//! `synthesize_rv32im_chunk_nifs_verifier_body` directly. In the current
//! specialization the program counter is fixed to `1`,
...
use super::recursive_cover::{
    ...
    Rv32imRecursiveCoverStateVar,
};
use super::{
    alloc_const_field_values, alloc_private_field_values, digest32_as_spartan_fields, digest_const_inputs,
    enforce_digest_eq, next_public_digest, Rv32imMainRecursionStepSpartanStatement,
};
...
pub use public_target::Rv32imMainRecursionStepSpartanPublishedTarget;

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Rv32imMainRecursionStepSpartanCircuitShape {
...
#[derive(Debug, Error)]
pub enum Rv32imMainRecursionStepSpartanError {
    #[error("rv32im main recursion step prepare failed: {0}")]
    Prepare(String),
    #[error("rv32im main recursion step verify failed: {0}")]
    Verify(String),
}

#[derive(Clone, Debug)]
pub(crate) struct Rv32imMainRecursionStepCircuit {
    spartan_shape: Rv32imMainRecursionStepSpartanShape,
    backend_relation: Rv32imMainRecursionFPrimeBackendRelation,
}

impl Rv32imMainRecursionStepCircuit {
    fn expected_public_values(&self) -> Vec<SpartanF> {
        let target = build_rv32im_main_recursion_step_spartan_published_target(&self.backend_relation)
...
fn canonical_main_recursion_step_spartan_statement(
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<Rv32imMainRecursionStepSpartanStatement, Rv32imMainRecursionStepSpartanError> {
    build_rv32im_main_recursion_step_spartan_statement_from_payload(&backend_relation.f_prime_advice)
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))
}

pub fn build_rv32im_main_recursion_step_spartan_published_target(
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<Rv32imMainRecursionStepSpartanPublishedTarget, Rv32imMainRecursionStepSpartanError> {
    ...
    if backend_relation.construction2_u_next.x_i() != &canonical_statement.x_out {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion published target requires backend construction2_u_next.x_i to match F' output"
                .into(),
        ));
    }
    backend_relation
        ...
        .ok_or_else(|| {
            Rv32imMainRecursionStepSpartanError::Prepare(
                "rv32im main recursion target requires a Construction-2 input fresh instance".into(),
            )
        })?;
    let terminal_verified_step_statement_digest =
        build_terminal_f_prime_verified_step_statement_digest(backend_relation)?;
    Ok(Rv32imMainRecursionStepSpartanPublishedTarget {
...
fn ensure_main_recursion_step_spartan_statement_binding(
    ...
    if backend_relation.spartan_statement != canonical_statement {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step circuit requires the canonical per-step Spartan statement derived from native F'"
                .into(),
        ));
    }
    Ok(())
...
fn dummy_backend_relation_from_chain_step(
    spartan_shape: &Rv32imMainRecursionStepSpartanShape,
    step_shape: &Rv32imChunkStepIvcShape,
    chunk_count_in: u64,
    running_state: &Rv32imChunkFoldState,
) -> Result<Rv32imMainRecursionFPrimeBackendRelation, Rv32imMainRecursionStepSpartanError> {
    let vk_fs = build_rv32im_main_recursion_verifier_key_fs_for_step_cap(step_shape.fresh_claim_count as usize)
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    ...
    let prepared_step_digests = crate::rv32im::kernel::prepared_step_digests(&public_chunk_input.steps);
    let mut handoff = Rv32imVerifiedKernelChunkHandoff {
        ...
        bridge_handoff: Rv32imChunkBridgeHandoff {
            ...
            step_bindings: prepared_step_digests
                ...
                .map(|(idx, prepared_step_digest)| {
                    ...
                    Rv32imPreparedStepBridgeBinding {
    ...
    handoff.public_chunk_digest = crate::rv32im::kernel::rv32im_public_chunk_digest(&handoff.public_chunk);
    ...
    let step_cap = usize::try_from(step_shape.fresh_claim_count).map_err(|_| {
        Rv32imMainRecursionStepSpartanError::Prepare("rv32im main recursion step step_cap overflow".into())
    })?;
    ...
    let me_input_accumulator_handle = crate::finalize::digest32_as_fields(
        crate::rv32im::final_relation::rv32im_chunk_fold_carry_recursive_accumulator_digest(&running_state.carry),
    );
    ...
    let transcript_out = crate::rv32im::final_relation::rv32im_chunk_fold_carried_transcript_snapshot(
    ...
    let state_out = Rv32imChunkFoldState {
        carry: next_carry,
    ...
    let native_verified_step_statement =
        build_rv32im_main_recursion_construction2_verified_step_statement_from_relation(&Rv32imChunkStepIvcRelation {
    ...
    let canonical_full_width =
        crate::rv32im::construction2_default::build_rv32im_main_recursion_construction2_canonical_full_width(
            &vk_fs,
            &crate::rv32im::main_recursion::Rv32imMainRecursionPhiSide::zero(),
        )
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    let canonical_u_i = crate::rv32im::construction2::build_rv32im_main_recursion_construction2_default_fresh_instance(
        &vk_fs,
    ...
    let advice = crate::rv32im::main_recursion::Rv32imMainRecursionFPrimeAdvice::from_parts(
        vk_fs.clone(),
        chunk_count_in,
        initial_state.carry.terminal_handle.0,
        running_state.carry.terminal_handle.0,
        crate::rv32im::main_recursion::RV32IM_MAIN_RECURSION_TRIVIAL_PC,
        crate::rv32im::main_recursion::Rv32imMainRecursionSideLaneWitness::zero(),
        crate::rv32im::main_recursion::Rv32imMainRecursionPhiSide::zero(),
        running_state.clone(),
        build_rv32im_main_recursion_backend_statement_from_parts_with_vk_fs(
            &vk_fs,
        ...
        state_out,
        replay_witness,
        crate::rv32im::construction2::build_rv32im_main_recursion_construction2_pi_fold_from_trace(&trace)
            .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?,
    )
    .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    let payload = build_rv32im_main_recursion_f_prime_payload(&advice, &spartan_shape)
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    ...
    {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion compressed-chain dummy setup derived a payload step shape whose fixed-shape fields drifted from the requested canonical chain step shape"
                .into(),
        ));
    }
    let statement = build_rv32im_main_recursion_step_spartan_statement_from_payload(&advice)
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    let construction2_u_next = crate::rv32im::main_recursion::evaluate_rv32im_main_recursion_f_prime_advice(&advice)
        .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?
        .construction2_u_next()
        .clone();
    Ok(Rv32imMainRecursionFPrimeBackendRelation {
...
fn main_recursion_x_out_circuit<CS: ConstraintSystem<SpartanF>>(
    ...
    transcript.append_message(
        ...
        b"neo.fold.next/rv32im/main_recursion_f_prime_x_out/version",
        b"v4",
    )?;
    transcript.append_message(
...
fn enforce_initial_transcript_when_base<CS: ConstraintSystem<SpartanF>>(
    ...
    next_chunk_count: u64,
    state_in_var: &Rv32imRecursiveCoverStateVar,
    label: &str,
) -> Result<(), SynthesisError> {
    ...
    let expected = crate::rv32im::final_relation::rv32im_chunk_fold_initial_transcript_snapshot();
...
fn enforce_inactive_side_lane_constraints<CS: ConstraintSystem<SpartanF>>(
    ...
    if !crate::rv32im::main_recursion::RV32IM_MAIN_RECURSION_SIDE_WITNESS_ACTIVE {
    ...
    if !crate::rv32im::main_recursion::RV32IM_MAIN_RECURSION_PHI_SIDE_ACTIVE {
...
pub(crate) fn synthesize_rv32im_main_recursion_step_body_with_outputs<CS: ConstraintSystem<SpartanF>>(
    ...
    let started = Instant::now();
    let chunk_replay = synthesize_rv32im_main_recursion_step_chunk_replay(
...
pub(crate) fn synthesize_rv32im_main_recursion_step_body<CS: ConstraintSystem<SpartanF>>(
    circuit: &Rv32imMainRecursionStepCircuit,
    cs: &mut CS,
    public_inputs: &[AllocatedNum<SpartanF>],
    public_cursor: &mut usize,
    ...
    synthesize_rv32im_main_recursion_step_body_with_outputs(circuit, cs, public_inputs, public_cursor, trace_prefix)
}

impl SpartanCircuit<Rv32imDeciderEngine> for Rv32imMainRecursionStepCircuit {
    ...
    fn synthesize<CS: ConstraintSystem<SpartanF>>(
        ...
        synthesize_rv32im_main_recursion_step_body(self, cs, &public_inputs, &mut public_cursor, None)?;
...
pub(crate) fn build_rv32im_main_recursion_step_circuit(
    spartan_shape: &Rv32imMainRecursionStepSpartanShape,
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<Rv32imMainRecursionStepCircuit, Rv32imMainRecursionStepSpartanError> {
    build_rv32im_main_recursion_step_circuit_inner(spartan_shape, backend_relation)
}

pub(crate) fn build_rv32im_terminal_f_prime_r2_circuit(
    spartan_shape: &Rv32imMainRecursionStepSpartanShape,
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<Rv32imMainRecursionStepCircuit, Rv32imMainRecursionStepSpartanError> {
    build_rv32im_main_recursion_step_circuit_inner(spartan_shape, backend_relation)
...
fn build_rv32im_main_recursion_step_circuit_inner(
    spartan_shape: &Rv32imMainRecursionStepSpartanShape,
    backend_relation: &Rv32imMainRecursionFPrimeBackendRelation,
) -> Result<Rv32imMainRecursionStepCircuit, Rv32imMainRecursionStepSpartanError> {
    let _ = rv32im_root_main_lane_context_for_claim_count(
        backend_relation
            .f_prime_advice
            .running_state()
            .carry
    ...
    if !spartan_shape.matches_payload(&backend_relation.payload) {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step circuit requires a canonical recursive-step payload matching the explicit Spartan shape".into(),
        ));
    }
    let canonical_step_image =
        crate::rv32im::main_recursion::evaluate_rv32im_main_recursion_f_prime_advice(&backend_relation.f_prime_advice)
            .map_err(|err| Rv32imMainRecursionStepSpartanError::Prepare(err.to_string()))?;
    if backend_relation.construction2_u_next.x_i() != canonical_step_image.construction2_u_next().x_i() {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step circuit requires construction2_u_next.x_i to match the terminal F' output hash"
                .into(),
        ));
    }
    ...
    {
        return Err(Rv32imMainRecursionStepSpartanError::Prepare(
            "rv32im main recursion step circuit payload/state claim counts are out of sync".into(),
        ));
    }
    ensure_main_recursion_step_spartan_statement_binding(backend_relation)?;
    Ok(Rv32imMainRecursionStepCircuit {
        spartan_shape: spartan_shape.clone(),
        backend_relation: backend_relation.clone(),
    })
}

fn build_rv32im_main_recursion_step_shape_only_circuit(
    spartan_shape: &Rv32imMainRecursionStepSpartanShape,
) -> Result<Rv32imMainRecursionStepCircuit, Rv32imMainRecursionStepSpartanError> {
