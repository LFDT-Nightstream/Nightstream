...
use neo_fold_clean::paper::f_prime::r1cs::{F_PRIME_ENC_INST_OFFSET, F_PRIME_PUBLIC_INPUT_LEN};
...
/// Stateful Fibonacci `(a, b) -> (b, a + b)` over
/// `[one, a, b, a_out, b_out, ...]`.
...
fn r1cs_stateful_audit_rejects_intermediate_public_carrier_padding() {
    ...
    let err = neo_fold_clean::verify_uncompressed_audit(&fixture.prep.prep, &fixture.audit)
        .expect_err("audit replay must reject nonzero intermediate carrier padding");
    assert!(
        matches!(
            err,
...
fn r1cs_stateful_terminal_fold_rejects_latest_link_or_padding_mutation() {
    for (label, coordinate) in [
        ...
        ("carrier padding", F_PRIME_PUBLIC_INPUT_LEN),
    ] {
        let fixture = build_single_fibonacci_fixture();
        let mut finished = neo_fold_clean::finish_uncompressed(&fixture.prep.prep, fixture.audit)
            .expect("finish single-step Fibonacci");
        let latest = &mut finished
            .final_fold
            .as_mut()
            ...
            .instances[0]
            .claim;
        latest.x[coordinate] += F::ONE;
        let error = neo_fold_clean::verify_uncompressed(&fixture.prep.prep, &finished)
            .expect_err("terminal public-carrier mutation must fail");
        assert!(
            matches!(
                error,
                neo_fold_clean::Error::TerminalLatestPublicInputMismatch { index: 0 }
            ),
            "expected {label} mismatch, got {error:?}"
        );
    }
}
