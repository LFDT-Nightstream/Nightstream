...
use neo_fold_legacy::paper::relations::{CcsClaim, CcsInstance, CcsWitness};
use neo_fold_legacy::{config, preprocess};
use neo_math::{D, F};
...
fn preprocessing_cached_structure_digest_matches_structure() {
    ...
    let recomputed = neo_fold_legacy::paper::digest::structure_digest(prep.structure());
    assert_eq!(
...
fn preprocessing_cache_accessors_expose_read_only_views() {
    ...
    let _params: &neo_fold_legacy::paper::params::Params = prep.params();
    let _setup: &neo_ajtai::AjtaiSModule = prep.commitment_scheme();
    let _key: &neo_fold_legacy::paper::construction2::VerifierKey = prep.verifier_key();
