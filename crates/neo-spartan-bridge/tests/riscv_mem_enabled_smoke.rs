...
fn test_riscv_rv32_b1_mem_enabled_spartan_phase1_smoke() {
    ...
    assert!(
        verify_fold_run(&vk, run.params(), run.ccs(), &steps_public, None, &spartan).expect("verify_fold_run"),
...
fn test_riscv_rv32_b1_mem_enabled_spartan_rejects_tampered_bus_opening() {
    ...
    let bad_witness = FoldRunWitness::new(bad_proof, steps_public, initial_accumulator, None);
    assert!(
