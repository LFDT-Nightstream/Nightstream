...
fn test_balanced_divrem_various_values_base2() {
    ...
    for (value, expected_max_digits) in test_cases {
        ...
        assert!(
            digits.len() <= expected_max_digits,
            "Value {} used {} digits, expected <= {}",
            value,
            digits.len(),
            expected_max_digits
        );
...
fn test_balanced_divrem_base3() {
    ...
    for (value, expected_max_digits) in test_cases {
        ...
        assert_eq!(
            reconstructed, value,
            "Value {} (base 3) should reconstruct correctly",
            value
        );
        assert!(
            digits.len() <= expected_max_digits,
            "Value {} (base 3) used {} digits, expected <= {}",
            value,
            digits.len(),
            expected_max_digits
        );
...
fn test_split_b_matrix_k_simple() {
    ...
    assert!(
        result.is_ok(),
        "Should successfully split simple matrix: {:?}",
        result.err()
    );
    ...
    for (i, mat) in split_matrices.iter().enumerate() {
        assert_eq!(
            mat.rows(),
            Z.rows(),
            "Split matrix {} should have same number of rows",
            i
        );
        assert_eq!(
            mat.cols(),
            Z.cols(),
            "Split matrix {} should have same number of cols",
            i
        );
    }
    ...
    for r in 0..Z.rows() {
        for c in 0..Z.cols() {
            assert_eq!(
                ...
                "Reconstructed Z[{},{}] should match original",
                r,
                c
            );
        }
