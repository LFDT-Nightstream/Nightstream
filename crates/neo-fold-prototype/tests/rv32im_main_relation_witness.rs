...
use neo_fold_prototype::circuit::superneo::witness::{
...
fn rv32im_main_relation_witness_gadgets_accept_consistent_projection_range_and_y_zcol() {
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_witness_gadgets_reject_tampered_projection() {
    let params = NeoParams::goldilocks_auto_r1cs_ccs(3).expect("params");
    let expected_m = 3;
    let m_in = 2;
    let witness = toy_witness();
