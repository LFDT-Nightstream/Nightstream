...
fn rv32im_mixed_opcode_perf_snapshot() {
    ...
    let public_proof_options = Rv32imPublicProofOptions {
    ...
    let (proof, prove_perf) = prove_rv32im_public_proof_with_options_and_perf(&input, public_proof_options)
        .expect("prove rv32im public proof");
    let prove_ms = prove_perf.total_ms;

    let verify_started = Instant::now();
    let verify_perf = audit_rv32im_public_proof_with_perf(&proof).expect("audit rv32im public proof");
    let verify_ms = millis_since(verify_started);

    let accepted_artifact = build_rv32im_accepted_proof_artifact(&proof).expect("build accepted artifact");
    ...
    let mut ivc_state = Rv32imIvcState::init_with_step_cap(1).expect("build initial rv32im ivc state");
    for relation in &relations {
        ivc_state = ivc_state.append(relation).expect("append rv32im ivc relation");
    }
    let native_append_ms = millis_since(native_append_started);

    let compress_started = Instant::now();
    let ivc_snark = ivc_state.compress().expect("compress rv32im ivc state");
    let compress_ms = millis_since(compress_started);
    let ivc_public_image = ivc_snark.public_image().clone();
    let ivc_snark_keys = setup_rv32im_ivc_snark_cached(&ivc_state).expect("setup rv32im ivc snark verifier key");

    let compressed_verify_started = Instant::now();
    ivc_snark
        .verify(&ivc_snark_keys.as_ref().1, &ivc_public_image)
        .expect("verify compressed rv32im ivc snark");
    let compressed_verify_ms = millis_since(compressed_verify_started);

    let (published_seam, published_seam_perf) =
        build_rv32im_published_proof_seam_with_perf(&proof).expect("build rv32im published seam");
    assert_eq!(
    ...
    let ((nightstream_statement, nightstream_proof), nightstream_build_perf) =
        build_rv32im_nightstream_from_published_proof_seam_with_perf(&published_seam, &published_seam_perf)
            .expect("build rv32im nightstream proof");
    ...
    let side_keys = neo_fold_prototype::public_proof::rv32im::side_relation_spartan::setup_rv32im_side_binding_cached(
        &side_statement,
        nightstream_proof.side_proof().opening_public(),
    )
    .expect("setup rv32im side binding");
    let (opening_statement, opening_witness) =
        neo_fold_prototype::public_proof::rv32im::side_opening_relation::build_rv32im_side_opening_relation_from_accepted_artifact(
            accepted_artifact,
        )
        .expect("build rv32im side opening relation");
    let side_opening_keys = neo_fold_prototype::public_proof::rv32im::side_opening_spartan::setup_rv32im_side_opening_spartan_cached(
        &opening_statement,
        &opening_witness,
    )
    ...
    let nightstream_verify_perf = verify_rv32im_nightstream_with_perf(
    ...
    .expect("verify Nightstream RV32IM proof");
    ...
    let root_ccs = rv32im_root_main_lane_ccs().expect("build RV32IM root CCS");
    let root_params = rv32im_simple_root_params();
    ...
    assert_eq!(
        ...
        u64::from(source.start_pc) + (total_opcodes as u64) * 4
    );
    assert!(proof.statement.halted);
    assert_eq!(
        output.kernel_claims.kernel.final_registers[1],
        x1_increment_count as u32
    );
    assert_eq!(u64::from(output.kernel_claims.kernel.final_pc), proof.statement.final_pc);
    ...
    // ── Input Shape ────────────────────────────────────────────────────────
    print_section("RV32IM Mixed Opcode Perf Snapshot");
    print_kv("ns_debug_n (non-halt ops)", opcode_count);
    print_kv("program_opcodes_total", total_opcodes);
    print_kv("mixed_block_len", RV32IM_MIXED_OPCODE_PERF_BLOCK_LEN);
    ...
    print_timing_table(
        ...
        &[
            ...
            ("prove_rv32im_public_proof", prove_ms),
            (
                "build_rv32im_published_seam.accepted_artifact",
                published_seam_perf.accepted_artifact_ms,
            ),
            (
                "build_rv32im_published_seam.kernel_export_source",
                published_seam_perf.kernel_export_source_ms,
            ),
            (
                "build_rv32im_published_seam.final_statement",
                published_seam_perf.final_statement_ms,
            ),
            (
                "build_rv32im_published_seam.main_proof",
                published_seam_perf.main_proof_ms,
            ),
            (
                "setup_rv32im_ivc_recursion_snark_from_final",
                ivc_recursion_snark_setup_ms,
            ),
            ("build_rv32im_nightstream", nightstream_build_ms),
        ],
        opcode_count,
        execution_row_count,
    );
    ...
    print_timing_table(
        "Raw Verify Timing",
        &[
            ("audit_rv32im_public_proof", verify_ms),
            ("verify_rv32im_nightstream", nightstream_verify_ms),
        ],
        opcode_count,
        execution_row_count,
    );
    ...
    print_kv("root_row_width", RV32IM_ROOT_ROW_WIDTH);
    print_kv("root_public_inputs", RV32IM_ROOT_PUBLIC_INPUTS);
    ...
    println!("╔══════════════════════════════════════════════════════════════════════════╗");
    println!(
        "║  RV32IM Perf Snapshot   N={:<6}  rows={:<6}  {:.2} rows/op              ║",
        opcode_count,
        execution_row_count,
        per_unit(execution_row_count as f64, opcode_count)
    );
