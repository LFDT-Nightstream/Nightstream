...
use toy_spartan::{
  ...
  spartan::R1CSSNARK,
  traits::{
    Engine, circuit::SpartanCircuit, pcs::PCSEngineTrait, snark::R1CSSNARKTrait,
...
impl SpartanCircuit<E> for CubicCircuit {
  ...
  fn shared<CS: ConstraintSystem<F>>(
    &self,
    _cs: &mut CS,
  ...
  fn synthesize<CS: ConstraintSystem<F>>(
    ...
    cs: &mut CS,
    _shared: &[AllocatedNum<F>],
    _precommitted: &[AllocatedNum<F>],
    _challenges: Option<&[F]>,
  ) -> Result<(), SynthesisError> {
    let x = AllocatedNum::alloc(cs.namespace(|| "x"), || Ok(self.x))?;
    let x_squared = AllocatedNum::alloc(cs.namespace(|| "x squared"), || Ok(self.x.square()))?;
    let x_cubed = AllocatedNum::alloc_input(cs.namespace(|| "x cubed"), || Ok(self.x_cubed))?;
...
#[test]
fn toy_spartan_proves_and_verifies_with_whir_without_superneo_wiring() {
  let x = F::from(3);
  ...
  let prepared = R1CSSNARK::<E>::prep_prove(&pk, circuit.clone(), true).unwrap();
  let proof = R1CSSNARK::<E>::prove(&pk, circuit.clone(), &prepared, true).unwrap();

  assert_eq!(proof.verify(&vk).unwrap(), circuit.public_values().unwrap());
}
