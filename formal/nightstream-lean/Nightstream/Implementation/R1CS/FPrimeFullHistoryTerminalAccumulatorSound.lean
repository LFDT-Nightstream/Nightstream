import Nightstream.Implementation.R1CS.FPrimeFullHistoryTerminalAccumulatorArtifact
import Nightstream.Implementation.R1CS.FPrimeFullHistoryTerminalAccumulatorCheckCoverage
import Nightstream.Implementation.R1CS.FPrimeFullHistoryTerminalAccumulatorSchedules
import Nightstream.Implementation.R1CS.FPrimeFullHistoryTerminalAccumulatorPoseidonHashes
...
def shiftedColumnMap (mapping : ShiftedTernaryMap) : List Nat :=
  FPrimeFullHistoryTerminalAccumulatorCheckCoverage.columnMap mapping
...
private theorem shiftedMap_one (mapping : ShiftedTernaryMap) :
  ...
  simp [shiftedColumnMap,
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.columnMap,
    Relabel.column]
...
private theorem parentCeClaimSourceColumns_schema :
    parentCeClaimSourceColumns =
      FPrimeFullHistoryParentCeSerialization.expectedSourceColumns
        FPrimeFullHistoryPiDec.terminalColumnMap
...
private theorem canonicalOpeningAt
  ...
  have schedule := FPrimeFullHistoryTerminalAccumulatorSchedule.rows_schedule index indexLt192
  change
    ((FPrimeFullHistoryTerminalAccumulatorCheckCoverage.shiftedOwnerRows
      mapping).drop
      (FPrimeFullHistoryTerminalAccumulatorCheckCoverage.shiftedLocalRowStart
        mapping)).take 124 =
      canonicalRows.map
        (Relabel.row
          (FPrimeFullHistoryTerminalAccumulatorCheckCoverage.columnMap
  ...
  · have scheduled :
      ...
      FPrimeFullHistoryTerminalAccumulatorCheckCoverage.shiftedOwnerRows,
      FPrimeFullHistoryTerminalAccumulatorCheckCoverage.shiftedLocalRowStart, owner] using schedule
...
theorem parentCeClaimSourceValues_sound
    ...
    parentCeClaimSourceColumns.map assignment =
      FPrimeFullHistoryParentCeSerialization.parentPreimage assignment := by
...
structure Facts (assignment : Nat → Nat) : Prop where
  ...
  seeded16 : FPrimeFullHistorySeededPhi81.block16.Holds assignment
  seeded17 : FPrimeFullHistorySeededPhi81.block17.Holds assignment
  parentClaimSource :
    parentCeClaimSourceColumns.map assignment =
...
/-- `CIR-SOUND` for every one of the exact 37,295 terminal-accumulator rows. -/
theorem sound (goldilocksPrime : EuclidPrime goldilocksP)
  ...
  have block16Satisfies :
      Satisfies FPrimeFullHistorySeededPhi81.block16.rows assignment :=
    pieces FPrimeFullHistorySeededPhi81.block16.rows (by simp [rowPieces])
  have segment1Satisfies : Satisfies segment1Rows assignment :=
    pieces segment1Rows (by simp [rowPieces])
  have block17Satisfies :
      Satisfies FPrimeFullHistorySeededPhi81.block17.rows assignment :=
    pieces FPrimeFullHistorySeededPhi81.block17.rows (by simp [rowPieces])
  ...
  refine {
    ...
    canonicalOpenings := ?_
    seeded16 := SeededPhi81.sound canonical one block16Satisfies
    seeded17 := SeededPhi81.sound canonical one block17Satisfies
    ...
    parentCeDigest := Poseidon2Sponge.trace_values_sound
      FPrimeFullHistoryTerminalAccumulatorPoseidonHashes.parentCeDigestTrace_valid
      canonical one segment2Satisfies
...
private theorem checkPatternTags_bounded : ∀ mapIndex,
    mapIndex <
        FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatternTags.length →
      (FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatternTags
        ).getD mapIndex 0 <
        FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatterns.length := by
  native_decide
...
private theorem selectedCanonicalRow_mem
    ...
    (rowIndexMember : rowIndex ∈
      FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatterns.getD
        ((FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatternTags
          ).getD mapIndex 0) []) :
    canonicalRows.getD rowIndex
        FPrimeFullHistoryTerminalAccumulatorCheckCoverage.defaultRow ∈
      canonicalRows := by
  have tagIndexLt : mapIndex <
      FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatternTags.length := by
    simpa [FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatternTags_length]
      using mapIndexLt
  ...
  have patternMember :
      FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatterns.getD
        ...
        FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatterns := by
    have member := List.getElem_mem
      (l := FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatterns)
      tagLt
    rwa [List.getElem_eq_getD []] at member
  have rowIndexLt :=
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checkPatterns_bounded
      _ patternMember rowIndex rowIndexMember
  have member := List.getElem_mem (l := canonicalRows) rowIndexLt
  rwa [List.getElem_eq_getD
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.defaultRow] at member
...
private theorem checksForMapIndex_complete
    ...
    (witness : ShiftedTernaryComplete.CanonicalWitness
      (Pulled (shiftedTernaryMaps.getD mapIndex default) assignment)) :
    Satisfies
      (FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checksForMapIndex
  ...
  have mappedSatisfies : Satisfies
    ...
    (Relabel.satisfies_mapped_iff canonicalRows
      ...
      canonicalSatisfies
  intro row member
  unfold FPrimeFullHistoryTerminalAccumulatorCheckCoverage.checksForMapIndex at member
...
private theorem segment0MapIndices_bounded : ∀ mapIndex ∈
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.segment0MapIndices,
      mapIndex < shiftedTernaryMaps.length := by
  native_decide

private theorem segment1MapIndices_bounded : ∀ mapIndex ∈
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.segment1MapIndices,
      mapIndex < shiftedTernaryMaps.length := by
  native_decide

private theorem segment2MapIndices_bounded : ∀ mapIndex ∈
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.segment2MapIndices,
      mapIndex < shiftedTernaryMaps.length := by
  native_decide
...
private theorem segment0ChecksHold {assignment : Nat → Nat}
  ...
  exact (congrArg (fun ownerRows => row ∈ ownerRows)
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.segment0_checks_covered).mp
      member
...
private theorem segment1ChecksHold {assignment : Nat → Nat}
  ...
  exact (congrArg (fun ownerRows => row ∈ ownerRows)
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.segment1_checks_covered).mp
      member
...
private theorem segment2ChecksHold {assignment : Nat → Nat}
  ...
  exact (congrArg (fun ownerRows => row ∈ ownerRows)
    FPrimeFullHistoryTerminalAccumulatorCheckCoverage.segment2_checks_covered).mp
      member
...
theorem complete {assignment : Nat → Nat}
  ...
  have block16Satisfies := SeededPhi81.complete canonical one witness.seeded16
  have block17Satisfies := SeededPhi81.complete canonical one witness.seeded17
  apply (satisfies_flatten_iff rowPieces assignment).mpr
  intro piece member
  change piece ∈
    [segment0Rows, FPrimeFullHistorySeededPhi81.block16.rows,
      segment1Rows, FPrimeFullHistorySeededPhi81.block17.rows,
      segment2Rows] at member
  simp only [List.mem_cons, List.not_mem_nil, or_false] at member
  rcases member with rfl | rfl | rfl | rfl | rfl
  · exact segment0Satisfies
  · exact block16Satisfies
  · exact segment1Satisfies
  · exact block17Satisfies
