...
@[simp] theorem parentCommitmentStart_eq :
    parentCommitmentStart = 19795693 := by
  rfl

@[simp] theorem parentPublicInputStart_eq :
    parentPublicInputStart = 19801201 := by
  rfl

@[simp] theorem parentEvalKStart_eq : parentEvalKStart = 19803199 := by
  rfl

@[simp] theorem parentEvalAStart_eq : parentEvalAStart = 19827499 := by
  rfl

@[simp] theorem parentStarts_eq :
    [parentCommitmentStart, parentPublicInputStart, parentEvalKStart,
      parentEvalAStart] = [19795693, 19801201, 19803199, 19827499] := by
  simp
