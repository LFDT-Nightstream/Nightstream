...
use neo_fold_prototype::rv32im::audit::{
    ...
    evaluate_rv32im_main_recursion_f_prime_advice, Rv32imChunkStepIvcRelation,
};
...
fn multi_chunk_relations() -> Vec<Rv32imChunkStepIvcRelation> {
    ...
    let public_proof = prove_rv32im_public_proof_with_options(&input, options).expect("prove chunked public proof");
...
fn rv32im_chunk_step_recursive_payload_matches_cover_shape_for_multi_step_chain() {
    let relations = multi_chunk_relations();
    let advices = build_rv32im_main_recursion_f_prime_advices(&relations).expect("build main recursion F' advices");
    let cover_shape =
        build_rv32im_chunk_step_ivc_recursive_step_cover_shape(&relations).expect("build recursive-step cover shape");
    let spartan_shape =
        build_rv32im_main_recursion_step_spartan_shape(&relations).expect("build recursive-step spartan shape");
    for (relation, advice) in relations.iter().zip(advices.iter()) {
        let payload =
            build_rv32im_main_recursion_f_prime_payload(advice, &spartan_shape).expect("build recursive-step payload");
        let expected_padding =
            build_rv32im_chunk_step_ivc_recursive_step_padding(&relation.statement, &relation.witness, &cover_shape)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_chunk_step_recursive_payload_zero_pads_tail_slots() {
    let relations = multi_chunk_relations();
    let advices = build_rv32im_main_recursion_f_prime_advices(&relations).expect("build main recursion F' advices");
    let cover_shape =
        build_rv32im_chunk_step_ivc_recursive_step_cover_shape(&relations).expect("build recursive-step cover shape");
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_chunk_step_recursive_payload_batch_builder_matches_single_step_builder() {
    let relations = multi_chunk_relations();
    let advices = build_rv32im_main_recursion_f_prime_advices(&relations).expect("build main recursion F' advices");
    let cover_shape =
        build_rv32im_chunk_step_ivc_recursive_step_cover_shape(&relations).expect("build recursive-step cover shape");
    let spartan_shape =
        build_rv32im_main_recursion_step_spartan_shape(&relations).expect("build recursive-step spartan shape");
    let batch_payloads =
        build_rv32im_main_recursion_f_prime_payloads(&advices, &spartan_shape).expect("build batched payloads");
    let (derived_spartan_shape, derived_payloads) =
        build_rv32im_main_recursion_f_prime_payloads_with_spartan_shape(&relations)
    ...
    {
        let single = build_rv32im_main_recursion_f_prime_payload(advice, &spartan_shape)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_chunk_step_recursive_claim_cover_dominates_multi_step_chain() {
    let relations = multi_chunk_relations();
    let advices = build_rv32im_main_recursion_f_prime_advices(&relations).expect("build main recursion F' advices");
    let claim_cover =
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_chunk_step_recursive_backend_relations_match_native_step_and_payload_builders() {
    let relations = multi_chunk_relations();
    let advices = build_rv32im_main_recursion_f_prime_advices(&relations).expect("build main recursion F' advices");
    ...
    for ((relation, backend_relation), payload) in relations
        ...
        .zip(batch_payloads.iter())
    {
        let _step_output = evaluate_rv32im_main_recursion_f_prime_advice(&backend_relation.f_prime_advice)
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_chunk_step_recursive_effective_chunk_trace_matches_native_trace() {
    let relations = multi_chunk_relations();
    let (_, backend_relations) = build_rv32im_main_recursion_f_prime_backend_relations_with_spartan_shape(&relations)
        .expect("build recursive-step backend relations");

    for backend_relation in &backend_relations {
        debug_check_rv32im_chunk_step_recursive_effective_chunk_trace_matches_native(backend_relation)
