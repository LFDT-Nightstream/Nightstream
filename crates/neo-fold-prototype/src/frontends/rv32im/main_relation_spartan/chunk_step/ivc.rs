...
pub struct Rv32imChunkStepIvcShape {
...
impl Rv32imChunkStepIvcShape {
    pub fn expected_digest(&self) -> [u8; 32] {
        let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/chunk_step_ivc/shape");
        tr.append_u64s(
            b"neo.fold.next/rv32im/chunk_step_ivc/shape/meta",
            &[
        ...
        for len in &self.fe_round_lengths {
            tr.append_u64s(b"neo.fold.next/rv32im/chunk_step_ivc/shape/fe_round", &[*len]);
        }
        for len in &self.nc_round_lengths {
            tr.append_u64s(b"neo.fold.next/rv32im/chunk_step_ivc/shape/nc_round", &[*len]);
        }
        tr.digest32()
    }
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Rv32imChunkStepIvcRecursiveStepPadding {
    pub terminal_step: bool,
...
impl Rv32imChunkStepIvcRecursiveStepPadding {
...
#[derive(Debug, Error)]
pub enum Rv32imChunkStepIvcSpartanError {
    #[error("rv32im chunk-step ivc prepare failed: {0}")]
    Prepare(String),
    #[error("rv32im chunk-step ivc verify failed: {0}")]
    Verify(String),
}

pub(crate) fn prepare_rv32im_chunk_step_ivc_circuit_inputs(
    statement: &Rv32imChunkStepIvcStatement,
    witness: &Rv32imChunkStepIvcWitness,
) -> Result<(Rv32imChunkStepIvcPublishedTarget, Rv32imMainCircuitChunkTrace), Rv32imChunkStepIvcSpartanError> {
    validate_rv32im_chunk_step_ivc_surface(statement, witness)
        .map_err(|err| Rv32imChunkStepIvcSpartanError::Verify(err.to_string()))?;
    let published_target = build_rv32im_chunk_step_ivc_published_target(statement)
        .map_err(|err| Rv32imChunkStepIvcSpartanError::Verify(err.to_string()))?;
    ...
    .map_err(|err| Rv32imChunkStepIvcSpartanError::Verify(err.to_string()))?;
    let canonical_statement = build_rv32im_chunk_step_ivc_statement_from_authoritative_parts(
        published_target.program_digest,
        witness,
        chunk_trace.handoff.chunk_relation_digest,
    );
    if canonical_statement != *statement {
        return Err(Rv32imChunkStepIvcSpartanError::Verify(
            "rv32im chunk-step ivc statement shell does not match the authoritative published step statement".into(),
        ));
    }
    Ok((published_target, chunk_trace))
}

pub fn build_rv32im_chunk_step_ivc_shape(
    statement: &Rv32imChunkStepIvcStatement,
    witness: &Rv32imChunkStepIvcWitness,
) -> Result<Rv32imChunkStepIvcShape, Rv32imChunkStepIvcSpartanError> {
    let (_, effective_chunk) = prepare_rv32im_chunk_step_ivc_circuit_inputs(statement, witness)?;
    Ok(rv32im_chunk_step_ivc_shape_from_trace(witness, &effective_chunk))
}

pub fn build_rv32im_chunk_step_ivc_recursive_step_cover_shape(
    relations: &[Rv32imChunkStepIvcRelation],
) -> Result<Rv32imChunkStepIvcShape, Rv32imChunkStepIvcSpartanError> {
    let mut cover = Rv32imChunkStepIvcShape::recursive_step_cover_seed();
    for relation in relations {
        let step_shape = build_rv32im_chunk_step_ivc_shape(&relation.statement, &relation.witness)?;
        cover = cover.recursive_step_cover_merge(&step_shape);
    }
    Ok(cover)
}

pub fn build_rv32im_chunk_step_ivc_recursive_step_padding(
    statement: &Rv32imChunkStepIvcStatement,
    witness: &Rv32imChunkStepIvcWitness,
    cover_shape: &Rv32imChunkStepIvcShape,
) -> Result<Rv32imChunkStepIvcRecursiveStepPadding, Rv32imChunkStepIvcSpartanError> {
    let step_shape = build_rv32im_chunk_step_ivc_shape(statement, witness)?;
    build_rv32im_chunk_step_ivc_recursive_step_padding_from_shape(&step_shape, cover_shape)
}

pub fn build_rv32im_chunk_step_ivc_recursive_step_padding_from_shape(
    ...
    if !cover_shape.covers_recursive_step_shape(step_shape) {
        return Err(Rv32imChunkStepIvcSpartanError::Prepare(
            "rv32im chunk-step recursive cover shape does not dominate the per-step shape".into(),
        ));
    }
    ...
    Ok(Rv32imChunkStepIvcRecursiveStepPadding {
...
fn rv32im_chunk_step_ivc_shape_from_trace(
    witness: &Rv32imChunkStepIvcWitness,
    effective_chunk: &Rv32imMainCircuitChunkTrace,
