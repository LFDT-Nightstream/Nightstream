...
use neo_fold_legacy::frontends::r1cs_f_prime::lean_manifest::{ColumnId, PhysicalOwner, TypedOwner, GOLDILOCKS_MODULUS};
...
use neo_fold_legacy::paper::construction2::{self, LatestInstance, ProofState, RunningInstance, State};
...
use neo_fold_legacy::paper::f_prime::r1cs::{encode_f_prime_superneo_public_input, F_PRIME_PUBLIC_INPUT_LEN};
...
fn direct_terminal_fixture(
    ...
    let zero_claim = CeClaim {
        ...
        eval_a: vec![vec![K::ZERO; D.next_power_of_two()]; step.structure().t()],
...
fn direct_combined_terminal_fixture(
    ...
    let zero_claim = CeClaim {
        ...
        r: vec![K::ZERO; joint_row_variables],
        eval_k: vec![K::ZERO; D.next_power_of_two()],
        eval_a: vec![vec![K::ZERO; D.next_power_of_two()]; emission.structure().t()],
        m_in: manifest.public_carrier_width(),
        fold_digest: [0; 32],
        adv: None,
    };
    let running_claim_count = neo_fold_legacy::config::K_RHO as usize;
    (
        log,
...
fn terminal_lifecycle_fixture(manifest: &LeanNativeCcsManifest) -> (neo_fold_legacy::Preprocessing, Uncompressed) {
    let probe = manifest
        .emit_phi81_step(|_| Some(F::ZERO))
        .expect("terminal relation probe");
    let params = neo_fold_legacy::config::ccs_params(
        probe.structure().n,
        probe.structure().m,
        probe.structure().t(),
    ...
    let prep = neo_fold_legacy::lifecycle::preprocess_with_test_log(
        params.clone(),
    ...
    let zero_claim = CeClaim {
        ...
        r: vec![K::ZERO; joint_row_variables],
        eval_k: vec![K::ZERO; D.next_power_of_two()],
        eval_a: vec![vec![K::ZERO; D.next_power_of_two()]; probe.structure().t()],
        m_in: manifest.public_carrier_width(),
        fold_digest: [0; 32],
        adv: None,
    };
    ...
    for step_count in 1..=256u64 {
        ...
        let digest = state_x_out_digest_with_mode(
            ...
            z_0,
            boundary,
            neo_fold_legacy::paper::construction2::TRIVIAL_PC,
            acc_digest,
            acc_digest,
            boundary,
            None,
        );
        let carrier = encode_f_prime_superneo_public_input(neo_fold_legacy::paper::digest::digest32_as_fields(digest));
    ...
    let state = State {
        ...
        z_0,
        z_i: boundary,
        pc: neo_fold_legacy::paper::construction2::TRIVIAL_PC,
...
fn manifest_owned_lifecycle_proves_and_verifies() {
    ...
    let default_running = RunningInstance::canonical_zero(
        ...
        prep.structure(),
        setup.manifest().public_carrier_width(),
        neo_fold_legacy::paper::construction2::LaneCommitmentMode::Plain,
    )
    ...
    let carrier = encode_f_prime_superneo_public_input(neo_fold_legacy::paper::digest::digest32_as_fields(expected));
...
fn combined_manifest_owned_lifecycle_proves_and_verifies() {
    ...
    let public = encode_f_prime_superneo_public_input(neo_fold_legacy::paper::digest::digest32_as_fields(expected));
...
fn terminal_statement_guard_audit_covers_every_native_statement_check() {
    ...
    let image = neo_fold_legacy::PublicImage {
