...
to the existing typed transition context.
...
construction of the exact fixed-active source product and context; and proof
that outgoing `Pi_RLC` source-structure consistency is true by construction.
