...
//! constants. Output limbs reuse wires already accepted by the joint phase.
//!
//! Authority boundary: a binding proves which R1CS column supplies a digest
//! field. It does not prove the output evaluation itself.
//!
...
//! | `y_ring` limbs | identity-first `message.yRing[source][matrix][lane].(c0,c1)` | existing output wire | `ActiveSemantics.encodeSourcePayload` |
    ...
    for (source, input) in inputs.iter().enumerate() {
        ...
        for (matrix, row) in input.y_ring.iter().enumerate() {
            ...
            for (lane, value) in row[..profile.lane_count()].iter().enumerate() {
                push_k_limbs(&mut output, source, matrix, lane, *value)?;
            }
        }
    }
...
fn push_k_limbs(
    ...
    let path = |limb| FieldPath::YRingLimb {
        source,
        matrix,
        lane,
        limb,
