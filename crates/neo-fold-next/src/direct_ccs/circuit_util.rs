...
use crate::superneo_circuit::claim::{packed_bytes_field_values, CeClaimVar};
...
pub(crate) fn direct_accumulator_digest_circuit_from_claims<CS: ConstraintSystem<SpartanF>>(
    ...
    if let Some(first) = claims.first() {
        let parent_len = first.c_data.len();
        field_terms.push(Vec::new());
...
fn push_constant_spartan_fields<I>(
    field_terms: &mut Vec<Vec<(bellpepper_core::Variable, SpartanF)>>,
    field_constants: &mut Vec<SpartanF>,
    field_values: &mut Vec<SpartanF>,
    values: I,
...
fn direct_domain_spartan_fields(domain: &[u8]) -> Vec<SpartanF> {
