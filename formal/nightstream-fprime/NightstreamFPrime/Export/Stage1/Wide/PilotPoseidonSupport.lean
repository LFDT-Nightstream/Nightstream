...
theorem prior_sbox (program : Program)
    ...
    CommonForm program ((PiRLCRetainedGeometry.priorPoseidonBlock program).form
...
theorem output_sbox (program : Program)
    ...
    CommonForm program ((PiRLCRetainedGeometry.outputPoseidonBlock program).form
...
theorem prior_word (program : Program)
    ...
    CommonForm program ((PiRLCPoseidonGeometry.priorInputBlock program).form
  ...
  exact shared_block_common program _ _ _ _ (Nat.le_refl _) (PiRLCPoseidonGeometry.priorInputFits within)

theorem output_word (program : Program)
    (geometry : PiRLCPoseidonGeometry.Geometry program (PerApplicationFixedPoint.logicalWidth program))
    (slot : Fin (PiRLCPoseidonGeometry.outputInputBlock program).slotCount) :
    CommonForm program ((PiRLCPoseidonGeometry.outputInputBlock program).form
      (PiRLCPoseidonGeometry.outputInputStart program) (PiRLCPoseidonGeometry.outputInputFits geometry) slot) := by
  let within : PiRLCPoseidonGeometry.Geometry program (RetainedLayout.sharedEnd program) :=
    ⟨by rw [PiRLCPoseidonGeometry.pilotLogicalWidth_eq, (RetainedLayout.boundaries program).2.2.1]; decide⟩
  apply shared_block_common program _ _ _ _ _ (PiRLCPoseidonGeometry.outputInputFits within)
...
theorem pilot_prior_input (program : Program)
    ...
    CommonForm program (PilotPoseidonPlan.priorInputState geometry invocation lane) := by
  ...
  · exact previous
  · exact previous
  · exact add previous (singleton _ _ (one_common program _ rfl))
  · exact previous

theorem pilot_output_input (program : Program)
    (geometry : PiRLCPoseidonGeometry.Geometry program (PerApplicationFixedPoint.logicalWidth program))
    (invocation : Fin PilotPoseidonPlan.invocationCount) (lane : Fin 8) :
    CommonForm program (PilotPoseidonPlan.outputInputState geometry invocation lane) := by
...
theorem pilot_poseidon (program : Program)
    (geometry : PiRLCPoseidonGeometry.Geometry program (PerApplicationFixedPoint.logicalWidth program)) :
    CommonPlans program (PilotPoseidonPlan.plan geometry) := by
  apply append
