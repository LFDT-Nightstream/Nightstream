...
use neo_fold_prototype::direct_ccs::{
    lower_sparse_r1cs_export_to_low_norm, lower_sparse_r1cs_export_to_low_norm_program_and_step, DirectCcsProgram,
    DirectCcsRecursiveIvcState, DirectCcsStep, DirectLowNormLaneKind, DirectR1csLowNormLayout, DirectSparseR1csExport,
};
...
fn r1cs_low_norm_lowering_builds_direct_step_and_appends() {
    ...
    let recursive = DirectCcsRecursiveIvcState::start(program.clone()).expect("direct recursive state");
...
#[test]
fn r1cs_low_norm_two_step_recursive_state_builds_compact_f_prime_authority() {
    ...
    assert_eq!(
        ...
        "two-step low-norm append should fold one compact F' source relation"
    );
    assert!(
        summary.proof.standalone_authority_ready,
        "compact F' authority must make the two-step direct recursion proof-ready"
    );
    assert!(
        summary.f_prime.carried_ce_claims > 0,
        "folded F' authority should expose carried CE claims from the compact source relation"
    );
    assert!(summary.f_prime.native_evaluator_available);
    assert!(summary.f_prime.low_norm_source.available);
    assert!(summary.f_prime.low_norm_source.r1cs.authority_constraints > 0);
    assert!(
        summary
            .f_prime
            .low_norm_source
    ...
    assert!(
        ...
        "the exact verifier body is still too large, so authority must come from the compact source relation: constraints={} cap={} blocker={:?}",
...
#[test]
fn r1cs_low_norm_lowered_product_uses_compact_f_prime_authority_despite_large_exact_body() {
    ...
    assert_eq!(
        summary.f_prime.folded_r2_steps, 1,
        "lowered arbitrary-field product should fold one compact F' source relation"
    );
    assert!(summary.proof.standalone_authority_ready);
    assert!(
        !summary.f_prime.encoder_required,
        "the compact F' chain was already folded during append, so no missing encoder remains"
    );
    assert_eq!(summary.proof.encoder_blocker, None);
    assert!(summary.f_prime.low_norm_source.r1cs.authority_constraints > 0);
    assert!(
    ...
    assert!(
        summary.f_prime.verifier_body.constraints > summary.f_prime.exact_encoder_row_cap,
        "exact verifier body should remain large while compact source authority keeps the path usable: constraints={} cap={}",
        summary.f_prime.verifier_body.constraints,
        summary.f_prime.exact_encoder_row_cap
    );
}
...
#[test]
#[ignore = "Long-running target: runs the real recursive Spartan compression path for compact F' authority."]
