...
/-!
Artifact check for derived-program rows 500 through 749.

Owns: executable well-formedness evidence for derived-program chunk two.
...
|---|---|---|
| rows 500--749 | predecessors are known and derived outputs are fresh | checked |
-/
...
private theorem chunk2Check :
    derivedShapeWellFormedCheck derivedProgramKnown2
      derivedProgramShapeChunk2 = true := by
  native_decide

theorem derivedProgramChunk2WellFormed :
    DerivedWellFormed derivedProgramKnown2 derivedProgramChunk2 :=
  derivedWellFormed_of_shape_check_true _ _ chunk2Check
