//! Owns prover-only Phase 0 RV32IM evaluation-claim witnesses and the accepted-artifact claim projection.
//!
//! It owns:
...
use crate::multilinear::build_eq_table;
use crate::rv32im::stage1::{stage1_row_words, Stage1ProofBundle};
use crate::rv32im::stage2::{
    ram_event_words, register_read_words, register_write_words, twist_link_words, Stage2ProofBundle,
};
use crate::rv32im::stage3::{continuity_event_words, Stage3ProofBundle};
...
use super::opening_eval_claims::{
    ...
    OpeningClaimAccumulator, PackedColumnEval, Rv32imEvalClaimBundle,
};
...
pub(crate) struct Rv32imEvalClaimWitnessBuildPerf {
...
impl Rv32imEvalClaimWitnessBuildPerf {
...
fn build_stage1_claim_witnesses_with_perf(
    artifact: &Stage1ArtifactSurface,
    proof: &Stage1ProofBundle,
) -> Result<(Vec<FamilyEvalClaimWitness>, Rv32imEvalClaimWitnessBuildPerf), SimpleKernelError> {
...
fn build_stage2_claim_witnesses_with_perf(
    ...
    let mut perf = Rv32imEvalClaimWitnessBuildPerf::default();
...
fn build_stage3_claim_witness_with_perf(
    artifact: &Stage3ArtifactSurface,
    proof: &Stage3ProofBundle,
) -> Result<Option<(FamilyEvalClaimWitness, Rv32imEvalClaimWitnessBuildPerf)>, SimpleKernelError> {
    assert_matching_digest(
...
pub fn build_rv32im_eval_claim_witnesses_from_accepted_artifact(
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<Vec<FamilyEvalClaimWitness>, SimpleKernelError> {
    let (claims, _) = build_rv32im_eval_claim_witnesses_from_accepted_artifact_with_perf(artifact)?;
    Ok(claims)
}

pub(crate) fn build_rv32im_eval_claim_witnesses_from_accepted_artifact_with_perf(
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<(Vec<FamilyEvalClaimWitness>, Rv32imEvalClaimWitnessBuildPerf), SimpleKernelError> {
...
pub fn build_rv32im_eval_claim_bundle_from_claim_witnesses(
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<Rv32imEvalClaimBundle, SimpleKernelError> {
    build_rv32im_eval_claim_bundle_from_claim_witnesses_with_validation(claim_witnesses, true)
}

pub(crate) fn build_rv32im_eval_claim_bundle_from_claim_witnesses_trusted_local(
    claim_witnesses: &[FamilyEvalClaimWitness],
) -> Result<Rv32imEvalClaimBundle, SimpleKernelError> {
    build_rv32im_eval_claim_bundle_from_claim_witnesses_with_validation(claim_witnesses, false)
}

fn build_rv32im_eval_claim_bundle_from_claim_witnesses_with_validation(
    claim_witnesses: &[FamilyEvalClaimWitness],
...
pub fn build_rv32im_eval_claim_bundle_from_accepted_artifact(
    artifact: &Rv32imAcceptedProofArtifact,
) -> Result<Rv32imEvalClaimBundle, SimpleKernelError> {
    let claim_witnesses = build_rv32im_eval_claim_witnesses_from_accepted_artifact(artifact)?;
    build_rv32im_eval_claim_bundle_from_claim_witnesses(&claim_witnesses)
}

pub fn verify_rv32im_eval_claim_bundle_from_accepted_artifact(
    artifact: &Rv32imAcceptedProofArtifact,
    bundle: &Rv32imEvalClaimBundle,
) -> Result<(), SimpleKernelError> {
    if bundle.digest != bundle.expected_digest() {
        return Err(SimpleKernelError::Bridge(
            "RV32IM Phase 0 eval-claim bundle digest mismatch".into(),
        ));
    }
    let expected = build_rv32im_eval_claim_bundle_from_accepted_artifact(artifact)?;
    if &expected != bundle {
        return Err(SimpleKernelError::Bridge(
            "RV32IM Phase 0 eval-claim bundle does not match the accepted artifact".into(),
        ));
    }
    Ok(())
}
...
fn phase0_pp_seed_digest(seed: [u8; 32]) -> [u8; 32] {
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase0/pp_seed");
    tr.append_message(b"neo.fold.next/rv32im/opening_convergence/phase0/pp_seed/value", &seed);
    tr.digest32()
}

fn phase0_module_shape_digest(d: u64, packed_column_count: u64) -> [u8; 32] {
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase0/module_shape");
    tr.append_u64s(
        b"neo.fold.next/rv32im/opening_convergence/phase0/module_shape/value",
...
pub(crate) fn phase0_binding_digest(
    ...
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase0/binding");
...
fn build_opened_object_witness_with_perf<const WORDS: usize, T>(
    ...
    context_label: &str,
) -> Result<(OpenedAjtaiObjectWitness, Rv32imEvalClaimWitnessBuildPerf), SimpleKernelError> {
    ...
    Ok((
        witness,
        Rv32imEvalClaimWitnessBuildPerf {
            packed_columns_ms,
            ...
            ..Rv32imEvalClaimWitnessBuildPerf::default()
        },
    ))
}
...
fn build_singleton_claim_witness_with_perf<const WORDS: usize, T>(
    ...
    stage_proof_binding_digest: [u8; 32],
    context_label: &str,
) -> Result<(FamilyEvalClaimWitness, Rv32imEvalClaimWitnessBuildPerf), SimpleKernelError> {
...
pub(crate) fn phase0_commitment_root_digest(commitment_vector: &[Commitment]) -> [u8; 32] {
    let mut tr = Poseidon2Transcript::new(b"neo.fold.next/rv32im/opening_convergence/phase0/commitment_root");
