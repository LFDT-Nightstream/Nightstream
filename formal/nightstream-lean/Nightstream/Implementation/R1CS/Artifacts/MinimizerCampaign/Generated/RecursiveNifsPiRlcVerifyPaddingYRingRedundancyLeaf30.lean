...
namespace Nightstream.Implementation.R1CS.Artifacts.MinimizerCampaign.Generated.RecursiveNifsPiRlcVerifyPaddingYRingRedundancyLeaf30
...
theorem candLeaf169 :
    (rowsChunk wire 169).filter
        (fun row => decide (row.family = certFamily)) =
      (certParts 169).map (fun scalar => scalar.candidate) := by
  native_decide
...
theorem scalarsGroup :
    ∀ k, 169 ≤ k → k < 170 → ∀ scalar ∈ certParts k,
      scalar.Valid ∧
  ...
  · subst is169
    intro scalar member
    rw [show certParts 169 = [] from rfl] at member
    cases member
  exact absurd upper (by omega)


end Nightstream.Implementation.R1CS.Artifacts.MinimizerCampaign.Generated.RecursiveNifsPiRlcVerifyPaddingYRingRedundancyLeaf30
