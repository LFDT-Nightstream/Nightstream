...
use super::{joint_term_metadata, nonempty, MetalJointMatrixPlan, MetalSession};
...
impl MetalSession {
    pub(crate) fn first_unsatisfied_row(
        ...
        drop(words);
        drop(blocks);
        // Verification must evaluate the actual rows, without the prover's
        // optional satisfied-row substitution for selective seeded matrices.
        let tables =
            self.build_joint_application_tables(plan, &plan.cache_owner, &masks, 1, plan.rows, plan.rows, false)?;
        ...
        let shape = self.buffer_from_slice(&[plan.rows as u64, structure.f.terms().len() as u64])?;
        ...
        self.dispatch(&encoder, &self.ccs_first_unsatisfied_row, plan.rows);
        encoder.endEncoding();
        self.finish(&command)?;
        ...
        Ok((first < rows).then_some(first as usize))
    }
}
