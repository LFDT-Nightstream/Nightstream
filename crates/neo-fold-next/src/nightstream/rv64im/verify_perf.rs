...
use crate::decider::spartan2::{verify_spartan2_decider, Spartan2DeciderProof, Spartan2DeciderVerifierKey};
...
pub fn verify_rv64im_nightstream_with_perf(
    ...
    verify_spartan2_decider(
        decider_vk,
        &proof.main_residual_proof.decider_relation.target(),
        decider_proof,
    )
    .map_err(|err| {
        SimpleKernelError::Bridge(format!(
            "RV64IM Nightstream separate Spartan proof failed verification: {err}"
        ))
    })?;
