...
use neo_reductions::common::{
    compute_y_zcol_from_witness_digits, decode_superneo_coeffs_from_witness_mat, project_x_from_witness_mat,
};
...
fn toy_claim(
    ...
    let ring_forms = build_superneo_ring_forms(structure, &Vec::<K>::new()).expect("build ring forms");
...
fn rv32im_main_relation_ce_consistency_accepts_consistent_claim() {
    ensure_toy_pp();
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_ce_consistency_rejects_tampered_commitment() {
    ensure_toy_pp();
    let params = NeoParams::goldilocks_auto_r1cs_ccs(3).expect("params");
    let structure = toy_structure();
    let witness = toy_witness();
    let log = AjtaiSModule::from_global_for_dims(D, witness.Z.cols()).expect("ajtai log");
    let mut claim = toy_claim(&params, &structure, &log, &witness);
    ...
    assert!(!cs.is_satisfied(), "tampered commitment must fail constraints");
}
