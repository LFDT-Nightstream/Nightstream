...
use neo_ccs::Mat;
use neo_fold_legacy::engine::r1cs_circuit::{R1csBuilder, TranscriptGadget};
...
use neo_fold_legacy::paper::nifs::circuit::{
...
use neo_fold_legacy::paper::reductions::pi_ccs_circuit::{enforce_pi_ccs, PiCcsVerifierConfig, PiCcsVerifierMessages};
use neo_fold_legacy::paper::relations::{superneo_public_x_cols, CcsClaim, LaneRanges, LaneScheme};
use neo_fold_legacy::paper::{pi_dec, pi_rlc};
use neo_fold_legacy::{CeClaim, Preprocessing};
use neo_math::ring::D;
use neo_math::{F, K};
...
fn emit_verifier(f: &Fixture) -> Result<(R1csBuilder, NifsVOutputs), neo_fold_legacy::paper::nifs::circuit::Error> {
...
fn recursive_pi_ccs_rejects_unforwarded_output_adv() {
    for mutate_running_output in [false, true] {
        ...
        assert!(
            neo_fold_legacy::paper::pi_ccs::verify(
                &mut native_tr,
                fixture.prep.params(),
