...
use neo_fold_prototype::direct_ccs::{direct_sparse_r1cs_export_from_spartan_circuit, DirectCcsRecursiveIvcState};
...
fn bellpepper_spartan_circuit_exports_append_through_direct_recursive_state() {
    ...
    let mut recursive = DirectCcsRecursiveIvcState::start(program.clone()).expect("direct recursive state");
    recursive = recursive
        .append_step(
            first
