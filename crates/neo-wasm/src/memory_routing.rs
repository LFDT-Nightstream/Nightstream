...
use crate::layout::{
    ...
    COL_LOCAL_WRITE_ENABLED, COL_LOCAL_ZERO_ACTIVE_BEFORE, COL_OUTPUT_CAPTURED, COL_PARAM_INIT_ACTIVE_BEFORE,
    COL_PC_FREF_ACTIVE, COL_PC_ROM_ACTIVE, COL_PROGRAM_CALL_INDIRECT_IMMEDIATES_ACTIVE,
    COL_PROGRAM_GLOBAL_INDEX_ACTIVE, COL_PROGRAM_LOCAL_INDEX_ACTIVE, COL_PROGRAM_TABLE_ID_ACTIVE,
    COL_STACK_READ_ACTIVE, COL_STACK_WRITE0_ACTIVE, COL_STACK_WRITE0_HI_ACTIVE, COL_TABLE_READ_ENABLED,
...
const AUXILIARY_ROW_SUPPORT_ATOMS: [usize; 5] = [
    COL_LOCAL_ZERO_ACTIVE_BEFORE,
...
fn activation_supports() -> BTreeMap<usize, BTreeSet<usize>> {
    ...
    for (name, gate, selectors) in [
        (
            ...
            COL_LINEAR_MEM_LANE_LOAD_ACTIVE[0],
            load_selectors.as_slice(),
        ),
        (
            "linear-memory lane 1 loads",
            COL_LINEAR_MEM_LANE_LOAD_ACTIVE[1],
            load_selectors.as_slice(),
        ),
        (
            "linear-memory lane 2 loads",
            COL_LINEAR_MEM_LANE_LOAD_ACTIVE[2],
            load_selectors.as_slice(),
        ),
        (
            "linear-memory lane 0 stores",
            COL_LINEAR_MEM_LANE_STORE_ACTIVE[0],
            store_selectors.as_slice(),
        ),
        (
            "linear-memory lane 1 stores",
            COL_LINEAR_MEM_LANE_STORE_ACTIVE[1],
            store_selectors.as_slice(),
        ),
        (
            "linear-memory lane 2 stores",
            COL_LINEAR_MEM_LANE_STORE_ACTIVE[2],
            store_selectors.as_slice(),
        ),
    ] {
    ...
    insert_derived_activation_support(
        &mut supports,
        "host-event exit latches",
        COL_HOST_EVENT_EXIT_LATCH,
        opcode_selectors([WasmOpcode::Return, WasmOpcode::End]),
    );
