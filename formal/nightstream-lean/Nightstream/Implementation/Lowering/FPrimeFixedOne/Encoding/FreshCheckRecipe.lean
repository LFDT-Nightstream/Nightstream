...
/-!
Contract: canonical `freshCheck` recipe for a selected terminal-equality
application profile.

Owns: equality of the authoritative fresh and fresh-witness coordinate
strings, exact transport to the frozen executable fresh checker, active and
inactive behavior, ownership, receipt support, and honest completion.

Does not own: the running relation, NIFS, a final fold, a deployment profile,
...
namespace FreshCheckRecipe

private theorem footprint_exact
    (parameters : Parameters)
    (profile : TerminalEqualityProfile parameters) :
    (signature parameters).callFootprint Call.freshCheck =
      equalityFootprint profile.codecs.fresh.width := by
  simpa [signature, callFootprint] using profile.freshFootprint

private theorem temporary_layouts_exact
    (parameters : Parameters)
    (profile : TerminalEqualityProfile parameters) :
    ((signature parameters).callFootprint Call.freshCheck).temporaries =
      [auxiliaryLayout profile.codecs.fresh.width,
        auxiliaryLayout profile.codecs.fresh.width,
        auxiliaryLayout profile.codecs.fresh.width.pred] := by
...
private def frameTemporaries
    ...
    {witness :
      Ref (typeSystem parameters) context (.data .freshWitness)}
    (frame :
      CallFrame (signature := signature parameters)
        profile.family Call.freshCheck
        (Refs.cons fresh (Refs.cons witness .nil))) :
    LayoutBundles
      [auxiliaryLayout profile.codecs.fresh.width,
        auxiliaryLayout profile.codecs.fresh.width,
        auxiliaryLayout profile.codecs.fresh.width.pred] :=
  ...
  have width :=
    frame.outputWidthsAgree (Ports.auxiliaryBit parameters) (by
      change Ports.auxiliaryBit parameters ∈
        callOutputs parameters Call.freshCheck
...
private def occurrence
  ...
  rightLength := by
    ...
    have leftExact :
        profile.codecs.fresh.width =
          fresh.port.layout.owners.length := by
      simpa [TerminalEqualityProfile.family, Profile.family,
        DataCodecs.family, Family.codecFor] using leftWidth
    have rightExact :
        profile.codecs.freshWitness.width =
          witness.port.layout.owners.length := by
      simpa [TerminalEqualityProfile.family, Profile.family,
        DataCodecs.family, Family.codecFor] using rightWidth
    calc
      witness.port.layout.owners.length =
          profile.codecs.freshWitness.width := rightExact.symm
      _ = profile.codecs.fresh.width := profile.freshWidthsEqual
      _ = fresh.port.layout.owners.length := leftExact
  inverseLength := by
    rw [ColumnBundle.length_eq, ColumnBundle.length_eq]
...
/-- Certified fresh relation recipe. -/
def recipe
    (parameters : Parameters)
    (profile : TerminalEqualityProfile parameters) :
    CallRecipe (signature parameters) profile.family Call.freshCheck := by
  refine
    { rows := ?_
      rowCount := ?_
      rowsOwned := ?_
        ...
        | cons witness tail =>
            ...
            change
              (occurrence parameters profile frame).rows.length =
                (equalityFootprint profile.codecs.fresh.width).recurringRows
                ...
                | cons witness inputs =>
                    ...
                    have coordinates :=
                      coordinate_values_of_decodes parameters profile frame
                        assignment fresh witness decoded
                    have raw :=
                    ...
                    have checkExact :
                        checked =
                          decide
                            (profile.codecs.fresh.encode fresh =
                              profile.codecs.freshWitness.encode witness) :=
                      profile.freshCheck_exact _ _ _
                    ...
                    | cons output outputs =>
                        ...
                        have encodedPair :=
                          (binaryOperand_encodes_iff profile.family assignment
                            frame.operands fresh witness).mp inputsEncoded
                        have coordinates :
                            (occurrence parameters profile frame).left.map
                                  (fun column => assignment column.id) =
                                profile.codecs.fresh.encode fresh ∧
                        ...
                        · rw [outputPreserved, outputCoordinate,
                          ...
                          have checkExact :
                              checked =
                                decide
                                  (profile.codecs.fresh.encode fresh =
                                    profile.codecs.freshWitness.encode
                                      witness) :=
                            profile.freshCheck_exact _ _ _
                          change
                            (if checked then 1 else 0) =
                              if profile.codecs.fresh.encode fresh =
                                  profile.codecs.freshWitness.encode witness
                                then 1 else 0
                          rw [checkExact]
                          by_cases equal :
