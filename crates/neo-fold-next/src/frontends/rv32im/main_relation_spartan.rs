//! Owns the shared RV32IM Spartan substrates used by chunk-step IVC, recursive
...
use crate::rv32im::final_relation::{Rv32imChunkFoldTranscriptSnapshot, RV32IM_CHUNK_DONE_RAW_TAG};
...
use crate::rv32im::main_relation_trace::{
    Rv32imMainCircuitCeClaimShape, Rv32imMainCircuitChunkCover, Rv32imMainCircuitChunkReplaySurface,
    Rv32imMainCircuitHandoff, Rv32imMainCircuitPublicInputLayout, CHUNK_META_RAW_TAG,
};
use crate::spartan_backend::SpartanF;
use crate::superneo_circuit::claim::{
    alloc_ce_claim, alloc_ce_claim_dec_surface, alloc_ce_claim_dec_surface_with_shared_r,
    alloc_ce_claim_public_surface_with_shared_point, CircuitCeClaim,
};
use crate::superneo_circuit::initial_sum::claimed_initial_sum_from_me_inputs;
use crate::superneo_circuit::k_field::{alloc_constant_k, alloc_k, KNum, KNumVar};
use crate::superneo_circuit::output_binding::enforce_me_outputs_against_inputs;
use crate::superneo_circuit::pi_ccs::{
    bind_header_and_instance_digest, bind_header_and_instance_digest_vars, bind_me_inputs, sample_challenges,
};
use crate::superneo_circuit::pi_dec::enforce_dec_public;
use crate::superneo_circuit::pi_rlc::enforce_rlc_dec_public_with_rho_coeffs_for_last_chunk;
use crate::superneo_circuit::rho_sampling::{materialize_goldilocks_rot_matrices, sample_goldilocks_rot_rhos};
use crate::superneo_circuit::sumcheck_replay::verify_sumcheck_rounds;
...
mod recursive_cover;
mod recursive_step;
mod stage_counting_cs;
mod step_statement;
mod transcript_k;

const RV32IM_MAIN_RELATION_DELTA: u64 = 7;
...
use nifs_v_stages::{
    ...
    synthesize_pi_dec_stage, synthesize_rv32im_chunk_nifs_verifier_body_with_outer_relation_mode,
    synthesize_rv32im_chunk_nifs_verifier_body_with_synthetic_chunk_relation_io, Rv32imChunkNifsVerifierCtx,
    Rv32imPiRlcStageOutput,
};
pub(crate) use recursive_step::terminal_f_prime_r2_public_values_from_parts;
#[allow(unused_imports)]
pub use recursive_step::Rv32imMainRecursionStepChunkReplayFingerprint;
pub use recursive_step::{
    ...
    debug_trace_rv32im_main_recursion_step_spartan_shape_synthesis, Rv32imMainRecursionStepAuthoritativeChunkSurface,
...
pub(crate) use recursive_step::{
    build_rv32im_terminal_f_prime_r2_circuit, synthesize_rv32im_main_recursion_step_body,
    Rv32imMainRecursionStepCircuit,
};
pub use step_statement::Rv32imMainRecursionStepSpartanStatement;
use transcript_k::append_k_to_transcript;

#[derive(Clone)]
pub(crate) struct Rv32imClaimBundle {
    claims: Vec<CircuitCeClaim>,
    effective_count: usize,
}

fn import_chunk_fold_transcript_in<CS: ConstraintSystem<SpartanF>>(
    cs: &mut CS,
    state_in_var: &recursive_cover::Rv32imRecursiveCoverStateVar,
...
impl Rv32imClaimBundle {
    pub(crate) fn from_effective_claims(claims: Vec<CircuitCeClaim>) -> Self {
    ...
    pub(crate) fn from_padded_claims(claims: Vec<CircuitCeClaim>, effective_count: usize) -> Self {
    ...
    pub(crate) fn effective_claims(&self) -> &[CircuitCeClaim] {
    ...
    pub(crate) fn into_effective_claims(self) -> Vec<CircuitCeClaim> {
...
) -> Result<(Rv32imClaimBundle, [AllocatedNum<SpartanF>; 4]), SynthesisError> {
    synthesize_direct_ccs_nifs_chunk_with_accumulator_handle(
        params,
        structure,
        dims,
        mat_digest,
...
pub(crate) fn synthesize_direct_ccs_nifs_chunk_with_accumulator_handle<CS: ConstraintSystem<SpartanF>>(
    params: &NeoParams,
    structure: &CcsStructure<F>,
    dims: Dims,
    mat_digest: &[Goldilocks; 4],
    ...
    let boundary_plan = Rv32imChunkBoundaryPlan::from_boundary_mode(
        Rv32imChunkBoundaryMode::Interior,
        chunk.fresh_claims.len(),
        chunk.pi_ccs.ccs_outputs.len(),
    );
    let body_output = synthesize_rv32im_chunk_nifs_verifier_body_with_outer_relation_mode(
        params,
        structure,
        dims,
        mat_digest,
...
pub(crate) enum Rv32imChunkBoundaryMode {
    Interior,
    TerminalPreserveIncoming,
    TerminalCarryChildren,
}

impl Rv32imChunkBoundaryMode {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum Rv32imChunkChildClaimSource {
    ReplayedChildren,
    TerminalFinalClaims,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum Rv32imChunkNextCarryMode {
    ReplaceWithEffectiveChildren,
    PreserveIncoming,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum Rv32imChunkRlcMode {
    Standard { constant_child_prefix: usize },
    TerminalLastChunkShortcut,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct Rv32imChunkBoundaryPlan {
    pub(crate) child_claim_source: Rv32imChunkChildClaimSource,
    pub(crate) next_carry_mode: Rv32imChunkNextCarryMode,
    pub(crate) rlc_mode: Rv32imChunkRlcMode,
}

impl Rv32imChunkBoundaryPlan {
    pub(crate) fn from_boundary_mode(
        boundary_mode: Rv32imChunkBoundaryMode,
        effective_fresh_claim_count: usize,
        effective_output_count: usize,
    ) -> Self {
        let child_claim_source = if matches!(boundary_mode, Rv32imChunkBoundaryMode::TerminalCarryChildren) {
            Rv32imChunkChildClaimSource::TerminalFinalClaims
        } else {
            Rv32imChunkChildClaimSource::ReplayedChildren
        };
...
pub(crate) fn rv32im_main_relation_delta() -> SpartanF {
    SpartanF::from_canonical_u64(RV32IM_MAIN_RELATION_DELTA)
}

pub(crate) fn synthesize_rv32im_main_relation_chunk<CS: ConstraintSystem<SpartanF>>(
    params: &NeoParams,
    structure: &CcsStructure<F>,
    dims: Dims,
    mat_digest: &[Goldilocks; 4],
...
) -> Result<Rv32imClaimBundle, SynthesisError> {
    let body_output = nifs_v_stages::synthesize_rv32im_chunk_nifs_verifier_body_with_outer_relation_mode(
        params,
        structure,
        dims,
        mat_digest,
...
fn cover_ce_claim_with_shared_point(
    shape: &Rv32imMainCircuitCeClaimShape,
...
fn cover_ccs_claim(
    shape: &crate::rv32im::main_relation_trace::Rv32imMainCircuitCcsClaimShape,
...
pub(crate) fn construction2_commitment_digest_circuit<CS: ConstraintSystem<SpartanF>>(
    ...
    field_terms.push(Vec::new());
    field_constants.push(SpartanF::from_canonical_u64(RV32IM_CONSTRUCTION2_COMMITMENT_RAW_TAG));
    field_values.push(SpartanF::from_canonical_u64(RV32IM_CONSTRUCTION2_COMMITMENT_RAW_TAG));
...
fn chunk_relation_digest_circuit<CS: ConstraintSystem<SpartanF>>(
    ...
    field_terms.push(Vec::new());
    field_constants.push(SpartanF::from_canonical_u64(RV32IM_CHUNK_RELATION_DIGEST_RAW_TAG));
    field_values.push(SpartanF::from_canonical_u64(RV32IM_CHUNK_RELATION_DIGEST_RAW_TAG));
