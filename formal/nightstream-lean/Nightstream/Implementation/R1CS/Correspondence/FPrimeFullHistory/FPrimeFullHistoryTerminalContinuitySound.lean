...
This module proves both directions for all 213,766 emitted rows.  The semantic
...
/-- Typed image of the CE-core data carried across the terminal child/running
boundary.  The accessors follow the production child serializer, which omits
the `y_zcol` projection validated later by the terminal CE owner. -/
structure CeCoreImage where
  serialized : List Nat
deriving DecidableEq, Repr

namespace CeCoreImage

def commitment (claim : CeCoreImage) : List Nat := claim.serialized.take 972
def publicMatrix (claim : CeCoreImage) : List Nat :=
  (claim.serialized.drop 972).take 13878
def shape (claim : CeCoreImage) : List Nat :=
  (claim.serialized.drop 14850).take 5
def evaluationPoints (claim : CeCoreImage) : List Nat :=
  (claim.serialized.drop 14855).take 20
def constantTerms (claim : CeCoreImage) : List Nat :=
  (claim.serialized.drop 14875).take 6
def ringEvaluations (claim : CeCoreImage) : List Nat :=
  (claim.serialized.drop 14881).take 384
def foldDigest (claim : CeCoreImage) : List Nat :=
...
def verifierChildCore
    (assignment : Nat → Nat) (pairs : List (Nat × Nat)) : CeCoreImage :=
  ⟨verifierChildValues assignment pairs⟩

def runningChildCore
    (assignment : Nat → Nat) (pairs : List (Nat × Nat)) : CeCoreImage :=
...
def Holds (assignment : Nat → Nat) : Prop :=
  ∀ pairs ∈ childPairs,
    verifierChildCore assignment pairs = runningChildCore assignment pairs

/-- Named CE fields transported by one child/running authority equality. -/
structure CoreComponentsEqual (left right : CeCoreImage) : Prop where
...
theorem Holds.components
    ...
    CoreComponentsEqual (verifierChildCore assignment pairs)
      (runningChildCore assignment pairs) := by
  have equal := holds pairs member
  exact
    ⟨ congrArg CeCoreImage.commitment equal
    , congrArg CeCoreImage.publicMatrix equal
    , congrArg CeCoreImage.shape equal
    , congrArg CeCoreImage.evaluationPoints equal
    , congrArg CeCoreImage.constantTerms equal
    , congrArg CeCoreImage.ringEvaluations equal
    , congrArg CeCoreImage.foldDigest equal ⟩
...
theorem child_widths :
    childPairs.map List.length = List.replicate 14 15269 := by
...
theorem coreImage_sound
    {assignment : Nat → Nat} {pairs : List (Nat × Nat)}
    (canonical : ∀ column, assignment column < goldilocksP)
    (one : assignment 0 = 1)
    (satisfies : Satisfies (EqualityPins.rows pairs) assignment) :
    verifierChildCore assignment pairs = runningChildCore assignment pairs := by
  have equalities := EqualityPins.rows_sound canonical one satisfies
  unfold verifierChildCore runningChildCore
  congr 1
...
theorem coreImage_complete
    {assignment : Nat → Nat} {pairs : List (Nat × Nat)}
    (canonical : ∀ column, assignment column < goldilocksP)
    (one : assignment 0 = 1)
    (equal : verifierChildCore assignment pairs = runningChildCore assignment pairs) :
  ...
  exact (child_values_eq_iff assignment pairs).mp
    (congrArg CeCoreImage.serialized equal)

/-- `CIR-SOUND` for the exact 213,766-row terminal continuity owner. -/
theorem sound
    {assignment : Nat → Nat}
    (canonical : ∀ column, assignment column < goldilocksP)
    (one : assignment 0 = 1)
...
theorem complete
  ...
  have childValuesEqual : verifierChildValues assignment child =
    ...
    exact congrArg CeCoreImage.serialized (holds child childMember)
