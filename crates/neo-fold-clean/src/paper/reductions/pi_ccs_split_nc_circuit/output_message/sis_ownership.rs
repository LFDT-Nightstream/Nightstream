...
//!
//! Does not own: output truth, `y_zcol` source authority, seeded Φ81
...
//! | `y_zcol` | canonical opening of accepted NC output limbs | exact opening rows for unique `y_zcol` columns; semantic authority remains open |
...
pub struct PiCcsOutputSisAudit {
    owners: [PiCcsOutputSisOwnerAudit; 3],
...
impl PiCcsOutputSisAudit {
    pub fn owners(&self) -> &[PiCcsOutputSisOwnerAudit; 3] {
        &self.owners
    }
...
pub fn audit_pi_ccs_output_sis(
    ...
    let mut counts = [OwnerCounts::default(); 3];
    ...
    Ok(PiCcsOutputSisAudit {
        owners: [
            ...
            finish_owner(R1csInputOwner::YZcolOutput, counts[2]),
        ],
