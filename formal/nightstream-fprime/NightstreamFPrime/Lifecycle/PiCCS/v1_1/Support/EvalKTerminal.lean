...
/-!
Owns variable-support propagation for the production PiCCS Eval-K terminal.
...
private theorem coefficientExprs_supported (interface : Interface)
    ...
    (outputEvalSupport : ∀ coordinate,
      Horner.KSupported (interface.outputEval_K offset coordinate) allowed) :
...
/-- Exact support propagation through the production Eval-K terminal. -/
...
/-- The separate Eval-K terminal output preserves the exact row support. -/
