...
mod tests {
    ...
    ///
    /// This test is helpful because it validates the entire commitment pipeline:
    /// 1. Generates random field elements within the base bounds
    /// 2. Decomposes them using base-b decomposition to create a matrix representation
    /// 3. Packs the decomposed matrix into ring elements suitable for commitment
    /// 4. Creates a commitment using the Ajtai lattice-based scheme with random error
    /// 5. Verifies that the commitment is valid by checking the linear relation
    ///
    /// This ensures that all components work together correctly - from decomposition
    /// through packing to commitment and verification. It's a critical integration test
    /// that catches issues in the interaction between different parts of the system.
    #[test]
    fn test_roundtrip() {
    ...
    fn test_blinded_commit_hiding_and_opening() {
        ...
        // The blinded witness should differ from the original
    ...
    #[test]
    fn test_verify_rejects_random_proof() {
        let params = test_params();
        let comm = AjtaiCommitter::setup_unchecked(params);
        let c = vec![RingElement::from_scalar(ModInt::zero(), params.n); params.k];
        let point = vec![ExtF::ZERO];
        ...
        let proof: Vec<RingElement<ModInt>> = (0..params.d)
            .map(|_| {
                let coeffs = (0..params.n)
                    .map(|_| ModInt::from_u64(rng.random_range(0..params.b)))
                    .collect();
                RingElement::from_coeffs(coeffs, params.n)
            })
            .collect();
    ...
    fn test_gpv_statistical_closeness() {
        ...
        let mean_norm = norms.iter().sum::<u64>() as f64 / (samples as f64);
        assert!(
            mean_norm > 10.0 && mean_norm < 30.0,
            "Unexpected mean norm: {}",
            mean_norm
        );
    }
    ...
    fn test_gpv_sample_chi_squared() {
        ...
        let samples: usize = std::env::var("NEO_CHI_SAMPLES")
            ...
            .unwrap_or(100);
        let mut counts = [0usize; 6]; // |0|, |1|, |2|, |3|, |4|, >=5
        for _ in 0..samples {
            let sample = comm.sample_gaussian_ring(&center, params.sigma, &mut rng).unwrap();
            let mut v = sample.coeffs()[0].as_canonical_u64() as i64;
            if v > params.q as i64 / 2 {
                v -= params.q as i64;
            }
        ...
        let p = [0.125, 0.239, 0.206, 0.161, 0.114, 0.155];
        let expected: Vec<f64> = p.iter().map(|&pr| pr * samples as f64).collect();
