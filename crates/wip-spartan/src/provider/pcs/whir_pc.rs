//! Standalone WHIR PCS for WIP Spartan's Goldilocks engine.
//!
//! This module owns the conversion between WIP Spartan's `ff` Goldilocks
...
//! does not by itself make the surrounding WIP Spartan proof zero knowledge.
//! The 125-bit target applies to this PCS; WIP Spartan's outer sum-check still
...
/// WHIR polynomial commitment engine for WIP Spartan's Goldilocks scalar field.
#[derive(Clone, Debug)]
  ...
  fn prove(
    ...
    if claimed_proof_evaluation(&proof) != Some(whir_eval) {
      return Err(internal_error(
        "WHIR returned an evaluation inconsistent with WIP Spartan",
      ));
    }
