...
namespace Nightstream.Implementation.Lowering.FPrimeFixedOne.Encoding.TerminalR1cs.Extension
...
def rows
  ...
  [ ⟨⟨owner, firstOrdinal⟩,
      ⟨left.low, right.low, Goldilocks.singleton frame.lowLow 1⟩⟩,
    ⟨⟨owner, firstOrdinal + 1⟩,
      ⟨left.high, right.high, Goldilocks.singleton frame.highHigh 1⟩⟩,
    ⟨⟨owner, firstOrdinal + 2⟩,
      ⟨sumValue left, sumValue right,
        Goldilocks.singleton frame.cross 1⟩⟩ ]
...
private theorem singleton_eval
    ...
    LinearCombination.eval assignment
        (Goldilocks.singleton column 1) =
      assignment column := by
  simp [Goldilocks.singleton, LinearCombination.eval, Fin.one_mul]
...
private theorem frame_products
    ...
    (assignment : ColumnId → F)
    (satisfied :
      Satisfies (rows owner firstOrdinal left right frame) assignment) :
  ...
  change
    (left.low.eval assignment * right.low.eval assignment =
        (Goldilocks.singleton frame.lowLow 1).eval assignment) ∧
      (left.high.eval assignment * right.high.eval assignment =
        (Goldilocks.singleton frame.highHigh 1).eval assignment) ∧
      ((sumValue left).eval assignment *
        (sumValue right).eval assignment =
          (Goldilocks.singleton frame.cross 1).eval assignment) ∧
      True at satisfied
...
theorem rows_honest
    ...
    (cross :
      assignment frame.cross =
        (sumValue left).eval assignment *
          (sumValue right).eval assignment) :
    Satisfies (rows owner firstOrdinal left right frame) assignment := by
  change
    (left.low.eval assignment * right.low.eval assignment =
        (Goldilocks.singleton frame.lowLow 1).eval assignment) ∧
      (left.high.eval assignment * right.high.eval assignment =
...
end Nightstream.Implementation.Lowering.FPrimeFixedOne.Encoding.TerminalR1cs.Extension
