...
theorem finishValue_probability_and_expected_work {SetupTape : Type*}
    ...
    (preparedContexts : FiatShamirTransfer.contextLaw inst.relation law =
      ...
      (call context alpha gamma point).value = InteractivePrefix.run
        (InteractiveComposition.firstPhase originalFirstPhase
          (SupportedExtraction.publicCheck
            (fun context => inst.running (inputs context))) context) alpha gamma point)
    (accessBound : Nat)
    ...
    (g Q (WideFiatShamir.realSuccessProbability inst.relation inst.ajtai running fresh law) - deltaFS Q -
      ...
      Real.sqrt ((∑' tape, (setupTapes tape).toReal * BindingProbability.localSuccessProbability
        inst.ajtai program (sourceProgram (prepare tape).value).access inst.relation running fresh
      ...
      PaperCompositionProbability.eventProbability contexts
        (InteractiveComposition.firstPhase originalFirstPhase (SupportedExtraction.publicCheck running))
        (InteractiveComposition.suffixLaw inst.relation inst.ajtai running fresh continuation)
        (InteractiveComposition.consume inst.relation inst.ajtai program)
  ...
  have storedEvent := PiCCSStoredSourceProbability.returnedSourceProbability_eq_finishValue inst
    inputs (FiatShamirTransfer.contextLaw inst.relation law) originalFirstPhase
    (SupportedExtraction.publicCheck (fun context => inst.running (inputs context)))
