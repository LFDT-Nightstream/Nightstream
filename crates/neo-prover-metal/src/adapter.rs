...
use neo_fold_clean::paper::reductions::accumulator_sis_circuit::{
    PI_CCS_OUTPUTS_SIS_CONFIG, PI_RLC_PROJECTION_SIS_CONFIG,
};
...
impl NifsProverAdapter for MetalNifsProver {
    fn prove(&mut self, request: NifsProverRequest<'_>) -> Result<NifsProverOutput, Error> {
        ...
        if running_carrier.is_some_and(|carrier| carrier.session_ownership_id() != self.session.ownership_id()) {
            return Err(backend_unavailable(
                "Metal running carrier belongs to a different session",
            ));
        }
        let running_parent_accumulator_digest = running_carrier.map(MetalRunningCarrier::parent_accumulator_digest);
        let running_accumulator_handle = running_parent_accumulator_digest.map(|parent_accumulator_digest| {
            AccumulatorHandle::from_parent_digest(request.running.claims.len(), Some(parent_accumulator_digest))
                .digest_fields()
        });
        let resident_running_id = running_carrier.and_then(MetalRunningCarrier::resident_id);
        if let Some(id) = resident_running_id {
        ...
        let pi_ccs_proof = pi_ccs::prove_from_parts_with_backends_and_transcript_mode(
            ...
            running_parent_accumulator_digest,
        ...
        let parent_accumulator_digest = digest::accumulator_ce_claim_digest(&rlc_output.claim);
        ...
        if let Some(lanes) = request.lanes {
            self.ensure_lane_ajtai_plan(lanes, resident_mix.cols())?;
        }
        let commitment_plan = &self
            ...
            .plan;
        let lane_commitment_plan = request.lanes.map(|_| {
            self.fresh_lane_commitment_plan
                .as_ref()
                .expect("Nebula lane commitment plan installed above")
        });
        ...
        let post_fold_summary = self.post_fold_summary(&next_running, parent_accumulator_digest)?;
