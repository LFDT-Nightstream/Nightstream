import Nightstream.Implementation.Nebula.Commitment.Lanes.ShiftedTernaryEncodingBridge

set_option autoImplicit false

namespace tests.NebulaShiftedTernaryEncodingBridge

open Nightstream.Implementation.Nebula.ShiftedTernaryEncodingBridge
open Nightstream.Protocol.Nebula.ShiftedTernary41V1

def maximum : CanonicalGoldilocks :=
  ⟨modulus - 1, by norm_num [modulus]⟩

example (index : Fin digitCount) :
    (trits maximum).getD index.val 0 =
      target maximum / 3 ^ index.val % 3 :=
  trits_getD_eq_quotient maximum index.val index.isLt

example (index : Fin digitCount) :
    Nightstream.Implementation.R1CS.ShiftedTernaryCanonicalWord.canonicalDigit
        maximum.val index.val =
      fieldDigit
        (Nightstream.Protocol.Nebula.CompactCommit.tritAt maximum index) :=
  canonicalDigit_eq_fieldDigit_tritAt maximum index

end tests.NebulaShiftedTernaryEncodingBridge
