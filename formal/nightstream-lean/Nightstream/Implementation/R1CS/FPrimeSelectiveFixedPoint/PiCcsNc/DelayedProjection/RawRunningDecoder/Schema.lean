...
and 54-live-lane/64-virtual-lane domains; and a compact proof-free
`{ child, logicalColumn, sourceColumn }` record schema for a later generated
artifact.

Does not own: any concrete `sourceColumn`, assignment value, R1CS row,
...
`SourceColumnMap.sourceColumn` field below is data, not evidence.

Emits constraints: none; typed coordinate schema only.

The Rust/R1CS exporter must eventually instantiate `SourceColumnMap` from
actual allocation columns and prove that its generated records are exactly
`SourceColumnMap.records`. This file deliberately contains no generated data
and no closed computation over the 3,780-record domain.
...
| `nifs.pi_ccs.nc.delayed.raw_decoder.records` | compact child-major source-column schema has one coordinate owner | derived | generated source-column values |
...
/-- Compact proof-free generated record. Bounds are checked by the generic
schema rather than embedded as proof terms in every generated datum. -/
...
def WellFormed (record : SourceColumnRecord) : Prop :=
  record.child < childCount /\ record.logicalColumn < logicalColumnCount

/-- Typed child coordinate after a `WellFormed` check. -/
def typedChild (record : SourceColumnRecord)
    (wellFormed : record.WellFormed) : Child :=
  ⟨record.child, wellFormed.1⟩

/-- Typed logical coordinate after a `WellFormed` check. -/
def typedLogicalColumn (record : SourceColumnRecord)
    (wellFormed : record.WellFormed) : LogicalColumn :=
  ⟨record.logicalColumn, wellFormed.2⟩

end SourceColumnRecord

/-- One source-column allocation for every typed child/logical coordinate.
This structure contains no provenance proof; an arbitrary function inhabits
it. -/
structure SourceColumnMap where
  sourceColumn : Child -> LogicalColumn -> Nat

namespace SourceColumnMap

/-- Canonical compact record at one typed coordinate. -/
def recordAt (columns : SourceColumnMap)
    (child : Child) (logicalColumn : LogicalColumn) : SourceColumnRecord where
  child := child.val
  logicalColumn := logicalColumn.val
  sourceColumn := columns.sourceColumn child logicalColumn

@[simp] theorem recordAt_child (columns : SourceColumnMap)
    (child : Child) (logicalColumn : LogicalColumn) :
    (recordAt columns child logicalColumn).child = child.val := by
  rfl

@[simp] theorem recordAt_logicalColumn (columns : SourceColumnMap)
    (child : Child) (logicalColumn : LogicalColumn) :
    (recordAt columns child logicalColumn).logicalColumn =
      logicalColumn.val := by
  rfl

@[simp] theorem recordAt_sourceColumn (columns : SourceColumnMap)
    (child : Child) (logicalColumn : LogicalColumn) :
    (recordAt columns child logicalColumn).sourceColumn =
      columns.sourceColumn child logicalColumn := by
  rfl

theorem recordAt_wellFormed (columns : SourceColumnMap)
    (child : Child) (logicalColumn : LogicalColumn) :
    (recordAt columns child logicalColumn).WellFormed := by
...
def records (columns : SourceColumnMap) : List SourceColumnRecord :=
...
@[simp] theorem records_length (columns : SourceColumnMap) :
...
theorem recordAt_mem_records (columns : SourceColumnMap)
    (child : Child) (logicalColumn : LogicalColumn) :
...
not claim that two coordinates have distinct physical source columns. -/
theorem recordAt_eq_recordAt_iff (columns : SourceColumnMap)
...
theorem recordAt_injective (columns : SourceColumnMap) :
...
theorem mem_records_iff (columns : SourceColumnMap)
...
theorem records_all_wellFormed (columns : SourceColumnMap) :
    ∀ record ∈ columns.records, record.WellFormed := by
  intro record member
  rcases (mem_records_iff columns record).1 member with
    ⟨child, logicalColumn, rfl⟩
  exact recordAt_wellFormed columns child logicalColumn

/-- Exact coordinate coverage and ownership, stated without a global
`Nodup` computation. -/
theorem existsUnique_record_with_coordinate (columns : SourceColumnMap)
    (child : Child) (logicalColumn : LogicalColumn) :
    ∃ record,
      (record ∈ columns.records /\
