...
use neo_fold_prototype::circuit::superneo::terminal_identity::{
...
fn rv32im_main_relation_terminal_identity_fe_matches_native_formula() {
...
#[test]
#[ignore = "Spartan-path tests are parked until native NIFS and F' replacement lands"]
fn rv32im_main_relation_terminal_identity_nc_matches_native_formula() {
    let mut cs = TestConstraintSystem::<SpartanF>::new();
    let delta = SpartanF::from_canonical_u64(7);
    let claims = synthetic_claims();
    let public_challenges = synthetic_public_challenges();
