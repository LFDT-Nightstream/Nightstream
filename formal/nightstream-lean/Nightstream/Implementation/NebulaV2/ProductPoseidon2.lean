...
namespace Nightstream.Implementation.NebulaV2.ProductPoseidon2
...
abbrev State := PaddedRowIdentityPoseidon2.State
...
def constants := PaddedRowIdentityPoseidon2.constants
def initialState : State := PaddedRowIdentityPoseidon2.initialState
def word (value : Nat) : Nat := PaddedRowIdentityPoseidon2.word value

def construction3DomainFields : List Nat :=
  PaddedRowIdentityPoseidon2.construction3DomainFields
def statementIdLabelFields : List Nat :=
  PaddedRowIdentityPoseidon2.statementIdLabelFields
def proofLabelFields : List Nat := PaddedRowIdentityPoseidon2.proofLabelFields
def proverMessageLabelFields : List Nat :=
  PaddedRowIdentityPoseidon2.proverMessageLabelFields
def verifierChallengeLabelFields : List Nat :=
  PaddedRowIdentityPoseidon2.verifierChallengeLabelFields

def statementIdentifierTag : Nat :=
  PaddedRowIdentityPoseidon2.statementIdentifierTag
def piRlcCandidateTag : Nat := PaddedRowIdentityPoseidon2.piRlcCandidateTag
def piDecOutputTag : Nat := PaddedRowIdentityPoseidon2.piDecOutputTag

abbrev Event := PaddedRowIdentityPoseidon2.Event
...
def eventScheduleFields : List Nat :=
  proverMessageLabelFields ++ verifierChallengeLabelFields ++
    [word 33, word eventSchedule.length] ++
      eventSchedule.flatMap PaddedRowIdentityPoseidon2.Event.fields
...
@[simp] theorem proofPrefixFields_length (statementId : StatementId) :
  ...
  simp [proofPrefixFields, construction3DomainFields, proofLabelFields,
    PaddedRowIdentityPoseidon2.proofLabelFields,
    PaddedRowIdentityPoseidon2.stringFields,
    PaddedRowIdentityPoseidon2.proofLabelBytes,
    PaddedRowIdentityPoseidon2.construction3DomainFields,
    PaddedRowIdentityPoseidon2.construction3DomainBytes,
...
def challengeValue (state : State) : K :=
  PaddedRowIdentityPoseidon2.challengeValue state
def squeezeK (state : State) : K × State :=
  PaddedRowIdentityPoseidon2.squeezeK state
...
abbrev Coefficient := PaddedRowIdentityPoseidon2.Coefficient
abbrev Scalar := PaddedRowIdentityPoseidon2.Scalar

def samplerAttemptCount : Nat := PaddedRowIdentityPoseidon2.samplerAttemptCount
def firstAttempt : Fin samplerAttemptCount :=
  PaddedRowIdentityPoseidon2.firstAttempt
def secondAttempt : Fin samplerAttemptCount :=
  PaddedRowIdentityPoseidon2.secondAttempt
def thirdAttempt : Fin samplerAttemptCount :=
  PaddedRowIdentityPoseidon2.thirdAttempt
def samplerCoefficientCount : Nat :=
  PaddedRowIdentityPoseidon2.samplerCoefficientCount
def candidateFields := PaddedRowIdentityPoseidon2.candidateFields
def candidateValue := PaddedRowIdentityPoseidon2.candidateValue
def candidateAccepted := PaddedRowIdentityPoseidon2.candidateAccepted
def candidateDigit := PaddedRowIdentityPoseidon2.candidateDigit
def sampleCoefficient := PaddedRowIdentityPoseidon2.sampleCoefficient
...
@[simp] theorem sampleCoefficient_of_first
  ...
  change PaddedRowIdentityPoseidon2.candidateAccepted
    (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
      PaddedRowIdentityPoseidon2.firstAttempt) = true at accepted
  change PaddedRowIdentityPoseidon2.sampleCoefficient state source coefficient =
    some (PaddedRowIdentityPoseidon2.candidateDigit
      (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
        PaddedRowIdentityPoseidon2.firstAttempt))
  simp only [PaddedRowIdentityPoseidon2.sampleCoefficient, accepted, if_true]
...
@[simp] theorem sampleCoefficient_of_second
  ...
  change PaddedRowIdentityPoseidon2.candidateAccepted
    (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
      PaddedRowIdentityPoseidon2.firstAttempt) = false at firstRejected
  change PaddedRowIdentityPoseidon2.candidateAccepted
    (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
      PaddedRowIdentityPoseidon2.secondAttempt) = true at secondAccepted
  change PaddedRowIdentityPoseidon2.sampleCoefficient state source coefficient =
    some (PaddedRowIdentityPoseidon2.candidateDigit
      (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
        PaddedRowIdentityPoseidon2.secondAttempt))
  simp only [PaddedRowIdentityPoseidon2.sampleCoefficient, firstRejected,
...
@[simp] theorem sampleCoefficient_of_third
  ...
  change PaddedRowIdentityPoseidon2.candidateAccepted
    (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
      PaddedRowIdentityPoseidon2.secondAttempt) = false at secondRejected
  change PaddedRowIdentityPoseidon2.candidateAccepted
    (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
      PaddedRowIdentityPoseidon2.thirdAttempt) = true at thirdAccepted
  change PaddedRowIdentityPoseidon2.sampleCoefficient state source coefficient =
    some (PaddedRowIdentityPoseidon2.candidateDigit
      (PaddedRowIdentityPoseidon2.candidateValue state source coefficient
        PaddedRowIdentityPoseidon2.thirdAttempt))
...
def SamplerShortfall := PaddedRowIdentityPoseidon2.SamplerShortfall
def SamplerAvailable := PaddedRowIdentityPoseidon2.SamplerAvailable
noncomputable def samplerSucceeded :=
  PaddedRowIdentityPoseidon2.samplerSucceeded
def scalarResponse := PaddedRowIdentityPoseidon2.scalarResponse
def piRlcResponse := PaddedRowIdentityPoseidon2.piRlcResponse
...
@[simp] theorem samplerSucceeded_eq_true_iff (state : State) :
  ...
  simpa only [samplerSucceeded, SamplerAvailable] using
    PaddedRowIdentityPoseidon2.samplerSucceeded_eq_true_iff state

def ResponseRefinesAt := PaddedRowIdentityPoseidon2.ResponseRefinesAt
...
theorem scalarResponse_refines_of_available
  ...
  exact PaddedRowIdentityPoseidon2.piRlcResponse_refines_of_available available
...
theorem scalarResponse_eq_of_sampled
  ...
  change PaddedRowIdentityPoseidon2.sampleCoefficient state source
    (Fin.cast (by rfl) coefficient) = some selected at sampled
  unfold scalarResponse PaddedRowIdentityPoseidon2.scalarResponse
  rw [sampled]
  rfl
...
theorem piRlcResponse_valid (state : State)
  ...
  PaddedRowIdentityPoseidon2.piRlcResponse_valid state index

end Nightstream.Implementation.NebulaV2.ProductPoseidon2
