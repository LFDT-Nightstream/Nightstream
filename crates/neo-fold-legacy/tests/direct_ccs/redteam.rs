...
use neo_fold_legacy::paper::construction2::{EncInst, SemanticStateMode};
use neo_fold_legacy::paper::digest::{self, StateXOutDigestMode};
use neo_fold_legacy::paper::f_prime::r1cs::F_PRIME_SUPERNEO_PUBLIC_INPUT_LEN;
use neo_fold_legacy::{finish_uncompressed, prove, verify_uncompressed, Preprocessing, Uncompressed};
...
fn preprocessing_rejects_partial_public_ring() {
    ...
    assert!(matches!(
        ...
        Err(FrontendError::Lifecycle(
            neo_fold_legacy::Error::PublicInputNotWholeRing { m_in: 1, d: D }
        ))
    ));
}
